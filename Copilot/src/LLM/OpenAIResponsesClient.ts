...
const DEFAULT_ENDPOINT: string = "https://api.openai.com/v1/responses";
...
export class OpenAIResponsesClient implements LLMClient {
  ...
  private mapResponse(body: OpenAIResponsesAPIResponse): ChatMessage {
  ...
  private createTimeoutError(originalError: Error): Error {
    const message: string = `OpenAI Responses request timed out after ${this.options.timeoutMs} ms while calling ${this.options.endpoint ?? DEFAULT_ENDPOINT}.`;
