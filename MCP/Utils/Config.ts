...
export const OneUptimeUrl: URL = process.env['ONEUPTIME_URL'] ? URL.fromString(process.env['ONEUPTIME_URL'].toString()) : URL.fromString("https://oneuptime.com");

if(!process.env['ONEUPTIME_PROJECT_ID']){
    throw new BadDataException("PROJECT_ID is not set in environment variables.");
}

export const ProjectID: string = process.env['ONEUPTIME_PROJECT_ID'].toString().trim();

if(!process.env['ONEUPTIME_API_KEY']){
    throw new BadDataException("ONEUPTIME_API_KEY is not set in environment variables.");
}

export const ApiKey: string = process.env['ONEUPTIME_API_KEY'].toString().trim();
