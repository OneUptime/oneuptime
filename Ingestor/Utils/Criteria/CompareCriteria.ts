import { CheckOn, CriteriaFilter, EvaluateOverTimeOptions, EvaluateOverTimeType, FilterType } from "Common/Types/Monitor/CriteriaFilter";
...
export default class CompareCriteria {
    ...
    public static greaterThan(data: {
        ...
        if (Array.isArray(data.value)) {
            if (data.evaluationType === EvaluateOverTimeType.AnyValue) {
                return data.value.some((value: number) => value > data.threshold);
            } else {
                return data.value.every((value: number) => value > data.threshold);
            }
        }
    ...
    public static convertThresholdToNumber(threshold: string | number | undefined): number | null {
        if (!threshold) {
            return null;
        }
    ...
    public static compareCriteriaNumbers(data: {
        ...
        if (data.criteriaFilter.filterType === FilterType.GreaterThan) {
            if (CompareCriteria.greaterThan({
                threshold: data.threshold as number,
                value: data.value,
                evaluationType: data.criteriaFilter.evaluateOverTimeOptions?.evaluateOverTimeType
            })) {
        ...
        if (
            ...
            if (CompareCriteria.greaterThanOrEqual({
                threshold: data.threshold as number,
                value: data.value,
                evaluationType: data.criteriaFilter.evaluateOverTimeOptions?.evaluateOverTimeType
            })) {
        ...
        if (
            data.criteriaFilter.filterType === FilterType.LessThanOrEqualTo
        ) {
            if (CompareCriteria.lessThanOrEqual({
                threshold: data.threshold as number,
                value: data.value,
                evaluationType: data.criteriaFilter.evaluateOverTimeOptions?.evaluateOverTimeType
            })) {
    ...
    public static getCompareMessage(data: {
        ...
        if (data.criteriaFilter.evaluateOverTimeOptions?.evaluateOverTimeType === EvaluateOverTimeType.AnyValue) {
            message += 'Any value of';
        }

        if (data.criteriaFilter.evaluateOverTimeOptions?.evaluateOverTimeType === EvaluateOverTimeType.AllValues) {
