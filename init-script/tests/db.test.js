...
const { collectionObject, mockDbCollection, mockDbClient } = require('./helper');
...
test('Should connect to db', async () => {
  await connectToDb();
  expect(global.client.connect).toBeCalledWith(url, { useUnifiedTopology: true });
});
