...
```mermaid
flowchart TB
  subgraph browser["Customer browser (packages/App/FeatureSet/BrowserRecorder, IIFE)"]
  ...
  subgraph app["App tier (packages/App/FeatureSet/Telemetry)"]
...
**Rejected: `rrweb-player`.** It is a Svelte component shipping its own stylesheet against an esbuild pipeline that turns CSS imports into runtime `<style>` injection (`packages/Common/UI/esbuild-config.js`). We must hand-build the scrubber anyway — there is no slider, timeline, scrubber, player, or video component in `packages/Common/UI/Components`. Adding a Svelte runtime for zero used functionality is pure risk.
...
**Location: `packages/App/FeatureSet/BrowserRecorder/`** — not a repo-root package. `packages/App/scripts/frontend-run.sh` already resolves `FeatureSet/<X>` and the Dockerfile already has a per-FeatureSet `npm ci` + `COPY` pattern, so this needs exactly three one-line edits instead of a new root COPY layer.

It gets **its own** `esbuild.config.js` and must not call `createConfig` from `packages/Common/UI/esbuild-config.js`, which hardcodes `format: "esm"` (:278), `splitting: true` (:284), `minify: false` (:282) and only accepts `entryPoint/outdir/additionalDefines/additionalExternal/additionalAlias`. An ESM+split bundle cannot be loaded by a plain `<script src>`. The recorder config is ~30 lines: `format: "iife"`, `globalName: "OneUptimeReplay"`, `minify: true`, `splitting: false`, `target: "es2019"`, no React/i18next aliases, no CSS plugin, no mermaid plugin.

**It must not import from `Common`.** `packages/Common/UI/Config.ts` reads `window.process.env`, which exists only because the server injects `/env.js` (`packages/App/FeatureSet/Frontend/Index.ts`), and `packages/Common/package.json` pulls express/typeorm/stripe/playwright/monaco/mermaid. Shared *pure* logic (masking functions, sampling hash, chunk math, URL scrubber, session identity) lives in `packages/Common/Utils/Rum/*` and is copied in at build time via the bundler — which is also what makes it testable (§12).

**Two-stage load, so a bad release is recoverable.** `GET /telemetry/session-replay/v1/recorder.js` is a ~1 KB **loader stub**, `Cache-Control: public, max-age=300`. Its only jobs: fetch config, honour `enabled`/consent/DNT/GPC, then load the mutable artifact at `/telemetry/session-replay/latest/recorder.js` (`no-store`, published SHA-384 for SRI). Live config is also `no-store`, keeping that SRI paired with the current bytes. This gives immediate replacement and a kill switch that stops *recording* rather than only stopping ingest.

Served under `/telemetry` deliberately: `packages/App/FeatureSet/Frontend/Index.ts`'s `DashboardFallbackRoutePrefixesToSkip` already contains `/telemetry`, and `FrontendRoutes.init()` runs **before** `TelemetryRoutes.init()`, so a root-level `/recorder.js` would be served as Dashboard SPA HTML on self-hosted installs. With `BILLING_ENABLED=true` the nginx catch-all `location /` proxies to the marketing Home app instead. We still add `/session-replay` to the skip list for the ingest routes.
...
| `SESSION_REPLAY_INGEST_ENABLED` | true | `packages/App/FeatureSet/Telemetry/Config.ts`, `!== "false"` idiom | instance kill switch |
...
| `maskInputFn` / `maskTextFn` | **fixed-length placeholder** | recorder, pre-emit | rrweb's default is `'*'.repeat(value.length)` — a length oracle for passwords, PANs, OTPs. Fixed 3-char block, or short/medium/long buckets. Unit-tested in `packages/Common/Tests`. |
...
| `enableMCP` | **unset** on both analytics models | model definition | `packages/App/FeatureSet/MCP/Tools/ToolGenerator.ts` auto-generates tools from it; an LLM agent must not get a tool that reads end-user screen recordings. Assert absence in a test. |
...
`packages/Common/Server/Utils/StartServer.ts:143` is `if (req.path.includes("/otlp/v1/")) { return next(); }` and :194 is the urlencoded twin. Extend **both**:
...
There is **no binary column type** (verified `packages/Common/Types/AnalyticsDatabase/TableColumnType.ts`: no Binary/Blob/FixedString/Nested; Text/JSON/JSONArray all compile to ClickHouse `String`). We store the **decompressed** rrweb JSON in a `Text` column with `ZSTD(3)` — base64-of-gzip would inflate 33% and hand ZSTD incompressible input. Precedents: `Log.body` (Text + ZSTD(3) + TokenBF), `Span.events` (JSONArray + ZSTD(3)), `ProfileSample.stacktrace` (ArrayText + ZSTD(3)).

Add to `packages/Common/Types/AnalyticsDatabase/AnalyticsTableName.ts` (note: the V-suffix is *not* a universal convention — `SloHistory`, `MutableMetricItem`, `MetricItemAggMV1m` have none — but the telemetry tables do):
...
### `packages/Common/Models/AnalyticsModels/RumSession.ts`

Template: `packages/Common/Models/AnalyticsModels/Profile.ts`. Engine **`ReplacingMergeTree`** → resolves to `ReplicatedReplacingMergeTree(version)` with the column name **hardcoded** in `packages/Common/Server/Utils/AnalyticsDatabase/ClusterConfig.ts:86`, so a column literally named `version` is mandatory (the trap `SloHistory.ts:148-153` documents).
...
### `packages/Common/Models/AnalyticsModels/RumSessionChunk.ts`

Template: `packages/Common/Models/AnalyticsModels/ProfileSample.ts` (its `super()` block is the shape to copy). Engine **`ReplacingMergeTree`** — needs a `version` column — so a retried or duplicated chunk collapses at merge instead of double-inserting into a plain MergeTree.
...
**Columns added to `packages/Common/Models/DatabaseModels/RumApplication.ts`**, beside `telemetryRetentionConfig`, each with `@ColumnAccessControl` mirroring the existing retention-column block:

`isSessionReplayEnabled` (Boolean, **false**) · `sessionReplayMaskingMode` (ShortText, `"MaskAllText"`) · `sessionReplayMaskSelectors` / `sessionReplayBlockSelectors` (simple-array, `[]`) · `sessionReplayAllowedOrigins` (simple-array, **`[]` = refused**) · `sessionReplayConsentMode` (ShortText, `"RequireExplicit"`) · `sessionReplayCaptureTrigger` (ShortText, `"Always"`) · `sessionReplaySamplePercentage` (Number, 100) · `sessionReplayCaptureUserIdentity` (Boolean, false) · `sessionReplayCaptureGeo` (Boolean, false) · `sessionReplayRecordCanvas` (Boolean, false) · `sessionReplayRetentionInDays` (Number, 7) · `sessionReplayMonthlyBudgetInGB` (Number, nullable) · `sessionReplayLastChunkReceivedAt` / `sessionReplayBudgetExceededAt` (Date, nullable — the two columns that answer most support tickets on their own).

Plus `Project.isSessionReplayAllowed` (Boolean, false), following `Project.enableAuditLogs`.

**`packages/Common/Models/DatabaseModels/RumSessionReplayView.ts`** — the read audit. Template `RumApplicationClient.ts` (`@TenantColumn("projectId")`, `@TableAccessControl({create: [], read: [...], update: [], delete: []})`, `@CrudApiEndpoint`, `@TableMetadata`, ManyToOne `onDelete: "CASCADE"`, **no** `@EnableDocumentation()`). Columns: `projectId` · `rumApplicationId` · `sessionId` · `viewedByUserId` + relation · `viewedByApiKeyId` · `viewedAt` · `ipAddress` (the *viewer's*) · `userAgent` · `secondsWatched` · `accessReason` (LongText, optionally mandatory per project) · `linkedIncidentId` · `linkedExceptionFingerprint`. Indexes `(projectId, sessionId, viewedAt)` and `(projectId, viewedByUserId, viewedAt)`. Deliberately **not** the `AuditLog` analytics table, which is `PlanType.Enterprise`-gated — a privacy control must not be a paid tier.

**`packages/Common/Models/DatabaseModels/RumSessionErasureRequest.ts`** — the first erasure primitive in this repo (verified: no erasure machinery anywhere; only `/legal/gdpr` marketing pages). `projectId` · `rumApplicationId` (**nullable, `ON DELETE SET NULL`** — with CASCADE, deleting the app would delete the request that was supposed to clean up after it) · `requestType` (`BySessionId` \| `ByIdentifiedUserKey` \| `ByDateRange` \| `ByRumApplication`) · `targetValue` (LongText — also holds the app id so the request survives its parent) · `startDate`/`endDate` · `status` · `requestedByUserId` · `requestedAt` · `completedAt` · `sessionsDeleted` · `chunksDeleted` · `failureReason`.

**`packages/Common/Models/DatabaseModels/RumSessionPin.ts`** — pin-to-incident. `projectId` · `rumApplicationId` · `sessionId` · `pinnedByUserId` · `reason` · `incidentId`/`alertId` · `expiresAt` · `materializedAt`. Unique `(projectId, rumApplicationId, sessionId)`.

Each new model needs **four** registrations or it does not exist: `packages/Common/Models/DatabaseModels/Index.ts` (**both** the import block and the export array), a thin `DatabaseService` subclass, a `BaseAPI` router in `packages/App/FeatureSet/BaseAPI/Index.ts` following the `RumApplicationClient` block, and a **generated** migration (`npm run generate-postgres-migration`) imported and appended in `packages/Common/Server/Infrastructure/Postgres/SchemaMigrations/Index.ts`.

### Permissions

Two edits per member in `packages/Common/Types/Permission.ts`: the `enum Permission` block near the RUM entries, and one `PermissionProps` object in `PermissionHelper.getAllPermissionProps()` with `group: PermissionGroup.Telemetry`, `isAssignableToTenant: true`, `isAccessControlPermission: true`. `PermissionsArray` derives from `Object.keys` — no third registry.
...
**And the split must be enforced in the handler, not only on the column.** All three designs claimed defence-in-depth via the `payload` column's `accessControl.read` plus `ModelPermission.checkSelectPermission`. That is false on the only path that reads payloads: with `crudApiPath` omitted, the sole reader is a bespoke raw-SQL endpoint, and `ModelPermission` is never invoked. Worse, the obvious helper — `createTelemetryReadAccessGuard` in `packages/Common/Server/API/TelemetryAPI.ts:102-121` — is an OR-list that already contains `ProjectMember`, `Viewer` and `TelemetryViewer`, so copying it produces exactly the outcome we rejected. Write a dedicated `requireSessionReplayPayloadAccess` whose list is only `ProjectOwner`, `ProjectAdmin`, `TelemetryAdmin`, `ReadRumSessionReplayPayload`; derive `projectId` strictly from `databaseProps.tenantId`, never from the body; and resolve `rumApplicationId` from the session header server-side and check it against the caller's accessible app set so label/team scope actually applies.
...
1. `packages/Common/Types/AnalyticsDatabase/AnalyticsTableName.ts` — two enum entries.
2. `packages/Common/Server/Services/RumSessionService.ts` and `RumSessionChunkService.ts` — 10-line classes copied verbatim from `packages/Common/Server/Services/ProfileSampleService.ts`.
3. **`packages/Common/Server/Services/Index.ts` `AnalyticsServices`** (18 entries today, ending `AuditLogService`) — **this** is what boot `createTables()` iterates. Omit it and the tables are silently never created.
4. `packages/Common/Models/AnalyticsModels/Index.ts` `AnalyticsModels` — a *separate* registry, for Realtime lookup and name resolution.
5. `packages/Common/Server/Utils/AnalyticsDatabase/ClickhouseCapacity.ts` `PRUNABLE_LOCAL_TABLES` — add **both** local tables, and make replay explicitly the *first* pruning victim so a disk-pressure event never sacrifices `LogItemV3`/`SpanItemV3` for recordings.
...
New endpoints in `packages/Common/Server/API/TelemetryAPI.ts`, placed with the `/telemetry/profiles/flamegraph` cluster. Note the real mount: `TelemetryAPI` is mounted at `/api`, so paths are `/api/telemetry/rum/session-replay/*`.
...
**Sandboxing — mandatory security-review gate.** A replay is arbitrary, attacker-influenceable HTML authored by a customer's end users, and this repo has no CSP header, no CSP meta tag, and no `<iframe>` in `packages/Common/UI` or the Dashboard except the GTM `<noscript>` block in `index.ejs`. Mount into an iframe with **`sandbox="allow-same-origin"` and nothing else** — no `allow-scripts`, no `allow-forms`, no `allow-popups`, no `allow-top-navigation`. rrweb's `Replayer` runs in the *parent* and writes into `contentDocument`, so it needs same-origin access but **not** script execution inside; `slimDOMOptions.script: true` already neuters `<script>` at capture. Reviewers split here: one proposed `sandbox="allow-scripts"` without `allow-same-origin` plus an inner `default-src 'none'` CSP — that combination is self-blocking (it forbids the Replayer's own bootstrap) and denies the parent DOM access the Replayer requires. Add a `<meta http-equiv="Content-Security-Policy">` inside the replay document with `script-src 'none'; default-src 'none'; img-src data: blob:; style-src 'unsafe-inline'; font-src data:; media-src 'none'; connect-src 'none'` as belt-and-braces. Do **not** attempt a Dashboard-wide CSP in this project: `packages/App/FeatureSet/Dashboard/views/index.ejs` has an inline theme bootstrap, an inline `tailwind.config`, a ~150-line inline service-worker script with `onclick=` handlers, runtime Tailwind, and GTM — a `script-src 'self'` meta tag would break the app outright. That is a separate hardening effort needing nonces or a refactor.

### Dashboard files

**Create:**
- `packages/App/FeatureSet/Dashboard/src/Pages/Rum/View/SessionReplay.tsx` — list page. `Navigation.getLastParamAsObjectID(1)`, per `Pages/Rum/View/Clients.tsx`.
...
- `.../Components/SessionReplay/ReplayScrubber.tsx` — **hand-built**, 5 lanes (chunk/buffer state · frustration · errors · network 4xx/5xx · route changes). Drag/hover math modelled on `packages/Common/UI/Components/TelemetryViewer/components/TelemetryHistogram.tsx:121-171`; structural model `Components/Profiles/FlamegraphView.tsx` (929 lines of pure div rendering with a keyboard handler). Controls: play/pause, 1/2/4/8x, skip-inactive, jump-to-next-error, Space / ←→ ±10 s / `,.` ±1 s.
...
Migration: `packages/App/FeatureSet/Workers/DataMigrations/AddSessionIdToTelemetryTables.ts`, a direct copy of `AddScalarEntityKeysToTelemetryTables.ts` (`getColumnTypeInDatabase()` guard → `service.addColumnInDatabase(column)`, idempotent, metadata-only, `runsInClusterMode() => false`, no-op rollback, no backfill).
...
5. **Incidents** — add `Replay` to `packages/Common/Types/Telemetry/TelemetryType.ts` (closed 5-member enum) and a branch to the `telemetryType` switch in `Pages/Incidents/View/Index.tsx`, reusing the existing `Incident.telemetryQuery` jsonb envelope. No new Incident column.
...
Also extend `packages/Common/UI/Components/Telemetry/TelemetryRetentionConfigForm.tsx` and `TelemetryRetentionConfigSummary.tsx`.
...
1. `packages/Common/Types/MeteredPlan/ProductType.ts` → `SessionReplay = "Session Replay"`.
2. `packages/Common/Server/Types/Billing/MeteredPlan/AllMeteredPlans.ts` → a `TelemetryMeteredPlanType` instance **plus** a branch in `MeteredPlanUtil.getMeteredPlanByProductType`, which **throws `BadDataException`** on unknown types.
3. `packages/Common/Server/Services/BillingService.ts` `getMeteredPlanPriceId` → **test + live Stripe price ids**. It throws on unknown types, so replay billing is dead on arrival without them. **GA-blocking.**
4. `packages/Common/Server/Services/TelemetryUsageBillingService.ts` — the `updateUsageBilling` guard rejects any ProductType outside `{Traces, Metrics, Logs, Profiles}` (verified); miss it and **every staging write throws**.
5. Same file — the aggregation branch, next to the two-table Profiles block.
6. Same file — `getAverageRowSizeForProduct`, plus `AVERAGE_SESSION_REPLAY_SESSION_BYTES` (default 49152) in `packages/Common/Server/EnvironmentConfig.ts`. Fallback only; dead code by design since `payloadBytes` is never 0 for a real session.
7. `packages/App/FeatureSet/Workers/Jobs/MeteredPlan/ReportTelemetryMeteredPlan.ts` — one `reportQuantityToBillingProvider` + `Sleep.sleep(1000)` pair. No second cron.
8. `packages/App/FeatureSet/Dashboard/src/Pages/Settings/UsageHistory.tsx` — appears automatically; it builds its filter from the enum.
...
Gates that apply to **every** phase: `npm run fix` at the repo root; `npm run compile` in each changed project; and for Dashboard changes, `npx tsc --noEmit` **and** `node esbuild.config.js` inside `packages/App/FeatureSet/Dashboard`.
...
**Modify:** `packages/Common/Server/Utils/StartServer.ts` (both bypass predicates with `.includes("/session-replay/v1/")`; add `x-oneuptime-token` + `x-oneuptime-app-identifier` to `setDefaultHeaders`' `Access-Control-Allow-Headers`; add `Access-Control-Max-Age`) · `packages/Nginx/default.conf.template` (`location /session-replay`) · `packages/App/FeatureSet/Frontend/Index.ts` (`/session-replay` in `DashboardFallbackRoutePrefixesToSkip`) · `packages/App/FeatureSet/Docs/Content/en/telemetry/real-user-monitoring.md` (correct the false "browser.* attributes are added automatically" claim and the `/otlp` path disagreement) · `.github/workflows/compile.yml` (add `node esbuild.config.js` to `compile-dashboard`).

**Create:** `packages/E2E/Tests/App/SessionReplayIngressRoutes.spec.ts` (add `/session-replay` to `IngressRoutes.spec.ts`'s array and assert `recorder.js` returns JavaScript, not Dashboard HTML) · a `TestServer/` fixture page on a **different origin** that POSTs with `x-oneuptime-token`.
...
**Create:** `packages/Common/Models/AnalyticsModels/RumSession.ts` · `RumSessionChunk.ts` · `packages/Common/Server/Services/RumSessionService.ts` · `RumSessionChunkService.ts` · `packages/Common/Types/Rum/SessionReplay.ts` · `packages/Common/Types/Rum/SessionReplayMaskingMode.ts` · `packages/Common/Types/Rum/SessionReplayTriggerReason.ts` · `packages/Common/Utils/Rum/SessionSampling.ts` · `packages/Common/Utils/Rum/UrlScrubber.ts` · `packages/Common/Utils/Rum/Masking.ts` · `packages/Common/Utils/Rum/SessionIdentity.ts` · `packages/Common/Utils/Rum/ChunkMath.ts` · `packages/Common/Server/Utils/SessionReplay/SessionReplayRetention.ts` · `packages/Common/Tests/Models/AnalyticsModels/RumSessionSchema.test.ts` · `packages/Common/Tests/Utils/Rum/*.test.ts`.

**Modify:** `packages/Common/Types/AnalyticsDatabase/AnalyticsTableName.ts` · `packages/Common/Models/AnalyticsModels/Index.ts` · `packages/Common/Server/Services/Index.ts` (`AnalyticsServices`) · `packages/Common/Types/Permission.ts` · `packages/Common/Types/Telemetry/TelemetryRetentionConfig.ts` · `packages/Common/Server/Utils/AnalyticsDatabase/ClickhouseCapacity.ts`.
...
**Create:** `packages/App/FeatureSet/Telemetry/API/SessionReplayIngest.ts` · `Middleware/SessionReplayRequestMiddleware.ts` · `Services/SessionReplayIngestService.ts` · `Services/SessionReplayScrubService.ts` · `Utils/SessionReplayChunkStore.ts` · `Utils/SessionReplayEnvelopeParser.ts` · `packages/Common/Server/Utils/SessionReplay/SessionReplayGateCache.ts` · `SessionReplayRateLimiter.ts` · `packages/App/FeatureSet/Workers/Jobs/Rum/FinalizeSessions.ts` · `packages/App/FeatureSet/Workers/Jobs/Rum/CleanupStaleResources.ts` (wire the orphaned `RumApplicationService.markDisconnectedApplications()`, which has zero callers, so the RUM status pill stops reading "Connected" forever).

**Modify:** `packages/App/FeatureSet/Telemetry/Index.ts` · `Config.ts` · `Services/Queue/TelemetryQueueService.ts` · `Jobs/TelemetryIngest/ProcessTelemetry.ts` (new case; **do not** add to `useInsertDedup`) · `packages/Common/Server/Utils/Telemetry/TelemetryFanInWriter.ts` (`maxBatchRowsByTable`) · `packages/Common/Models/DatabaseModels/RumApplication.ts` (13 columns) · `packages/Common/Models/DatabaseModels/Project.ts` · `packages/App/FeatureSet/Workers/Index.ts` · `packages/App/FeatureSet/Telemetry/Services/LogScrubRuleService.ts` (export `BUILT_IN_PATTERNS`, `SENSITIVE_KEY_REGEX`; widen `applyScrubAction`) · `config.example.env`.

**Migrations:** `npm run generate-postgres-migration` for the `RumApplication` + `Project` columns; import and append the generated class in `packages/Common/Server/Infrastructure/Postgres/SchemaMigrations/Index.ts` or it never runs at startup.
...
**Create:** `packages/App/FeatureSet/BrowserRecorder/{package.json,package-lock.json,tsconfig.json,jest.config.json,esbuild.config.js,README.md}` · `src/{Index,Loader,Recorder,RollingBuffer,Chunker,Transport,Config,Consent,FrustrationDetector,ErrorRecorder,NetworkRecorder,ConsoleRecorder,RouteRecorder,SessionId,Masking}.ts` · `Tests/*.test.ts` · `packages/App/FeatureSet/Docs/Content/en/telemetry/session-replay.md` · `.github/workflows/test.browser-recorder.yaml`.

**Modify:** `packages/App/package.json` (`build-frontend:browser-recorder` + `:prod`, added to `build-frontends` and `build-frontends:prod`) · `packages/App/Dockerfile.tpl` (one `npm ci` layer + one `COPY --chown=1000:1000 ./packages/App/FeatureSet/BrowserRecorder /usr/src/app/FeatureSet/BrowserRecorder`) · `.github/workflows/compile.yml` (`compile-browser-recorder`) · `packages/App/FeatureSet/Docs/Utils/Nav.ts` · `packages/App/FeatureSet/Dashboard/src/Components/TelemetryResource/documentationMarkdown.ts` (`getRumDocMarkdown`) · `packages/App/FeatureSet/Dashboard/src/Index.tsx` + `packages/Common/UI/Utils/Telemetry/Telemetry.ts` (dogfood `session.id`).

`packages/App/scripts/frontend-run.sh` needs **no** change — it already resolves `FeatureSet/<X>`. `Scripts/Install/SyncPackageVersions.js` needs **no** change — it recursively walks every `package.json`.
...
**Create:** the eleven Dashboard files in §7 · `packages/App/Tests/Dashboard/SessionReplayRoutes.test.ts` · `packages/Common/Tests/UI/Rum/ChunkLoader.test.ts` · `packages/Common/Tests/UI/Rum/SeekAnchor.test.ts` · `packages/E2E/Tests/Dashboard/SessionReplay.spec.ts`.

**Modify:** `packages/Common/Server/API/TelemetryAPI.ts` (five endpoints + `requireSessionReplayPayloadAccess`) · the six routing files · `Pages/Rum/View/Overview.tsx` · `packages/App/FeatureSet/Dashboard/package.json` + `package-lock.json`.
...
**Create:** `packages/App/FeatureSet/Workers/DataMigrations/AddSessionIdToTelemetryTables.ts` · `packages/Common/Models/DatabaseModels/RumSessionReplayView.ts` · `RumSessionErasureRequest.ts` · `RumSessionPin.ts` · their three services · `packages/App/FeatureSet/Workers/Jobs/Rum/ProcessSessionErasureRequests.ts` · `MaterializePinnedSessions.ts` · `packages/App/FeatureSet/Dashboard/src/Pages/Rum/Settings/SessionReplay.tsx` · `Pages/Rum/View/SessionReplayAudit.tsx` · `Components/SessionReplay/ReplayCard.tsx`.

**Modify:** `packages/Common/Models/AnalyticsModels/{Span,Log,ExceptionInstance}.ts` · `packages/App/FeatureSet/Telemetry/Services/OtelTracesIngestService.ts` (`ExceptionEventPayload` + construction site + `buildExceptionRow` + `buildSpanRow`) · `OtelLogsIngestService.ts` (log row + `collectExceptionFromLog`) · `packages/App/FeatureSet/Workers/DataMigrations/Index.ts` · `packages/Common/Types/Telemetry/TelemetryType.ts` · `packages/Common/Models/DatabaseModels/IncidentFeed.ts` · `Pages/Incidents/View/Index.tsx` · `Components/Exceptions/{ExceptionExplorer,ExceptionInstanceTable,OccuranceTable}.tsx` · `Components/Logs/LogsViewer.tsx` · `packages/Common/Types/AuditLog/AuditLogAction.ts` (+ `Read`) · `packages/Common/Server/Services/AuditLogService.ts` · `packages/Common/Types/MeteredPlan/ProductType.ts` · `AllMeteredPlans.ts` · `BillingService.ts` · `TelemetryUsageBillingService.ts` · `packages/Common/Server/EnvironmentConfig.ts` · `ReportTelemetryMeteredPlan.ts` · `packages/Common/UI/Components/Telemetry/TelemetryRetentionConfig{Form,Summary}.tsx` · `packages/Common/Models/DatabaseModels/Index.ts` · `packages/App/FeatureSet/BaseAPI/Index.ts` · `packages/Common/Server/Services/RumApplicationService.ts` (`onBeforeDelete` → enqueue erasure; `onUpdateSuccess` → `clearCache`).
...
1. ~~**Default capture trigger — error-triggered or always-on?**~~ **RESOLVED: always-on.** The default is now `Always` with `sessionReplaySamplePercentage: 100` (migration `1791400000000-SessionReplayRecordEverySessionByDefault`, which also backfills applications still sitting on the old pair). `OnErrorOrFrustration` at 0% was ~15x cheaper and it was the wrong shape for the product: the session list filled with rows carrying signals and no footage, "Metadata only" was the normal badge, and a session with no recording is indistinguishable from a feature that does not work. It also answered only half of what people open a replay for — the abandoned checkout and the page that "looked wrong" never threw. Sample percentage is now the cost dial, and the error-triggered mode remains one dropdown away for anyone who wants data minimisation more than coverage.
