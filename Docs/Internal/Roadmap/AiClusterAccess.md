...
| Model | `KubernetesCluster` (+ `RunnerJob.kubernetesClusterId`, `AutoRemediationSuggestion.kubernetesClusterId`) | `aiAccessRunnerId`, `aiAccessCredentialId`, `isAiInvestigationEnabled`, `aiRemediationMode` (Disabled / RequireApproval / Automatic / BypassApproval), `aiKubectlCommandAllowlist`, `aiAccessLastVerifiedAt`, `aiAccessLastError`. Migration `AddKubernetesClusterAiAccess1794000000000`. |
