...
A snippet of JavaScript run in a sandboxed `isolated-vm`. The sandbox lives on a [Runbook Agent](./agents.md) inside your own infrastructure — not on the OneUptime Worker.
...
A bash script (`bash -c <script>`) run on a [Runbook Agent](./agents.md) in your own infrastructure. Bash never executes on the OneUptime Worker.
