...
const ArgumentsForm: FunctionComponent<ComponentProps> = (
    ...
    const [_showComponentPickerModal, setShowComponentPickerModal] = useState<boolean>(false);
            ...
            <div className='mt-5 mb-5'>
                ...
                {component.metadata.arguments && component.metadata.arguments.length > 0 && <BasicForm
                    ...
                    fields={component.metadata.arguments && component.metadata.arguments.map((arg: Argument) => {
                        return {
                            title: `${arg.name}`,
                            footerElement: (<div className='text-gray-500'>
                                <p>Pick this value from other <a className='underline text-blue-500 hover:text-blue-600 cursor-pointer' onClick={() => {
                                    setSelectedArgId(arg.id);
                                    setShowComponentPickerModal(true);
                                }}>component</a> or from <a className='underline text-blue-500 hover:text-blue-600 cursor-pointer' onClick={() => {
                                    setSelectedArgId(arg.id);
                            ...
                            ...componentInputTypeToFormFieldType(arg.type)
                        }
                    })}
                />}
            </div>
            {showVariableModal && <VariableModal workflowId={props.workflowId} onClose={()=>{
                setShowVariableModal(false);
            }} onSave={(variableId: string)=>{
                formRef.current.setFieldValue(selectedArgId, variableId);
            }}/>}
        </div>
    );
};
