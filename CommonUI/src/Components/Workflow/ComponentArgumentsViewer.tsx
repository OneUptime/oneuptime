...
const ComponentArgumentViewer: FunctionComponent<ComponentProps> = (
    ...
    return (
        <div className='mt-5 mb-5'>
            ...
            {props.arguments && props.arguments.length === 0 && <ErrorMessage error={'This component does not take any arguments.'} />}
            <div className="mt-3">
                {props.arguments && props.arguments.length > 0 && props.arguments.map((argument: Argument) => {
                    return (
                        <div className="mt-2 mb-2 relative flex items-center space-x-3 rounded-lg border border-gray-300 bg-white px-6 py-5 shadow-sm focus-within:ring-2 focus-within:ring-pink-500 focus-within:ring-offset-2">
                            <div className="min-w-0 flex-1 flex justify-between">
                                <div className="focus:outline-none">
                                    ...
                                    <p className="text-sm font-medium text-gray-900">{argument.name} <span className='text-gray-500 font-normal'>(ID: {argument.id})</span></p>
                                    <p className="truncate text-sm text-gray-500">{argument.required ? "Required. ": "Optional. "}{argument.description}</p>
                                </div>
                                <div>
                                    <Pill color={Black} text={argument.type} />
                                    </div>
                            </div>
                        </div>
                    )
