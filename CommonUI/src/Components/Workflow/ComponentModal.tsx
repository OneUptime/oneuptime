...
const ComponentsModal: FunctionComponent<ComponentProps> = (
    ...
    useEffect(() => {
        setComponents(Components.filter((component) => component.componentType === props.componentsType));
    }, []);
    ...
    useEffect(() => {
        ...
        setComponents(Components.filter((component) => {
            return component.componentType === props.componentsType && (component.title.toLowerCase().includes(search.trim().toLowerCase()) || component.description.toLowerCase().includes(search.trim().toLowerCase()) || component.category.toLowerCase().includes(search.trim().toLowerCase()))
        }));
    ...
    return <Modal
        ...
        rightElement={<div>
            <Input placeholder='Search...' onChange={(text) => {
                setSearch(text);
            }} />
        </div>}
    >
        ...
        <>
            <div>
                ...
                <div className='flex flex-wrap h-[60rem] overflow-y-auto overflow-x-hidden pt-10 pb-10'>
                    {!components || components.length === 0 && <p className='text-sm text-gray-400'>No components that match your search.</p>}
                    {components && components.length > 0 && components.map((component: Component, i: number) => {
                        return (<div className='m-5'><ComponentElement key={i} data={{
