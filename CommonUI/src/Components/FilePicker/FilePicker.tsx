...
const FilePicker: FunctionComponent<ComponentProps> = (
    ...
    const getThumbs = (): Array<ReactElement> => {
        return filesModel.map((file: FileModel, i: number) => {
            ...
            const blob = new Blob([new Uint8Array((file.file as any).data).buffer], {type: file.type as string});
            ...
            return (
                <div key={file.name} className="file-picker-thumb">
                    <div className='file-picker-delete-logo'>
                        <Icon style={{
                            marginTop: "-5px"
                        }} icon={IconProp.Close} color={White} thick={ThickProp.Thick} size={SizeProp.Regular} onClick={() => {
    ...
    return (
        ...
        >
            <section className="container">
                {props.isMultiFilePicker || filesModel.length === 0 && <div {...getRootProps({ className: 'file-picker-dropzone' })}>
                    <input {...getInputProps()} />
                ...
                <aside className="file-picker-thumbs-container">{getThumbs()}</aside>
            </section>
        </div>
    );
};
