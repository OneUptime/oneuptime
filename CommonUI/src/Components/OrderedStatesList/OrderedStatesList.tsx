...
const OrderedStatesList: FunctionComponent<ComponentProps> = (
    ...
    return (
        <div>
            ...
            {props.items && props.items.length > 0 && props.items.map((item: ListItem, i: number) => {
                return (<div>
                    <Item
                        item={item}
                    ...
                    {props.onCreateNewItem && <div><Item
                        onClick={(item) => {
                            props.onCreateNewItem && props.onCreateNewItem(parseInt(item.id))
                        }}
                        item={{
                            text: "Create",
                            id: i.toString()
                        }} />
                        <div className="vertical-list"></div></div>}
                </div>);
            })}


            {!props.items || props.items.length === 0 && <p> {props.noItemsMessage || 'No items in this list.'}</p>}
        </div>
    );
};
