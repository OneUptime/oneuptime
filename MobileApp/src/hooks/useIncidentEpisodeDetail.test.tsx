...
/*
 * These five hooks are the whole data layer behind the incident episode detail
 ...
 * incidentEpisodes api suite.
 */

jest.mock("../api/incidentEpisodes", () => {
  return {
    fetchIncidentEpisodeById: jest.fn(),
    fetchIncidentEpisodeStates: jest.fn(),
    fetchIncidentEpisodeStateTimeline: jest.fn(),
    fetchIncidentEpisodeNotes: jest.fn(),
    fetchIncidentEpisodeFeed: jest.fn(),
  };
});

const PROJECT_A: string = "project-a";
const PROJECT_B: string = "project-b";
const EPISODE_1: string = "incident-episode-1";
const EPISODE_2: string = "incident-episode-2";

const byIdMock: jest.Mock = fetchIncidentEpisodeById as unknown as jest.Mock;
const statesMock: jest.Mock =
  fetchIncidentEpisodeStates as unknown as jest.Mock;
const timelineMock: jest.Mock =
  fetchIncidentEpisodeStateTimeline as unknown as jest.Mock;
const notesMock: jest.Mock = fetchIncidentEpisodeNotes as unknown as jest.Mock;
const feedMock: jest.Mock = fetchIncidentEpisodeFeed as unknown as jest.Mock;
...
beforeEach(() => {
  ...
  statesMock.mockImplementation((projectId: string) => {
    return Promise.resolve([
      makeIncidentState({ _id: `${projectId}-incident-state` }),
    ]);
  });
  timelineMock.mockImplementation((projectId: string, episodeId: string) => {
    return Promise.resolve([
      makeStateTimelineItem({ _id: `${projectId}-${episodeId}-timeline` }),
    ]);
  });
...
describe("useIncidentEpisodeDetail", () => {
  test("hands the caller the episode the api resolved, unchanged", async () => {
    ...
    const { result } = await renderHook(
      () => {
        return useIncidentEpisodeDetail(PROJECT_A, EPISODE_1);
      },
  ...
  test("caches the episode under a key naming both the project and the episode", async () => {
    ...
    /*
     * IncidentEpisodeDetailScreen writes an optimistic state change straight
     * into this exact key by hand, so the literal matters beyond cache
     * identity: if the hook's key moved, the screen's optimistic update would
     * land on a cache entry nothing is reading.
     */
    const cached: IncidentEpisodeItem | undefined = client.getQueryData([
      "incident-episode",
      PROJECT_A,
      EPISODE_1,
    ]);
  ...
  test("a second episode in the same project does not inherit the first one's cache", async () => {
    ...
    const { result: second } = await renderHook(
      () => {
        return useIncidentEpisodeDetail(PROJECT_A, EPISODE_2);
      },
      { wrapper: createQueryWrapper(client) },
    );
    await waitFor(() => {
  ...
  test("the same episode id under a different project is fetched separately", async () => {
    ...
    const { result: inProjectB } = await renderHook(
      () => {
        return useIncidentEpisodeDetail(PROJECT_B, EPISODE_1);
      },
      { wrapper: createQueryWrapper(client) },
    );
    await waitFor(() => {
  ...
  test("does not fetch anything before a project is known", async () => {
    ...
    const { result } = await renderHook(
      () => {
        return useIncidentEpisodeDetail("", EPISODE_1);
      },
      { wrapper: createQueryWrapper(client) },
    );
  ...
  test("does not fetch anything before an episode is known", async () => {
    ...
    const { result } = await renderHook(
      () => {
        return useIncidentEpisodeDetail(PROJECT_A, "");
      },
      { wrapper: createQueryWrapper(client) },
    );
  ...
  test("surfaces an api rejection as an error rather than as a missing episode", async () => {
     ...
     */
    const failure: Error = new Error("incident episode fetch failed");
  ...
  test("an episode that no longer exists settles as null data, not as a failure", async () => {
     ...
     * `fetchIncidentEpisodeById` ends in `?? null`. react-query v5 caches
     * `null` like any other value but REFUSES `undefined`, rejecting the query
     * with a synthetic "data is undefined" error - so a deleted episode used to
     * arrive at IncidentEpisodeDetailScreen through isError, wearing the same
     * clothes as the failure above and told apart only by the wording of a
    ...
    const { result } = await renderHook(
      () => {
        return useIncidentEpisodeDetail(PROJECT_A, "deleted-episode");
      },
      { wrapper: createQueryWrapper(client) },
    );
...
describe("useIncidentEpisodeStates", () => {
  /*
   * The literal key of this hook is deliberately not pinned here. It is
   * ["incident-states", projectId] today, which is byte for byte the key
   * useIncidentStates in useIncidentDetail.ts uses for a different api
   * function - see the concern raised alongside this suite. Freezing that
   * literal into a test would make the collision harder to fix, so what
   * follows asserts only what has to hold whichever name the key ends up
   * with: the project is part of it, the guard works, and the rows reach the
   * caller.
   */
  test("hands the caller the states the api resolved, unchanged", async () => {
    const states: IncidentState[] = [
      makeIncidentState({ _id: "created", name: "Created", order: 1 }),
      makeIncidentState({
        _id: "acknowledged",
        name: "Acknowledged",
    ...
    const { result } = await renderHook(
      () => {
        return useIncidentEpisodeStates(PROJECT_A);
      },
      { wrapper: createQueryWrapper(client) },
    );
  ...
  test("keeps each project's states apart", async () => {
    /*
     * Incident states are per project rows, and this list is what the detail
    ...
    const { result: inProjectB } = await renderHook(
      () => {
        return useIncidentEpisodeStates(PROJECT_B);
      },
      { wrapper: createQueryWrapper(client) },
    );
    await waitFor(() => {
    ...
    const statesForB: IncidentState[] | undefined = inProjectB.current.data;
    const statesForA: IncidentState[] | undefined = inProjectA.current.data;
    expect(statesForB?.[0]?._id).toBe(`${PROJECT_B}-incident-state`);
    expect(statesForA?.[0]?._id).toBe(`${PROJECT_A}-incident-state`);
  });
  ...
  test("does not fetch anything before a project is known", async () => {
    ...
    const { result } = await renderHook(
      () => {
        return useIncidentEpisodeStates("");
      },
      { wrapper: createQueryWrapper(client) },
    );
  ...
  test("surfaces an api rejection as an error rather than as an empty list", async () => {
     ...
     */
    const failure: Error = new Error("incident states fetch failed");
...
describe("useIncidentEpisodeStateTimeline", () => {
  test("hands the caller the timeline rows under the episode's own key", async () => {
    const client: QueryClient = createTestQueryClient();

    const { result } = await renderHook(
      () => {
        return useIncidentEpisodeStateTimeline(PROJECT_A, EPISODE_1);
      },
      { wrapper: createQueryWrapper(client) },
    );
  ...
  test("a second episode does not inherit the first one's timeline", async () => {
    ...
    const { result: second } = await renderHook(
      () => {
        return useIncidentEpisodeStateTimeline(PROJECT_A, EPISODE_2);
      },
      { wrapper: createQueryWrapper(client) },
    );
    await waitFor(() => {
  ...
  test("does not fetch anything before a project is known", async () => {
    ...
    const { result } = await renderHook(
      () => {
        return useIncidentEpisodeStateTimeline("", EPISODE_1);
      },
      { wrapper: createQueryWrapper(client) },
    );
  ...
  test("does not fetch anything before an episode is known", async () => {
    ...
    const { result } = await renderHook(
      () => {
        return useIncidentEpisodeStateTimeline(PROJECT_A, "");
      },
      { wrapper: createQueryWrapper(client) },
    );
  ...
  test("surfaces an api rejection as an error rather than as an empty timeline", async () => {
    const failure: Error = new Error("incident episode timeline fetch failed");
...
describe("useIncidentEpisodeNotes", () => {
  test("hands the caller the notes under the episode's own key", async () => {
    const client: QueryClient = createTestQueryClient();

    const { result } = await renderHook(
      () => {
        return useIncidentEpisodeNotes(PROJECT_A, EPISODE_1);
      },
      { wrapper: createQueryWrapper(client) },
    );
  ...
  test("a second episode does not inherit the first one's notes", async () => {
    ...
    const { result: second } = await renderHook(
      () => {
        return useIncidentEpisodeNotes(PROJECT_A, EPISODE_2);
      },
      { wrapper: createQueryWrapper(client) },
    );
    await waitFor(() => {
  ...
  test("does not fetch anything before a project is known", async () => {
    ...
    const { result } = await renderHook(
      () => {
        return useIncidentEpisodeNotes("", EPISODE_1);
      },
      { wrapper: createQueryWrapper(client) },
    );
  ...
  test("does not fetch anything before an episode is known", async () => {
    ...
    const { result } = await renderHook(
      () => {
        return useIncidentEpisodeNotes(PROJECT_A, "");
      },
      { wrapper: createQueryWrapper(client) },
    );
  ...
  test("surfaces an api rejection as an error rather than as no notes", async () => {
    const failure: Error = new Error("incident episode notes fetch failed");
...
describe("useIncidentEpisodeFeed", () => {
  test("hands the caller the feed under the episode's own key", async () => {
    const client: QueryClient = createTestQueryClient();

    const { result } = await renderHook(
      () => {
        return useIncidentEpisodeFeed(PROJECT_A, EPISODE_1);
      },
      { wrapper: createQueryWrapper(client) },
    );
  ...
  test("a second episode does not inherit the first one's feed", async () => {
    ...
    const { result: second } = await renderHook(
      () => {
        return useIncidentEpisodeFeed(PROJECT_A, EPISODE_2);
      },
      { wrapper: createQueryWrapper(client) },
    );
    await waitFor(() => {
  ...
  test("does not fetch anything before a project is known", async () => {
    ...
    const { result } = await renderHook(
      () => {
        return useIncidentEpisodeFeed("", EPISODE_1);
      },
      { wrapper: createQueryWrapper(client) },
    );
  ...
  test("does not fetch anything before an episode is known", async () => {
    ...
    const { result } = await renderHook(
      () => {
        return useIncidentEpisodeFeed(PROJECT_A, "");
      },
      { wrapper: createQueryWrapper(client) },
    );
  ...
  test("surfaces an api rejection as an error rather than as an empty feed", async () => {
    const failure: Error = new Error("incident episode feed fetch failed");
...
describe("the incident episode hooks used together, as the detail screen uses them", () => {
  test("the four episode-scoped hooks each keep their own cache entry", async () => {
    ...
    const { result } = await renderHook(
      () => {
        return {
          detail: useIncidentEpisodeDetail(PROJECT_A, EPISODE_1),
          timeline: useIncidentEpisodeStateTimeline(PROJECT_A, EPISODE_1),
          notes: useIncidentEpisodeNotes(PROJECT_A, EPISODE_1),
          feed: useIncidentEpisodeFeed(PROJECT_A, EPISODE_1),
        };
      },
      { wrapper: createQueryWrapper(client) },
    );
  ...
  test("nothing at all is fetched while the project id is still empty", async () => {
    ...
    await renderHook(
      () => {
        return {
          detail: useIncidentEpisodeDetail("", EPISODE_1),
          states: useIncidentEpisodeStates(""),
          timeline: useIncidentEpisodeStateTimeline("", EPISODE_1),
          notes: useIncidentEpisodeNotes("", EPISODE_1),
          feed: useIncidentEpisodeFeed("", EPISODE_1),
        };
      },
      { wrapper: createQueryWrapper(client) },
    );
