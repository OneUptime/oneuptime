...
async function renderIncidentsScreenDark(): Promise<void> {
...
describe("The incidents/episodes switch", () => {
  beforeEach(() => {
    mockIncidents.current = incidentsWith({ items: [activeIncident()] });
    mockEpisodes.current = episodesWith({ items: [activeEpisode()] });
  });
  ...
  test("both views are always offered as buttons, with the current one selected", async () => {
    ...
    expect(isSelected(screen.getByRole("button", { name: "Incidents" }))).toBe(
      false,
    );
  ...
  test("each half explains what it shows to a screen reader", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Incidents" }).props.accessibilityHint,
    ).toBe("Show individual incidents");
    expect(
      screen.getByRole("button", { name: "Episodes" }).props.accessibilityHint,
    ).toBe("Show related incidents grouped into episodes");
  });

  test("the line under the switch describes the list that is showing", async () => {
    await renderIncidentsScreen();

    expect(screen.getByText("Each incident, listed on its own.")).toBeTruthy();
    expect(
      screen.queryByText("Related incidents, grouped into episodes."),
    ).toBeNull();

    await fireEvent.press(screen.getByRole("button", { name: "Episodes" }));

    expect(
      screen.getByText("Related incidents, grouped into episodes."),
    ).toBeTruthy();
    expect(screen.queryByText("Each incident, listed on its own.")).toBeNull();
  });

  test("choosing the view that is already showing changes nothing", async () => {
    await renderIncidentsScreen();

    await fireEvent.press(screen.getByRole("button", { name: "Incidents" }));
  ...
  test("the count is the full section total, not the rendered page", async () => {
    mockIncidents.current = incidentsWith({
      items: Array.from({ length: 30 }, (_: unknown, index: number) => {
        return {
          ...activeIncident(),
          item: { ...activeIncident().item, _id: `active-${index}` },
        };
      }),
    });
    ...
    expect(
      screen.getAllByRole("button", { name: ACTIVE_INCIDENT_LABEL }).length,
...
describe("The acknowledge swipe as drawn", () => {
  ...
  test("a resolved row has no swipe panel behind it at all", async () => {
    mockIncidents.current = incidentsWith({ items: [resolvedIncident()] });
    await renderIncidentsScreen();
...
describe("In dark mode", () => {
  ...
  test("the canvas, headings, counts and cards read from the dark palette", async () => {
    await renderIncidentsScreenDark();
    ...
    expect(screen.getByLabelText(ACTIVE_INCIDENT_LABEL)).toHaveStyle({
  ...
  test("the view switch uses the dark track and raised segment", async () => {
    ...
    expect(screen.getByTestId("response-view-incidents")).toHaveStyle({
      backgroundColor: darkColors.backgroundElevated,
    });
  });
