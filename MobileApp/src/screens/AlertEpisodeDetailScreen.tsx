...
export default function AlertEpisodeDetailScreen({
  ...
  } = useAlertEpisodeStates(projectId);
  ...
  const {
    ...
    isError: feedError,
    refetch: refetchFeed,
  } = useAlertEpisodeFeed(projectId, episodeId);
  const {
    data: notes,
    ...
    return (
      ...
      >
        <EmptyState
          ...
          subtitle="This alert episode no longer exists, or it is not part of this project."
          icon="episodes"
        />
      </ScrollView>
    );
  ...
  return (
    ...
    >
      <ResponseDetailHeader
        ...
        meta={describeResponseAge("Created", episode.createdAt)}
      />
      {statesError ? (
        <QueryErrorNotice
