...
const CLICK_ID_KEYS: Array<string> = ["gclid", "wbraid", "gbraid"];
const TOUCH_STRING_KEYS: Array<string> = [
  "utmSource", "utmMedium", "utmCampaign", "utmTerm", "utmContent", "channel",
];
...
const sanitizeStringValue: (value: JSONValue, max?: number) => string | null = (
  ...
  if (typeof value === "string" && value.trim().length > 0) return value.trim().slice(0, max);
  if (typeof value === "number") return value.toString().slice(0, max);
  return null;
};

const sanitizeUrl: (value: JSONValue) => string | null = (value: JSONValue): string | null => {
  ...
  try {
    ...
    parsed.password = "";
    parsed.hash = "";
    CLICK_ID_KEYS.concat(["utm_source", "utm_medium", "utm_campaign", "utm_term", "utm_content"]).forEach((key: string) => parsed.searchParams.delete(key));
...
export default class Attribution {
  ...
  public static sanitizeTouch(value: JSONValue): JSONObject | undefined {
    ...
    const landingPage: string | null = sanitizeUrl(input["landingPage"] || input["landingUrl"]);
    ...
    if (timestamp && !Number.isNaN(Date.parse(timestamp))) result["timestamp"] = new Date(timestamp).toISOString();
    const clickIds: JSONObject | undefined = this.sanitizeClickIds(input["clickIds"]);
    if (clickIds) result["clickIds"] = clickIds;
    return Object.keys(result).length > 0 ? result : undefined;
  }

  public static sanitizeFirstTouchAttribution(value: JSONValue): JSONObject | undefined {
    if (
      value &&
      typeof value === "object" &&
      !Array.isArray(value) &&
      (value as JSONObject)["anonymousVisitorId"]
    ) {
      return this.sanitizeAcquisitionAttribution(value);
    }
    return this.sanitizeTouch(value);
  }

  public static sanitizeAcquisitionAttribution(value: JSONValue): JSONObject | undefined {
    if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
    const input: JSONObject = value as JSONObject;
    const result: JSONObject = {};
    const visitorId: string | null = sanitizeStringValue(input["anonymousVisitorId"], 100);
    if (visitorId && /^[a-zA-Z0-9_-]{16,100}$/.test(visitorId)) result["anonymousVisitorId"] = visitorId;
    const consentState: JSONValue = input["consentState"];
    if (Object.values(AttributionConsentState).includes(consentState as AttributionConsentState)) result["consentState"] = consentState;
