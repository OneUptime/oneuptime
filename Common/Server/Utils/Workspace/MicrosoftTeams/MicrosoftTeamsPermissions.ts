...
export const PERMISSION_DESCRIPTIONS: { [key: string]: string } = {
  ...
  "openid": "Returns an id_token so we can read tenant id - required for OAuth flow",
  "profile": "Basic user profile claims - required for user identification",
  "email": "View users' email address - used for user notifications and identification", 
  ...
  "Team.ReadBasic.All": "Read the names and descriptions of teams - enables team discovery",
  "Channel.ReadBasic.All": "Read the names and descriptions of channels - enables channel discovery",
  "ChannelMessage.Send": "Send channel messages as the signed-in user",
  "TeamMember.ReadWrite.All": "Add and remove members from teams - enables user management",
  "Teamwork.Read.All": "Read organizational teamwork settings - provides additional context",

  // Application permissions
  "Channel.Create": "Create channels - enables automatic channel creation for incidents",
  "Channel.Delete.All": "Delete channels - enables cleanup and archival operations",
  "ChannelMember.Read.All": "Read the members of all channels - enables membership verification",
  "ChannelMember.ReadWrite.All": "Add and remove members from all channels - enables automated user management",
  "ChannelMessage.Read.All": "Read all channel messages - enables message monitoring and context",
  "ChannelMessage.UpdatePolicyViolation.All": "Flag channel messages for violating policy - compliance feature",
  "ChatMessage.Read.All": "Read all chat messages - enables direct message monitoring",
  "Team.ReadBasic.All.App": "Get a list of all teams - enables team discovery for app operations",
  "TeamMember.Read.All": "Read the members of all teams - enables team membership verification",
  "TeamMember.ReadWrite.All.App": "Add and remove members from all teams - enables automated team management",
  "Teamwork.Migrate.All": "Create chat and channel messages with anyone's identity and with any timestamp - advanced messaging features",
  "Teamwork.Read.All.App": "Read organizational teamwork settings - provides organizational context"
};

// Categories for permission organization
export const PERMISSION_CATEGORIES = {
  AUTHENTICATION: ["openid", "profile", "email", "offline_access", "User.Read"],
  TEAM_ACCESS: ["Team.ReadBasic.All", "TeamMember.Read.All", "TeamMember.ReadWrite.All"],
  CHANNEL_ACCESS: ["Channel.ReadBasic.All", "Channel.Create", "Channel.Delete.All", "ChannelMember.Read.All", "ChannelMember.ReadWrite.All"],
  MESSAGING: ["ChannelMessage.Send", "ChannelMessage.Read.All", "ChannelMessage.UpdatePolicyViolation.All", "ChatMessage.Read.All"],
...
export function validateApplicationTokenPermissions(token: string): {
  ...
  try {
    ...
    const missingRoles = REQUIRED_APPLICATION_PERMISSIONS.filter(
      permission => !tokenRoles.includes(permission)
    );
    
    const extraRoles = tokenRoles.filter(
      role => !REQUIRED_APPLICATION_PERMISSIONS.includes(role) && 
              !role.startsWith('Directory.') && // Exclude common directory roles
              !role.startsWith('Application.')   // Exclude application management roles
    );
...
export function validateDelegatedTokenPermissions(token: string): {
  ...
  try {
    const payload = decodeJWTToken(token);
    const tokenScopes: Array<string> = (payload.scp || payload.scope || "").split(" ").filter(Boolean);
...
export function generatePermissionChecklist(): string {
  let checklist = "Microsoft Teams Integration - Azure AD Permission Checklist\n";
  checklist += "=".repeat(60) + "\n\n";
  
  checklist += "DELEGATED PERMISSIONS (add these to 'API permissions' → 'Microsoft Graph' → 'Delegated permissions'):\n";
  checklist += "-".repeat(40) + "\n";
  ...
  checklist += "☐ Verify all permissions show green checkmark (granted status)\n";
  checklist += "☐ Generate client secret and copy the SECRET VALUE (not Secret ID)\n";
  checklist += "☐ Copy Application (client) ID from app registration overview\n";
  checklist += "☐ Configure OneUptime environment variables with client ID and secret\n";
