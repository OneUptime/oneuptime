...
export default class AlertWorkspaceMessages {
  ...
  public static async createChannelsAndInviteUsersToChannels(data: {
    ...
    try {
      ...
      logger.error("Error in createChannelsAndInviteUsersToChannels", { projectId: data.projectId?.toString(), alertId: data.alertId?.toString() });
      logger.error(err, { projectId: data.projectId?.toString(), alertId: data.alertId?.toString() });
      return null;
    }
  }
