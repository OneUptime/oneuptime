...
export default class GreenlockUtil {
  ...
  public static async orderCert(data: {
    ...
    try {
      ...
      if (IsBillingEnabled) {
        throw new ServerException(
          `Unable to order certificate for ${data.domain}. Error: ${errorMessage}. Please contact support at support@oneuptime.com for more information.`,
        );
      } else {
        throw new ServerException(
          `Unable to order certificate for ${data.domain}. Error: ${errorMessage}. Please make sure that your server can be accessed publicly over port 80 (HTTP) and port 443 (HTTPS). If the problem persists, please refer to server logs for more information. Please also set up LOG_LEVEL=DEBUG to get more detailed server logs.`,
        );
      }
    }
  }
