...
} from "../../../../../Types/AI/SentinelInsightStatus";
...
/*
 * Sentinel Insights — the dedupe/upsert store between detectors and the
 * SentinelInsight table. Detectors re-emit the same finding on every scan
 ...
 * SentinelInsight column definitions (LongText/ShortText).
 */
  ...
  public static async upsertCandidates(data: {
    ...
    for (const candidate of data.candidates) {
      ...
      try {
         ...
         */
        const existing: SentinelInsight | null =
          await SentinelInsightService.findOneBy({
            query: {
        ...
        if (
          existing &&
          existing.status &&
          !SentinelInsightStatusHelper.isTerminalStatus(existing.status)
        ) {
          /*
        ...
        if (existing && existing.status === SentinelInsightStatus.Dismissed) {
          /*
          ...
          if (withinCooldown) {
            logger.debug(
              `Sentinel Insights: suppressing candidate ${fingerprint} for project ${data.projectId.toString()} — dismissed by a human within the last ${DISMISSED_COOLDOWN_DAYS} days.`,
            );
        ...
        const insight: SentinelInsight = new SentinelInsight();
        ...
        insight.status = SentinelInsightStatus.Detected;
        ...
        const created: SentinelInsight = await SentinelInsightService.create({
          data: insight,
          props: { isRoot: true },
        });
        ...
        logger.error(
          `Sentinel Insights: upsert failed for candidate ${candidate.fingerprint} (project ${data.projectId.toString()}) — continuing with the remaining candidates: ${error}`,
        );
      }
    }
    ...
    if (result.droppedByCap > 0) {
      logger.debug(
        `Sentinel Insights: candidate fingerprints dropped by the per-scan cap for project ${data.projectId.toString()}: ${droppedFingerprints.join(", ")}`,
      );
      logger.info(
        `Sentinel Insights: per-scan new-insight cap (${MAX_NEW_INSIGHTS_PER_PROJECT_PER_SCAN}) reached for project ${data.projectId.toString()} — dropped ${result.droppedByCap} candidate(s) this tick. Recurring findings will re-surface on later ticks if their signal persists.`,
      );
    }
