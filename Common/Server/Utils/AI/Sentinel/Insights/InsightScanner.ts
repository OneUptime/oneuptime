...
/*
 * Sentinel Insights — the scan orchestration (the "watch loop").
 *
 ...
 * enableSentinelInsights flag are scanned, insights never page and never
...
export default class InsightScanner {
   ...
   * enableSentinelInsights defaults to false, so nothing runs for a project
  ...
  public static async scanAllProjects(): Promise<void> {
    ...
    try {
      projects = await ProjectService.findBy({
        query: { enableSentinelInsights: true },
        select: {
          _id: true,
      ...
      logger.error(
        `Sentinel Insights: failed to list opted-in projects for the scan tick: ${error}`,
      );
      return;
    }
    ...
    for (const project of projects) {
      try {
        ...
        logger.error(
          `Sentinel Insights: scan failed for project ${project.id?.toString()}: ${error}`,
        );
      }
    }
  }
  ...
  public static async scanProjectForInsights(project: Project): Promise<void> {
    ...
    for (const detector of detectors) {
      try {
        ...
        logger.error(
          `Sentinel Insights: ${detector.insightType} detector failed for project ${projectId.toString()} — continuing with the remaining detectors: ${error}`,
        );
      }
    }
    ...
    for (const insight of upsertResult.created) {
      try {
        ...
        logger.error(
          `Sentinel Insights: routing failed for insight ${insight.id?.toString()} (project ${projectId.toString()}) — continuing with the remaining insights: ${error}`,
        );
      }
    }
  }
  ...
  private static async rerouteStrandedInsights(data: {
    ...
    let stranded: Array<SentinelInsight> = [];

    try {
      stranded = await SentinelInsightService.findBy({
        query: {
          projectId: projectId,
      ...
      logger.error(
        `Sentinel Insights: failed to sweep unrouted (Detected) insights for project ${projectId.toString()} — continuing with the scan: ${error}`,
      );
      return;
    }
    ...
    for (const insight of stranded) {
      try {
        ...
        logger.error(
          `Sentinel Insights: re-routing failed for stranded insight ${insight.id?.toString()} (project ${projectId.toString()}) — continuing with the remaining stranded insights: ${error}`,
        );
      }
    }
  }
  ...
  private static async routeNewInsight(data: {
    ...
    try {
      ...
      logger.error(
        `Sentinel Insights: fix routing threw for insight ${insight.id?.toString()} (treating as no fix opened): ${error}`,
      );
    }
    ...
    try {
      ...
      logger.error(
        `Sentinel Insights: triage enqueue threw for insight ${insight.id?.toString()} (continuing without triage): ${error}`,
      );
    }
