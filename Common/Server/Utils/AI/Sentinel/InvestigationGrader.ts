...
/*
 * Sentinel — on-resolve investigation grading (Phase 2 measurement layer).
 *
 * When an incident resolves with a human-recorded root cause AND a completed
 * Sentinel investigation exists for it, ONE constrained LLM call compares the
...
const FEATURE_NAME: string = "Sentinel Investigation Grading";
...
export default class InvestigationGrader {
  ...
  public static async gradeInvestigationOnResolve(data: {
    ...
    try {
      ...
      /*
       * The posted analysis: the latest RootCause feed item — Sentinel's
      ...
      if (!analysisMarkdown) {
        logger.debug(
          `Sentinel grading: no posted analysis found for incident ${incidentId.toString()} — skipping.`,
        );
        return;
      }
      ...
      if (!grade) {
        logger.warn(
          `Sentinel grading: unparseable grade response for incident ${incidentId.toString()} (run ${run.id!.toString()}) — no grade stored.`,
        );
        return;
      }
      ...
      logger.debug(
        `Sentinel grading: incident ${incidentId.toString()} investigation graded ${grade}.`,
      );
    } catch (error) {
      logger.error(
        `Sentinel grading: failed for incident ${incidentId.toString()}: ${error}`,
      );
    }
  }
}
