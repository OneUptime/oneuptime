  ...
  public static async routeInsightFix(data: {
    ...
    try {
      ...
      if (project.enableAi === false) {
        logger.debug(
          `AI insights: not routing a fix for insight ${insight.id.toString()} — AI is disabled for the project.`,
        );
        return {};
      }
      ...
      if (project.enableInsightFixTasks !== true) {
        logger.debug(
          `AI insights: not routing a fix for insight ${insight.id.toString()} — project has not opted in to insight fix tasks.`,
        );
        return {};
      }
      ...
      const isExceptionInsight: boolean =
        ...
        insight.insightType === AIInsightType.ExceptionSpike;
      ...
      if (isExceptionInsight && !insight.telemetryExceptionId) {
        logger.debug(
          `AI insights: not routing a fix for insight ${insight.id.toString()} — no telemetryExceptionId on the insight.`,
        );
        return {};
      }
      ...
      if (
        ...
        logger.debug(
          `AI insights: not routing a fix for insight ${insight.id.toString()} — the latency evidence has no span-tree findings or no trace id.`,
        );
        return {};
      }
      ...
      if (!budget.allowed) {
        logger.debug(
          `AI insights: not routing a fix for insight ${insight.id.toString()} — ${FixRunBudget.describeRejection(budget)}`,
        );
        return {};
      }
      ...
      if (isExceptionInsight) {
        ...
        if (!readiness.ready) {
          logger.debug(
            `AI insights: not routing a fix for insight ${insight.id.toString()} — AI fix readiness checks failed.`,
          );
          return {};
        }
      ...
      try {
        ...
        logger.error(
          `AI insights: failed to stamp fix run ${createdRun.id.toString()} with insight ${insight.id.toString()}: ${error}`,
        );
      }

      logger.debug(
        `AI insights: queued ${isExceptionInsight ? "FixException" : "FixPerformance"} run ${createdRun.id.toString()} for insight ${insight.id.toString()}.`,
      );
       ...
       */
      logger.debug(
        `AI insights: fix routing skipped for insight ${insight.id?.toString()}: ${error}`,
      );
      return {};
    }
  }
