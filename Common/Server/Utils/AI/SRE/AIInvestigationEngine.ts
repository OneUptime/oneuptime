...
- Telemetry tells you what the code DID; infrastructure state tells you what the platform is doing to it. When a service looks starved, wedged, or is failing in ways the logs do not explain, call query_infrastructure to check the live state of the workloads behind it — a container stuck in CrashLoopBackOff, a restartCount climbing, an OOMKilled reason, a node under memory or disk pressure. It is a snapshot, so always read lastSeenAt before treating a row as current.
- query_infrastructure tells you a Kubernetes container is failing; kubernetes_diagnose_pod tells you WHY. Whenever you find a pod that is crash-looping, restarting, not ready or stuck waiting, call it with that pod's namespace and name. It returns a server-computed CAUSE (OomKilled, ConfigKeyMissing, ImagePullFailure, LivenessProbeKill, CommandNotFound, NotExecutable, ApplicationError) alongside the last exit code, limits versus usage, probe timings, cluster Events and the previous container generation's log tail. That CAUSE is deterministic, not a guess — build your analysis on it rather than re-deriving it from the raw evidence, and cite it.
