...
export class Service extends DatabaseService<MonitorStatusTimeline> {
  ...
  protected override async onBeforeCreate(
    ...
    try {
      ...
      logger.error(e, { projectId: createBy.data.projectId?.toString(), monitorId: createBy.data.monitorId?.toString() } as LogAttributes);
    }
    ...
    logger.debug("State Before this", { projectId: createBy.data.projectId?.toString(), monitorId: createBy.data.monitorId?.toString() } as LogAttributes);
    logger.debug(stateBeforeThis, { projectId: createBy.data.projectId?.toString(), monitorId: createBy.data.monitorId?.toString() } as LogAttributes);
    ...
    logger.debug("State After this", { projectId: createBy.data.projectId?.toString(), monitorId: createBy.data.monitorId?.toString() } as LogAttributes);
    logger.debug(stateAfterThis, { projectId: createBy.data.projectId?.toString(), monitorId: createBy.data.monitorId?.toString() } as LogAttributes);
  ...
  protected override async onCreateSuccess(
    ...
    logger.debug("Status Timeline Before this", { projectId: createdItem.projectId?.toString(), monitorId: createdItem.monitorId?.toString() } as LogAttributes);
    logger.debug(onCreate.carryForward.statusTimelineBeforeThisStatus, { projectId: createdItem.projectId?.toString(), monitorId: createdItem.monitorId?.toString() } as LogAttributes);

    logger.debug("Status Timeline After this", { projectId: createdItem.projectId?.toString(), monitorId: createdItem.monitorId?.toString() } as LogAttributes);
    logger.debug(onCreate.carryForward.statusTimelineAfterThisStatus, { projectId: createdItem.projectId?.toString(), monitorId: createdItem.monitorId?.toString() } as LogAttributes);

    logger.debug("Created Item", { projectId: createdItem.projectId?.toString(), monitorId: createdItem.monitorId?.toString() } as LogAttributes);
    logger.debug(createdItem, { projectId: createdItem.projectId?.toString(), monitorId: createdItem.monitorId?.toString() } as LogAttributes);
    ...
    if (!onCreate.carryForward.statusTimelineBeforeThisStatus) {
      ...
      logger.debug("This is the first status.", { projectId: createdItem.projectId?.toString(), monitorId: createdItem.monitorId?.toString() } as LogAttributes);
      ...
      logger.debug("This is the last status.", { projectId: createdItem.projectId?.toString(), monitorId: createdItem.monitorId?.toString() } as LogAttributes);
    } else {
      /*
      ...
      logger.debug("This status is in the middle.", { projectId: createdItem.projectId?.toString(), monitorId: createdItem.monitorId?.toString() } as LogAttributes);
    }
  ...
  protected override async onBeforeDelete(
    ...
    if (deleteBy.query._id) {
      ...
      if (monitorId) {
        ...
        if (!stateBeforeThis) {
          // This is the first status, no need to update previous status.
          logger.debug("This is the first status.", { monitorId: monitorId?.toString() } as LogAttributes);
          ...
          logger.debug("This is the last status.", { monitorId: monitorId?.toString() } as LogAttributes);
        } else {
          /*
          ...
          logger.debug("This status is in the middle.", { monitorId: monitorId?.toString() } as LogAttributes);
        }
      }
