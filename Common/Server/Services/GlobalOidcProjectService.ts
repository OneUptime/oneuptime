...
import type { Service as GlobalOidcServiceType } from "./GlobalOidcService";
...
export class Service extends DatabaseService<Model> {
  ...
  protected override async onCreateSuccess(
    ...
    if (createdItem.globalOidcId) {
      await Service.getGlobalOidcService().syncForceForProvider(
        createdItem.globalOidcId,
      );
    }
  ...
  protected override async onBeforeDelete(
    ...
    for (const row of rows) {
      const id: string | undefined = row.globalOidcId?.toString();
      if (id && !seen.has(id)) {
        seen.add(id);
        providerIds.push(row.globalOidcId!);
      }
    }
  ...
  protected override async onDeleteSuccess(
    ...
    for (const providerId of providerIds) {
      await Service.getGlobalOidcService().syncForceForProvider(providerId);
    }
  ...
  /*
   * Lazy require to avoid an import cycle: GlobalOidcService statically imports
   ...
   */
  private static getGlobalOidcService(): GlobalOidcServiceType {
    // eslint-disable-next-line @typescript-eslint/no-require-imports, @typescript-eslint/no-var-requires
    return require("./GlobalOidcService").default;
  }
}
