...
export class Service extends DatabaseService<PodmanHostFeed> {
  public constructor() {
    super(PodmanHostFeed);
  ...
  @CaptureSpan()
  public async createPodmanHostFeedItem(data: {
    podmanHostId: ObjectID;
    feedInfoInMarkdown: string;
    podmanHostFeedEventType: PodmanHostFeedEventType;
    projectId: ObjectID;
     ...
     */
    try {
      const feed: PodmanHostFeed = new PodmanHostFeed();

      if (!data.podmanHostId) {
        throw new BadDataException("Podman Host ID is required");
      }
      ...
      if (!data.podmanHostFeedEventType) {
        throw new BadDataException("Podman Host feed event is required");
      }
      ...
      feed.podmanHostId = data.podmanHostId;
      feed.feedInfoInMarkdown = data.feedInfoInMarkdown;
      feed.podmanHostFeedEventType = data.podmanHostFeedEventType;
      ...
      logger.error("Error in creating Podman host feed", {
        projectId: data.projectId?.toString(),
        podmanHostId: data.podmanHostId?.toString(),
      } as LogAttributes);
      logger.error(e, {
        projectId: data.projectId?.toString(),
        podmanHostId: data.podmanHostId?.toString(),
