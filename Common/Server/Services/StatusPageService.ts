...
export class Service extends DatabaseService<StatusPage> {
  ...
  protected override async onCreateSuccess(
    ...
    if (
      ...
      this.addOwners(
        ...
        logger.error(`Error in StatusPageService owner assignment: ${error}`, { projectId: createdItem.projectId?.toString(), statusPageId: createdItem.id?.toString() } as LogAttributes);
      });
    }
  ...
  public async hasReadAccess(data: {
    ...
    try {
      ...
      if (statusPage?.ipWhitelist && statusPage.ipWhitelist.length > 0) {
        ...
        if (!ipAccessedFrom) {
          logger.error("IP address not found in request.", { statusPageId: statusPageId?.toString() } as LogAttributes);
          return {
            hasReadAccess: false,
        ...
        if (!isIPWhitelisted) {
          logger.error(
            `IP address ${ipAccessedFrom} is not whitelisted for status page ${statusPageId.toString()}.`, { statusPageId: statusPageId?.toString() } as LogAttributes,
          );
      ...
      if (token) {
        try {
          ...
          logger.error(err, { statusPageId: statusPageId?.toString() } as LogAttributes);
        }
      }
  ...
  private hasValidMasterPasswordCookie(data: {
    ...
    try {
      ...
      logger.error(err, { statusPageId: data.statusPageId?.toString() } as LogAttributes);
    }
  ...
  public async sendEmailReport(data: {
    ...
    const sendEmail: SendEmailFunction = async (
      ...
      MailService.sendMail(
        ...
        logger.error(err, { projectId: statuspage.projectId?.toString(), statusPageId: statuspage.id?.toString() } as LogAttributes);
      });
    };
