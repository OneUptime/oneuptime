...
export class Service extends DatabaseService<CloudResourceFeed> {
  public constructor() {
    super(CloudResourceFeed);
  ...
  @CaptureSpan()
  public async createCloudResourceFeedItem(data: {
    cloudResourceId: ObjectID;
    feedInfoInMarkdown: string;
    cloudResourceFeedEventType: CloudResourceFeedEventType;
    projectId: ObjectID;
     ...
     */
    try {
      const feed: CloudResourceFeed = new CloudResourceFeed();

      if (!data.cloudResourceId) {
        throw new BadDataException("Cloud Resource ID is required");
      }
      ...
      if (!data.cloudResourceFeedEventType) {
        throw new BadDataException("Cloud Resource feed event is required");
      }
      ...
      feed.cloudResourceId = data.cloudResourceId;
      feed.feedInfoInMarkdown = data.feedInfoInMarkdown;
      feed.cloudResourceFeedEventType = data.cloudResourceFeedEventType;
      ...
      logger.error("Error in creating cloud resource feed", {
        projectId: data.projectId?.toString(),
        cloudResourceId: data.cloudResourceId?.toString(),
      } as LogAttributes);
      logger.error(e, {
        projectId: data.projectId?.toString(),
        cloudResourceId: data.cloudResourceId?.toString(),
