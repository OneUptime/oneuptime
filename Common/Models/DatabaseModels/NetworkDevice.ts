...
/*
 * Indexes for a fleet of eighty thousand devices in one project. Each one is
 * here for a query the product runs constantly, and nothing here is
 * speculative — the columns a poll rewrites every cycle are indexed only
 * where a sequential scan is the alternative.
 */

/*
 * The device list's default page, and every fleet-wide count over it.
 *
 * `createdAt` is the third column because the list has no explicit sort and
 * DatabaseService falls back to `createdAt DESC`: without it, showing the
 * first ten devices means scanning the whole project and top-N sorting eighty
 * thousand rows. The two-column prefix still serves every count and filter
 * that only cares about project and archived-ness, so this replaces the
 * `(projectId, isArchived)` index rather than joining it — a redundant prefix
 * index is pure write amplification on a table every poll updates.
 */
@Index(["projectId", "isArchived", "createdAt"])
/*
 * The Status facet, and the three counts in the summary strip that have to
 ...
 *
 * The foreign key on `probeId` creates no index of its own in Postgres, so
 * without this each probe scanned the entire NetworkDevice table (every
 * project's devices, not just its own) several times a minute. `nextPollAt`
 * IS rewritten by every claim, which is real write cost — and still by far
 * the cheaper side of the trade at this size.
 */
@Index(["probeId", "nextPollAt"])
/*
...
export default class NetworkDevice extends BaseModel {
  ...
  @TableColumn({
    ...
    description:
      "How this device's health is established: SNMP (an assigned probe walks it on a schedule) or Monitor (no polling — the linked monitor's status is the device's status). Devices created before this existed are SNMP.",
    example: "SNMP",
  })
  @Column({
    nullable: true,
  ...
  @TableColumn({
    ...
    title: "Monitor ID",
    description:
      "ID of the monitor that reports this device's health when its monitoring method is Monitor.",
  })
  @Column({
    type: ColumnType.ObjectID,
    nullable: true,
  ...
  @TableColumn({
    ...
    description:
      "Deprecated: SNMP v3 auth is now stored in the snmpV3* columns below. Retained for reading legacy devices.",
  })
  @Column({
   ...
   * The device owns its own polling: the assigned probe walks every
   * polling-enabled device on this schedule, independent of any Monitor.
   * Monitors reference the device only to alert on what these walks (and
   * traps) report.
   */
  ...
  @TableColumn({
    ...
    description:
      "Whether the assigned probe polls this device on a schedule. Disable to pause SNMP polling without deleting the device.",
    defaultValue: true,
  })
  @Column({
  ...
  public isPollingEnabled?: boolean = undefined;
  ...
  @TableColumn({
    ...
    description:
      "How often, in minutes, the assigned probe polls this device via SNMP",
    example: "5",
  })
  @Column({
    type: ColumnType.Number,
  ...
  public pollingIntervalInMinutes?: number = undefined;
  ...
  @TableColumn({
    ...
    description:
      "Walk the IF-MIB interface tables on each poll to inventory interfaces, bandwidth, and errors. Also collects LLDP/CDP neighbors for the topology graph.",
    defaultValue: true,
  })
  @Column({
    type: ColumnType.Boolean,
  ...
  public walkInterfaces?: boolean = undefined;
  ...
  @TableColumn({
    ...
    description:
      "Also walk the device's ARP cache and bridge forwarding database on each poll to discover endpoints (laptops, printers, POS terminals) attached to it. Strictly opt-in: costs extra SNMP table walks per poll. Only meaningful when Walk Interfaces is on.",
    defaultValue: false,
  })
  @Column({
    type: ColumnType.Boolean,
  ...
  public collectEndpoints?: boolean = undefined;
  ...
  @ColumnAccessControl({
    ...
    /*
     * Editable so the dashboard can offer "Poll Now" — setting this to the
     * current time makes the assigned probe pick the device up on its next
     * claim cycle.
     */
    update: [
      Permission.ProjectOwner,
  ...
  public lastWalkLog?: JSONObject = undefined;
  ...
  @TableColumn({
    ...
    description:
      "sysObjectID — the vendor's registered OID for this device model, enriched from SNMP walks. Used to fingerprint the vendor and suggest an OID template.",
    example: "1.3.6.1.4.1.9.1.1208",
  })
  @Column({
    nullable: true,
    type: ColumnType.ShortText,
  ...
  public sysObjectId?: string = undefined;
  ...
  @TableColumn({
    ...
    description:
      "System location (sysLocation) enriched from SNMP walks of this device",
    example: "DC1, Rack 14",
  })
  @Column({
    nullable: true,
  ...
  public sysLocation?: string = undefined;
  ...
  @TableColumn({
    ...
    description:
      "System contact (sysContact) enriched from SNMP walks of this device",
  ...
  public sysContact?: string = undefined;
  ...
  @TableColumn({
    ...
    title: "Vendor",
    description:
      "Hardware vendor, from ENTITY-MIB or derived from sysObjectID. Managed by the probe.",
    example: "Cisco",
  })
  @Column({
    nullable: true,
  ...
  @TableColumn({
    ...
    title: "Model",
    description:
      "Hardware model from ENTITY-MIB (entPhysicalModelName). Managed by the probe.",
    example: "WS-C2960X-48TS-L",
  })
  @Column({
    nullable: true,
    type: ColumnType.ShortText,
  ...
  @TableColumn({
    ...
    title: "Serial Number",
    description:
      "Chassis serial number from ENTITY-MIB (entPhysicalSerialNum). Managed by the probe.",
    example: "FOC1932X0K1",
  })
  @Column({
    nullable: true,
  ...
  @TableColumn({
    ...
    description:
      "Firmware revision from ENTITY-MIB (entPhysicalFirmwareRev). Managed by the probe.",
    example: "15.2(2)E7",
  })
  @Column({
    nullable: true,
  ...
  @TableColumn({
    ...
    description:
      "Operating system / software revision from ENTITY-MIB (entPhysicalSoftwareRev). Managed by the probe.",
    example: "15.2(2)E7",
  })
  @Column({
    nullable: true,
  ...
  @TableColumn({
    ...
    type: TableColumnType.Date,
    canReadOnRelationQuery: true,
    title: "Last Rebooted At",
    description:
      "Device boot time computed from sysUpTime on each walk. Managed by the probe.",
  })
  @Column({
    nullable: true,
    type: ColumnType.Date,
  })
  public lastRebootedAt?: Date = undefined;
  ...
  @TableColumn({
    ...
    title: "CDP Neighbors",
    description:
      "CDP neighbors discovered on the last SNMP walk, complementing LLDP for the topology graph. Managed by the probe.",
  })
  @Column({
    nullable: true,
    type: ColumnType.JSON,
  })
  public cdpNeighbors?: Array<CdpNeighbor> = undefined;
