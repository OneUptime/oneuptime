...
/*
 * Restores a UNIQUE index that an autogenerated migration deleted — the slug is the public URL identifier for this resource, so two live
 * rows must never share one.
 *
 * The real definition is partial (WHERE "deletedAt" IS NULL) and lives in
 * migration 1786200000000-RestoreDroppedUniqueIndexes; @Index() cannot express a
 * partial index, so the migration owns it and UNSYNCHRONIZED_INDEX stops the
 * schema builder dropping it again. See Types/Database/UnsynchronizedIndex.
 */
@Index("IDX_network_device_slug", ["slug"], UNSYNCHRONIZED_INDEX)
@TableMetadata({
