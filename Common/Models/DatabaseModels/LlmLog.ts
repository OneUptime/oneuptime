...
export default class LlmLog extends BaseModel {
  ...
  /*
   * Output (completion) tokens alone. Backs the per-run output-token loop
   * budget for server-mediated code-fix agent completions (B4 Tier 0), which
   * caps what the model may PRODUCE per run — input tokens grow with the
   * conversation and would make a total-token cap strangle long loops.
   */
  @ColumnAccessControl({
    create: [],
    read: [
  ...
  @TableColumn({
    ...
    description: "Output (completion) tokens generated by this call",
    canReadOnRelationQuery: false,
    isDefaultValueColumn: true,
    defaultValue: 0,
  })
