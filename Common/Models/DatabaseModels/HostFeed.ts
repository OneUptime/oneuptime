...
/*
 * Every state change worth remembering about a host. The overview page
 * answers "what does this host look like right now"; the feed answers
...
@TableAccessControl({
  ...
  read: [
    ...
    Permission.ReadHostFeed,
  ],
  delete: [],
  update: [],
})
@CrudApiEndpoint(new Route("/host-feed"))
@OwnedThrough("hostId", Host)
@Entity({
  name: "HostFeed",
})
@TableMetadata({
  tableName: "HostFeed",
  singularName: "Host Feed",
  pluralName: "Host Feeds",
  icon: IconProp.List,
  tableDescription:
    "Log of everything that happened to this host - creation, updates, owner changes and the rules that made them.",
})
@Index(["hostId", "postedAt"]) // Host feed page: feed sorted by postedAt
export default class HostFeed extends BaseModel {
  @ColumnAccessControl({
    create: [
      Permission.ProjectOwner,
      Permission.ProjectAdmin,
  ...
  @TableColumn({
    ...
    modelType: Host,
    title: "Host",
    description: "Relation to Host in which this resource belongs",
  ...
  @TableColumn({
    ...
    required: true,
    title: "Host ID",
    description: "Relation to Host ID in which this resource belongs",
    example: "e5f6a7b8-d9c0-1234-5678-901234efab56",
  })
  @Column({
    type: ColumnType.ObjectID,
  ...
  @TableColumn({
    ...
    description: "Log of the host change in Markdown",
    example: "👨🏻‍💻 Added **Jane Doe** as the owner of this host.",
  })
  @Column({
  ...
  @TableColumn({
    ...
    example:
      "### Additional Context\n\nThis host was discovered from incoming telemetry.",
  })
  @Column({
    type: ColumnType.Markdown,
    nullable: true,
  ...
  @TableColumn({
    ...
    required: true,
    title: "Host Feed Event",
    description: "Host Feed Event",
    example: "HostCreated",
  })
  @Column({
  ...
  public hostFeedEventType?: HostFeedEventType = undefined;
