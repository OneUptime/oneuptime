...
/*
 * Every state change worth remembering about a Podman host. The overview page
 * answers "what does this Podman host look like right now"; the feed answers
 ...
 */
export enum PodmanHostFeedEventType {
  PodmanHostCreated = "PodmanHostCreated",
  PodmanHostUpdated = "PodmanHostUpdated",
  PodmanHostArchived = "PodmanHostArchived",
  PodmanHostRestored = "PodmanHostRestored",
...
@EnableDocumentation()
@CanAccessIfCanReadOn("podmanHost")
...
@TableAccessControl({
  create: [
    ...
    Permission.CreatePodmanHostFeed,
  ],
  read: [
    Permission.ProjectOwner,
    Permission.ProjectAdmin,
    ...
    Permission.ReadPodmanHostFeed,
  ],
  delete: [],
  update: [],
})
@CrudApiEndpoint(new Route("/podman-host-feed"))
@OwnedThrough("podmanHostId", PodmanHost)
@Entity({
  name: "PodmanHostFeed",
})
@TableMetadata({
  tableName: "PodmanHostFeed",
  singularName: "Podman Host Feed",
  pluralName: "Podman Host Feeds",
  icon: IconProp.List,
  tableDescription:
    "Log of everything that happened to this Podman host - creation, updates, owner changes and the rules that made them.",
})
@Index(["podmanHostId", "postedAt"]) // Podman Host feed page: feed sorted by postedAt
export default class PodmanHostFeed extends BaseModel {
  @ColumnAccessControl({
    create: [
      Permission.ProjectOwner,
      Permission.ProjectAdmin,
  ...
  @TableColumn({
    ...
    title: "Podman Host",
    description: "Relation to Podman Host in which this resource belongs",
  ...
  @TableColumn({
    ...
    description: "Relation to Podman Host ID in which this resource belongs",
    example: "e5f6a7b8-d9c0-1234-5678-901234efab56",
  })
  @Column({
    type: ColumnType.ObjectID,
  ...
  @TableColumn({
    ...
    description: "Log of the Podman host change in Markdown",
    example: "👨🏻‍💻 Added **Jane Doe** as the owner of this Podman host.",
  })
  @Column({
  ...
  @TableColumn({
    ...
    example:
      "### Additional Context\n\nThis Podman host was discovered from incoming telemetry.",
  })
  @Column({
    type: ColumnType.Markdown,
    nullable: true,
  ...
  @TableColumn({
    ...
    required: true,
    title: "Podman Host Feed Event",
    description: "Podman Host Feed Event",
    example: "PodmanHostCreated",
  })
  @Column({
  ...
  public podmanHostFeedEventType?: PodmanHostFeedEventType = undefined;
