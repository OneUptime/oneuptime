...
describe("SubscriptionPlan", () => {
  ...
  describe("isCustomPricingPlan", () => {
    it("should return false if plan is not custom pricing", () => {
      const isCustomPricingPlan: boolean = SubscriptionPlan.isCustomPricingPlan(
        monthlyPlanId,
        env,
      );
  ...
  describe("getSubscriptionPlans", () => {
    it("should return an array of SubscriptionPlan objects", () => {
      ...
      expect(subscriptionPlans?.[0]?.getYearlyPlanId()).toBe("yearly_plan_id");
    });
  });
  ...
  describe("isFeatureAccessibleOnCurrentPlan", () => {
    ...
    it("should return true if the feature is on the current plan", () => {
      const env: JSONObject = {
        SUBSCRIPTION_PLAN_1: "Free,monthly_plan_id,yearly_plan_id,0,0,3,7",
        SUBSCRIPTION_PLAN_2:
          "Growth,growth_monthly_plan_id,growth_yearly_plan_id,9,99,2,14",
      };
      const featureSubscriptionPlan: SubscriptionPlan = new SubscriptionPlan(
