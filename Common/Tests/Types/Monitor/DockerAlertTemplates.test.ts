...
/*
 * Lock in the Docker alert-template contracts (mirrors the DockerSwarm /
 * Ceph precedents). Three layers:
 *
 *   1. ENUMERATED invariants run over getAllDockerAlertTemplates(), so a
 ...
 *
 *   3. Blackout contract: the worker's monitorDocker fetcher stamps
 *      MetricMonitorResponse.isTelemetrySourceReporting via the shared
 *      checkTelemetrySourceReporting liveness probe. Docker templates
...
function buildArgs(): DockerAlertTemplateArgs {
  return {
    hostIdentifier: "docker-host-prod",
...
function getReferencableAliases(
  monitor: MonitorStepDockerMonitor,
): Set<string> {
...
const ALL_TEMPLATES: Array<DockerAlertTemplate> = getAllDockerAlertTemplates();

describe("DockerAlertTemplates - registry", () => {
  test("template ids are unique and match the expectation table exactly", () => {
    const ids: Array<string> = ALL_TEMPLATES.map((t: DockerAlertTemplate) => {
      return t.id;
    });
  ...
  test("getDockerAlertTemplateById resolves every id (and misses cleanly)", () => {
    for (const expected of EXPECTED_TEMPLATES) {
      expect(getDockerAlertTemplateById(expected.id)).toBeDefined();
    }
    expect(getDockerAlertTemplateById("does-not-exist")).toBeUndefined();
  });

  test("getDockerAlertTemplatesByCategory partitions the registry", () => {
    const resource: Array<DockerAlertTemplate> =
      getDockerAlertTemplatesByCategory("Resource");
    const container: Array<DockerAlertTemplate> =
      getDockerAlertTemplatesByCategory("Container");
    const host: Array<DockerAlertTemplate> =
      getDockerAlertTemplatesByCategory("Host");

    expect(
      resource
        .map((t: DockerAlertTemplate) => {
          return t.id;
        })
        .sort(),
    ).toEqual(
      ["docker-high-cpu", "docker-high-memory", "docker-cpu-throttling"].sort(),
    );
    expect(
      container
        .map((t: DockerAlertTemplate) => {
...
describe("DockerAlertTemplates - enumerated invariants (every template)", () => {
  test.each(
    ALL_TEMPLATES.map((t: DockerAlertTemplate) => {
      return [t.id, t];
    }),
  )("%s builds a valid MonitorStep", (_id: unknown, template: unknown) => {
    const args: DockerAlertTemplateArgs = buildArgs();
    const step: MonitorStep = (template as DockerAlertTemplate).getMonitorStep(
      args,
    );
  ...
  test.each(
    ...
    (_id: unknown, template: unknown) => {
      ...
      for (const queryConfig of monitor.metricViewConfig
        ...
        expect(getDockerMetricByMetricName(metricName)).toBeDefined();
      }
...
describe("DockerAlertTemplates - spec table expectations", () => {
  test.each(
    EXPECTED_TEMPLATES.map((t: TemplateExpectation) => {
      return [t.id, t];
    }),
...
/*
 * Blackout contract: a total telemetry blackout — the Docker agent died,
 * so the host reports NOTHING — must never read as recovery. The
 * worker's monitorDocker fetcher stamps isTelemetrySourceReporting=false
 * on the response in that case (via checkTelemetrySourceReporting).
 * Docker templates keep the default Ignore no-data policy on every
 ...
 */
describe("DockerAlertTemplates - blackout contract (no auto-resolve on absent data)", () => {
  async function evaluateSnapshot(input: {
    ...
    monitor.monitorType = MonitorType.Docker;
    ...
    const dataToProcess: MetricMonitorResponse = {
      ...
      metricViewConfig: getDockerMonitor(input.step).metricViewConfig,
  ...
  test("docker-container-down: fires on a stopped container, then HOLDS (not resolves) when the agent goes dark", async () => {
    const template: DockerAlertTemplate | undefined =
      getDockerAlertTemplateById("docker-container-down");
    expect(template).toBeDefined();
