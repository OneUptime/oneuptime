 ...
 * behind a delete the user has already asked for, and its contract is to FAIL
 * CLOSED: the provider being unreachable, the notification app being
 * mid-restart, or the endpoint answering 500 must preserve the parent row and
 * its retry metadata instead of silently orphaning a billable number.
 *
 ...
 * rejected and the parent delete must stop before it loses the number's config
...
describe("releaseIncomingCallPhoneNumber", () => {
  ...
  describe("the request it makes", () => {
     ...
     * request is rejected and the number stays provisioned, so that rejection
     * must reach the delete caller.
     */
  ...
  describe("it fails closed, preserving provider retry metadata", () => {
    /*
     * The parent delete has not committed yet. A provider or network problem
     * must stop it before the only retry metadata is removed.
     */
    test("a rejected request is propagated to the delete caller", async () => {
      api.post.mockRejectedValue(new Error("provider unreachable") as never);

      await expect(release()).rejects.toThrow("provider unreachable");
    });

    test("an HTTP failure response is propagated to the delete caller", async () => {
      api.post.mockResolvedValue({
        isFailure: (): boolean => {
          return true;
        },
      ...
      await expect(release()).rejects.toThrow("HTTP 503");
     ...
     * missing cluster key secret, say - must stop the delete for the same
     * reason.
     */
    test("a throw while building the request is logged and propagated", async () => {
      clusterKey.getClusterKeyHeaders.mockImplementation((): never => {
        throw new Error("no cluster key configured");
      });

      await expect(release()).rejects.toThrow("no cluster key configured");
      expect(mockedLogger.error).toHaveBeenCalled();
