 ...
 *      MORE owner emails. It is deleted: the invoice already goes out through
 *      generateInvoiceAndChargeCustomer and the Slack balance-refill
 *      notification still fires, so nothing actionable was lost.
 *
 ...
 *
 *   3. Guarding the catch exposes a double-send that was invisible before it:
 *      the no-payment-method branch mails and then THROWS, and that throw
 *      lands in this method's own catch. Without also setting the in-memory
 *      flag in that branch, a first no-payment-method failure sends TWO
 *      identical "ACTION REQUIRED" emails. That is the assertion below that
 *      only makes sense once you trace the throw.
 *
 *   4. AIBillingService had bugs 1-3 byte-for-byte.
 *
...
describe("NotificationService.rechargeBalance - SMS and call owner emails", () => {
  test("a successful recharge mails project owners zero times", async () => {
  ...
  test("a repeat recharge failure with the flag already true mails owners zero times", async () => {
    findOneByIdSpy.mockResolvedValue(
      makeSmsProject({ failedFlagAlreadySet: true }),
    );
    generateInvoiceSpy.mockRejectedValue(new Error("card_declined"));
