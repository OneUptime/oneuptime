import { AddOnCallCalendarFeeds1790200000000 } from "../../../../Server/Infrastructure/Postgres/SchemaMigrations/1790200000000-AddOnCallCalendarFeeds";
...
const MIGRATION_NAME: string = "AddOnCallCalendarFeeds1790200000000";
...
async function run(direction: "up" | "down"): Promise<Array<string>> {
  ...
  const migration: AddOnCallCalendarFeeds1790200000000 =
    new AddOnCallCalendarFeeds1790200000000();
...
describe("AddOnCallCalendarFeeds migration", () => {
  test("names itself after its timestamp, in the class and the recorded name", () => {
    ...
    expect(AddOnCallCalendarFeeds1790200000000.name).toBe(MIGRATION_NAME);
  ...
  test("is registered LAST, above every timestamp already registered", () => {
    ...
    expect(
      registered.filter((migration: MigrationClass) => {
        return migration === AddOnCallCalendarFeeds1790200000000;
      }),
    ).toHaveLength(1);
  });
  ...
  test("is backed by exactly one file carrying the same timestamp", () => {
    ...
    const files: Array<string> = fs
      ...
      .filter((file: string) => {
        return file.startsWith("1790200000000-");
      });

    expect(files).toEqual(["1790200000000-AddOnCallCalendarFeeds.ts"]);
  });
