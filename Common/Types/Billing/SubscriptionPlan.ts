export default class SubscriptionPlan {
    ...
    constructor(monthlyPlanId: string, yearlyPlanId: string, name: string, monthlySubscriptionAmountInUSD: number, yearlySubscriptionAmountInUSD: number, order: number, trialPeriodInDays: number) {
    ...
    public static getSubscriptionPlans(): Array<SubscriptionPlan> { 
        ...
        for (const key in process.env) {
            if (key.startsWith("SUBSCRIPTION_PLAN_")) {
                const content = process.env[key] as string || '';
                const values = content.split(",");

                if (values.length > 0) {
                    plans.push(new SubscriptionPlan(values[1] as string,values[2] as string,  values[0] as string, parseInt(values[3] as string), parseInt(values[4] as string),parseInt(values[5] as string), parseInt(values[6] as string)));
                }
            }
        }
