module.exports = {
  findBy: async function(query, limit, skip) {
      try {
          ...
          const ssos = await ssoDefaultRoles.find(query, {
              _id: 1,
              project:1,
              role:1,
              domain: 1,
  ...
  create: async function(data) {
      ...
      if(!['Owner', 'Administrator', 'Member', 'Viewer'].includes(data.role)) {
      ...
      try {
          ...
          ErrorService.log('ssoDefaultRolesService.create', error);
          if( error.code === 11000 ) {
            const err = new Error('The combinaison [Domain - Project - Role] already exists.');
            err.code = 400;
            throw err;
          }
          throw error;
  ...
  findOneBy: async function(query) {
      try {
          ...
          const sso = await ssoDefaultRoles.findOne(query)
              .populate('domain',['_id','domain'])
              .populate('project',['_id','name']);
          return sso;
      } catch (error) {
