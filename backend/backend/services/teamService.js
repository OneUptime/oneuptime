...
module.exports = {
        ...
        try {
            ...
            if (projects && projects.length > 0) {
                ...
                if (query.parentProjectId && projects[0]) {
                    const parentProject = await ProjectService.findOneBy({ _id: projects[0].parentProjectId, deleted: { $ne: null } });
    ...
    getTeamMemberBy: async function (projectId, teamMemberUserId) {
        ...
        try {
            ...
            if (index === -1) {
                ...
                const user = await UserService.findOneBy({ _id: teamMemberUserId });
                return user;
            }
        } catch (error) {
            ErrorService.log('teamService.getTeamMemberBy', error);
    ...
    getSeats: async function (members) {
        try {
            let seats = members.filter(async user => {
                let count = 0;
                const user_member = await UserService.findOneBy({ _id: user.userId });
    ...
    inviteTeamMembers: async function (addedByUserId, projectId, emails, role) {
        ...
        if (duplicateEmail) {
            ...
            if (subProject) {
                teamMembers = await _this.getTeamMembersBy({ _id: subProject._id });
            } else {
            ...
            if (isUserInProject) {
                const error = new Error('These users are already members of the project.');
                error.code = 400;
                ...
                try {
                    const invite = await _this.inviteTeamMembersMethod(projectId, emails, role, addedBy, extraUsersToAdd);
                    return invite;
                } catch (error) {
    ...
    async checkUser(teamMembers, emails) {
        ...
        for (let i = 0; i < teamMembers.length; i++) {
            const user = await UserService.findOneBy({ _id: teamMembers[i].userId });
    ...
    inviteTeamMembersMethod: async function (projectId, emails, role, addedBy, extraUsersToAdd) {
        try {
            ...
            let subProject = null;
            let project = await ProjectService.findOneBy({ _id: projectId });
            const registerUrl = ACCOUNTS_HOST ? `${ACCOUNTS_HOST}/register` : 'https://accounts.fyipe.com/register';
            if (project.parentProjectId) {
                subProject = project;
                project = await ProjectService.findOneBy({ _id: subProject.parentProjectId });
            }
            ...
            for (const member of invitedTeamMembers) {
                if (member.name) {
                    ...
                    const userInProject = projectUsers.find(user => user.userId === member._id);
                    if (userInProject) {
                        if (role === 'Viewer') {
            ...
            const subProjectTeams = await ProjectService.findBy({ parentProjectId: project._id });
            if (subProjectTeams.length > 0) {
                const subProjectTeamsUsers = await Promise.all(subProjectTeams.map(async (subProject) => {
    ...
    removeTeamMember: async function (projectId, userId, teamMemberUserId) {
        ...
        if (userId === teamMemberUserId) {
            ...
            if (index === -1) {
                const error = new Error('Member to be deleted from the project does not exist.');
                error.code = 400;
                ...
                const member = await UserService.findOneBy({ _id: teamMemberUserId });
                if (subProject) {
    ...
    updateTeamMemberRole: async function (projectId, userId, teamMemberUserId, role) {
        try {
            ...
            if (index === -1) {
                const error = new Error('User whose role is to be changed is not present in the project.');
                error.code = 400;
                ...
                if (nextRole === previousRole) {
                    const error = new Error('Please provide role different from current role and try again.');
                    error.code = 400;
                    ErrorService.log('teamService.updateTeamMemberRole', error);
                    throw error;
                } else {
