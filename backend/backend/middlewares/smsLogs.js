...
module.exports = {
    log: async function(req, res, next) {
        try {
            const blackListedRoutes = ['/sms-logs/'];
            ...
            // SMS logging is attached to res 'finish' event, because of below reasons.
            ...
            res.on('finish', async () => {
                ...
                const smsLogStatus = await GlobalConfigService.findOneBy({
                    name: 'smsLogMonitoringStatus',
                });
                // check if the global config has SMSLog flag and is storing logs before trying to store logs
                const shouldStoreLogs = !(smsLogStatus && !smsLogStatus.value);
                //  skip storing if SMS log config exist and it is not storing

                if (shouldStoreLogs) {
                    // store logs if storing
                    const parsedUrl = url.parse(req.originalUrl);

                    // Avoiding logging any SMS data, if its a blacklisted url/route.
            ...
            ErrorService.log('smsLogs.log', error);
