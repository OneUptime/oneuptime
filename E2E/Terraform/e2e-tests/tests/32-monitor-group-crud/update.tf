...
resource "oneuptime_monitor_group" "basic" {
  name        = "TF Basic Monitor Group Updated ${random_id.suffix.hex}"
  description = "Basic monitor group updated by the E2E suite"
}
...
resource "oneuptime_monitor_group" "secondary" {
  ...
  description = "Secondary monitor group (updated)"
}
