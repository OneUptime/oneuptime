...
const capture: (
  ...
  const originalViewport: { width: number; height: number } | null =
    page.viewportSize();
  const tallCapture: boolean =
    name === "edit-rule" || name === "monitor-editor-overview";
  try {
    if (tallCapture) {
      await page.setViewportSize({
        width: 1440,
        height: name === "edit-rule" ? 2000 : 1600,
      });
    }
    if (name === "edit-rule") {
      await editor(page)
    ...
    if (tallCapture && originalViewport) {
...
test.describe("Simple monitor editor", () => {
  ...
  test.beforeEach(async () => {
    await ctx.page.setViewportSize({ width: 1440, height: 1200 });
  ...
  test("keeps the editor and save controls usable on a narrow screen", async ({
    ...
    const bounds: { viewport: number; scroll: number } = await editor(
      ctx.page,
    ).evaluate((element: HTMLElement) => {
      return {
    ...
    await saveEditor(ctx.page);
    expect(rules(await getSteps(ctx))[0]!["name"]).toEqual("Mobile rule edit");
  });
});
