...
const requestJson: (data: RequestData) => Promise<JSONish> = async (
  ...
  expect(
    response.ok(),
    `${data.path}: ${response.status()} ${await response.text()}`,
  ).toBe(true);
...
export const listItems: (data: {
  ...
  query?: JSONish;
  select: JSONish;
}) => Promise<Array<JSONish>> = async (data: {
  page: Page;
  projectId: string;
  path: string;
  query?: JSONish;
  ...
  const response: JSONish = await requestJson({
    ...
    body: { query: data.query || {}, select: data.select, limit: 100, skip: 0 },
  });
...
export const deleteItem: (data: {
  ...
  const response: APIResponse = await data.page.request.delete(
    buildUrl(`${data.path}/${data.id}`),
    {
  ...
  expect(
    response.ok(),
    `Cleanup ${data.path}: ${response.status()} ${await response.text()}`,
  ).toBe(true);
};

export const registerAndCreateProject: (data: {
  page: Page;
  ...
  await data.page
    ...
    .first()
    .fill(`${data.projectNamePrefix} ${unique}`);
  await data.page.getByTestId("modal-footer-submit-button").click();
  await expect(data.page).toHaveURL(
    /\/dashboard\/([a-f0-9-]+)(?:\/home\/?)?$/,
    { timeout: 120000 },
  );
  const projectId: string | undefined = data.page
    .url()
    .match(/\/dashboard\/([a-f0-9-]+)/)?.[1];
