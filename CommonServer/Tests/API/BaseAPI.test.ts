...
jest.mock('../../Utils/Response', () => ({
  sendEntityArrayResponse: jest.fn().mockImplementation((...args: []) => args),
  sendJsonObjectResponse: jest.fn().mockImplementation((...args: []) => args),
  sendEmptyResponse: jest.fn(),
  sendEntityResponse: jest.fn().mockImplementation((...args: []) => args),
}))

jest.mock('../../Services/DatabaseService', () => {
  return jest.fn().mockImplementation(() => {
    return {
      countBy: () => new PositiveNumber(42),
      findBy: () => {
        return [{id: 'mock'}]
      },
      findOneById: jest.fn().mockImplementation(() => ({id: 'mock'})),
      deleteOneBy: jest.fn(),
      updateOneBy: jest.fn(),
...
jest.mock('Common/Models/BaseModel', () => {
  return jest.fn().mockImplementation((initObject) => {
    return {
      ...initObject,
      getCrudApiPath: jest.fn().mockImplementation(() => '/mock'),
      getTableColumnMetadata: jest.fn().mockImplementation(() => null)
    };
  });
});

jest.mock('../../Services/ProjectService', () => ({
  getCurrentPlan: () => ({currentPlan: 'Free', isSubscriptionUnpaid: false})
}))
...
const TestService = new DatabaseService(BaseModel);
...
describe('BaseAPI', () => {
  ...
  beforeAll(async () => {
    ...
    emptyDatabaseCommonInteractionProps = await baseApiInstance.getDatabaseCommonInteractionProps(emptyRequest)
  });
    ...
    for (const [method, uri, shouldBeCalled] of checkRoutes) {
      describe(`${method} ${uri}`, () => {
        it(`should call ${shouldBeCalled}`, async () => {
          const spy = jest.spyOn(baseApiInstance as any, shouldBeCalled as any);
          await mockRouter.match(method, uri).handlerFunction(emptyRequest, res, next)
          expect(spy).toHaveBeenCalledWith(emptyRequest, res);
        })

        it('should call next(err) on exception', async () => {
          const error = new Error('Mocked Error')
          jest.spyOn(baseApiInstance as any, shouldBeCalled as any)
  ...
  describe('BaseAPI.getPermissionsForTenant', () => {
    it('should return empty permissions if userTenantAccessPermission is not set', async () => {
      jest.spyOn(baseApiInstance, 'getDatabaseCommonInteractionProps').mockResolvedValueOnce({});
      const permissions = await baseApiInstance.getPermissionsForTenant(emptyRequest);
  ...
  describe('getTenantId', () => {
    ...
    it('should return ObjectID if tennantId is passed', () => {
      const tenantId = new ObjectID('123')
      let tenantRequest: OneUptimeRequest = { tenantId } as OneUptimeRequest
   ...
   describe('getDatabaseCommonInteractionProps', () => {
    ...
    it('should initialize props with undefined values', async () => {
      const props = await baseApiInstance.getDatabaseCommonInteractionProps(request);
  ...
  describe('getList', () => {
    it('should call onBeforeList', async () => {
      const onBeforeListSpy = jest.spyOn(baseApiInstance as any, 'onBeforeList');
    ...
    it('should call service.findBy with the correct parameters', async () => {
      const findBySpy = jest.spyOn(TestService, 'findBy')
      await baseApiInstance.getList(emptyRequest, res);
      expect(findBySpy).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should call service.count with the correct parameters', async () => {
      const findBySpy = jest.spyOn(TestService, 'countBy')
      await baseApiInstance.getList(emptyRequest, res)
      expect(findBySpy).toHaveBeenCalledWith(expect.objectContaining({
        query: {},
        props: emptyDatabaseCommonInteractionProps,
    ...
    it('should return values retrieved from service',async () => {
      const result = await baseApiInstance.getList(emptyRequest, res) as any
    ...
    it('should throw BadRequestException if limit is greater than LIMIT_PER_PROJECT', async () => {
      ...
      await expect(baseApiInstance.getList(emptyRequest, res)).rejects.toThrow(BadRequestException);
    });
  ...
  describe('getCount', () => {
    it('should call onBeforeCount lifecycle method', async () => {
      const onBeforeCountSpy = jest.spyOn(baseApiInstance as any, 'onBeforeCount')
    ...
    it('should process empty query if no body is provided', async () => {
      const countBySpy = jest.spyOn(TestService, 'countBy')
      await baseApiInstance.count(emptyRequest, res);
    ...
    it('should send a json response with the count', async () => {
      await baseApiInstance.count(emptyRequest, res);
      expect(Response.sendJsonObjectResponse).toHaveBeenCalledWith(emptyRequest, res, {
        count: 42,
      });
    });
  })
  ...
  describe('getItem', () => {
    it('should call onBeforeDelete lifecycle method', async () => {
      const onBeforeGetSpy = jest.spyOn(baseApiInstance as any, 'onBeforeGet');
    ...
    it('should call service.findOneById', async () => {
      const findOneByIdSpy = jest.spyOn(baseApiInstance.service, 'findOneById');
      await baseApiInstance.getItem(getRequest, res);
      expect(findOneByIdSpy).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should return EntityResponse', async () => {
      const sendEntityResponseSpy = jest.spyOn(Response as any, 'sendEntityResponse');
      await baseApiInstance.getItem(getRequest, res);
  ...
  describe('deleteItem', () => {
    it('should call onBeforeDelete lifecycle method', async () => {
      const onBeforeDeleteSpy = jest.spyOn(baseApiInstance as any, 'onBeforeDelete');
    ...
    it('should convert request param id to query', async () => {
      await baseApiInstance.deleteItem(deleteRequest, res);
      const deleteOneBySpy = jest.spyOn(baseApiInstance.service, 'deleteOneBy');
      expect(deleteOneBySpy).toHaveBeenCalledWith(expect.objectContaining({
        query: {_id: 'delete-me'},
        props: emptyDatabaseCommonInteractionProps,
      }))
    });
    ...
    it('should send empty response on success', async () => {
      await baseApiInstance.deleteItem(deleteRequest, res);
      const sendEmptyResponseSpy = jest.spyOn(Response as any, 'sendEmptyResponse');
   ...
   describe('updateItem', () => {
    ...
    it('should call onBeforeUpdate lifecycle method', async () => {
      const onBeforeUpdateSpy = jest.spyOn(baseApiInstance as any, 'onBeforeUpdate');
      await baseApiInstance.updateItem(updateRequest, updateResponse);
      expect(onBeforeUpdateSpy).toHaveBeenCalledWith(updateRequest, updateResponse);
    });
    ...
    it('should remove forbidden fields from the item', async () => {
      ...
      const updateOneBySpy = jest.spyOn(baseApiInstance.service, 'updateOneBy');
      expect(updateOneBySpy).toHaveBeenCalledWith(expect.objectContaining({
  ...
  describe('createItem', () => {
    ...
    it('should call onBeforeCreate lifecycle method', async () => {
      const onBeforeCreateSpy = jest.spyOn(baseApiInstance as any, 'onBeforeCreate');
      await baseApiInstance.createItem(createRequest, createResponse);
      expect(onBeforeCreateSpy).toHaveBeenCalledWith(createRequest, createResponse);
    });

    it('should return EntityResponse with the saved item', async () => {
      jest.spyOn(baseApiInstance.service, 'create').mockResolvedValue(savedItem);
      await baseApiInstance.createItem(createRequest, createResponse);
      const sendEntityResponseSpy = jest.spyOn(Response as any, 'sendEntityResponse');
      expect(sendEntityResponseSpy).toHaveBeenCalledWith(
        createRequest,
  ...
  describe('getRouter', () => {
    it('should return an ExpressRouter instance', () => {
      const router = baseApiInstance.getRouter();
  ...
  describe('getEntityName', () => {
    it('should return the name of the entity', () => {
      const entityName = baseApiInstance.getEntityName();
