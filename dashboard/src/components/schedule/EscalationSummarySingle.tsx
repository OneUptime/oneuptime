...
const EscalationSummarySingle = ({
    ...
    const teams = teamMembers && teamMembers.filter((team: $TSFixMe) => team.userId);
    const groups = teamMembers && teamMembers.filter((group: $TSFixMe) => group.groupId);

    const teamMembersOnPartialDutyCount = teamMembers.filter((member: $TSFixMe) => {
        return (
            member.startTime &&
            member.startTime !== '' &&
            member.endTime &&
    ...
    return <>
        ...
        {teams.length > 0 ? (
            <div className="bs-Fieldset-row">
                ...
                >
                    {teams.length > 0 &&
                        teams.map((member: $TSFixMe) => {
                            let membersFromList = teamMemberList.filter(
                                (memberFromList: $TSFixMe) => {
                                    return (
