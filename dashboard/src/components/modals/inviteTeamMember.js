...
import { teamCreate, teamCreateRequest, teamCreateSuccess, teamCreateError } from '../../actions/team';
	...
	submitForm = (values) => {
		...
		if(subProjects && subProjects.length >= 1 && data.subProjectId === currentProject._id){
			openModal({
				...
				onConfirm: () => teamCreate(data.subProjectId, values).then(function () {
					closeThisDialog();
						...
						<form id={`frm_${data.subProjectName}`} lpformnum="2" onSubmit={handleSubmit(this.submitForm)}>
							<div className="bs-Modal-header">
								<div className="bs-Modal-header-copy">
									<div className="db-InviteSetting-header">
										<h2><span>Invite new team members</span></h2>
										<p className="db-InviteSetting-headerDescription"><span>Enter the email addresses of the users you&#39;d like to invite, and choose the role they should have.</span></p>
										<div className="db-MultiEmailInput bs-TextInput">
											<div className="DraftEditor-root">
												...
												<div className="DraftEditor-editorContainer">
													<div aria-describedby="placeholder-ek1bp" className="notranslate public-DraftEditor-content" role="textbox" spellCheck="false" style={{ outline: 'none', whiteSpace: 'pre-wrap', wordWrap: 'break-word' }}>
														<div data-contents="true">
															<div data-block="true" data-editor="ek1bp" data-offset-key="chvmc-0-0">
																<div
									...
									<div className="db-RoleRadioList">
										...
										<div className="db-RoleRadioList-row">
											<label className="bs-Radio" htmlFor={`Member_${data.subProjectName}`}>
												...
												<span className="bs-Radio-button"></span><span className="bs-Radio-label"><div className="db-RoleRadioListLabel"><div className="db-RoleRadioListLabel-name"><span>Member</span></div><div className="db-RoleRadioListLabel-description"><span>Best for developers, devops, and people might be concerned about the downtime.</span></div><div className="db-RoleRadioListLabel-info"><div className="Box-root Flex-inlineFlex"><div className="Box-root Flex-flex"><div className="Box-root Flex-flex"></div></div></div></div></div></span></label></div>
										<div className="db-RoleRadioList-row">
											<label className="bs-Radio" htmlFor={`Administrator_${data.subProjectName}`}>
												<Field id={`Administrator_${data.subProjectName}`} className="bs-Radio-source" name="role" component="input" type="radio" value="Administrator" />
												<span className="bs-Radio-button"></span><span className="bs-Radio-label"><div className="db-RoleRadioListLabel"><div className="db-RoleRadioListLabel-name"><span>Administrator</span></div><div className="db-RoleRadioListLabel-description"><span>Administrators have complete control over project they are invited to. They can manage resources and invite new members.</span></div><div className="db-RoleRadioListLabel-info"><div className="Box-root Flex-inlineFlex"><div className="Box-root Flex-flex"><div className="Box-root Flex-flex"></div></div></div></div></div></span></label></div>
										<ShouldRender if={this.props.data.subProjectId === this.props.currentProject._id}>
											<div className="db-RoleRadioList-row">
							...
							<div className="bs-Modal-footer">
								<div className="bs-Modal-footer-actions">
									<ShouldRender if={this.props.team.teamCreate.error}>
										<div className="bs-Tail-copy">
											<div className="Box-root Flex-flex Flex-alignItems--stretch Flex-direction--row Flex-justifyContent--flexStart" style={{ marginTop: '10px' }}>
												...
												<div className="Box-root">
													<span style={{ color: 'red' }}>{this.props.team.teamCreate.error}</span>
												</div>
											</div>
										</div>
									</ShouldRender>
									...
									<button
										...
										disabled={this.props.team && this.props.team.teamCreate.requesting}
										type="submit">
										{!this.props.team.teamCreate.requesting && <span>Invite</span>}
										{this.props.team && this.props.team.teamCreate.requesting && <FormLoader />}
									</button>
								</div>
							</div>
						</form>
...
const mapDispatchToProps = (dispatch) => {
	return bindActionCreators({ teamCreate, teamCreateRequest, teamCreateSuccess, teamCreateError, openModal, closeModal }, dispatch)
}
...
FormModal.propTypes = {
	...
	teamCreate: PropTypes.oneOfType([
		PropTypes.func
	]).isRequired,
