...
import { smsTemplateTitles, smsTemplateDescriptions } from '../basic/SmsTitleList';
                    ...
                    <div className="bs-ContentSection-content Box-root Box-divider--surface-bottom-1 Flex-flex Flex-alignItems--center Flex-justifyContent--spaceBetween Padding-horizontal--20 Padding-vertical--16">
                        <div className="Box-root">
                            <span className="Text-color--inherit Text-display--inline Text-fontSize--16 Text-fontWeight--medium Text-lineHeight--24 Text-typeface--base Text-wrap--wrap">
                                <span>{template ? smsTemplateTitles[[template.smsType]] : 'Default SMS Template'}</span>
                            </span>
                            <p>
                                <span>
                                    {template ? smsTemplateDescriptions[[template.smsType]] : 'Default SMS Template'}
                                </span>
                            </p>
                        </div>
                    </div>
                        ...
                        <div className="bs-ContentSection-footer bs-ContentSection-content Box-root Box-background--white Flex-flex Flex-alignItems--center Flex-justifyContent--spaceBetween Padding-horizontal--20 Padding-vertical--12">
                            ...
                            <div className="bs-Tail-copy">
                                <div className="Box-root Flex-flex Flex-alignItems--stretch Flex-direction--row Flex-justifyContent--flexStart" style={{ marginTop: '10px' }}>
                                    <ShouldRender if={(editSmsTemplates && editSmsTemplates.error) || (resetSmsTemplates && resetSmsTemplates.error)}>
                                        ...
                                        <div className="Box-root">
                                            <span style={{ color: 'red' }}>{editSmsTemplates.error || resetSmsTemplates.error}</span>
                                        </div>
                                    </ShouldRender>
                                </div>
                            </div>
                            <div>
                                <button
                                    ...
                                    disabled={(editSmsTemplates && editSmsTemplates.requesting) || (resetSmsTemplates && resetSmsTemplates.requesting)}
                                    type="submit"
                                >
                                    <ShouldRender if={!(editSmsTemplates && editSmsTemplates.requesting) || (resetSmsTemplates && resetSmsTemplates.requesting)}>
                                        <span>Save</span>
                                    </ShouldRender>
                                ...
                                <button
                                    className="bs-Button"
                                    disabled={(resetSmsTemplates && resetSmsTemplates.requesting) || !template._id}
                                    type="button"
                                    onClick={() => { this.props.resetTemplate(template._id) }}
                                >
                                    <ShouldRender if={!(resetSmsTemplates && resetSmsTemplates.requesting)}>
                                        <span>Reset</span>
                                    </ShouldRender>
