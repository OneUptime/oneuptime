...
import { updateStatusPageSetting, updateStatusPageSettingRequest, updateStatusPageSettingSuccess, updateStatusPageSettingError } from '../../actions/statusPage';
...
export class Setting extends Component {
  ...
  submitForm = (values) => {
    ...
    this.props.updateStatusPageSetting(projectId._id || projectId, values).then( ()=> {
      reset()
    }, function () {
  ...
  render() {
    ...
    else if(this.props.statusPage && this.props.statusPage.status && this.props.statusPage.status._id){
      hosted = `${this.props.statusPage.status._id}.fyipeapp.com`;
    }
    if (this.props.statusPage && this.props.statusPage.status && this.props.statusPage.status._id) {
        ...
        <div className="Box-root">
          <div className="bs-ContentSection-content Box-root Box-divider--surface-bottom-1 Flex-flex Flex-alignItems--center Flex-justifyContent--spaceBetween Padding-horizontal--20 Padding-vertical--16">
            <div className="Box-root">
              ...
              <p><span>Change the domain settings of where the status page will be hosted.</span></p>
            </div>
          </div>
                    ...
                    <div className="bs-Fieldset-rows">
                      {
                        ...
                        <div className="bs-Fieldset-row">
                          <label className="bs-Fieldset-label"> Your Status Page is hosted at </label>
                        ...
                        <div className="bs-Fieldset-row">
                          <label className="bs-Fieldset-label">Your Status Page is hosted at </label>
                          <div className="bs-Fieldset-fields">
                            <span className="value" style={{ marginTop: '6px' }}>{hosted}</span>
                          </div>
                        </div>
                        )
                      }
            ...
            <div className="bs-ContentSection-footer bs-ContentSection-content Box-root Box-background--white Flex-flex Flex-alignItems--center Flex-justifyContent--spaceBetween Padding-horizontal--20 Padding-vertical--12"><span className="db-SettingsForm-footerMessage"></span>
              ...
              <div className="bs-Tail-copy">
                <div className="Box-root Flex-flex Flex-alignItems--stretch Flex-direction--row Flex-justifyContent--flexStart">
                  ...
                  <div className="Box-root">
                    <ShouldRender if={this.props.statusPage.setting.error}>
                      <span style={{ color: 'red' }}>{this.props.statusPage.setting.error}</span>
                    </ShouldRender>
                    <ShouldRender if={!this.props.statusPage.setting.error}>
                      <span>Changes to these settings will take 72 hours to propogate.</span>
                    </ShouldRender>
                  </div>
                </div>
              </div>
              ...
              <div>
                <RenderIfSubProjectAdmin subProjectId={ projectId }>
                  <button id="btnAddDomain" className="bs-Button bs-DeprecatedButton bs-Button--blue" disabled={this.props.statusPage.setting.requesting} type="submit">{!this.props.statusPage.setting.requesting && <span>Save Domain Settings </span>}
                    {this.props.statusPage.setting.requesting && <FormLoader />}</button>
...
const mapDispatchToProps = (dispatch) => {
  return bindActionCreators({ updateStatusPageSetting, updateStatusPageSettingRequest, updateStatusPageSettingSuccess, updateStatusPageSettingError }, dispatch)
}
...
function mapStateToProps(state) {
  return {
    ...
    initialValues: { domain: state.statusPage && state.statusPage.status && state.statusPage.status.domain ? state.statusPage.status.domain : '' },
