...
import {
    updateStatusPageMonitors, updateStatusPageMonitorsRequest,
    updateStatusPageMonitorsSuccess, updateStatusPageMonitorsError, fetchProjectStatusPage
...
export class Monitors extends Component {
    ...
    submitForm = (values) => {
        ...
        this.props.updateStatusPageMonitors(projectId._id || projectId, { _id: status._id, monitorIds })
            .then(() => {
                ...
                <div className="Box-root">
                    ...
                    <div className="bs-ContentSection-content Box-root Box-divider--surface-bottom-1 Flex-flex Flex-alignItems--center Flex-justifyContent--spaceBetween Padding-horizontal--20 Padding-vertical--16">
                        <div className="Box-root">
                            ...
                            <p><span>What monitors do you want to show on the status page?</span></p>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleSubmit(this.submitForm)}>
                                        ...
                                        <fieldset data-test="RetrySettings-failedAndExpiring" className="bs-Fieldset">
                                            <div className="bs-Fieldset-rows">
                                                <div className="bs-Fieldset-row">
                                                    <label className="bs-Fieldset-label" style={{ flex: '35% 0 0' }}><span>Add these to my status page</span></label>
                        ...
                        <ShouldRender if={!this.props.monitors.length > 0 && !this.props.monitors.requesting}>
                            <div className="bs-ContentSection-content Box-root Box-background--offset Box-divider--surface-bottom-1 Padding-horizontal--8 Padding-vertical--2">
                                <div>
                                    <div className="bs-Fieldset-wrapper Box-root Margin-bottom--2">
                                        <div id="app-loading" style={{ 'display': 'flex', 'justifyContent': 'center', 'alignItems': 'center', 'flexDirection': 'column' }}>
                                            <div style={{ 'marginTop': '20px', 'marginBottom': '20px' }}>No monitors are added to this project. <Link to={'/project/' + this.props.currentProject._id + '/monitoring'}> Please create one. </Link></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="bs-ContentSection-footer bs-ContentSection-content Box-root Box-background--white Flex-flex Flex-alignItems--center Flex-justifyContent--spaceBetween Padding-horizontal--20 Padding-vertical--12"><span className="db-SettingsForm-footerMessage"></span>
                            <div className="bs-Tail-copy">
                                <div className="Box-root Flex-flex Flex-alignItems--stretch Flex-direction--row Flex-justifyContent--flexStart" style={{ marginTop: '10px' }}>
                                    <ShouldRender if={this.props.statusPage.monitors.error} >
                                        ...
                                        <div className="Box-root">
                                            <span style={{ color: 'red' }}>{this.props.statusPage.monitors.error}</span>
                                        </div>
                            ...
                            <div>
                                <ShouldRender if={this.props.monitors.length > 0}>
                                    <button id="btnAddStatusPageMonitors" className="bs-Button bs-DeprecatedButton bs-Button--blue" disabled={this.props.statusPage.monitors.requesting} type="submit">{!this.props.statusPage.monitors.requesting && <span>Save Changes </span>}
                                        {this.props.statusPage.monitors.requesting && <FormLoader />}</button>
                                </ShouldRender>
                            </div>
                        </div>
                    </form>
...
const mapStateToProps = state => {
    ...
    if (status && status.monitorIds && status.monitorIds.length > 0 && monitors.length > 0) {
