...
import { 
	...
	createProjectError } from '../../actions/project';
import {
	CardElement,
	StripeProvider,
	Elements,
	...
	createToken = (values) => {
		...
		if (cardRegistered) {
			...
			stripe
				...
				.then(({ token }) => {
					if (token) {
						return checkCard({ tokenId: token.id, email, companyName })
					}
					else {
				...
				.then(result => {
					if (result.paymentIntent && result.paymentIntent.status === 'succeeded') {
						submitForm({ ...values, paymentIntent: result.paymentIntent.id });
					}
					else {
				...
				<div className="bs-Modal bs-Modal--medium">
										...
										<div className="bs-Modal-block bs-u-paddingless">
											<div className="bs-Modal-content">
												<div className="bs-Fieldset-wrapper Box-root Margin-bottom--2">
													<label>
														<CardElement
															{...createOptions(this.props.elementFontSize)}
														/>
													</label>
												</div>
											</div>
					...
					<div className="bs-Modal-footer">
						<div className="bs-Modal-footer-actions">
							<button
								id="btnCancelProject"
								className={`bs-Button bs-DeprecatedButton ${requesting && 'bs-is-disabled'}`}
								type="button"
								onClick={hideForm}
								disabled={requesting}
							>
							...
							<button
								className={`bs-Button bs-DeprecatedButton bs-Button--blue ${requesting && 'bs-is-disabled'}`}
								type="submit"
								disabled={requesting}
							>
...
const mapStateToProps = (state) => {
	return {
		email: state.profileSettings.profileSetting && state.profileSettings.profileSetting.data.email,
		companyName: state.profileSettings.profileSetting && state.profileSettings.profileSetting.data.companyName,
	}
}
...
const ProjectFormStripe = injectStripe(connect(mapStateToProps, mapDispatchToProps)(ProjectForm));
