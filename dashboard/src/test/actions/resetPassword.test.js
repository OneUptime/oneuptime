...
describe('actions', () => {
    it('should create an action of type PASSWORDRESET_REQUEST', () => {
        const promies = Promise.resolve(true);
        const expectedAction = {
            type: actions.PASSWORDRESET_REQUEST,
        };
        const action = actions.resetPasswordRequest(promies);
            ...
            switch (dispatched.type) {
                case actions.PASSWORDRESET_REQUEST:
                    expect(dispatched.type).toEqual(
                        actions.PASSWORDRESET_REQUEST
                    );
                    break;
                case actions.PASSWORDRESET_SUCCESS:
                    expect(dispatched.type).toEqual(
                        actions.PASSWORDRESET_SUCCESS
                    );
                    break;
                default:
                    expect(dispatched.type).toEqual(
                        actions.PASSWORDRESET_FAILED
                    );
