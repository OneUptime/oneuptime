...
describe('actions', async () => {
    it('should create an action of type CHANGEPASSWORD_REQUEST, and Return promise in payload', () => {
        const promise = Promise.resolve('CHANGEPASSWORD_REQUEST response');
        const action = actions.changePasswordRequest(promise);
...
describe('actions', () => {
    it('should create an action of type CHANGEPASSWORD_FAILED with error in payload', () => {
        ...
        const action = actions.changePasswordError('change pass error');
...
describe('actions', () => {
    it('should create an action of type CHANGEPASSWORD_SUCCESS, and response values in payload', () => {
        const action = actions.changePasswordSuccess('values');
...
describe('actions', () => {
    it('should despatch CHANGEPASSWORD_REQUEST and CHANGEPASSWORD_SUCCESS  actions', () => {
        axiosMock
            .onPost(`${API_URL}/user/reset-password`)
            .reply(
                200,
                { email: 'test', link: 'some link', someprops: 'value' },
                {}
            );
            ...
            switch (dispatched.type) {
                case actions.CHANGEPASSWORD_REQUEST:
                    expect(dispatched.type).toEqual(
                        actions.CHANGEPASSWORD_REQUEST
                    );

                    break;
                case actions.CHANGEPASSWORD_SUCCESS:
                    expect(dispatched.type).toEqual(
                        actions.CHANGEPASSWORD_SUCCESS
                    );
                ...
                default:
                    expect(dispatched.type).toEqual(
                        actions.CHANGEPASSWORD_FAILED
                    );
                    expect(dispatched.payload).toEqual('fail test');
            }
        };
        actions.changePassword('projectId', 5, 10)(dispatch);
    });
});
