...
describe('actions', async () => {
    it('should create an action of type ALERT_FETCH_REQUEST, and Return promise in payload', () => {
        const promise = Promise.resolve('alert fetch response');
        const action = actions.alertRequest(promise);
...
describe('actions', () => {
    it('should create an action of type ALERT_FETCH_FAILED', () => {
        ...
        const action = actions.alertError('error that occurred');
...
describe('actions', () => {
    it('should create an action of type ALERT_FETCH_SUCCESS or ALERT_FETCH_FAILED , and alert in payload', () => {
        const action = actions.alertSuccess('alert');
...
describe('actions', () => {
    it('should despatch ALERT_FETCH_REQUEST and ALERT_FETCH_SUCCESS  actions', () => {
        axiosMock
            .onGet(`${API_URL}/alert/projectId/getAlerts?skip=5&limit=10`)
            .reply(200, [], {});
        ...
        actions.fetchAlert('projectId', 5, 10)(dispatch);
    });
});
...
describe('actions', async () => {
    it('should create an action of type INCIDENTS_ALERT_FETCH_REQUEST, and Return promise in payload', () => {
        const promise = Promise.resolve('incident alert fetch response');
        const call = actions.incidentAlertRequest(promise);
...
describe('actions', () => {
    it('should create an action of type INCIDENTS_ALERT_FETCH_FAILED with error in payload', () => {
        ...
        const action = actions.incidentAlertError('incident alert error');
        expect(action.type).toEqual(expectedAction.type);
        expect(action.payload).toEqual(expectedAction.payload);
    });
});
...
describe('actions', () => {
    it('should create an action of type INCIDENTS_ALERT_FETCH_SUCCESS with alert in payload', () => {
        ...
        const action = actions.incidentAlertSuccess('alert');
        expect(action.type).toEqual(expectedAction.type);
        expect(action.payload).toEqual(expectedAction.payload);
    });
});
...
describe('actions', () => {
    it('should despatch ALERT_FETCH_REQUEST and ALERT_FETCH_SUCCESS  actions', () => {
        axiosMock
            .onGet(
                `${API_URL}/alert/projectId/incidentId/getIncidentAlerts?skip=5&limit=10`
            )
            .reply(200, [], {});
        ...
        const dispatch = dispatched => {
            switch (dispatched.type) {
                case actions.INCIDENTS_ALERT_FETCH_SUCCESS:
                    expect(dispatched.type).toEqual(
                        actions.INCIDENTS_ALERT_FETCH_SUCCESS
                    );
                    expect(dispatched.payload).toEqual([]);
                    break;
                case actions.INCIDENTS_ALERT_FETCH_REQUEST:
                    expect(dispatched.type).toEqual(
                        actions.INCIDENTS_ALERT_FETCH_REQUEST
                    );

                    break;
                default:
                    expect(dispatched.type).toEqual(
                        actions.INCIDENTS_ALERT_FETCH_FAILED
                    );
                    expect(dispatched.payload).toEqual('fail test');
            }
        };
        actions.fetchIncidentAlert('projectId', 'incidentId', 5, 10)(dispatch);
    });
});
