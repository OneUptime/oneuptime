...
storiesOf('Monitor', module)
    ...
    .add('NewMonitor', () => <NewMonitor {...props} />);
storiesOf('Monitor', module)
    .addDecorator(story => (
        <Provider store={store_requesting}>{story()}</Provider>
    ))

    .addDecorator(story => <div style={{ margin: '5%' }}>{story()}</div>)

    .add('NewMonitor Requesting', () => <NewMonitor {...props} />);
storiesOf('Monitor', module)
    .addDecorator(story => <Provider store={store_error}>{story()}</Provider>)

    .addDecorator(story => <div style={{ margin: '5%' }}>{story()}</div>)

    .add('NewMonitor Error', () => <NewMonitor {...props} />);
