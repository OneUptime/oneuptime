...
| Monitor-backed devices | Gear that cannot be walked (`monitoringMethod = Monitor`) is a first-class device: a bound monitor is never required to register one (all forms, the API, discovery); an unbound device reads Pending tagged "No monitor" on the list, the site tab and the Overview hero; the server keeps `isReachable` in step with the bound monitor's status so the summary tiles and the Status chip count it correctly; the status bridge fires on probe-driven changes; a Ping monitor can be created and bound from the create form, the device page, or in bulk from the device list | `Common/Types/NetworkDevice/NetworkDeviceMonitoringMethod.ts`, `Common/Server/Services/NetworkDeviceService.ts` (`refreshStampedMonitorStatus`), `Common/Utils/NetworkDiscovery/PingMonitorBuilder.ts`, `App/FeatureSet/Dashboard/src/Components/NetworkDevice/PingMonitorProvisioning.ts`, `useBulkCreatePingMonitors.tsx` |
