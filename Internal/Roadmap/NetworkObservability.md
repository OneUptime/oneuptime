...
| Topology | LLDP + CDP neighbor walks; opt-in ARP + FDB endpoint collection (row-bounded walks, wall-clock deadline); per-site uplink inference that places monitor-backed (ping-only) devices by recognising them in a switch's FDB, derived per build and never stored | `SnmpMonitor.ts` (endpoint phase), `Common/Models/DatabaseModels/NetworkEndpoint.ts`, `Common/Utils/Monitor/EndpointUplinkInferenceUtil.ts` |
