...
15. **Environment-aware remediation on your own infrastructure — propose-first, honestly scoped.** After a confident RCA, the Brain proposes concrete remediation _actions_ — start one of your existing runbooks, or a drafted command for one of your self-hosted Runbook Agents — as structured, persisted, approvable artifacts on the incident/alert, never as prose suggestions. The scope is deliberately narrow: the Brain picks from **human-authored runbooks and human-deployed agents only** (server-side allowlists — it cannot invent targets), drafted commands **always** require a human approval everywhere, and unattended execution exists only for runbook actions on agents a human explicitly tagged as non-production — with untagged agents counting as production, so autonomy is opt-in per host, never assumed. Execution rides the ordinary audited runbook substrate, attributed and budget-capped. This is not "self-healing infrastructure"; it is a diagnosed proposal with a one-click approve, plus a fenced-off test-environment fast path for teams that want it.
...
- **Execution on customer environments is propose-first, and a drafted command never runs itself.** Any Brain-initiated action on customer infrastructure is a persisted, approvable proposal before it is anything else. Free-form commands the Brain drafted require explicit human approval **always and everywhere** — no flag, plan, or environment relaxes this, because a drafted command has been reviewed by nobody. Unattended execution is reserved for human-authored runbooks on agents a human explicitly tagged as non-production, and the tagging fails safe: an untagged or unknown agent counts as production, so nothing auto-runs anywhere until someone deliberately says "this host is a test box". Executions are budget-capped (a daily cap where _unset means the default, never unlimited_ — actions on infrastructure are default-bounded in a way token spend is not — plus a per-incident cap), proposals expire rather than lingering, and every execution is fully attributed end to end: the proposing run, the approving human (or the auto-approval decision), and the resulting runbook execution are all linked records. The targets the Brain may name are server-side allowlists — injected telemetry can influence what it suggests, never mint where things run.
