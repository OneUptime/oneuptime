...
| RCA also written as incident internal note (system-authored; the note service skips its generic announcement for user-less notes so quiet mode is preserved) | ✅ | `Sentinel/IncidentInvestigationRunner.ts` (postAnalysis), `IncidentInternalNoteService.onCreateSuccess` |
...
- **2026-07-10** — RCA now also posted as a system-authored incident internal note; `IncidentInternalNoteService.onCreateSuccess` skips its generic announcement for user-less notes so the RootCause feed item (quiet-mode gated) stays the single notification source.
