...
| G11 | Preventive precision + noise budget (vision §4.14/§6): per-finding-type measured precision (human confirm/dismiss rates) gates any automation; per-repo `maxOpenPullRequests` + per-project daily fix-task budget + fingerprint dedupe enforced; fix PRs human-reviewed always — auto-merge is a non-goal, not a setting | ❌ NOT MET (lane not built; first step is outcome instrumentation on the existing manual fix path) | Exception-fix path is manual-only: user click per exception, one active task per (exception, recipe). **One carve-out since 2026-07-13:** the ImproveInstrumentation recipe auto-creates a fix task from an inconclusive investigation — behind an explicit default-FALSE project opt-in (`enableInstrumentationFixTasks`), per-subject dedupe, and covered by the shipped PR-outcome sync; per-repo PR caps, the daily fix-task budget, draft PRs, and CI verify are still missing, so the gate stays NOT MET. Since 2026-07-13 task creation also fail-early validates all prerequisites (readiness API) and on Cloud agents can only receive project-owned LLM keys — the global/billed provider is unreachable from the agent path when billing is enabled (self-host installs fall back to the operator's own global provider, `getLlmProviderForAgentTasks`); findings inbox does not exist, so nothing to be noisy yet | Any auto-created fix task; any preventive PR automation; any "incidents prevented" claim |
...
- [ ] Auto-created fix tasks for graduated finding types only (new-fingerprint exceptions first — highest precision, handler exists), honoring per-repo guardrails (**re-introduce on `CodeRepository`** — the original `restrictedImprovementActions`/`maxOpenPullRequests` columns were dropped with the `ServiceCodeRepository` table on 2026-07-13) + a per-project daily fix-task budget + a retry cap (the 30-min stuck-task reset currently retries forever); **draft PRs, human review always**; requires the code-fix track's sandbox + verify loop (today's path commits the agent diff and opens a non-draft PR with zero build/test/lint) *(first auto-created recipe shipped 2026-07-13 outside the findings lane: **ImproveInstrumentation** — the vision §5 "instrumentation-gap ticket", upgraded to a PR, triggered by an inconclusive investigation; default-false `Project.enableInstrumentationFixTasks` opt-in + per-subject dedupe, but NONE of this item's guardrails — per-repo caps, daily budget, draft PRs, CI verify — exist yet, so they remain open here and apply to that recipe too)*
