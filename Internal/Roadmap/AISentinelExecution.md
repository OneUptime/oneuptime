...
- **2026-07-13** — **Shipped UX-overhaul C1: service ↔ repository link suggestions.** Deterministic name-matcher (`Common/Server/Utils/CodeRepository/ServiceRepoLinkSuggester.ts`: exact > containment > token-overlap, no LLM, already-linked services excluded) exposed via `GET /service-code-repository/suggest-links` (new `ServiceCodeRepositoryAPI`, caller-props-scoped) and rendered as one-click "Suggested links" cards on both linking pages (repo → Services tab, service → Code Repositories tab). This is the on-ramp the Preventive lane's repo-mapping dependency needs. Tests: `ServiceRepoLinkSuggester.test.ts`.
