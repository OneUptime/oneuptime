...
### Tier 1 (ships next): verify via the customer's own CI on draft PRs
...
- **Regression-test SHOULD-FAIL check**: for `WriteRegressionTest` PRs, a failing test job on the PR is the *expected* outcome — reported as `expected-failure-observed` rather than red. Closes the honesty gap deterministically.
- Runs annotated with the CI conclusion as a `ProgressLog` event.

Limits, stated: repos without CI verify nothing (reported as `unverified`, per G9 "verification fails to unverified, never verified"); we never re-run or gate the customer's CI; check-run polling shares the PR-state sync cadence.
...
3. **B4-PR3** — Tier 1 CI verify: check-run polling, `ciStatus` on PR rows, verified-green rate, SHOULD-FAIL reporting.
