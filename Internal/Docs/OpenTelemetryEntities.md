...
> - **Deferred (not yet built):** the **frontend** read-switch + net-new entity reads (e.g. "everything on pod X"), scalar per-type key columns (`hostEntityKey`, ...), host-MV re-key + OTLP `entity_refs` parsing (phase 6). Registry / ClickHouse backfill is intentionally **not** done — forward-only by decision.
