...
> **Implementation status (2026-06-09) — phases 1 + 2 + 3 landed; phase 4 read-enablement foundation landed (frontend swap deferred).**
...
> - **Deferred (not yet built):** the **frontend** read-switch + net-new entity reads (see above), scalar per-type key columns (`hostEntityKey`, ...), `TelemetryEntityRelationship` relationships/topology (phase 5), host-MV re-key + OTLP `entity_refs` parsing (phase 6). Registry / ClickHouse backfill is intentionally **not** done — forward-only by decision.
