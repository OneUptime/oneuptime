...
> - **Deferred (not yet built):** the **frontend** read-switch for the *existing* host/docker/k8s tabs (gated on backfill or retention aging — see above), scalar per-type key columns (`hostEntityKey`, ...), host-MV re-key + OTLP `entity_refs` parsing (phase 6). Registry / ClickHouse backfill is intentionally **not** done — forward-only by decision.
