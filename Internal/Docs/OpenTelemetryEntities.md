...
> - **Frontend read-switch deferred (deliberate):** swapping the Host/Docker/K8s telemetry tabs from `attributes['resource.host.name'] = X` to `has(entityKeys, key)` is **not** done. For those special-cased views the attribute predicate already returns identical rows; with no backfill, old rows have empty `entityKeys`, so a switch must keep the attribute predicate as an OR fallback — same results, marginally slower. The payoff (drop the fallback → bloom-indexed fast path) only arrives once old rows age out of retention **or** a backfill runs. The foundation above makes that switch a per-tab one-liner when worthwhile, and is the prerequisite for net-new entity reads (pods/containers/nodes) that have no attribute view today.
...
> - **Deferred (not yet built):** the **frontend** read-switch for the *existing* host/docker/k8s tabs (gated on backfill or retention aging — see above), scalar per-type key columns (`hostEntityKey`, ...), host-MV re-key + OTLP `entity_refs` parsing (phase 6), a `lastSeenAt`-based prune cron for the registry tables, and the per-project opt-in + entity budget for promoting high-churn types. Registry / ClickHouse backfill is intentionally **not** done — forward-only by decision.
