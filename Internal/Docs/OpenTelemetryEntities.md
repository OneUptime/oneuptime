...
> - **Deferred (not yet built):** the **frontend** read-switch + net-new entity reads + the **topology/service-map** graph viz (the relationship data layer is built; the visualization is not), scalar per-type key columns (`hostEntityKey`, ...), host-MV re-key + OTLP `entity_refs` parsing (phase 6). Registry / ClickHouse backfill is intentionally **not** done — forward-only by decision.
