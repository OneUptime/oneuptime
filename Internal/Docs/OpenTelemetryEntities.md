...
> **Implementation status (2026-06-09) — phases 1 + 2 + 3 landed.**
...
> - **Deferred (not yet built):** scalar per-type key columns (`hostEntityKey`, ...), read-path switch to `has(entityKeys, ...)` (phase 4), `TelemetryEntityRelationship` relationships/topology (phase 5), host-MV re-key + OTLP `entity_refs` parsing (phase 6). Registry backfill of existing Service/Host/... rows is intentionally **not** done — forward-only by decision.
