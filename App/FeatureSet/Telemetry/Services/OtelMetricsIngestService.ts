...
export default class OtelMetricsIngestService extends OtelIngestBaseService {
  private static async flushMetricsBuffer(
    ...
    while (
      ...
      /*
       * Emit the reactive fresh-shape signal so the telemetry-monitor scheduler
       * can skip provably-idle monitors. Best-effort: any failure is swallowed
       * inside recordFreshShapes so it can never affect ingest.
       */
