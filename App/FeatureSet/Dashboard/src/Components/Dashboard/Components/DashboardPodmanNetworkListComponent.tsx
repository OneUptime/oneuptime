...
import DashboardPodmanNetworkListComponent from "Common/Types/Dashboard/DashboardComponents/DashboardPodmanNetworkListComponent";
...
export interface ComponentProps extends DashboardBaseComponentProps {
  component: DashboardPodmanNetworkListComponent;
}
...
const DashboardPodmanNetworkListComponentElement: FunctionComponent<
  ComponentProps
> = (props: ComponentProps): ReactElement => {
  const [networks, setNetworks] = useState<Array<PodmanResource>>([]);
  const [error, setError] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState<boolean>(true);

  const args: DashboardPodmanNetworkListComponent["arguments"] =
  ...
  const rows: Array<ReactElement> = networks.map((n: PodmanResource) => {
    const id: string = (n._id as string) || "";
    const name: string = (n.name as string) || "Unnamed";
    const driverScope: string = (n.state as string) || "—";
    const hostName: string = (n.podmanHost?.name as string) || "—";
...
export default React.memo(
  DashboardPodmanNetworkListComponentElement,
  arePropsEqual,
);
