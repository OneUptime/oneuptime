...
export interface ComponentProps {
  ...
  onTemplateSelected: (template: PodmanAlertTemplate) => void;
}

const PodmanTemplatePicker: FunctionComponent<ComponentProps> = (
  props: ComponentProps,
): ReactElement => {
  const allTemplates: Array<PodmanAlertTemplate> = getAllPodmanAlertTemplates();

  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-500">
        Select a pre-built alert template to quickly set up Podman container
        monitoring. The template will auto-configure the metric, aggregation,
        time range, and thresholds.
      </p>

      <div className="grid grid-cols-1 gap-2">
        {allTemplates.map((template: PodmanAlertTemplate) => {
...
export default PodmanTemplatePicker;
