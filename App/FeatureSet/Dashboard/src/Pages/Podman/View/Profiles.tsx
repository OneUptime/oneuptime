...
const PodmanHostProfiles: FunctionComponent<
  PageComponentProps
> = (): ReactElement => {
  ...
  const profileQuery: Query<Profile> = useMemo(() => {
     ...
     * inline attribute maps — same workaround the Host/Podman logs pages use.
     */
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const q: any = {
      /*
       * Podman hosts scope by resource attribute only — there is no
       * entity-key builder for Podman hosts (see
       * Common/Utils/Telemetry/EntityKey), so no entityScope predicate
       * here. This mirrors the Podman logs / traces pages.
       */
      attributes: {
   ...
   * primaryEntityId, but profiles produced on a Podman host rarely
   * carry the PodmanHost row id there — batches with a resolvable
   ...
   * PodmanHost id (see OtelIngestBaseService.selectPrimaryEntity).
  ...
  return (
    <Fragment>
      <ProfileTable
        ...
        noItemsMessage="No performance profiles found for this Podman host."
      />
    </Fragment>
  );
};

export default PodmanHostProfiles;
