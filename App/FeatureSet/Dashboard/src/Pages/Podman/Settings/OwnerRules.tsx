...
const podmanHostOwnerDocumentation: string = `
### How Podman Host Owner Rules Work

Podman Host Owner Rules add owner users and teams to a Podman host automatically when it matches your criteria — without anyone having to remember to assign owners.

### Match Criteria

A rule matches a Podman host only when **all** specified criteria pass. Empty criteria are skipped.

- **Podman Host Labels** — any-of (M2M)
...
const PodmanHostOwnerRulesPage: FunctionComponent<RuleSettingsPageProps> = (
  props: RuleSettingsPageProps,
): ReactElement => {
  return (
    <RuleTable<PodmanHostOwnerRule>
      modelType={PodmanHostOwnerRule}
      viewRuleId={RuleViewPageUtil.getViewRuleId(props, PodmanHostOwnerRule)}
      listRoute={RuleViewPageUtil.getListRoute(
        PageMap.PODMAN_SETTINGS_OWNER_RULES,
      )}
      getRuleViewRoute={(rule: PodmanHostOwnerRule): Route => {
        return RuleViewPageUtil.getRuleViewRoute(
          PageMap.PODMAN_SETTINGS_OWNER_RULE_VIEW,
          rule,
        );
      }}
      id="podmanHost-owner-rules-table"
      name="Settings > Podman Host Owner Rules"
      userPreferencesKey="podmanHost-owner-rules-table"
      saveFilterProps={{
        tableId: "podman-host-owner-rules-table",
      }}
      isDeleteable={true}
      isEditable={true}
      isCreateable={true}
      ...
      cardProps={{
        title: "Podman Host Owner Rules",
        description:
          "Auto-assign owner users and teams when matching Podman hosts are created.",
      }}
      helpContent={{
        title: "How Podman Host Owner Rules Work",
        description:
          "Match Podman hosts and add owner users/teams automatically.",
        markdown: podmanHostOwnerDocumentation,
      }}
      sortBy="name"
      ...
      columns={[
        ...
        },
        {
          ...
          title: "Status",
          type: FieldType.Boolean,
          getElement: (item: PodmanHostOwnerRule): ReactElement => {
      ...
      formFields={[
        {
          ...
          required: true,
          placeholder: "Assign team to matching Podman hosts",
        ...
        },
        {
          ...
          sectionDescription:
            "Filter Podman hosts by labels. Leave empty to skip the filter.",
...
export default PodmanHostOwnerRulesPage;
