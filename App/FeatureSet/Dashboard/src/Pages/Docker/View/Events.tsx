...
const DockerHostEvents: FunctionComponent<
  PageComponentProps
> = (): ReactElement => {
  const modelId: ObjectID = Navigation.getLastParamAsObjectID(1);

  const [events, setEvents] = useState<Array<DockerEventRow>>([]);
  ...
  const fetchData: PromiseVoidFunction = async (): Promise<void> => {
    ...
    try {
      ...
      const dockerEvents: Array<DockerEventRow> = [];
      ...
      for (const log of listResult.data) {
        ...
        dockerEvents.push({
          ...
          action: (attrs["docker.event.action"] as string) || log.severityText || "info",
          message: typeof log.body === "string" ? log.body : JSON.stringify(log.body),
        });
      }

      setEvents(dockerEvents);
    } catch (err) {
  ...
  const paginatedData: Array<DockerEventRow> = useMemo(() => {
  ...
  const tableColumns: Array<Column<DockerEventRow>> = [
    {
      title: "Time",
      type: FieldType.Text,
      key: "timestamp",
  ...
  return (
    <Card
      title="Docker Events"
      description="Events from the last 24 hours on this host."
      buttons={cardButtons}
    >
      <Table<DockerEventRow>
        id="docker-events-table"
        columns={tableColumns}
        data={paginatedData}
        singularLabel="Docker Event"
        pluralLabel="Docker Events"
        isLoading={false}
        error=""
        ...
        sortBy={sortBy as keyof DockerEventRow | null}
        sortOrder={sortOrder}
        onSortChanged={(
          newSortBy: keyof DockerEventRow | null,
          newSortOrder: SortOrder,
        ) => {
          setSortBy(newSortBy as string | null);
          setSortOrder(newSortOrder);
        }}
        noItemsMessage="No Docker events found in the last 24 hours. Events will appear here once the Docker agent is sending data."
      />
    </Card>
  );
};
