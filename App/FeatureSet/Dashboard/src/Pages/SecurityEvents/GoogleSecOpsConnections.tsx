...
A poll's budgets are 20 search pages shared by the two created-time passes, 16 alerts-view requests and 4 minutes; a pass cut short by them is a warning check, never a success. When a window holds more detections than that, polling never re-reads the same window forever. The poll is **Partial**, the cursor stays put, and the next poll reads half as long a window from the same starting point (\`This window holds more records than one poll can read; the next poll reads a {n} minute window from the same starting point.\`), halving down to one minute; each complete poll doubles the window back towards 24 hours. If even one minute cannot be read in full, polling moves past it so newer detections keep arriving, and stores this in **Last Error**: \`More records were created in the one minute from {start} to {end} than one poll can read. Polling moved past this minute so newer records keep arriving; use Import this time range in Diagnostics on this minute to recover what one run can read.\` Recover that minute with **Import this time range** in **Diagnostics** (the range inputs use your local time; the message is in UTC). A failed poll keeps its cursor and window length and retries on the next tick. Shortening the poll interval does not change the window.
            ...
            getCustomElement: (
              ...
              return (
                <InlineConnectionTest
                  ...
                  /*
                   * A report describes the settings it was run with; editing
                   * the region, instance, key or data to import hides it.
                   */
                  settingsKey={connectionTestSettingsKey((): JSONObject => {
                    return googleSecOpsTestBody(formValues);
                  })}
                  disabledReason={
