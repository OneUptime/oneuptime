...
const DockerSwarmClusterViewLayout: FunctionComponent<
  PageComponentProps
> = (): ReactElement => {
  const { id } = useParams();
    ...
    const fetchCounts: () => Promise<void> = async (): Promise<void> => {
      try {
        ...
        const summaryUrl: URL = URL.fromString(APP_API_URL.toString())
          .addRoute("/docker-swarm-resource/inventory-summary/")
        ...
        setResourceCounts({
          ...
          services: readNum("serviceCount"),
          tasks: readNum("taskCount"),
          stacks: readNum("stackCount"),
          networks: readNum("networkCount"),
          secrets: readNum("secretCount"),
          configs: readNum("configCount"),
...
export default DockerSwarmClusterViewLayout;
