...
- A **OneUptime Telemetry Ingestion Token** — create one from _Project Settings → 텔레메트리 및 APM → 수집 키_ and copy the value
...
| `ONEUPTIME_SERVICE_TOKEN` | Yes      | Telemetry ingestion token from _Project Settings → 텔레메트리 및 APM → 수집 키_                                        |
