...
> **이것은 노드 메트릭도 함께 제거합니다.** kubelet, cAdvisor, hostmetrics 리시버는 로그 컬렉터 DaemonSet 안에 있으므로, 파드 로그를 끄면 이들도 함께 삭제됩니다 — OOM-kill, CPU 스로틀링, PVC 저디스크 모니터도 마찬가지입니다. 클러스터 수준 메트릭과 Kubernetes 이벤트는 유지되지만, 노드별 또는 컨테이너별 메트릭은 유지되지 않습니다. 메트릭은 유지하면서 로그 볼륨을 줄이려면 대신 [`filters.logs.minSeverity`](#filtering-by-log-severity) 또는 [`namespaceFilters`](#namespace-filtering)를 사용하세요.
...
> `api`와 `disabled`는 둘 다 로그 컬렉터 DaemonSet을 제거하며, 그와 함께 위에서 설명한 노드, 파드, 컨테이너 및 호스트 메트릭도 제거합니다 — `logs.enabled=false`와 동일한 트레이드오프입니다. 오직 `daemonset` 모드만 이들을 수집합니다. `api` 모드를 강제하는 GKE Autopilot 및 EKS Fargate 프리셋이 kubelet 메트릭을 보고하지 않는 이유가 바로 이것입니다.
