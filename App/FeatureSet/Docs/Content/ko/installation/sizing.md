...
이 가이드는 Kubernetes(Helm)에서 자체 호스팅 OneUptime 배포의 크기를 정하는 데 도움이 됩니다. OneUptime이 의존하는 세 가지 데이터스토어 — **PostgreSQL**, **Valkey**, **ClickHouse** — 와 애플리케이션 컴퓨트를 다루며, 실제 수치를 확보한 뒤 조정할 수 있는 시작 티어를 제공합니다.
...
| **Valkey**     | 캐시, 작업 큐, 세션                                                                          | **큐 깊이와 활성 세션**. 메모리에 좌우되며 규모가 작습니다. 신뢰할 수 있는 원본이 아닙니다.    |
...
캐시 계층은 Redis 7.2의 BSD 라이선스 포크인 [Valkey](https://valkey.io)를 실행하며, 캐시, 작업 큐, 세션 저장소로 사용됩니다. Redis 프로토콜을 사용하는 서버라면 무엇이든 그 자리를 대신할 수 있으며, 아래 사이징은 어느 쪽이든 동일하게 적용됩니다. 이는 **메모리에 좌우되며** 영속성은 **기본적으로 비활성화되어 있습니다**(여기서 Redis는 신뢰할 수 있는 원본이 아닙니다 — 재구축할 수 있습니다). 예상 큐 깊이와 동시 세션으로 크기를 정하세요. 대부분의 배포는 2–8 GB의 메모리로 충분합니다. 기본 축출 정책이 `noeviction`이라는 점에 유의하세요. 따라서 지속적인 과부하 상황에서 큐가 쌓이면 Redis 메모리를 모니터링하세요.
...
- **Valkey** — 차트에는 차트 내 복제 기능이 없습니다. HA를 위해서는 OneUptime이 **외부 관리형 Redis**(또는 AI/클러스터 배포)를 가리키도록 하세요.
