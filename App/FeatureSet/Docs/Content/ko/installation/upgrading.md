...
유지되며 보존 TTL에 따라 점진적으로 스스로 삭제됩니다.
...
- **업그레이드 이전 텔레메트리를 UI에서 볼 필요가 없는 경우:** 할 일이
  ...
  이전 데이터는 보이지 않은 채 기존 테이블에서 만료됩니다.
- **업그레이드 이전 텔레메트리를 계속 보고 싶은 경우:** 업그레이드 후
  언제든지 아래의 수동 복사를 실행합니다.
...
다음 작업은 **업그레이드가 완전히 기동된 후**에 실행합니다(새 테이블과
해당 materialized view가 존재해야 합니다). ClickHouse 호스트에서 직접
연결하십시오 — 네이티브 프로토콜에는 HTTP 타임아웃이 없으므로 몇 시간이
걸리는 문(statement)도 문제없이 실행됩니다:
