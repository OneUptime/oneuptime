...
2. OneUptime에서 **워크플로 → 전역 변수 → 만들기** 로 이동하고, 이름을 `SERVICENOW_AUTH` 로 지정하고, base64 문자열을 붙여넣고 **Is Secret** 를 켭니다.

## 2단계 — 워크플로 구성

1. **워크플로 → 워크플로 생성** 을 열고, 이름을 `Incidents → ServiceNow` 로 지정하고 **빌더** 를 엽니다.
