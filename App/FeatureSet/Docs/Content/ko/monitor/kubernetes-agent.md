...
| `oneuptime.url` | *(필수)* | OneUptime 인스턴스의 URL입니다. |
| `oneuptime.apiKey` | *(필수)* | 프로젝트 API 키(Settings → API Keys). |
| `clusterName` | *(필수)* | 이 클러스터의 고유한 이름입니다. 모든 레코드에 `k8s.cluster.name`으로 기록됩니다. |
