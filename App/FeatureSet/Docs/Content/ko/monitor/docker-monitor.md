...
- **그룹화 기준** — 선택적으로 `resource.container.name`별로 그룹화하여 각 컨테이너가 독립적으로 평가되도록 함
...
베이스라인 이상 탐지(임계값 없음 — 폼에 대신 **민감도**와 **베이스라인 기간**이 표시되며, 각 샘플을 요일·시간대가 같은 베이스라인과 비교함):

- **비정상적으로 높음** — 값이 예상 범위를 넘어 상승함
- **비정상적으로 낮음** — 값이 예상 범위 아래로 하락함
- **비정상** — 값이 어느 방향으로든 예상 범위를 벗어남

이상 탐지 조건은 선택한 베이스라인 기간 이상의 이력이 쌓이기 전까지 알림을 생성하지 않습니다(Learning 상태).
...
컨테이너가 `json-file` 로그 드라이버를 사용하지 않을 가능성이 큽니다. 위의 [로그 드라이버 요구 사항](#로그-드라이버-요구-사항) 섹션의 진단 명령을 실행하고 로그를 전송해야 하는 컨테이너를 전환하십시오.
