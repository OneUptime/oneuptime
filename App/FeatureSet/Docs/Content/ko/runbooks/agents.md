...
| **실행 타임아웃**   | 30초   | 에이전트가 스크립트를 종료시키기 전까지 허용하는 시간. (Bash는 `SIGKILL`. JavaScript의 isolate는 분해됨.)                                                                                                       |

둘 다 단계별로 설정할 수 있습니다. **Runbooks &rsaquo; 여러분의 Runbook &rsaquo; 단계**를 열고, Bash 또는 JavaScript 단계를 펼쳐, 스크립트 아래에서 **실행 타임아웃**과 **클레임 타임아웃**을 초 단위로 설정하세요. 필드를 비워 두면 기본값이 사용됩니다. 각각 1초에서 1시간까지 지정할 수 있고, 그 범위를 벗어난 값은 단계가 실행될 때 범위 안으로 제한됩니다.
