...
**Runbook 에이전트**는 Runbook의 Bash 단계를 **여러분의 자체 인프라 안에서** 실행하는 작은 자체 호스팅 프로세스입니다. OneUptime 워커는 여러분의 셸 명령을 직접 실행하지 않습니다 — 큐에 적재할 뿐이고, 여러분이 환경에 설치한 Runbook 에이전트가 그것을 가져가서 실행하고 결과를 회신합니다.

이 페이지에서는 에이전트를 설치하고, Bash 단계를 그쪽으로 라우팅하고, 일상적으로 운영하는 방법을 설명합니다.

## 에이전트가 존재하는 이유

이전 OneUptime 버전은 Bash 단계를 워커에서 직접 실행했습니다. 운영자가 이미 머신에 셸을 갖고 있는 싱글 테넌트 자체 호스팅 환경에서는 동작했지만, 다른 모두에게는 두 가지 문제가 있었습니다:

- **신뢰 경계**: Runbook을 작성할 수 있는 사람이라면 누구나 워커에서 셸을 실행할 수 있고, 워커가 가진 모든 환경 변수와 파일시스템에 접근할 수 있습니다.
- **도달 범위**: 유용한 Bash 단계의 대부분은 *고객 측* 인프라에서 동작하길 원합니다 ("이 서비스 재시작", "우리 클러스터에 kubectl") — OneUptime 측이 아닙니다.

Runbook 에이전트는 이를 뒤집습니다. Bash 단계는 우리 쪽에서 돌지 않습니다. 여러분이 통제하는 호스트에서 돌고, 그 호스트가 무엇을 할 수 있는지는 여러분이 결정합니다.
...
4. Bash 단계가 실행되면, 워커는 그 단계의 **Agent Tag**가 표시된 작업 행을 삽입하고 상태를 `Pending`으로 설정합니다.
5. 같은 프로젝트에서 그 태그를 가진 건강한 임의의 에이전트가 작업을 원자적으로 차지하고 (두 에이전트가 같은 작업을 동시에 실행하는 일은 없습니다) 로컬에서 `bash -c <당신의 스크립트>`를 실행하고, stdout/stderr/exit 코드를 캡처해 결과를 회신합니다.
...
| **태그** | 쉼표로 구분. Bash 단계는 태그를 지정하고, 프로젝트에서 그 태그를 가진 임의의 에이전트가 실행할 수 있습니다. 흔한 패턴: `prod`, `staging`, `eu-west-1`, `db-host`. |
...
태그는 Bash 단계가 에이전트를 찾는 방법입니다. 몇 가지 패턴:
...
Bash 단계는 정확히 하나의 에이전트 태그를 **반드시** 지정해야 합니다. 다중 태그 라우팅 (`prod` AND `db`를 모두 가진 임의의 에이전트에서 실행) 은 로드맵에 있지만 이번 릴리스에는 없습니다.

## Bash 단계를 에이전트로 지정

Runbook에 Bash 단계를 추가하면 폼이 **Agent Tag**를 묻습니다:

- 실행시키고 싶은 에이전트(들)에 매치되는 태그를 입력하세요.
- 아래 에디터에 스크립트를 작성하세요.

Runbook이 실행되어 이 단계에 도달하면 워커는 그 태그로 작업을 큐에 넣습니다. 그 태그를 가진 건강한 에이전트가 최소 하나 온라인이라면 작업은 몇 초 안에 차지되어 실행됩니다.

## 운영 메모

### 타임아웃

모든 Bash 단계에 두 개의 타임아웃이 적용됩니다:

| 타임아웃 | 기본값 | 제어 대상 |
| --- | --- | --- |
| **Claim 타임아웃** | 2분 | 워커가 *어떤* 에이전트라도 작업을 차지할 때까지 기다리는 시간. 시간 내에 아무도 가져가지 않으면 단계는 `TimedOut`으로 실패하고 Runbook은 (**실패 시 계속**에 따라) 계속하거나 멈춥니다. |
| **실행 타임아웃** | 30초 | 에이전트가 스크립트에 `SIGKILL`을 보내기 전까지 실행하게 두는 시간. 단계별 구성 가능. |
...
스크립트의 자식 프로세스는 리스 만료 시 자동으로 취소되지 **않습니다** — 다만 워커는 더 이상 그것을 기다리지 않고, 다른 클레임이 인계받은 뒤에는 에이전트가 결과를 제출할 수 없습니다. exactly-once가 중요하면 스크립트를 재실행 안전하게 설계하세요.
