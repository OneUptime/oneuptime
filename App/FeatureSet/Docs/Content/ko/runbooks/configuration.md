...
- 단계별 실행 타임아웃 기본값: JavaScript, Bash, HTTP 모두 **30초**. Runbook의 **단계** 페이지에서 단계별로 설정하세요 — 필드를 비워 두면 기본값이 유지됩니다.
- Bash와 JavaScript 단계의 **클레임 타임아웃**: **2분** — 워커가 실패 처리하기 전 선택된 에이전트가 작업을 가져가기까지 기다리는 시간. 이것도 단계별 설정 가능.
- 두 타임아웃 모두 **1초에서 1시간**까지 지정할 수 있습니다. 그 범위를 벗어난 값은 단계가 실행될 때 범위 안으로 제한되므로, 잘못 입력한 값이 타임아웃을 무력화하거나 워커 슬롯을 무한정 점유하는 일은 없습니다.
