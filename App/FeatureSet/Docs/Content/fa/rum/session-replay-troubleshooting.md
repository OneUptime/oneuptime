...
| `config-recorder-version-invalid` | سیاست `recorderVersion`ای حمل می‌کرد که خارج از دستور زبان نسخهٔ پذیرفته‌شده و امن برای مسیر بود، پس هیچ نشانی فرآورده‌ای ساخته نشد. پراکسی‌ای که بازنویسی می‌کند یا پاسخی دست‌کاری‌شده — استقراری که ساختی ندارد به‌جایش `config-disabled` با `recorder-not-built` گزارش می‌دهد. |
...
| `artifact-url-invalid` | سیاست نسخه‌ای را نام برد که خارج از دستور زبان نسخهٔ پذیرفته‌شده و امن برای مسیر بود، پس هیچ نشانی‌ای ساخته نشد. پاسخ پیکربندی دست‌کاری‌شده یا از پراکسی گذشته. |
