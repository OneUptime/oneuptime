...
این کار نسخه ضبط‌کننده، شناسه نشست، `state` آن (`recording`، `uploading`، `not-sampled`، `stopped`…)، اینکه اگر ایستاده چرا ایستاده، `bootstrapDecision`، پاسخ هر دروازه‌ای زیر `decisions` (نمونه‌برداری‌شده یا نه، وضعیت رضایت، اینکه آیا ارسال مجاز است و چه چیزی جلویش را می‌گیرد، آخرین دستور کارساز)، `capabilities` ضبط‌کننده، و فهرست کامل رکوردها را برمی‌گرداند — از جمله رکوردهای **خرده‌اسکریپت راه‌انداز**، که پیش از وجود فرآورده ضبط‌کننده اجرا می‌شود و بیشتر تصمیم‌های کنار کشیدن آنجا گرفته می‌شوند. بنا به ساختش هیچ محتوای صفحه‌ای در بر ندارد؛ [آنچه تشخیص‌ها هرگز در بر ندارند](#what-the-diagnostics-never-contain) را ببینید. فیلدها در [`getDiagnostics()`](/docs/telemetry/session-replay#getdiagnostics) شرح داده شده‌اند.
...
| `config-fetch-rejected` | نقطه پایانی پاسخی غیر 2xx داد. [وضعیت‌های واکشی پیکربندی](#config-fetch-statuses) پایین را ببینید. |
...
| `server-directive` | کارساز میان نشست تغییر داد که ضبط‌کننده باید چه کند. مقدار `reason` واژگانی بسته است — [وضعیت‌های POST قطعه](#chunk-post-statuses) پایین را ببینید. |
...
2. **آیا `GET .../session-replay/v1/config`ای هست؟**
   ...
   - **بله، غیر 2xx** → [وضعیت‌های واکشی پیکربندی](#config-fetch-statuses) را ببینید.
