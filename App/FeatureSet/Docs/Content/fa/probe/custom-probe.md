...
- `PROBE_DISCOVERY_REVERSE_DNS_BUDGET_IN_MS` — سقف زمانی، به میلی‌ثانیه، برای یافتن نام‌های DNS معکوس (PTR) میزبان‌هایی که جاروی کشف یافته است. تنظیم‌نشده (یا ۰) رهایش کنید تا از شمار میزبان‌ها اندازه‌گیری شود: ۶۰ ثانیه برای حداکثر حدود ۸۶۰ میزبان، که تا بیشینه ۱۰ دقیقه رشد می‌کند. میزبان‌هایی که جست‌وجو به‌موقع به آن‌ها نرسد نام DNS معکوس نمی‌گیرند (و اگر SNMP یا NetBIOS نامشان را نیابد با نشانی IP فهرست می‌شوند) و پیام وضعیت پویش می‌گوید چند میزبان از قلم افتاده‌اند. برای شبکه‌ای بزرگ پشت کارساز DNS کُند آن را بالا ببرید (پیش‌فرض: ۰، بازه: ۱۰۰۰ تا ۱۲۰۰۰۰۰)
