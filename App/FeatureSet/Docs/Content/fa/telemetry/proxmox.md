...
عامل API ‏PVE را روی شبکه پرس‌وجو می‌کند، پس لازم نیست روی گره‌ای از خوشه زندگی کند — و در حالت ایده‌آل نباید: آن را روی ماشینی اجرا کنید که از شکست گره‌ای جان به در می‌برد (ماشین مجازی کوچکی برای مانیتورینگ روی سخت‌افزار جدا، میزبانی مدیریتی)، یا `PVE_HOST` را به‌جای نشانی گرهی تنها به VIP / نام DNS چرخشی نشانه بگیرید. اگر هدف API عامل همان گره‌ای باشد که تازه مرده، مانیتورینگ شما هم با آن می‌میرد. (تنها استثنا خط لوله اختیاری گزارش‌های journald است، که باید روی گره PVE اجرا شود — [فرستادن گزارش‌های سرویس Proxmox](#optional-ship-proxmox-service-logs) را ببینید.)
