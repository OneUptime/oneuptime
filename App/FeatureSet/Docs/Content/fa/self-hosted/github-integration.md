...
| Pull requests | Read & Write | ساخت و مدیریت درخواست‌های ادغام، و منتشر کردن بازبینی‌ها |
| Issues | Read & Write | خواندن issueها، و منتشر کردن نظرهای برنامه — **از جمله روی درخواست‌های ادغام**، که GitHub گفت‌وگویشان را از راه API مربوط به issueها می‌گذراند |
...
**همان Issues: Read & write است که برنامه را تعاملی می‌کند.** بدون آن، نام‌بردن‌ها دریافت می‌شوند و بعد وقتی برنامه می‌خواهد پاسخ بدهد خاموش شکست می‌خورند — GitHub نظرهای گفت‌وگوی درخواست ادغام را از API مربوط به issueها سرو می‌کند، پس همین یک دسترسی، دروازه هر پاسخی است که برنامه می‌نویسد. [کار با OneUptime از درون GitHub](/docs/ai/github-app) را ببینید.
...
OneUptime از دو دسته رویداد استفاده می‌کند، و کار این دو فرق می‌کند.

**همگام‌سازی مخزن** — `installation` و `installation_repositories`. GitHub Apps این‌ها را خودکار دریافت می‌کنند؛ مجموعه مخزن‌های وصل‌شده را با آنچه برنامه رویش نصب است همگام نگه می‌دارند.

**برنامه تعاملی** — در این‌ها باید صریحاً مشترک شوید، و هرکدام راه مشخصی برای سپردن کار به برنامه را فعال می‌کنند:

| رویداد | چه چیزی را فعال می‌کند |
| ------------------------------- | --------------------------------------------------------------------- |
| **Issue comment** | فرمان‌های `@mention` روی issueها **و** روی درخواست‌های ادغام |
| **Issues** | واگذاری یک issue به برنامه، و برچسب تریگر مخزن |
| **Pull request** | درخواست بازبینی از برنامه |
| **Pull request review** | نام بردنی که در بدنه بازبینی ثبت‌شده نوشته شده باشد |
| **Pull request review comment** | نام بردن روی نظری درون‌خطی در diff |

اگر در هیچ‌کدام از این‌ها مشترک نباشید، GitHub App همچنان مخزن‌ها را وصل می‌کند و همچنان از دل OneUptime درخواست‌های ادغام رفع باز می‌کند — فقط هرگز به چیزی که در GitHub نوشته می‌شود پاسخ نمی‌دهد. این رایج‌ترین علت «بات مرا نادیده می‌گیرد» است. برای اینکه فرمان‌ها چه‌اند و چه کسی اجازه صادر کردنشان را دارد، [کار با OneUptime از درون GitHub](/docs/ai/github-app) را ببینید.

رویدادهای دیگر (**Push** و **Workflow run**) فقط تأیید دریافت و نادیده گرفته می‌شوند؛ اشتراک آن‌ها اعلان یا خودکارسازی CI/CD را فعال نمی‌کند.
