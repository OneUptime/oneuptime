...
- A **Paused** or **Misconfigured** evaluation measures nothing, so it posts nothing rather than a number that would read as real. Charts show a gap.
...
In a chart or value widget, pick one of the `oneuptime.slo.*` metrics. Group by `sloName` for one line per SLO, or filter on `sloName` or on an `oneuptime.label.*` attribute to narrow it down. Use the aggregations from the table above: Avg for the percentages and durations, Max for burn rate and status.
...
1. **Service Level Objectives** — an SLO list with every SLO that is not archived: its status, SLI against target, error budget bar and burn rate, least budget first, with a count of SLOs by status.
2. **Error Budget & Burn Rate** — the **Lowest Error Budget Remaining**, **Least Error Budget Time Left** and **Peak Burn Rate** tiles, and **Error Budget Remaining by SLO** and **Burn Rate by SLO** charts with one line per SLO.
...
Archived SLOs do not appear in the list or the Selected SLO widgets. Because the toolbar's options come from metrics, an archived SLO's name stays in the picker until its last points age out (30 days by default); picking it shows that no active SLO has that name.
