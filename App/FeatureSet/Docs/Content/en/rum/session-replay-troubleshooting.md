...
| `config-recorder-version-invalid`  | The policy carried a `recorderVersion` outside the accepted path-safe version grammar, so no artifact URL could be built. A rewriting proxy or a tampered response — a deployment with no build reports `config-disabled` with `recorder-not-built` instead. |
...
| `artifact-url-invalid`   | The policy named a version outside the accepted path-safe version grammar, so no URL was built. A tampered or proxied config response.                                                                    |
