...
- `OTEL_EXPORTER_OTLP_ENDPOINT` is the **base** URL. Claude Code appends the per-signal path itself. Only use `OTEL_EXPORTER_OTLP_METRICS_ENDPOINT` / `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT` if you need to split signals, and give those the full path (`https://oneuptime.com/otlp/v1/metrics`, `.../v1/logs`).
...
**Use a drop-in instead of the main file if you already have a policy.** A `managed-settings.d/` directory next to the main file merges `*.json` in alphabetical order, so a telemetry-only `10-telemetry.json` ships cleanly without touching an existing security policy file. Since v2.1.223 the `env` block is merged **per variable** across admin sources, so a telemetry drop-in still applies for variables the higher-ranked sources leave unset.
...
Precedence, highest first: server-managed → MDM/OS policy → managed-settings files → `HKCU`.

One gotcha worth planning around: when a **server-managed** change touches an `env` variable, the developer has to accept a security-approval dialog in an interactive session before it takes effect. File-based managed settings do not have that step.

**Verify on a developer machine.** Run `/status` in Claude Code and look at the `Setting sources` line — it should read `Enterprise managed settings (file)`. That confirms the file was found and parsed, before you go hunting for missing data in OneUptime.
...
| `claude_code.commit.count`            | count  | standard attributes only                                                                                                                                                                                                                                                                       |
| `claude_code.pull_request.count`      | count  | standard attributes only                                                                                                                                                                                                                                                                       |
...
| `claude_code.code_edit_tool.decision` | count  | `tool_name` = `Edit` \| `Write` \| `NotebookEdit`; `decision` = `accept` \| `reject`; `source` = `config` \| `hook` \| `user_permanent` \| `user_temporary` \| `user_abort` \| `user_reject`; `language` (for example `TypeScript`, `Python`, `Markdown`, `unknown`)                           |
...
Third-party agent, skill, plugin, marketplace, MCP server and MCP tool names are **redacted** to generic values like `custom` / `third-party` unless you set `OTEL_LOG_TOOL_DETAILS=1`. First-party names are not redacted.

## Events (OTLP logs) reference

Events arrive as OTLP log records. Every record also carries `event.name` (the bare name without the `claude_code.` prefix), `event.timestamp` (ISO 8601) and `event.sequence` (a monotonic counter). Records additionally carry `prompt.id`, `message.uuid`, `workspace.host_paths`, `workflow.run_id`, `workflow.name` and `client_request_id` for correlation, and pick up `trace_id` / `span_id` from an inbound `TRACEPARENT` even when no traces exporter is configured.
...
| `claude_code.mcp_server_connection`   | `status`, `transport_type` (`stdio` / `sse` / `http`), `server_scope`, `duration_ms`, `error_code`, `server_name`                                                     |
...
| `claude_code.plugin_loaded`           | `plugin.name`, `plugin.version`, `marketplace.name`                                                                                                                   |

`claude_code.api_request` carries both `cost_usd` (a float) and `cost_usd_micros` (an integer in millionths of a USD). Use the integer when you need per-request cost you can add up without rounding drift.
...
**Turn it off for fleet-wide rollouts.** You lose nothing for cost and usage reporting; per-session detail still exists in the events, which are not affected by this toggle.
...
| `OTEL_LOG_TOOL_CONTENT`        | off     | Tool input and output content in span events                                                   |
...
Span names are `claude_code.interaction` at the top, with `claude_code.llm_request`, `claude_code.hook` and `claude_code.tool` beneath it, and `claude_code.tool.blocked_on_user` / `claude_code.tool.execution` under the tool span. The LLM spans carry GenAI semantic convention keys — `gen_ai.system` (always `anthropic`), `gen_ai.request.model`, `gen_ai.response.id`, `gen_ai.response.finish_reasons`, `gen_ai.tool.call.id` — alongside `input_tokens`, `output_tokens`, `cache_read_tokens`, `cache_creation_tokens`, `ttft_ms`, `duration_ms`, `stop_reason` and `llm_request.context`. Because they use the standard GenAI keys, OneUptime classifies them as LLM calls the same way it classifies spans from any other [GenAI instrumentation](/docs/telemetry/ai-llm-observability).

`CLAUDE_CODE_PROPAGATE_TRACEPARENT=1` forwards the W3C `traceparent` through a custom `ANTHROPIC_BASE_URL` proxy. Bash and PowerShell subprocesses inherit `TRACEPARENT` automatically, so a build script that is itself instrumented joins the same trace.
...
**Check what settings Claude Code actually loaded.** Run `/status`. The `Setting sources` line tells you whether the managed settings file was picked up (`Enterprise managed settings (file)`). If it is not listed, the file is at the wrong path, is not valid JSON, or is not readable by the developer's user account.

**Managed settings look right but the env block is ignored.** If the change came from **server-managed** settings rather than a file, the developer has to accept a security-approval dialog in an interactive session first. Ask them to open Claude Code interactively once.
