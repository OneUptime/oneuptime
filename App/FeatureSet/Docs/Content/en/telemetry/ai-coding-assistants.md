...
| -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| **Claude Code**                                                      | Native — metrics and logs, traces in beta                                                                                                                                                                                                                                                                    | Direct OTLP from each developer machine; environment variables or a managed-settings file                                           | **Yes** — `user.email`, emitted natively on every metric datapoint and event. Falls back to `user.account_uuid` / `user.account_id`. Note it also emits an anonymous per-install `user.id` — see the caveat under [Attributes read as the employee](#attributes-read-as-the-employee) | [Claude Code](/docs/telemetry/claude-code)                            |
...
| **GitHub Copilot**                                                   | Native on **agent surfaces only** — VS Code Chat and agent mode, the agent host behind Copilot CLI, the Copilot SDK, JetBrains agent workflows. GitHub documents no OTel for inline completions, Copilot on github.com or Copilot code review, and marks `telemetry` unsupported for the Copilot cloud agent | Direct OTLP from the editor or agent host; enterprise managed settings for a fleet                                                  | **No** — Copilot emits no user attribute of any kind. Inject `enduser.id` and `user.email` per machine via `resourceAttributes` or `OTEL_RESOURCE_ATTRIBUTES`                                                                                                                         | [Gemini CLI & GitHub Copilot](/docs/telemetry/gemini-cli-and-copilot) |
| **Cline**                                                            | Native — metrics and logs, no traces                                                                                                                                                                                                                                                                         | Direct OTLP, but configured centrally: an admin sets the endpoint and protocol in the Cline organization dashboard, not per machine | **No** — Cline documents `user_id` as an anonymized user identifier, and publishes no way to resolve it to a person                                                                                                                                                                   | —                                                                     |
| **LiteLLM / Portkey gateways**                                       | Native trace export — LiteLLM's `otel` callback; Portkey's self-hosted and enterprise export, which Portkey marks experimental                                                                                                                                                                               | Direct OTLP from the gateway process. One configuration covers every application behind it                                          | **Yes, with LiteLLM** — the key owner's id AND email, when you issue one virtual key per person. Works on a stock proxy: OneUptime reads both the default v1 spellings (`metadata.user_api_key_user_id`, `metadata.user_api_key_user_email`) and the v2 `litellm.`-namespaced ones    | [AI Gateways](/docs/telemetry/ai-gateways)                            |
...
| **Windsurf**                                                         | **None documented.** Windsurf's published docs index lists no telemetry, OpenTelemetry or OTLP page at all                                                                                                                                                                                                   | Only through a gateway you put in front of it                                                                                       | Not over telemetry. Per-user data lives in Windsurf's Enterprise HTTP API, which OneUptime does not poll                                                                                                                                                                              | —                                                                     |
...
| **Amazon Bedrock**                                                   | **None from the InvokeModel / Converse APIs.** But **Bedrock AgentCore is different** — AWS documents it as emitting OTEL-compatible metrics, spans and logs                                                                                                                                                 | Instrument the calling application, or front Bedrock with a gateway that emits OTLP                                                 | Whatever your app or gateway stamps. AWS's own path is `identity.arn` in model invocation logs plus `iamPrincipal/` cost allocation tags — neither reaches OneUptime                                                                                                                  | —                                                                     |
...
The Bedrock row is about the **model-invocation APIs** (`InvokeModel`, `Converse`), which push no OpenTelemetry to you. **Amazon Bedrock AgentCore is a separate case:** AWS documents it as emitting OTEL-compatible telemetry — built-in metrics for agents, gateway resources and memory resources, plus spans and logs for memory resources when enabled — routed to CloudWatch by default. Setting `DISABLE_ADOT_OBSERVABILITY=true` unsets the runtime's default ADOT configuration so you can point the ADOT SDK at OneUptime's OTLP endpoint instead.
...
LiteLLM names these attributes differently in its two OpenTelemetry modes: the default v1 `otel` callback uses a bare `metadata.` prefix, and the opt-in v2 mode (`LITELLM_OTEL_V2=true`) uses the `litellm.` namespace. **OneUptime reads both spellings**, so attribution works on a stock proxy with no extra configuration. The employee email list follows the same rule — `metadata.user_api_key_user_email` on v1, `litellm.metadata.user_api_key_user_email` on v2.

`user.id` leads because it is the canonical OpenTelemetry key for a human actor — the one the semantic-conventions maintainers point people at, and the one to standardize on if you are stamping identity yourself. **One exception: Claude Code.** It emits `user.id` as a random anonymous identifier persisted in `~/.claude.json`, not a person. On its metric datapoints that is harmless, because the metric list below is email-first — but if you enable Claude Code's traces beta, that anonymous `user.id` outranks `user.email` on the spans. Drop or remap `user.id` in a collector processor for that fleet. `enduser.id` is still an active attribute (the 1.25 deprecation removed `enduser.role` and `enduser.scope`, not `enduser.id`), so it is accepted as an equal alias. `cursor.user.id` sorts last because it is an opaque integer that needs Cursor's admin API to resolve to a person.
...
All three carry the **caller's own downstream customer**, not the employee. `gen_ai.user` and `llm.user` are how instrumentations echo back OpenAI's `user` request parameter — now deprecated in favour of `safety_identifier` and `prompt_cache_key` — which OpenAI documents as "a stable identifier for your end-users", sent so OpenAI can detect abuse by _your_ users. LiteLLM's `user_api_key_end_user_id` is its explicit end-user id, distinct from the `user_api_key_user_id` key-owner id above.
...
| Provider              | **No.** The coding-agent counters carry no GenAI provider attribute (`gen_ai.provider.name`, or its deprecated predecessor `gen_ai.system`) — Cursor and Claude Code route to several providers behind one subscription and never say which |
...
For Windsurf, JetBrains AI, the Bedrock model-invocation APIs, Azure OpenAI and anything else in the matrix with no native export, the clean architecture is an AI gateway. Every call flows through one process, that process emits OpenTelemetry, and you configure the export once instead of on every machine.

**Use LiteLLM with one virtual key per employee.** That single decision is what makes the whole thing attributable:

- **Either OpenTelemetry mode works.** The two LiteLLM OTel versions name these attributes differently: the default v1 `otel` callback emits bare `metadata.user_api_key_user_id` / `metadata.user_api_key_user_email` / `metadata.user_api_key_team_id`, while `LITELLM_OTEL_V2=true` emits `litellm.metadata.user_api_key_user_id`, `litellm.metadata.user_api_key_team_id` and `litellm.team.id`. **OneUptime recognizes both**, so you do not need to switch modes or rewrite attributes in a collector to get per-employee attribution.
- LiteLLM stamps the key-owner id on the span — the internal user who owns the key the request authenticated with. OneUptime reads it as the employee id.
- The team id becomes the team column, so cost-centre rollups work without touching resource attributes.
...
Do **not** reach for the end-user id for this — `metadata.user_api_key_end_user_id` on v1, `litellm.metadata.user_api_key_end_user_id` and `litellm.end_user.id` on v2. It is the downstream customer, and all three spellings are excluded for the reason above.
...
| LiteLLM        | **Depends on the OTel version.** v1 (the default `otel` callback): **ON** — turn it off with `turn_off_message_logging`, the callback's `message_logging`, or `OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT`. v2 (`LITELLM_OTEL_V2=true`): **OFF** — it records metadata only and `OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT` defaults to `no_content`; opt in with `span_only`, `event_only` or `span_and_event` |
