...
If you need prompt-level traces from Cursor, this pipeline cannot give them to you. The closest thing Cursor offers is client-side [hooks](https://cursor.com/docs/hooks) — spawned processes that communicate over stdio and can forward structured telemetry wherever you like. Cursor's OpenTelemetry pages do not point at hooks as the alternative; it is simply the mechanism that exists, and it is a build-it-yourself path not covered here.
...
2. **Test connection.** Cursor documents this as a check of the URL and auth; what the check reports back is not specified.
...
`cursor.model.name` is the requested **public** model after routed-intent collapse: Cursor rewrites `auto:` to `Auto`, `thinking:` to `Thinking`, `pro:` to `Pro` and `premium:` to `Premium`, and passes anything else through. It is absent on Bugbot and whenever the source had no model. So a model breakdown will contain a bucket literally named `Auto` that cannot be decomposed into the underlying models Cursor routed to — the same reason the Provider breakdown is unavailable.
...
| `cursor.cloud_agent.setup` (body `cloud_agent_setup_<kind>`)               | INFO for `started` / `completed`; ERROR for `failed` | `cursor.cloud_agent.setup.kind` (`started`, `completed`, `failed`), `cursor.cloud_agent.setup.duration_ms` (terminal kinds only — `completed` / `failed`, and only when present), `cursor.cloud_agent.setup.reason` (failures only; open vocabulary, e.g. `install_command_failed`)                                                                                                          |
| `cursor.cloud_agent.artifact` (body `cloud_agent_artifact_created`)        | INFO                                                 | `cursor.cloud_agent.artifact.file_name` (always), `cursor.cloud_agent.artifact.content_type` (optional; MIME type)                                                                                                                                                                                                                                                                           |
| `cursor.cloud_agent.pull_request` (body `cloud_agent_pull_request_<kind>`) | INFO for `opened`; WARN for `creation_failed`        | `cursor.cloud_agent.pull_request.kind` (`opened`, `creation_failed`), `cursor.cloud_agent.pull_request.number` (`opened` only), `cursor.cloud_agent.pull_request.draft` (`opened` only). Cursor notes `creation_failed` is live and `opened` may be sparse while the producer rolls out                                                                                                      |
...
- **`GET /teams/members`** — the roster. Each member has `id`, `email`, `name`, `role` (Cursor documents `member` and `owner` as examples, not as a closed set — tolerate other values) and `isRemoved`. Note that `id` here is an **encoded string** like `user_PDSPmvukpYgZEDXsoNirw3CFhy`, not an integer. Rate limit 20 requests/minute per team — the Admin API default for most endpoints; the 60/minute below is specific to `/teams/filtered-usage-events`.
- **`POST /teams/filtered-usage-events`** — the money view. Every event carries `userEmail`, `model`, `kind`, `chargedCents`, `requestsCosts`, `isTokenBasedCall`, `isChargeable`, `isHeadless` and a `timestamp` in epoch milliseconds — sent as a JSON **string**, so parse it before doing arithmetic. Two fields are conditional: `tokenUsage` (`inputTokens`, `outputTokens`, `cacheWriteTokens`, `cacheReadTokens`, `totalCents`, optional `discountPercentOff`) is present only when `isTokenBasedCall` is `true`, and `conversationId` is omitted for events with no associated conversation. Sum `chargedCents` for spend — Cursor documents it as the field that reconciles event-level costs with `/teams/spend` totals and the dashboard totals, including both the model cost and the Cursor Token Rate where the rate applies. Body takes `startDate` and `endDate` as epoch milliseconds (both bounds inclusive), plus optional `userId` (number), `email`, `page` and `pageSize` (default 100, max 1000). Rate limit 60 requests/minute; the data is aggregated hourly, so poll at most once an hour.

The join back to your OneUptime data is **`conversationId`**: Cursor documents it as a join key with other sources that expose conversation ids, and it is the same id space as `cursor.conversation.id` on the OTel wire, which lets you attach an email and a real dollar figure to a session you already have log events for. It is **optional on both sides** — the Admin API omits it for events with no associated conversation, and `cursor.conversation.id` is an optional log attribute. Expect a residue of usage events and log records that cannot be joined, and bucket them as unattributed rather than assuming full coverage.
...
- Re-run **Test connection** in Cursor — Cursor documents it as checking the URL and auth.
