...
- Each poll is bounded (requests per run, alerts per run, and Falcon's own limit of 10,000 results per query). When a bound stops a poll before the window is fully read, the run is reported as **Partial** with a warning and the cursor is held, so the next poll re-reads the same window instead of losing alerts.
...
The report then adds the OneUptime platform checks — worker consumers on the Worker queue, the poll scheduler, the security event store, and (for a saved connection) whether polling is overdue — so "nothing is imported" can be told apart from "Falcon has nothing to import". Each failed check carries remediation text. **Copy report** copies the checklist as JSON; credentials and tokens are never included.
...
- `CrowdStrike Falcon alerts query failed (HTTP 429)` or `CrowdStrike Falcon alerts fetch failed (HTTP 429)` — Falcon is rate limiting this API client. The message names the `X-RateLimit-RetryAfter` header Falcon sent (when the header was available). Wait for that period; scheduled polls retry automatically on the next tick. If it recurs, give OneUptime its own API client instead of sharing one with other integrations.
...
- A run reports **Partial** with `Falcon's alerts query cannot page past 10000 results` — the window holds more than 10,000 alerts, which Falcon's offset paging cannot read in one query. Shorten the **Poll interval (minutes)** so each window holds fewer alerts, or import the period with **Import history** in smaller ranges.
