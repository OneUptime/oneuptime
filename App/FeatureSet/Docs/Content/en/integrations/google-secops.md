...
- **Google returns zero records** — check the exact time window, the basis and the saved scope in Diagnostics, and run Test connection to read the availability counts under both scopes. A non-alerting detection may require Alerts and detections. Preview a period covering both its detection time and creation time. The initial scheduled poll looks back 24 hours by created time; use Import history for older records.
