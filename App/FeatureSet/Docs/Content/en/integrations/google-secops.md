...
- **Test connection** runs synchronously in the API process. It does not need a background worker, which is exactly why it can tell you when no worker is running. It reports a checklist: **Configuration**, **Authenticate with Google**, **Read rule detections**, **Read curated rule detections**, **Read the alerts view**, **Detections available to import**, **Background workers**, **Poll scheduler**, **Security event storage** and, for a saved connection, **Scheduled polling**. Each read check is a one-record probe over the last 24 hours. The availability check counts what Google has under the saved scope *and* under the other scope (Alerts only versus Alerts and detections) over the last 24 hours and the last 7 days, one page per range, reported as a number or as `1000+`; when the saved scope has nothing but the other scope does, the check warns: your rules create detections but alerting is not enabled on them; switch the scope to Alerts and detections or enable alerting in Google SecOps. The storage check runs the same duplicate lookup polling runs before every insert, so a broken analytics database fails here with a name rather than an hour later as a poll error. Every request in a test has a 20 second deadline. Tests leave the polling cursor and event store untouched; a saved connection gets a run-history row of type test. You can also test settings from the create form before saving them; nothing is stored until you save.
...
1. **Scope.** The default scope is **Alerts only**, and a rule detection is only an alert when alerting is enabled on the rule in Google SecOps. A tenant whose rules create detections without alerting polls honestly empty forever. Run **Test connection**: the **Detections available to import** check counts both scopes and says so when the other scope has records. Switch the scope to **Alerts and detections** or enable alerting on the rules.
2. **Rule run frequency and detection latency.** Detections are created when the rule runs, which can be up to an hour or a day after the events they describe. The connector polls by created time, so they arrive on the first poll after Google creates them, not at their detection time. A rule you just enabled has created nothing yet, and a quiet tenant has nothing to create. The availability check reports how many detections were created in the last 24 hours and 7 days; zero under both scopes means Google has nothing for OneUptime to import yet.
...
- **Last Error is populated** — a poll was attempted and something in it failed. The field stores the complete error message with credentials redacted. When a connection has an error, select **View Error** in its **Actions** column to open the full message, then **Copy Error** in the dialog to copy it for support. The error clears on the next successful poll, which also removes the **View Error** action. Read the prefix first. Only three prefixes carry an HTTP status, so a message without one is not evidence of a fault on OneUptime's side:
