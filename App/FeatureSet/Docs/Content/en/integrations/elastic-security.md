...
- **Each scheduled poll covers at most 24 hours.** A connection whose cursor is older catches up in consecutive daily windows instead of skipping ahead.
- **The cursor moves only when a window was read completely.** A poll reads up to 1,000 alerts per request and up to 20 requests. If it hits either bound, or a page cannot be advanced (more than 1,000 alerts sharing one creation timestamp), the run is reported as **Partial** with a warning, the cursor is held, and the next poll re-reads the same window. Nothing is silently dropped.

Kibana's detection alerts search accepts a size and a sort but no page cursor, so the connector pages by advancing the lower bound of its `@timestamp` range to the last alert on each page and dropping the re-read alerts by `_id`.
...
- `Elastic Security alerts search did not complete` — the search timed out or the connection dropped after authentication succeeded. Large windows on a slow cluster can hit the 60 second request timeout; the cursor is held and the next poll retries the same window.
...
- **Health is Partial** — the last poll hit a bound and held its cursor; read the run's warnings in **Diagnostics**. A connection that is Partial on every poll needs a shorter poll interval, so each window holds fewer than 20,000 new alerts.
