...
- Detect tasks that crash, restart, or get rescheduled (a replacement task shows under 60s of uptime)
...
- **Group By** — Optionally group by the `resource.container.name` attribute so each Swarm task is evaluated independently — one incident per task
...
Each container's metrics arrive as their own OTLP **resource**, so container identity is stored `resource.`-prefixed in ClickHouse:

| Attribute                       | Meaning                                                     | Example        |
| ------------------------------- | ----------------------------------------------------------- | -------------- |
| `resource.container.name`       | A Swarm task's container, named `<service>.<slot>.<taskid>` | `web.1.abc123` |
| `resource.container.image.name` | The container's image                                       | `nginx:latest` |
...
Filter on `resource.container.image.name`, `docker.swarm.service.name`, or `docker.swarm.node.name` to scope a query, on `resource.container.name` to scope to one task, and group by `resource.container.name` to evaluate each task independently — one incident per task.
...
| --------------------------- | -------- | -------------------------------------------------------------------------------------------------------------- |
| **Task Down (Low Uptime)**  | Critical | Any task's `container.uptime` is under 60s (rescheduled, restarted, or crashed and replaced)                   |
| **High Task CPU Usage**     | Warning  | Any task's `container.cpu.utilization` exceeds 80% of one CPU core (100% = one full core)                      |
| **High Task Memory Usage**  | Warning  | Any task's `container.memory.percent` exceeds 85% of its limit (or of node memory, for services with no limit) |
| **High Task Process Count** | Warning  | Any task's `container.pids.count` exceeds 500                                                                  |

Each template groups by `resource.container.name`, so one incident fires per affected task.
