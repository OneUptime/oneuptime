...
После установки ваши сервисы начинают появляться в разделе **Продукты → Трассировка** и на карте сервисов в течение пары минут, причём `k8s.cluster.name` устанавливается равным вашему `clusterName`, что позволяет фильтровать по кластеру.
...
OBI также может распространять контекст трассировки через границы сервисов, чтобы span на стороне Pod B связывался с той же трассировкой, что и исходящий запрос Pod A — без изменений SDK в обоих приложениях. **По умолчанию это отключено; включите с помощью `--set ebpf.contextPropagation=true`.**

| Опция                      | По умолчанию | Описание                                                                                                                                                                                         |
| -------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `ebpf.contextPropagation`  | выкл.        | Вставка W3C `traceparent` в исходящий трафик, чтобы span связывались между сервисами. По умолчанию отключено — сначала прочитайте предупреждение ниже. Дополнительных требований к ядру нет; никакая версия ядра не делает её безвредной. |
| `ebpf.contextPropagationMode` | headers   | Как вставляется `traceparent`: `headers` (заголовки запросов HTTP/1.1, наименьшая зона влияния), `tcp` (TCP-опция через Linux Traffic Control) или `all` (оба). Читается только если `contextPropagation` имеет значение true. |
...
> **⚠️ Почему это отключено по умолчанию.** Любой режим распространения изменяет трафик, который уже находится в пути: незашифрованные HTTP-запросы расширяются на месте в ядре, а к TLS и «сырому» TCP добавляется TCP-опция через хук Traffic Control. Если учёт байтов соединения не скорректирован в точности, поток теряет синхронизацию — сообщаемый симптом состоит в том, что передачи через L7-прокси вроде nginx зависают в середине тела, как только ответ превышает ~64 КБ, тогда как небольшие запросы продолжают работать, из-за чего проблема выглядит как ошибка приложения, а не агента. Трассировки, RED-метрики и карта сервисов от этого не зависят; для связывания между сервисами SDK OpenTelemetry, распространяющий заголовок в пространстве пользователя без переписывания в ядре, — более безопасный вариант.
...
| `oneuptime.apiKey`                        | _(обязательно)_                                  | API-ключ проекта (Настройки → Ключи API).                                                                                                                                                       |
