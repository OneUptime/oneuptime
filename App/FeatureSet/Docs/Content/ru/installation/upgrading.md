...
Уровень кэша и очередей работает на [Valkey](https://valkey.io) — форке Redis 7.2 под лицензией BSD — вместо Redis: Redis 7.4 перешёл на более строгую лицензию, и большинство первоначальных разработчиков продолжили работу над Valkey. Valkey использует тот же сетевой протокол, что и Redis, поэтому выше уровня сокета ничего не изменилось, и при желании вы по-прежнему можете направить OneUptime на настоящий Redis (или на управляемый Redis-совместимый сервис).

Теперь всё названо соответственно: настройки — `VALKEY_*`, значения Helm — `valkey:` / `externalValkey:`, а объекты Kubernetes — `<release>-valkey*`. **Все прежние имена по-прежнему работают**, поэтому нетронутые `config.env` или `values.yaml` продолжат работать — но, прежде чем обновлять нагруженный production-кластер, прочитайте примечание про Helm ниже.

**Docker Compose — действий не требуется.** Сервис теперь называется `valkey` и сохраняет `redis` в качестве сетевого алиаса, поэтому существующий `config.env` с `REDIS_HOST=redis` разрешается без правок. Приложение читает `VALKEY_*` и при их отсутствии возвращается к `REDIS_*`, а `npm run update` **не** заменит ваши текущие настройки кэша: обычно он добавляет каждую новую настройку, которую находит в `config.example.env`, но эти он распознаёт как переименования и оставляет ваши значения — включая ваш `REDIS_PASSWORD` — ровно там, где они были. Обновляйтесь через `npm run update` или любую команду `docker compose up --remove-orphans`, чтобы старый контейнер `redis` был удалён; если оставить его работающим, за одним и тем же именем хоста `redis` окажутся два контейнера и половина подключений будет попадать на устаревший.

Если вы задаёте переменные кэша вручную в `docker-compose.override.yml`, переименуйте их в `VALKEY_*`. Базовый файл задаёт `VALKEY_HOST` из вашего `REDIS_HOST`, и переопределение, которое задаёт только `REDIS_HOST`, больше не имеет приоритета.

**Helm — менять значения не нужно, но кэш один раз перезапустится.**

- `redis:` и `externalRedis:` по-прежнему работают; они накладываются поверх новых значений по умолчанию `valkey:` / `externalValkey:`, а `helm upgrade` выводит уведомление со списком найденных устаревших ключей.
- Сгенерированный пароль переносится из прежнего Secret `<release>-redis` в `<release>-valkey`, поэтому ничего не ротируется. Прежний Secret сохраняется и хранит теперь уже неиспользуемую копию — удалите его, как только обновление окончательно закрепится.
- Переименование StatefulSet приводит к пересозданию его пода. Встроенный кэш ничего не пишет на диск, поэтому поднимается «холодным»: закэшированные значения теряются, а задачи BullMQ, которые ожидали выполнения, были отложены или ждали повторной попытки, пропадают. Повторяющиеся и cron-задачи регистрируются заново при переподключении. Обновляйтесь в спокойный момент, если для вас важны телеметрия в обработке или повторные попытки рабочих процессов.
- Если вы указываете на управляемый кэш через `extraEnv`, а не через `externalValkey:`, переименуйте эти записи в `VALKEY_*`. Запись `REDIS_HOST` там теперь игнорируется, поскольку чарт также задаёт `VALKEY_HOST`, и приложение отдаёт предпочтение ему.
