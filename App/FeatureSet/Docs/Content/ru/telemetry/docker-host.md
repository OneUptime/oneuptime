...
- **OneUptime Telemetry Ingestion Token** — создайте его в разделе _Настройки проекта → Телеметрия и APM → Ключи приема_ и скопируйте значение
...
| `ONEUPTIME_SERVICE_TOKEN` | Да          | Токен загрузки телеметрии из _Настройки проекта → Телеметрия и APM → Ключи приема_                                                                                                                              |
...
| `DOCKER_API_VERSION`      | Нет         | Версия Docker Engine API, которую использует агент. По умолчанию `1.44`; понизьте её на хостах с более старым демоном или задайте пустое значение для автоматического согласования (см. «Устранение неполадок») |
...
Демон отклоняет клиент новее его собственного максимума, поэтому приёмник так и не
запускается, и коллектор завершает работу вместе с ним. Узнайте максимальную версию
демона и передайте её агенту:

```bash
docker version --format '{{ .Server.APIVersion }}'
```

Затем добавьте `-e DOCKER_API_VERSION=1.41` (или полученное значение) в `docker run`
либо задайте `DOCKER_API_VERSION` в Compose. Более новые демоны по-прежнему обслуживают
...
строку**. Тогда агент попросит Docker SDK согласовать версию с демоном (один запрос
`HEAD /_ping`, затем собственный максимум демона), что работает как со старыми, так и
с новыми демонами:
