...
- **OneUptime Telemetry Ingestion Token** — создайте его в разделе _Project Settings → Телеметрия и APM → Ключи приема_ и скопируйте значение
...
| `ONEUPTIME_SERVICE_TOKEN` | Да          | Токен загрузки телеметрии из _Project Settings → Телеметрия и APM → Ключи приема_                                                               |
