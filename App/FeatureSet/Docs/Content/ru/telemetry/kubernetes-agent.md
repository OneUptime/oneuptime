...
> **Это также удаляет ваши метрики узлов.** Приёмники kubelet, cAdvisor и hostmetrics находятся внутри DaemonSet сборщика логов, поэтому отключение логов подов удаляет и их — вместе с мониторами OOM-kill, троттлинга CPU и нехватки места на PVC. Вы сохраните метрики уровня кластера и события Kubernetes, но не метрики по узлам или контейнерам. Чтобы сократить объём логов, сохранив метрики, используйте вместо этого [`filters.logs.minSeverity`](#filtering-by-log-severity) или [`namespaceFilters`](#namespace-filtering).
...
> `api` и `disabled` оба удаляют DaemonSet сборщика логов, а вместе с ним и описанные выше метрики узлов, подов, контейнеров и хоста — тот же компромисс, что и у `logs.enabled=false`. Только режим `daemonset` собирает их. Именно поэтому пресеты GKE Autopilot и EKS Fargate, которые принудительно включают режим `api`, не сообщают метрики kubelet.
