...
`filters.logs.minSeverity` отбрасывает записи логов ниже заданной важности на стороне агента, до того как что-либо будет отправлено:

```bash
  --set filters.logs.minSeverity=WARN
```

Принимает `TRACE`, `DEBUG`, `INFO`, `WARN`, `ERROR`, `FATAL`. `WARN` сохраняет WARN, ERROR и FATAL и отбрасывает INFO, DEBUG и TRACE. Значение по умолчанию (`""`) сохраняет всё.

Это работает, несмотря на то что среды выполнения контейнеров не записывают важность в строку лога: агент извлекает её из текста лога (`[ERROR]`, `WARN:`, `level=info`, …) и в качестве запасного варианта использует `stderr → ERROR` / `stdout → INFO`. Это применяется в **обоих** режимах логирования — в режиме `daemonset` через сборщик, в режиме `api` внутри самого сборщика логов, — поэтому пресеты не могут незаметно изменить это поведение.

> Записи, важность которых всё же не удалось определить, **сохраняются**, а не отбрасываются. Безопасный сбой фильтра — отправить слишком много, а не тихо удалить лог, о неклассифицированности которого никто не знал.
...
- **Важны только предупреждения и ошибки?** `filters.logs.minSeverity` отбрасывает остальное на стороне агента. На «болтливом» кластере это часто даёт самое большое сокращение из возможных, потому что INFO и DEBUG составляют основную массу вывода большинства приложений:
  ...
  О том, как определяется важность и что происходит с логами, которые не удалось классифицировать, см. [Фильтрация по важности логов](#filtering-by-log-severity).
...
- **Совсем не нужны логи подов в OneUptime?** Отключите их:
  ...
  > **Это также отключает метрики узлов, подов, контейнеров и хоста.** Приёмники kubelet, cAdvisor и hostmetrics — все живут в том же DaemonSet сборщика логов, поэтому отключение логов подов убирает и их — вместе с мониторами OOM-kill, троттлинга CPU и низкого дискового пространства PVC. То же самое относится к `logs.mode: api` и `logs.mode: disabled`.
  >
  > Если вам нужно меньше логов, но вы хотите сохранить метрики, оставайтесь на `logs.mode: daemonset` и используйте вместо этого `namespaceFilters` или `filters.logs.minSeverity`, описанные выше.
...
- **Отбрасывайте конкретные метрики по имени.** Приведённые выше списки разрешённых действуют в рамках отдельного приёмника; `filters.metrics.exclude` охватывает их все, поэтому используйте его для всего, что нельзя выразить регуляторами уровня приёмника:
  ...
  О сопоставлении по точному имени и по регулярному выражению, а также о форме списка разрешённых см. [Включение или исключение метрик по имени](#including-or-excluding-metrics-by-name).
...
# Сохраните DaemonSet — именно он собирает метрики kubelet, cAdvisor и хоста,
# а также логи — но передавайте только те логи, по которым стоит оповещать.
logs:
  enabled: true
  mode: daemonset
