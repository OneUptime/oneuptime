...
Все три **необратимы**: то, что вы отбросили здесь, никогда не попадёт в OneUptime, и все три способны заставить монитор замолчать. Первые два заставляют монитор замолчать, убирая сигнал, за которым он следит. Сэмплирование действует более узко: RED-метрики eBPF вычисляются до того, как отработает сэмплер, поэтому мониторы на основе метрик остаются точными, — но мониторы, которые считают *спаны* (**Traces** по `Span Count`, **Exceptions** по `Exception Count`), видят пропорционально меньше, и их пороги нужно перенастроить на тот же коэффициент. Если вы предпочитаете решить позже, OneUptime может отбрасывать данные на стороне сервера (**Logs → Settings → Drop Filters**, **Metrics → Settings → Pipeline Rules**) — это по-прежнему расходует исходящий трафик, но это настройка, которую можно изменить без повторного развёртывания.
