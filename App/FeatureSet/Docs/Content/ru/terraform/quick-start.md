...
2. Go to **Настройки проекта** > **Ключи API**.
...
- **Мониторы** — the `Homepage` monitor is listed with the `critical` label.
- **Страницы статуса** — `Internal Status` appears.
- **Настройки проекта > Метки** — the `critical` label exists with the color you set.
