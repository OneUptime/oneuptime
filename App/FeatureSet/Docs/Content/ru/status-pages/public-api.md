...
- [Обзор страниц статуса](/docs/status-pages/index) — что такое страница статуса и как её части связаны между собой.
- [Ресурсы и группы страницы статуса](/docs/status-pages/resources-and-groups) — ресурсы, которые возвращают эти конечные точки.
- [Оформление и домены страницы статуса](/docs/status-pages/branding-and-domains) — пользовательский домен, с которого обслуживаются эти конечные точки.
- [Подписчики и объявления](/docs/status-pages/subscribers) — объявления, которые предоставляет конечная точка объявлений.
- [Обзор инцидентов](/docs/incidents/index) — откуда берутся инциденты в этих ответах.
