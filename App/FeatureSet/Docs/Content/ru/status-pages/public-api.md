...
- [Оформление и домены страницы статуса](/docs/status-pages/branding-and-domains) — собственный домен, с которого обслуживаются эти конечные точки.
- [Подписчики и объявления](/docs/status-pages/subscribers) — объявления, которые отдаёт конечная точка объявлений.
