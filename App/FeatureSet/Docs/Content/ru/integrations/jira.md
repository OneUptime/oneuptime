...
2. В OneUptime перейдите в **Рабочие процессы → Глобальные переменные → Создать**.
...
1. Включите рабочий процесс (**Включено**).
2. Создайте тестовый инцидент в OneUptime (или вызовите его через монитор).
3. Откройте вкладку **Журналы** рабочего процесса. Блок **API** должен показать статус `201` и тело ответа, содержащее `key` новой задачи (например, `OPS-1234`).
...
- **Приоритет** — добавьте `"priority": { "name": "High" }` в `fields`. Ветвясь по `{{Incident.incidentSeverity.name}}` с помощью **Условия**, можно сопоставлять уровни серьёзности OneUptime с приоритетами Jira.
