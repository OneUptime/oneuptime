...
Когда оповещение сбрасывается, Grafana отправляет ещё одно уведомление со значением `status: resolved`. Добавьте вторую ветвь **Условия** (`status == resolved`), найдите соответствующий инцидент и переведите его в состояние разрешено с помощью **Update Incident**.
...
- **Запуск не появляется** — убедитесь, что Grafana может достичь URL (проверьте серверные журналы Grafana) и рабочий процесс **Включено**.
- **Пустые поля** — проверьте вывод триггера на вкладке **Журналы**; ссылайтесь на поля, существующие для вашей версии системы оповещений.
