...
1. Откройте **Рабочие процессы → Создать рабочий процесс**, назовите его `Incidents → Opsgenie` и откройте **Конструктор**.
...
2. Добавьте блок **Условия**, проверяющий, что инцидент теперь разрешён (ветвление по `{{Incident.currentIncidentState.name}}`).
3. Из выхода **Да** добавьте блок **API**:
   - **Method**: `POST`
...
Приоритеты Opsgenie — от `P1` до `P5`. Сопоставьте с уровнями серьёзности OneUptime с помощью ветвей **Условия** по `{{Incident.incidentSeverity.name}}` перед блоком API.
...
- **Ничего не происходит** — убедитесь, что рабочий процесс **Включено**.
