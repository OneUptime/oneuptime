...
3. Перетащите блок **Conditions** на холст и соедините выход триггера с ним. Настройте:
   ...
   - **Operator**: `==`
   - **Right value**: `1`  *(Zabbix отправляет `1` для проблемы, `0` — для восстановления)*
...
5. Сохраните. Пока оставьте **Enabled** *выключенным* — включите после тестирования.
...
3. **Name**: `OneUptime`.
4. Добавьте следующие **Parameters** (нажмите *Add* для каждого). Они сопоставляют [макросы](https://www.zabbix.com/documentation/current/en/manual/appendix/macros/supported_by_location) Zabbix с понятной нагрузкой:
...
Zabbix отправляет уведомления *пользователю*. Создайте выделенного пользователя, чтобы интеграцию было легко найти и отключить.
...
3. **Conditions** (опционально): сузьте область, например *Trigger severity >= Warning*. Оставьте пустым, чтобы отправлять всё.
...
Основной рабочий процесс выше только *открывает* инциденты. Чтобы также *закрывать* их при восстановлении Zabbix:
...
4. Соедините с блоком **Update Incident** и переведите инцидент в состояние *разрешено*.

Поскольку разрешение зависит от того, как вы моделируете состояния инцидентов в проекте, сначала убедитесь, что путь *создания* работает надёжно, и только потом добавляйте путь разрешения. См. [Компоненты → Компоненты данных OneUptime](/docs/workflows/components#oneuptime-data-components).
