...
1. Откройте **Рабочие процессы → Создать рабочий процесс**. Назовите его `Zabbix → Incidents` и откройте вкладку **Конструктор**.
...
3. Перетащите блок **Условия** на холст и соедините выход триггера с ним. Настройте:
   - **Left value**: `{{Zabbix.Request Body.status}}`
   - **Operator**: `==`
   - **Right value**: `1` _(Zabbix отправляет `1` для проблемы, `0` — для восстановления)_
4. Перетащите блок **Создать инцидент** и соедините его с выходом **Да** блока **Условия**. Заполните:
   - **Заголовок**: `Zabbix: {{Zabbix.Request Body.name}}`
   - **Описание**: `Host: {{Zabbix.Request Body.host}}\nSeverity: {{Zabbix.Request Body.severity}}\nZabbix event: {{Zabbix.Request Body.event_id}}`
   - **Серьёзность**: выберите нужный уровень серьёзности инцидента OneUptime (позже можно уточнить, добавив ветви **Условия** для сопоставления уровней серьёзности Zabbix).
5. Сохраните. Пока оставьте **Включено** _выключенным_ — включите после тестирования.
...
1. Вернитесь в рабочий процесс OneUptime и включите **Включено**.
...
3. Откройте вкладку **Журналы** рабочего процесса. Вы должны увидеть запуск с нагрузкой Zabbix, ветку **Да** блока **Условия** и созданный инцидент.
4. Проверьте раздел **Инциденты** в OneUptime — проблема Zabbix теперь является инцидентом.
...
2. В рабочем процессе добавьте вторую ветвь **Условия**: левое значение `{{Zabbix.Request Body.status}}`, оператор `==`, правое `0`.
3. Из выхода **Да** добавьте блок **Find Incident**, который находит открытый ранее созданный инцидент — совпадение по `event_id` Zabbix, сохранённому в описании или метке.
...
Уровни серьёзности Zabbix (`Not classified`, `Information`, `Warning`, `Average`, `High`, `Disaster`) приходят в `{{Zabbix.Request Body.severity}}`. Чтобы сопоставить их с уровнями серьёзности инцидентов OneUptime, добавьте ветви **Условия** перед **Создать инцидент** — например, направьте `Disaster` и `High` на «Критический» инцидент, а всё остальное — на «Серьёзный». Для каждой ветви создайте отдельный блок **Создать инцидент**.

## Устранение неполадок

**Рабочий процесс никогда не запускается.**

- Убедитесь, что переключатель **Включено** рабочего процесса включён.
...
- Откройте вкладку **Журналы** рабочего процесса и проверьте вывод триггера. Убедитесь, что имена полей в разделе **Тело запроса** совпадают с теми, на которые вы ссылаетесь (`name`, `host`, `severity`, `status`, `event_id`).
