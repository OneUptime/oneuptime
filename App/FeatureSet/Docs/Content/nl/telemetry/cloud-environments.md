...
- Een **OneUptime Telemetry Ingestion Token** — maak er een aan via _Projectinstellingen → Telemetrie & APM → Ingestiesleutels_.
...
| `service.instance.id` | nee     | Per taak/instance bijgehouden onder **Instanties** (met live CPU / geheugen)                      |
...
- **CPU** en **Geheugen** per draaiende taak/instance (uit `container.cpu.utilization` / `container.memory.usage`), plus een lijst **Topinstanties op CPU**.
- **Instanties** — een live telling van taken.
- **Aanvragen** en trendgrafieken afgeleid van je traces.
- Volledige tabbladen **Logboeken**, **Traces**, **Metrieken** en **Instanties**.
