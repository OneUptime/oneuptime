...
Workflow-triggers voor incidenten worden niet met de hand geschreven — OneUptime genereert ze uit de datamodellen, dus elk model uit de incidentfamilie krijgt componenten **On Create X**, **On Update X** en **On Delete X**, vernoemd naar de enkelvoudsnaam van het model. De drie belangrijkste zijn **On Create Incident**, **On Update Incident** en **On Delete Incident**, en ze staan in de categorie **Incident** van het workflow-componentenpalet op `/dashboard/{projectId}/workflows`.
