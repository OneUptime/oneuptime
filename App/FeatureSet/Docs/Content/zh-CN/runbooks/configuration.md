...
- 每步执行超时默认值：JavaScript、Bash 和 HTTP 都是 **30 秒**。可在 Runbook 的 **步骤** 页面上按步骤设置 — 该字段留空即保持默认值。
- Bash 与 JavaScript 步骤的 **领取超时**：**2 分钟** — Worker 在判定失败前等待被选定代理来领任务的时长。同样可按步骤设置。
- 两个超时的取值范围都是 **1 秒到 1 小时**。超出的值会在步骤运行时被限制到这个范围内，因此配错的值既不能关掉超时，也不能无限占住某个 Worker 槽位。
