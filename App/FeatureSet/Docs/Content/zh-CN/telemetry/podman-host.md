...
- A **OneUptime Telemetry Ingestion Token** — create one from _Project Settings → 遥测与 APM → 摄取密钥_ and copy the value
...
| `ONEUPTIME_SERVICE_TOKEN` | Yes      | Telemetry ingestion token from _Project Settings → 遥测与 APM → 摄取密钥_                                        |
