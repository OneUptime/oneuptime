...
OneUptime 将托管的云计算资源归类为 **云环境（Cloud Environments）**—— AWS ECS / Fargate、Google Cloud Run、Azure Container Apps / Container Instances、AWS Elastic Beanstalk、AWS App Runner 以及 Azure App Service。每一个由 `cloud.platform` + `cloud.account.id` + `cloud.region` 组成的唯一组合都会创建一个环境，因此诸如 _“AWS ECS · us-east-1 · 123456789012”_ 这样的组合就是一个单一实体，它聚合了在其上运行的每一个工作负载。
