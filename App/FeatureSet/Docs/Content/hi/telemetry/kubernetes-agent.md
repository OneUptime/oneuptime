...
- एक **OneUptime API key** — इसे _प्रोजेक्ट सेटिंग्स → API कुंजियाँ_ से बनाएँ
...
`namespaceFilters.rules` नेमस्पेस पैटर्न को चार स्कोप पर स्वतंत्र रूप से लागू करता है:

- `podLogs`: hostPath filelog रिसीवर या API लॉग टेलर पर पॉड stdout/stderr फ़िल्टर करता है; Kubernetes इवेंट और ऑडिट लॉग प्रभावित नहीं होते।
- `ebpfDiscovery`: OBI प्रोसेस डिस्कवरी को फ़िल्टर करता है और इसलिए eBPF ट्रेस व eBPF मेट्रिक दोनों को नियंत्रित करता है।
- `metrics`: मेटाडेटा एनरिचमेंट के बाद नेमस्पेस वाले मेट्रिक फ़िल्टर करता है; बिना नेमस्पेस वाली नोड और क्लस्टर सीरीज़ रखी जाती हैं।
- `traces`: मेटाडेटा एनरिचमेंट के बाद eBPF स्पैन और एप्लिकेशन द्वारा भेजे गए OTLP स्पैन दोनों फ़िल्टर करता है।

नेमस्पेस पैटर्न पूरे नाम से मेल खाते हैं और * वाइल्डकार्ड का समर्थन करते हैं, जैसे team-*. यदि किसी स्कोप में कोई include नियम है, तो उस स्कोप में केवल मेल खाने वाले नेमस्पेस रखे जाते हैं। exclude नियम हमेशा प्राथमिकता लेते हैं। डिफ़ॉल्ट नियम kube-system को केवल podLogs और ebpfDiscovery से बाहर करता है।

पॉड लॉग और eBPF डिस्कवरी को चुनिंदा नेमस्पेस तक सीमित करने के लिए:
...
किसी अधिक शोर वाले नेमस्पेस के लॉग रोकते हुए उसके eBPF ट्रेस, सर्विस मैप और मेट्रिक रखने के लिए, एक्सक्लूज़न को केवल podLogs तक सीमित करें:
...
podLogs और ebpfDiscovery नियम स्रोत पर फ़िल्टर करते हैं: बाहर की गई लॉग फ़ाइलें कभी खोली नहीं जातीं और बाहर किए गए वर्कलोड कभी इंस्ट्रूमेंट नहीं होते। metrics और traces नियम बाद में collector में चलते हैं, जब नेमस्पेस मेटाडेटा जोड़ दिया जाता है।

#### नेमस्पेस के आधार पर मेट्रिक और ट्रेस फ़िल्टर करना

यदि आप नेमस्पेस वाले मेट्रिक या स्पैन भी फ़िल्टर करना चाहते हैं, तो उन स्कोप को सीधे नियम में जोड़ें:

```bash
  --set-json 'namespaceFilters.rules=[{"action":"exclude","namespaces":["kube-system","noisy-*"],"scopes":["podLogs","ebpfDiscovery","metrics","traces"]}]'
```

> **नोड और क्लस्टर स्तर के मेट्रिक हमेशा रखे जाते हैं। नेमस्पेस पॉड की विशेषता है, नोड की नहीं; इसलिए बिना नेमस्पेस वाली सीरीज़ नियम से मेल नहीं खातीं और हटाई नहीं जातीं।**

Kubernetes इवेंट को एजेंट में नेमस्पेस के आधार पर फ़िल्टर नहीं किया जा सकता। वे k8sobjects रिसीवर से k8s.namespace.name एट्रिब्यूट के बिना आते हैं; नेमस्पेस इवेंट बॉडी में होता है। इन्हें सर्वर की ओर फ़िल्टर करें।
...
| **Kubernetes इवेंट्स**                                             | चेतावनियाँ, त्रुटियाँ, scheduling events                                                                                                            |
...
| **Metric cardinality**         | विशिष्ट series की संख्या (प्रति-container, प्रति-PVC, …) | `filters.metrics.exclude`, `namespaceFilters.rules` (`metrics`), `cadvisor.metricsAllowlist`, `kubeletstats.volumeMetrics` |
...
- **filter processor पर** — डेटा एकत्र किया जाता है, फिर निर्यात से पहले गिरा दिया जाता है। `filters.logs.minSeverity`, `filters.metrics.*`, `namespaceFilters.rules` (`metrics`/`traces`)। थोड़ी अधिक collector CPU, लेकिन यह receivers के आर-पार काम करता है और ऐसी चीज़ें व्यक्त कर सकता है जो कोई receiver नहीं कर सकता।
...
- **क्या केवल कुछ नेमस्पेस के लॉग चाहिए? podLogs स्कोप वाला include नियम उपयोग करें। मिलान लॉग स्रोत पर होता है, इसलिए फ़िल्टर किए गए नेमस्पेस कभी पढ़े नहीं जाते और eBPF टेलीमेट्री स्वतंत्र रहती है।**
  ...
  किसी शोर वाले समूह को छोड़कर बाकी सभी नेमस्पेस रखने के लिए namespaces: [noisy-*] और scopes: [podLogs] वाला exclude नियम उपयोग करें।
...
- **क्या पूरे नेमस्पेस के मेट्रिक हटाने हैं? metrics स्कोप वाला exclude नियम जोड़ें। प्रति-पॉड और प्रति-कंटेनर सीरीज़ फ़िल्टर होती हैं, जबकि बिना नेमस्पेस वाली नोड और क्लस्टर सीरीज़ रखी जाती हैं।**
...
ज़रूरत के अनुसार और कड़ा करें: minSeverity को ERROR करें, नेमस्पेस नियम के scopes में metrics जोड़ें, या यदि आप OTel SDK से ट्रेस भेजते हैं तो ebpf.enabled=false करें।
...
Telemetry उपयोग प्रति दिन एकत्रित किया जाता है, इसलिए कमी की पुष्टि करने के लिए **प्रोजेक्ट सेटिंग्स → उपयोग इतिहास** के अंतर्गत एक या दो दिन में रुझान की जाँच करें — जैसे ही आप कोई परिवर्तन लागू करते हैं यह तुरंत नहीं हिलेगा। एक बार में एक लीवर बदलें ताकि आप अंतर का श्रेय दे सकें — logs बंद, फिर interval ऊपर, फिर eBPF छाँटा गया — बजाय इसके कि एक ही बार में सब कुछ कम कर दें और एक monitor खो दें जिस पर आप वास्तव में निर्भर थे।
...
3. **Ingestion key सत्यापित करें।** OneUptime से सीधे पूछें कि आपका token स्वीकार किया गया है या नहीं (`200` = वैध, `401` = अज्ञात/निरस्त):
   ...
   यदि यह `401` लौटाता है, तो आपके रिलीज़ में key गलत है या निरस्त कर दी गई थी। _प्रोजेक्ट सेटिंग्स → टेलीमेट्री और APM → इंजेशन कुंजियाँ_ से एक live key कॉपी करें और पुनः-डिप्लॉय करें:
...
5. यदि आपने reinstall पर `clusterName` बदला, तो एजेंट एक **नए** क्लस्टर के रूप में दिखाई देता है — पुरानी प्रविष्टि "डिस्कनेक्टेड" बनी रहती है (यह अपेक्षित है; यह पुरानी है)।
...
4. बहुत बड़े क्लस्टर में एक रेप्लिका बाधा बन सकती है—namespaceFilters.rules में podLogs स्कोप वाले include नियमों के साथ अलग-अलग रिलीज़ शार्ड करें।
