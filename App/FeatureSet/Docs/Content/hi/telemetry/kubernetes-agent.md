...
मात्रा घटाने के दो तरीके हैं, और यह जानना सार्थक है कि आप किसका उपयोग कर रहे हैं:
...
- **केवल चेतावनियों और त्रुटियों की परवाह है?** `filters.logs.minSeverity` बाकी को एजेंट पर गिरा देता है। एक बातूनी क्लस्टर पर यह अक्सर उपलब्ध सबसे बड़ी एकल कमी होती है, क्योंकि अधिकांश एप्लिकेशन आउटपुट का बड़ा हिस्सा INFO और DEBUG होता है:
  ...
  गंभीरता कैसे निर्धारित की जाती है और जिन logs को यह वर्गीकृत नहीं कर सकता उनका क्या होता है, इसके लिए [Log गंभीरता द्वारा फ़िल्टरिंग](#filtering-by-log-severity) देखें।
...
- **OneUptime से pod logs की बिल्कुल भी आवश्यकता नहीं है?** उन्हें बंद कर दें:
  ...
  > **यह node, pod, container, और host मेट्रिक्स को भी अक्षम कर देता है।** kubelet, cAdvisor, और hostmetrics receivers सभी उसी log-collector DaemonSet में रहते हैं, इसलिए pod logs बंद करने से वे भी हट जाते हैं — साथ ही OOM-kill, CPU-throttling, और PVC-low-disk monitors भी। यही बात `logs.mode: api` और `logs.mode: disabled` पर लागू होती है।
  >
  > यदि आप कम logs चाहते हैं लेकिन अपने मेट्रिक्स रखना चाहते हैं, तो `logs.mode: daemonset` पर बने रहें और इसके बजाय ऊपर दिए गए `namespaceFilters` या `filters.logs.minSeverity` का सहारा लें।
...
- **नाम से विशिष्ट मेट्रिक्स गिराएँ।** ऊपर दी गई allowlists प्रति-receiver हैं; `filters.metrics.exclude` उन सभी तक फैलता है, इसलिए इसका उपयोग किसी भी ऐसी चीज़ के लिए करें जिसे receiver-स्तरीय घुंडियाँ व्यक्त नहीं कर सकतीं:
  ...
  सटीक-बनाम-regex मिलान और allowlist रूप के लिए [नाम द्वारा मेट्रिक्स शामिल या बाहर करना](#including-or-excluding-metrics-by-name) देखें।
...
> **जो आप काटते हैं उस पर ध्यान दें।** कुछ monitors विशिष्ट signals पर निर्भर करते हैं: `cadvisor` को अक्षम करने से OOM-kill और CPU-throttling monitors हट जाते हैं; `kubeletstats.volumeMetrics` को अक्षम करने से PVC low-disk monitor हट जाता है; logs को अक्षम करने (या DaemonSet को बंद करने) से log-आधारित अलर्ट *और* आपके node मेट्रिक्स हट जाते हैं। उन signals को छाँटें जिन पर आप कार्रवाई नहीं करते, न कि उन्हें जिन्हें कोई monitor देख रहा है।
