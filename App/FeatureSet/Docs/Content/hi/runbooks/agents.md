...
एक **Runbook एजेंट** एक छोटा सेल्फ-होस्टेड प्रोसेस है जो आपके runbooks के Bash steps को **आपके अपने इन्फ्रास्ट्रक्चर के अंदर** चलाता है। OneUptime का Worker आपकी shell कमांड्स को कभी खुद नहीं चलाता — वह उन्हें कतार में डालता है, और आपने अपने environment में जो Runbook एजेंट इंस्टॉल किया है वह उन्हें उठाकर लोकल में चलाता है और परिणाम वापस भेजता है।

यह पेज समझाता है कि एजेंट कैसे इंस्टॉल करें, Bash steps को उसकी ओर रूट कैसे करें, और रोज़मर्रा में उसे कैसे चलाएँ।

## एजेंट क्यों ज़रूरी हैं

पहले OneUptime के संस्करण Bash steps सीधे Worker पर चलाते थे। यह सिंगल-टेनेंट सेल्फ-होस्टेड डिप्लॉयमेंट के लिए ठीक था जहाँ ऑपरेटरों के पास उस बॉक्स पर पहले से shell थी, लेकिन बाकी सबके लिए दो समस्याएँ हैं:

- **विश्वास सीमा।** जो भी runbook लिख सकता है वह Worker पर shell चला सकता है, जिसके पास Worker की सारी env variables और filesystem तक पहुँच होगी।
- **पहुँच।** अधिकांश उपयोगी Bash steps *ग्राहक* के इन्फ्रास्ट्रक्चर पर काम करना चाहते हैं ("इस service को restart करो", "हमारे cluster पर kubectl") — OneUptime के नहीं।

Runbook एजेंट इसे उलट देते हैं। Bash steps हमारे यहाँ नहीं चलते। वे उस host पर चलते हैं जिसे आप नियंत्रित करते हैं, और आप तय करते हैं कि वह host क्या कर सकता है।
...
4. जब कोई Bash step चलता है, Worker उस step के **Agent Tag** के साथ एक job पंक्ति डालता है और उसकी स्थिति `Pending` कर देता है।
5. एक ही project में उस tag को रखने वाला कोई भी स्वस्थ एजेंट उस job को atomic रूप से claim करता है (कभी भी दो एजेंट एक ही job नहीं चलाएँगे), `bash -c <आपकी script>` को लोकल में चलाता है, stdout/stderr/exit-code कैप्चर करता है, और परिणाम वापस भेजता है।
...
| **टैग** | अल्पविराम से अलग। Bash steps एक tag को निशाना बनाते हैं; project में उस tag वाला कोई भी एजेंट उन्हें चला सकता है। आम पैटर्न: `prod`, `staging`, `eu-west-1`, `db-host`। |
...
Tag एक Bash step के लिए एजेंट खोजने का तरीका है। कुछ पैटर्न:
...
एक Bash step को **ठीक एक** एजेंट tag देना होगा। Multi-tag routing (किसी ऐसे एजेंट पर चलाओ जिसके पास `prod` AND `db` दोनों हों) roadmap पर है पर इस release में नहीं है।

## Bash step को एजेंट की ओर इंगित करें

अपने runbook में एक Bash step जोड़ें। फ़ॉर्म आपसे **Agent Tag** माँगेगा:

- वही tag लिखें जो आप जिस एजेंट(ओं) पर चलाना चाहते हैं उससे मेल खाता है।
- नीचे editor में अपनी script लिखें।

जब runbook चलकर इस step तक पहुँचेगा, Worker उस tag के साथ एक job queue में डालता है। अगर उस tag वाला कम से कम एक स्वस्थ एजेंट online है, तो job कुछ ही सेकंड में claim होकर चल जाता है।

## संचालन संबंधी नोट

### Timeouts

हर Bash step पर दो timeouts लागू होते हैं:
...
| **Execution timeout** | 30 सेकंड | एजेंट script को `SIGKILL` भेजने से पहले कितनी देर चलने देता है। प्रति step कॉन्फ़िगर करने योग्य। |
...
lease expire होने पर script का child process अपने आप cancel **नहीं** होता — लेकिन Worker उसका इंतज़ार छोड़ देता है, और किसी और claim के संभाल लेने के बाद एजेंट परिणाम submit नहीं कर पाएगा। अगर exactly-once मायने रखता है, scripts को safely re-runnable डिज़ाइन करें।
