...
- A **OneUptime Telemetry Ingestion Token** — create one from _Project Settings → Télémétrie & APM → Clés d'ingestion_ and copy the value
...
| `ONEUPTIME_SERVICE_TOKEN` | Yes      | Telemetry ingestion token from _Project Settings → Télémétrie & APM → Clés d'ingestion_                                        |
