...
- **Sauvegardes** : Sauvegardez régulièrement vos bases de données (Clickhouse, Postgres). Le cache est sans état et peut être ignoré en toute sécurité.
- **Cache et files d'attente** : Le service `valkey` exécute [Valkey](https://valkey.io), le fork sous licence BSD de Redis 7.2, configuré via les paramètres `VALKEY_*` de votre fichier `config.env`. N'importe quel serveur parlant le protocole Redis convient — pointez `VALKEY_HOST` vers un Redis managé si vous le préférez. Ces paramètres s'appelaient `REDIS_*` jusqu'à la version 12.0.36 ; les anciens noms sont toujours lus, le conteneur répond toujours au nom d'hôte `redis`, et `npm run update` ne les réécrira pas : un ancien fichier `config.env` n'a donc besoin d'aucune modification.
