...
**Runbook 代理**是一个小型的自托管进程,**在你自己的基础设施内**执行 Runbook 的 Bash *和* JavaScript 步骤。OneUptime Worker 永远不会自己执行你的脚本 — 它只把任务放入队列,而由你安装在自己环境中的 Runbook 代理把它们取走、执行、并将结果回报。

JavaScript 仍然在 `isolated-vm` 沙箱中运行;区别在于沙箱住在你的代理主机上,而不是我们这边。

本页说明如何安装一个代理、把 Bash 和 JavaScript 步骤路由到它、以及如何日常运维。

## 为什么需要代理

OneUptime 早期版本在 Worker 上运行 Bash 和 JavaScript 步骤。JavaScript 在沙箱(`isolated-vm`)里,Bash 不在。对于超出单租户自托管部署的任何场景,这两者都有问题:

- **信任边界**:任何能编写 Runbook 的人都能在 Worker 上执行代码,可以访问 Worker 拥有的所有环境变量和文件系统。JavaScript 沙箱挡住了明显的东西,但挡不住一个坚决的用户去探测我们的网络可以触及什么。
- **可达性**:大多数有用的步骤想要操作的是*客户*侧基础设施(「重启这个服务」、「在我们的集群上跑 kubectl」、「在我们内部数据库里查一条记录」),而不是 OneUptime 侧的。

Runbook 代理把这反过来。Bash 和 JavaScript 步骤不在我们这里运行。它们运行在你控制的主机上,而你决定那台主机可以做什么。
...
4. 当一个 Bash 或 JavaScript 步骤运行时,Worker 插入一行作业记录,标上该步骤的 **Agent Tag** 和步骤类型(Bash 或 JavaScript),并把状态设为 `Pending`。
5. 同一项目中带有该标签的任一健康代理会原子地认领该作业(永远不会有两个代理执行同一个作业),在本地运行 — Bash 用 `bash -c <脚本>`,JavaScript 用 `isolated-vm` 沙箱 — 捕获结果并回送。
...
| **标签** | 用逗号分隔。Bash 和 JavaScript 步骤指向一个标签;项目里任何带有该标签的代理都可以执行。常见模式:`prod`、`staging`、`eu-west-1`、`db-host`。 |
...
标签是 Bash 或 JavaScript 步骤找到代理的方式。几个模式:
...
Bash 和 JavaScript 步骤各自**必须**指定且只能指定一个代理标签。多标签路由(运行在任何同时带有 `prod` AND `db` 的代理上)在路线图上,这个版本暂未提供。

## 把步骤指向代理

在你的 Runbook 里加一个 Bash 或 JavaScript 步骤。表单会问你 **Agent Tag**:

- 填入与你想运行它的代理匹配的标签。
- 在下方编辑器里写脚本。

Runbook 运行到该步骤时,Worker 用那个标签和步骤类型把作业加入队列。只要至少有一个带该标签的健康代理在线,作业就会在几秒内被认领并执行。Bash 通过 `bash -c` 执行;JavaScript 在代理上的 `isolated-vm` 沙箱里运行(无文件系统、无网络、无 `Function`/`eval`)。

## 运维提示

### 超时

每个 Bash 或 JavaScript 步骤有两个超时:

| 超时 | 默认 | 控制什么 |
| --- | --- | --- |
| **认领超时** | 2 分钟 | Worker 等待*某个*代理来认领作业的最长时间。如果没人按时认领,这一步以 `TimedOut` 失败,Runbook 继续(或停止,取决于**失败时继续**)。 |
| **执行超时** | 30 秒 | 代理给脚本运行的最长时间,超过后终止它。每步可单独配置。(Bash 收到 `SIGKILL`;JavaScript 的隔离体被拆除。) |
...
Bash 的子进程在租约过期时**不会**自动取消(JavaScript 的隔离体如果会结束,也只是被放着跑完)— 但是 Worker 不再等它们,而且其他认领接手后,代理就不能再提交结果了。如果你在意 exactly-once,请把脚本设计成可安全重试的。
