...
- A **OneUptime Telemetry Ingestion Token** — create one from _Configurações do projeto → Telemetria e APM → Chaves de ingestão_ and copy the value
...
| `ONEUPTIME_SERVICE_TOKEN` | Yes      | Telemetry ingestion token from _Configurações do projeto → Telemetria e APM → Chaves de ingestão_                   |
...
| **Logs de contêineres** | stdout / stderr logs from all containers                       |
