...
1. Abra **Fluxos de trabalho → Criar fluxo de trabalho**, nomeie-o `Datadog → Incidents` e abra o **Construtor**.
...
4. A partir de **Sim**, adicione um bloco **Criar incidente**:
   ...
   - **Descrição**: `{{Datadog.Request Body.body}}\nHost: {{Datadog.Request Body.host}}\n{{Datadog.Request Body.link}}`
   - **Gravidade**: escolha uma.
...
3. Verifique a aba **Registros** do workflow e sua lista de **Incidentes**.

## Resolvendo na recuperação (opcional)

`$ALERT_TRANSITION` é `Recovered` quando um monitor é limpo. Adicione uma segunda ramificação **Condições** (`transition == Recovered`), encontre o incidente correspondente (combine com o `id` enviado) e mova-o ao estado resolvido com **Update Incident**.

## Solução de problemas

- **Nenhuma execução aparece** — confirme que a mensagem do monitor inclui `@webhook-oneuptime` e que o workflow está **Habilitado**.
- **Campos estão vazios** — o Datadog só substitui variáveis de template que se aplicam ao evento. Inspecione a saída do gatilho na aba **Registros** e ajuste o payload do webhook.
