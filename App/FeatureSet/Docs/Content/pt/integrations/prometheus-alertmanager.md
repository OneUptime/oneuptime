...
Com `send_resolved: true`, o Alertmanager também faz POST quando um alerta é limpo, desta vez com `status: resolved`. Adicione uma segunda ramificação **Condições** (`status == resolved`), encontre o incidente correspondente (combine com `commonLabels.alertname`) e mova-o ao estado resolvido com **Update Incident**.

## Solução de problemas

- **Nenhuma execução aparece** — confirme que o Alertmanager consegue alcançar a URL (verifique seus logs para erros de entrega) e que o workflow está **Habilitado**.
- **Campos do incidente estão vazios** — regras diferentes definem anotações diferentes. Inspecione a saída do gatilho na aba **Registros** e referencie campos que realmente existem (`commonAnnotations` vs `annotations` por alerta).
