...
- A **OneUptime Telemetry Ingestion Token** — create one from _Project Settings → 遙測與 APM → 擷取金鑰_ and copy the value
...
| `ONEUPTIME_SERVICE_TOKEN` | Yes      | Telemetry ingestion token from _Project Settings → 遙測與 APM → 擷取金鑰_                                        |
