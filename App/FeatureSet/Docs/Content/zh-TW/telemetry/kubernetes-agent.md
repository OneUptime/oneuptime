...
- **在取樣器端**——資料會先被收集，然後保留具代表性的一小部分。`sampling.traces.percentage`。它是這裡的異類：上述兩者會移除整個*類別*的遙測資料，因此它們所捨棄的東西，會從每一筆追蹤中消失。取樣則會保留每一個類別，改為將母體變稀疏，因此存活下來的資料仍然是完整且具代表性的。

三者都是**不可逆的**：您在此處捨棄的資料永遠不會抵達 OneUptime，而且三者都可能讓某個監控變得靜默。前兩者是藉由移除監控所監看的訊號來讓它靜默。取樣的影響則比較窄：eBPF RED 指標是在取樣器執行之前就計算好的，因此以指標為基礎的監控仍然精確——但會計數 *span* 的監控（以 `Span Count` 為條件的 Traces、以 `Exception Count` 為條件的 Exceptions）看到的數量會按比例變少，需要以相同的倍數重新調校它們的門檻。如果您寧可稍後再決定，OneUptime 可以改為在伺服器端捨棄資料（**Logs → Settings → Drop Filters**、**Metrics → Settings → Pipeline Rules**）——那仍然會耗用 egress，但它是一個您無需重新部署即可變更的設定。
