...
| **JavaScript Expression** | 必須運算結果為真的自訂 JavaScript 運算式 |
...
唯一的篩選條件是 **Evaluates To True**：運算式回傳真值時即符合該條件。

運算式在沙箱中執行，且未繫結任何郵件欄位，因此無法讀取觸發本次檢查之郵件的主旨、寄件者、內文或收件者。若要依郵件內容比對，請使用 **Email Subject**、**Email From**、**Email Body** 與 **Email To** 檢查類型。
