...
外部狀態頁面監控可讓您監控第三方狀態頁面,並在您所依賴的服務發生中斷或效能降級時收到警示。OneUptime 會定期檢查外部狀態頁面(例如 AWS、GCP、Azure、GitHub 等),並評估其狀態。
...
當設定為 **Auto** 時,OneUptime 會嘗試自動偵測狀態頁面格式:

1. 首先,它會嘗試 Atlassian Statuspage JSON API(`/api/v2/status.json` 與 `/api/v2/components.json`)
2. 如果失敗,它會嘗試將該頁面解析為 RSS 或 Atom 摘要
3. 作為最後的後備方案,它會執行基本的 HTTP 可達性檢查
...
輸入您想要監控的外部狀態頁面 URL。對於由 Atlassian Statuspage 提供支援的網站,這通常是根 URL(例如 `https://status.example.com`)。對於 RSS/Atom 摘要,請直接輸入摘要 URL。

### 供應商類型

選擇狀態頁面的供應商類型。使用 **Auto**(預設)讓 OneUptime 自動偵測格式,或在您知道格式時指定特定的供應商類型。

### 元件名稱篩選

如果狀態頁面回報多個元件,您可以選擇指定一個元件名稱,以僅監控該特定元件。例如,若要僅監控 us-east-1 中的 AWS EC2,您會輸入 `EC2 us-east-1`(狀態頁面上所顯示的確切元件名稱)。

當未指定元件名稱時,則會監控狀態頁面的整體狀態。
...
您可以根據下列項目設定條件,以判斷外部服務何時被視為上線、降級或離線:

- **Is Online** – 狀態頁面是否可達並回傳狀態資料
- **Overall Status** – 狀態頁面的整體狀態指標(例如「operational」、「major_outage」)
- **Component Status** – 特定元件的狀態(使用元件名稱篩選時)
- **Active Incidents** – 狀態頁面上目前回報的進行中事件數量
