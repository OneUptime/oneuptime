...
- **沒有出現執行記錄** — 確認 Alertmanager 能連線到該 URL（檢查其記錄是否有傳送錯誤），並確認該工作流程為 **已啟用**。
- **事件欄位為空** — 不同的規則會設定不同的 annotations。請檢查 **日誌** 分頁中的觸發器輸出，並參照實際存在的欄位（`commonAnnotations` 與每個告警各自的 `annotations`）。
