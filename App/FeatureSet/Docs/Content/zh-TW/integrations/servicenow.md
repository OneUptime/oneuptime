...
2. 在 OneUptime 中，前往 **工作流程 → 全域變數 → 建立**，將其命名為 `SERVICENOW_AUTH`，貼上該 base64 字串，並開啟 **Is Secret**。

## 步驟 2 — 建立 workflow

1. 開啟 **工作流程 → 建立工作流程**，將其命名為 `Incidents → ServiceNow`，然後開啟 **建構器**。
...
4. **儲存**、啟用，並建立一則測試事件。workflow 記錄中的 `201 Created` 回應會傳回新記錄的 `sys_id` 與 `number`（例如 `INC0012345`）。
