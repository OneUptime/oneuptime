...
| 欄位                 | 用途                                                              |
| -------------------- | ----------------------------------------------------------------- |
| **名稱**             | 簡短、易讀的標籤。會顯示於稽核紀錄中。                            |
| **描述**             | 提供給團隊成員的選填情境說明。                                    |
| **已啟用**           | 切換以暫停某規則，而無需將其刪除。                                |
| **標題模式**         | 不分大小寫的正規表示式，用於比對實體的標題。留空 = 比對任何標題。 |
| **描述模式**         | 不分大小寫的正規表示式，用於比對實體的描述。留空 = 比對任何描述。 |
| **待開始的執行手冊** | 當規則觸發時要啟動的一個或多個 runbook。                          |
...
4. 該執行會連結至來源實體——它會顯示在該事件、警示或排程維護事件的頁面上，以及該 runbook 的「執行次數」清單中。

您可以在 **運行手冊 → 執行次數** 下查看所有由規則觸發的執行，並依狀態、runbook 或日期進行篩選。
