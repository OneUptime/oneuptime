...
- **Backup**: Esegui regolarmente il backup dei tuoi database (Clickhouse, Postgres). La cache è stateless e può essere tranquillamente ignorata.
- **Cache e code**: Il servizio `valkey` esegue [Valkey](https://valkey.io), il fork con licenza BSD di Redis 7.2, configurato tramite le impostazioni `VALKEY_*` nel file `config.env`. Va bene qualsiasi server che parli il protocollo Redis — se preferisci, punta `VALKEY_HOST` a un Redis gestito. Queste impostazioni si chiamavano `REDIS_*` fino alla 12.0.36; i vecchi nomi vengono ancora letti, il container risponde ancora all'hostname `redis` e `npm run update` non li riscriverà, quindi un `config.env` già esistente non richiede modifiche.
