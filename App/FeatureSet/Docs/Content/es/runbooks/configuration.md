...
- **Claim timeout** por defecto para pasos Bash y JavaScript: **2 minutos** — cuánto espera el Worker a que un Agente de Runbook recoja el job antes de fallarlo.
...
- `RunbookAdmin` (rol) — agrupa todo lo anterior; asígnalo a un equipo para darle capacidad completa sobre runbooks.
...
- Los **pasos Bash y JavaScript** nunca corren en el Worker de OneUptime. Se despachan como jobs a un [Agente de Runbook](/docs/runbooks/agents) que tú has instalado en tu propia infraestructura. El Worker encola el job marcado con el **Agent Tag** y el tipo de paso, un agente lo reclama atómicamente, lo ejecuta localmente — Bash vía `bash -c <script>`, JavaScript dentro de un sandbox `isolated-vm` con el preámbulo habitual (corta cadenas de prototipos, elimina `Function` y `eval`, congela los prototipos nativos) — y devuelve el resultado. El propio proceso Worker no ejecuta scripts de clientes.
...
- `RunbookAgentJob` — una fila por paso Bash o JavaScript despachado: tag requerido, tipo de paso, script, estado (Pending → Claimed → Running → Succeeded/Failed/TimedOut/Cancelled), claim deadline, lease, output, exit code.
