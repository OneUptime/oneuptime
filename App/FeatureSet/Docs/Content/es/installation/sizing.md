...
Esta guía le ayuda a dimensionar una implementación autoalojada de OneUptime en Kubernetes (Helm). Cubre los tres almacenes de datos de los que depende OneUptime — **PostgreSQL**, **Valkey** y **ClickHouse** — más el cómputo de la aplicación, y ofrece niveles iniciales que puede ajustar una vez que tenga cifras reales.
...
| **Valkey**       | Caché, colas de trabajo y sesiones                                                                                                   | **Profundidad de cola y sesiones activas**. Limitado por memoria y modesto. No es una fuente de verdad.           |
...
- **Dé a ClickHouse RAM generosa.** Las consultas de agregación consumen mucha memoria. Como regla general, dimensione la RAM a una fracción significativa (25–50%) de su conjunto de datos comprimido _en caliente_ (consultado recientemente), con un piso práctico de 16 GB para cualquier flota de producción real.
...
- **Valkey** — el chart no tiene replicación interna. Para HA, apunte OneUptime a un **Redis gestionado externo** (o una implementación de AI/clúster).
