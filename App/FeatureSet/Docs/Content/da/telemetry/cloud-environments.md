...
Rene virtuelle maskiner (EC2, Compute Engine, Azure VM) forbliver **Værter**, og Kubernetes hører fortsat under **Kubernetes**. Denne visning er specifikt til administreret / PaaS-compute.

## Forudsætninger

- En **OneUptime Telemetry Ingestion Token** — opret en fra _Projektindstillinger → Telemetri og APM → Indtagelsesnøgler_.
...
| `service.instance.id` | nej      | Spores pr. task/instans under **Instanser** (med live CPU / hukommelse)                                 |
...
- **CPU** og **Hukommelse** pr. kørende task/instans (fra `container.cpu.utilization` / `container.memory.usage`), plus en **Øverste instanser efter CPU**-liste.
- **Instanser** — et live-antal af tasks.
- **Anmodninger** og trenddiagrammer udledt fra dine traces.
- Komplette faner for **Protokoller**, **Spor**, **Metrikker** og **Instanser**.

Opdeling pr. service for de samme workloads er tilgængelig under **Tjenester**.
