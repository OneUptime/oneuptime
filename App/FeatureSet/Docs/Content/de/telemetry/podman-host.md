...
- A **OneUptime Telemetry Ingestion Token** — create one from _Project Settings → Telemetrie & APM → Ingestion-Schlüssel_ and copy the value
...
| `ONEUPTIME_SERVICE_TOKEN` | Yes      | Telemetry ingestion token from _Project Settings → Telemetrie & APM → Ingestion-Schlüssel_                                        |
