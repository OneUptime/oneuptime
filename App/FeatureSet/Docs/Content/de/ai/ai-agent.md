...
2. A fix task is created and picked up by an available Runner with the **Führt KI-Codekorrekturen aus** capability.
...
- **OneUptime Cloud**: zero-config — if your project has no LLM provider of its own, agent tasks use the shared global provider and the usage is billed as metered AI tokens, exactly like every other AI feature. To use your own keys instead, configure a provider under **Projekteinstellungen** > **KI** > **LLM-Anbieter** — a project-owned provider always takes precedence.
...
1. Create a Runner under **Einstellungen** > **Runbook-Agents** and use **Einrichtungsanweisungen anzeigen** on its row for a pre-filled install command. The key is shown once — save it securely. The command looks like:
...
2. Enable **Führt KI-Codekorrekturen aus** on the Runner — the capability is off by default, and the Runner adopts the change on its next heartbeat (about a minute); no restart needed.
...
The Runner shows as connected on the **Einstellungen** > **Runbook-Agents** page within a minute or two. If it does not, check the container logs (`docker logs oneuptime-runner`) for credential or network errors.
