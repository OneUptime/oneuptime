...
この連携は**アウトバウンド**です: OneUptime が [Opsgenie Alert API](https://docs.opsgenie.com/docs/alert-api) を呼び出します。**インシデント → On Create** トリガーと **API コンポーネント**を持つ OneUptime の **[ワークフロー](/docs/workflows/index)** を使います。
...
1. **ワークフロー → ワークフローを作成** を開き、`Incidents → Opsgenie` という名前にして **ビルダー** を開きます。
...
4. **保存** して有効化し、テスト用インシデントを作成します。ワークフローのログに `202 Accepted` レスポンスが表示されれば Opsgenie がアラートをキューに入れたことを意味します。

## ステップ 3 — OneUptime の解決時にクローズする (推奨)

1. **インシデント → On Update** トリガーを持つ `Close Opsgenie` という名前の**2 つ目のワークフロー**を作成します。
2. インシデントが解決済みになったことを確認する **条件** ブロックを追加します (`{{Incident.currentIncidentState.name}}` で分岐)。
3. **はい** から **API** ブロックを追加します:
   - **Method**: `POST`
...
Opsgenie の優先度は `P1`〜`P5` です。API ブロックの前に `{{Incident.incidentSeverity.name}}` で **条件** ブランチを追加して、OneUptime の重大度からマッピングします。
