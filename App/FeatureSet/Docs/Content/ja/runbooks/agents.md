...
**Runbook → 設定 → エージェント** に行き、新しいエージェントを作成します。次を入力:
...
**Runbook → 設定 → エージェント** に戻ります。約 60 秒以内に、エージェントの行が `Connected` に切り替わり、**最終確認** が更新されているはずです。`Disconnected` のままなら:
...
| **実行タイムアウト** | 30 秒      | エージェントがスクリプトを終了させるまでの最大時間。(Bash は `SIGKILL`。JavaScript の isolate は破棄。)                                                                                      |

どちらもステップごとに設定できます。**Runbook → 対象の Runbook → ステップ** を開き、Bash または JavaScript のステップを展開して、スクリプトの下にある **実行タイムアウト** と **取得タイムアウト** を秒単位で設定します。フィールドを空のままにするとデフォルトが使われます。それぞれ 1 秒から 1 時間まで指定でき、その範囲外の値はステップ実行時に範囲内に収められます。
