...
どちらもステップごとに設定できます。**Runbooks → 対象の Runbook → ステップ** を開き、Bash または JavaScript のステップを展開して、スクリプトの下にある **実行タイムアウト** と **取得タイムアウト** を秒単位で設定します。フィールドを空のままにするとデフォルトが使われます。それぞれ 1 秒から 1 時間まで指定でき、その範囲外の値はステップ実行時に範囲内に収められます。
