...
**Runbook エージェント** は、Runbook の Bash *と* JavaScript ステップを**お客様自身のインフラ内で**実行する、小さなセルフホスト型のプロセスです。OneUptime ワーカーがお客様のスクリプトを実行することはありません — ワーカーはキューに入れるだけで、ステップの作成者が選んだ Runbook エージェントがそれを取得し、実行し、結果を投稿します。
