...
- ステップあたりの実行タイムアウトのデフォルト: JavaScript、Bash、HTTP で **30 秒**。Runbook の **ステップ** ページでステップごとに設定します — フィールドを空のままにするとデフォルトのままになります。
- Bash と JavaScript ステップの**取得タイムアウト**: **2 分** — 選択されたエージェントがジョブを取得するまでワーカーが失敗扱いにせずに待つ時間。こちらもステップごとに設定します。
- どちらのタイムアウトも **1 秒から 1 時間** まで指定できます。その範囲外の値はステップ実行時に範囲内に収められるため、設定を打ち間違えてもタイムアウトを無効にしたり、ワーカーのスロットを無期限に占有したりはできません。
