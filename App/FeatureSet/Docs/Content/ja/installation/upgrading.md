...
この切り替えは**前方のみ（forward-only）**です。新しいテーブルは空の状態で始まり、アップグレード後に取り込まれたすべてのテレメトリーは即座に新テーブルに書き込まれ、履歴は時間の経過とともに自然に埋まっていきます。旧テーブルはディスク領域を回収するため、アップグレード中に**自動的に削除（DROP）されます** — 履歴を引き継ぐ選択肢を残したい場合は、アップグレードの**前に**旧テーブルをリネームしてください（以下のステップ 0）。

> **すでに 11.0.0 または 11.0.1 を使用していますか？** これらのリリースでは旧テーブルは保持されていました（TTL によって徐々に縮小し、コピーは「アップグレード後の任意のタイミングで」実行できました）。それ以降のアップデートでは、起動時に旧テーブルが**削除（DROP）されます**。履歴のコピーをまだ実行しておらず、今後も実行したい場合は、アップデートを適用する前に以下のステップ 0 を実行してください。

### 対応が必要なケース

- **新規インストール:** 対応は不要です。
- **アップグレード前のテレメトリーを UI に表示する必要がないアップグレード:** 対応は不要です。テレメトリーページにはアップグレード時点以降のデータのみが表示され、旧テーブルはアップグレード中に削除されます。
- **アップグレード前のテレメトリーを表示したいアップグレード:** アップグレードの**前に**旧テーブルをリネームし（以下のステップ 0）、その後の任意のタイミングで手動コピーを実行してください。
