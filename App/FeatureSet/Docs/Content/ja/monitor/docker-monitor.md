...
- **より大きい**、**より小さい**、**以上**、**以下**、**等しい**

ベースライン異常検知（しきい値は不要 — フォームには代わりに **感度** と **ベースライン期間** が表示され、各サンプルを曜日・時刻が同じベースラインと比較します）：

- **異常に高い** — 値が想定範囲を上回る
- **異常に低い** — 値が想定範囲を下回る
- **異常** — 値がいずれかの方向で想定範囲を外れる

異常検知の条件は、選択したベースライン期間以上の履歴がたまるまでアラートを発しません（Learning 状態）。
