...
- A **OneUptime Telemetry Ingestion Token** — create one from _Project Settings → テレメトリと APM → 取り込みキー_ and copy the value
...
| `ONEUPTIME_SERVICE_TOKEN` | Yes      | Telemetry ingestion token from _Project Settings → テレメトリと APM → 取り込みキー_                                        |
