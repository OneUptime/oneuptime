...
- **Säkerhetskopior**: Säkerhetskopiera dina databaser regelbundet (Clickhouse, Postgres). Cachen är tillståndslös och kan ignoreras utan risk.
- **Cache och köer**: Tjänsten `valkey` kör [Valkey](https://valkey.io), den BSD-licensierade forken av Redis 7.2, som konfigureras via `VALKEY_*`-inställningarna i `config.env`. Vilken server som helst som talar Redis-protokollet fungerar — peka `VALKEY_HOST` mot ett hanterat Redis om du föredrar det. De här inställningarna hette `REDIS_*` fram till 12.0.36; de gamla namnen läses fortfarande, containern svarar fortfarande på värdnamnet `redis`, och `npm run update` skriver inte om dem, så en äldre `config.env` behöver inte redigeras.
