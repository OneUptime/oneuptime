...
`filters.logs.minSeverity` släpper loggposter under en allvarlighetsgrad, i agenten, innan något skickas:

```bash
  --set filters.logs.minSeverity=WARN
```

Accepterar `TRACE`, `DEBUG`, `INFO`, `WARN`, `ERROR`, `FATAL`. `WARN` behåller WARN, ERROR och FATAL och släpper INFO, DEBUG och TRACE. Standardvärdet (`""`) behåller allt.

Detta fungerar trots att containerkörtider inte registrerar någon allvarlighetsgrad på loggraden: agenten tolkar fram en ur loggtexten (`[ERROR]`, `WARN:`, `level=info`, …) och faller tillbaka på `stderr → ERROR` / `stdout → INFO`. Det gäller i **båda** logglägena — i `daemonset`-läge via insamlaren, i `api`-läge inuti loggläsaren själv — så förinställningarna kan inte ändra beteendet bakom ryggen på dig.

> Poster vars allvarlighetsgrad ändå inte kunde fastställas **behålls**, de släpps aldrig. Det säkra felet för ett filter är att skicka för mycket, inte att tyst radera en logg som ingen visste var oklassificerad.
...
- **Bryr du dig bara om varningar och fel?** `filters.logs.minSeverity` släpper resten i agenten. På ett pratsamt kluster är detta ofta den enskilt största minskningen som finns att få, eftersom INFO och DEBUG utgör merparten av de flesta applikationers utdata:
  ...
  Se [Filtrering efter loggallvarlighetsgrad](#filtering-by-log-severity) för hur allvarlighetsgraden fastställs och vad som händer med loggar den inte kan klassificera.
...
- **Släpp specifika mått efter namn.** Tillåtelselistorna ovan gäller per mottagare; `filters.metrics.exclude` spänner över alla, så använd den för allt som rattarna på mottagarnivå inte kan uttrycka:
  ...
  Se [Inkludera eller exkludera mått efter namn](#including-or-excluding-metrics-by-name) för exakt matchning kontra regex och för tillåtelselisteformen.
