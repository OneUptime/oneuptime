...
The API key is a **project API key** created in your instance's dashboard under **Projektinställningar > API-nycklar** — exactly as on cloud. Self-hosted master keys do not work and fail with `ProjectId required` (see [Troubleshooting](/docs/terraform/troubleshooting)).
