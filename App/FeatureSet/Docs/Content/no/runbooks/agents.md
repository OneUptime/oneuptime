...
En **Runbook-agent** er en liten selvhostet prosess som kjører Bash-stegene i runbookene dine **inne i din egen infrastruktur**. OneUptime-Worker'en kjører aldri shell-kommandoene dine selv — den setter dem i kø, og en Runbook-agent du har installert i miljøet ditt henter dem, kjører dem og sender resultatet tilbake.

Denne siden forklarer hvordan du installerer en agent, ruter Bash-steg til den og drifter den til daglig.

## Hvorfor agenter finnes

Tidligere versjoner av OneUptime kjørte Bash-steg direkte på Worker'en. Det fungerte for single-tenant selvhostede oppsett der operatørene allerede hadde shell på maskinen, men gir to problemer for alle andre:

- **Tillitsgrense.** Den som kan skrive et runbook, kan kjøre shell på Worker'en, med tilgang til alle environment variables og hele filsystemet Worker'en har.
- **Rekkevidde.** De fleste nyttige Bash-steg vil operere på *kundens* infrastruktur ("restart denne tjenesten", "kubectl på vårt cluster") — ikke på OneUptimes.

Runbook-agenter snur dette på hodet. Bash-steg kjører ikke hos oss. De kjører på en vert du kontrollerer, og du bestemmer hva den verten har lov til.
...
4. Når et Bash-steg kjører, setter Worker'en inn en jobrad merket med stegets **Agent Tag** og setter statusen til `Pending`.
5. Hvilken som helst sunn agent i samme prosjekt som bærer den taggen claimer jobben (atomisk — aldri to agenter på samme jobb), kjører `bash -c <ditt skript>` lokalt, fanger stdout/stderr/exit-kode, og sender resultatet tilbake.
...
| **Tags** | Komma-separerte. Bash-steg sikter etter en tag; enhver agent i prosjektet med den taggen får kjøre dem. Vanlige mønstre: `prod`, `staging`, `eu-west-1`, `db-host`. |
...
Tagger er måten et Bash-steg finner en agent på. Noen mønstre:
...
Et Bash-steg **må** spesifisere nøyaktig én agent-tag. Multi-tag-ruting (kjør på enhver agent som har `prod` AND `db`) står på roadmap'en, men er ikke i denne releasen.

## Pek et Bash-steg mot en agent

Legg til et Bash-steg i runbook'et ditt. Skjemaet spør om en **Agent Tag**:

- Skriv inn taggen som matcher agenten/agentene du vil ha det til å kjøre på.
- Skriv skriptet ditt i editoren under.

Når runbook'et kjører og når dette steget, køer Worker'en en jobb med den taggen. Hvis minst én sunn agent med den taggen er online, blir jobben claimet innen få sekunder og kjørt.

## Driftsmerknader

### Timeouts

To timeouts gjelder for hvert Bash-steg:
...
| **Execution timeout** | 30 sekunder | Hvor lenge agenten lar skriptet kjøre før den sender `SIGKILL`. Konfigurerbar per steg. |
...
Skriptets barneprosess avbrytes **ikke** automatisk når lease'en utløper — men Worker'en slutter å vente, og agenten kan ikke sende inn et resultat når en annen claim har tatt over. Design skript til å være trygge å kjøre om igjen hvis exactly-once betyr noe for deg.
