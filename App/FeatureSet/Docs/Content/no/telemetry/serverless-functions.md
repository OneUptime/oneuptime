...
- Et **OneUptime Telemetry Ingestion Token** — opprett ett fra _Prosjektinnstillinger → Telemetri og APM → Inntaksnøkler_ og kopier `x-oneuptime-token`-verdien.
...
| `faas.instance`                                        | nei     | Spores per instans under fanen **Instanser**                |
...
> En funksjon som også setter `service.name`, vises fortsatt under **Tjenester** i tillegg. Visningen **Serverløse funksjoner** er det FaaS-fokuserte perspektivet, avgrenset etter `faas.name`.
...
Når funksjonen sender ut et spor, en logg eller en metrikk, dukker den opp under **Serverløse funksjoner**. Oversikten viser:

- **Anrop**, **error rate** og **p95-varighet** — utledet fra sporene dine, over et valgbart tidsrom, med trenddiagrammer.
- **Instanser** — en sanntidstelling av `faas.instance`-verdiene som er sett.
- Fullstendige faner for **Logger**, **Spor** og **Målinger** avgrenset til denne funksjonen.

Du kan også automatisk bruke etiketter og eiere via _Serverløs → Innstillinger → Etikettregler / Eierregler_.
