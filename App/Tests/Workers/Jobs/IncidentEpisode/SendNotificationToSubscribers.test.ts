...
/*
 * Incident episode "created" notifications to status page subscribers.
 *
...
const DESCRIPTION: string = "Several **network** incidents are open.";
const DESCRIPTION_HTML: string =
  "<p>Several <strong>network</strong> incidents are open.</p>";
const DESCRIPTION_TEXT: string = "Several network incidents are open.";
...
const CUSTOM_EMAIL_BODY: string =
  "<div>{{episodeDescription}}</div><div>{{resourcesAffected}}</div>";
const CUSTOM_EMAIL_SUBJECT: string =
  "{{episodeTitle}}: {{episodeDescription}} ({{resourcesAffected}})";
const CUSTOM_SMS_BODY: string =
  "SMS {{episodeDescription}} ({{resourcesAffected}})";
const CUSTOM_SLACK_BODY: string =
  "Slack {{episodeDescription}} ({{resourcesAffected}})";
const CUSTOM_TEAMS_BODY: string =
  "Teams {{episodeDescription}} ({{resourcesAffected}})";
...
beforeEach(() => {
  ...
  mock(IncidentEpisodeService.findAllBy).mockImplementation(
    async (args: unknown): Promise<Array<IncidentEpisode>> => {
      ...
      // Nothing to mark as Skipped; only the pending episodes are sent.
      if (
...
describe("IncidentEpisode:SendNotificationToSubscribers", () => {
  test("sends the default episode-created email", async () => {
    await runJob();
  ...
  test("sends the default SMS, Slack and Teams messages, matching the dashboard defaults", async () => {
    ...
    expect(sentSms()).toEqual([
      `Incident ${EPISODE_TITLE} (Major) on Acme Status. Impact: Edge network. Details: ${DETAILS_URL}. Unsub: ${UNSUBSCRIBE_URL}`,
    ]);
...
describe("IncidentEpisode:SendNotificationToSubscribers, with custom templates and grouped resources", () => {
  beforeEach(() => {
...
describe("IncidentEpisode default email, with grouped resources", () => {
