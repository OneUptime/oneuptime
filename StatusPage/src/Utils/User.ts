...
export default class User {
    public static getAccessToken(statusPageId: ObjectID): string {
        return LocalStorage.getItem(statusPageId.toString()+'access_token') as string;
    }
    ...
    public static setUserId(statusPageId: ObjectID, userId: ObjectID): void {
        LocalStorage.setItem(statusPageId.toString()+'user_id', userId.toString());
    }

    public static getUserId(statusPageId: ObjectID,): ObjectID {
        return new ObjectID((LocalStorage.getItem(statusPageId.toString()+'user_id') as string) || '');
    }

    public static getName(statusPageId: ObjectID,): Name {
        return new Name((LocalStorage.getItem(statusPageId.toString()+'user_name') as string) || '');
    }

    public static setName(statusPageId: ObjectID,name: Name): void {
        LocalStorage.setItem(statusPageId.toString()+'user_name', name.toString());
    }

    public static getEmail(statusPageId: ObjectID,): Email {
        return new Email(LocalStorage.getItem(statusPageId.toString()+'user_email') as string);
    }
    ...
    public static isLoggedIn(statusPageId: ObjectID): boolean {
        return LocalStorage.getItem(statusPageId.toString()+'access_token') ? true : false;
    }
