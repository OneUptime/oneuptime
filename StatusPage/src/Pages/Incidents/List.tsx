...
const Overview: FunctionComponent<PageComponentProps> = (
    ...
    useAsyncEffect(async () => {
        try {
            ...
            const response = await BaseAPI.post<JSONObject>(URL.fromString(DASHBOARD_API_URL.toString()).addRoute(`/status-page/incidents/${id.toString()}`), {}, {});
            const data = response.data;


            const incidentPublicNotes = BaseModel.fromJSONArray(data['incidentPublicNotes'] as JSONArray || [], IncidentPublicNote);
            const incidents = BaseModel.fromJSONArray(data['incidents'] as JSONArray || [], Incident);
    ...
    useEffect(() => {
        ...
        for (const incident of incidents) {
            ...
            for (const incidentPublicNote of incidentPublicNotes) {
                ...
                if (incidentPublicNote.incidentId?.toString() === incident.id?.toString()) {
                    timeline.push({
            ...
            for (const incidentStateTimeline of incidentStateTimelines) {
                ...
                if (incidentStateTimeline.incidentId?.toString() === incident.id?.toString()) {

                    timeline.push({
                        text: incidentStateTimeline.incidentState?.name || '',
                        date: incidentStateTimeline?.createdAt!,
                        isBold: true,
                        color: incidentStateTimeline.incidentState?.color || Red
                    })
                }
            }
