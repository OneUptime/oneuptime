  ...
  const data: EventItemComponentProps = {
    ...
    eventViewRoute: !isSummary
      ...
      : RouteUtil.populateRouteParams(
          isPreviewPage
            ? (RouteMap[PageMap.PREVIEW_EPISODE_DETAIL] as Route)
            : (RouteMap[PageMap.EPISODE_DETAIL] as Route),
          episode.id!,
        ),
...
const EpisodeDetail: FunctionComponent<PageComponentProps> = (
    ...
    try {
      ...
      const response: HTTPResponse<JSONObject> = await API.post<JSONObject>({
        url: URL.fromString(STATUS_PAGE_API_URL.toString()).addRoute(
          `/episodes/${id.toString()}/${episodeId?.toString()}`,
        ),
        data: {},
      ...
      const rawEpisodes: JSONArray = (data["episodes"] as JSONArray) || [];
  ...
  return (
    ...
    >
      {episode ? <EventItem {...parsedData} /> : <></>}
      {!episode ? (
        <EmptyState
          id="episode-empty-state"
          title={"No Episode"}
          description={"Episode not found on this status page."}
