...
const CONTENT_DIR: string = path.resolve(__dirname, "../../App/FeatureSet/Docs/Content");
...
const headingsOf: (file: string) => Array<string> = (file: string): Array<string> => {
 ...
 */
const anchorsByPosition: (file: string) => Map<string, string> = (file: string): Map<string, string> => {
  ...
  fs.readFileSync(file, "utf8").split("\n").forEach((line: string, i: number): void => {
    ...
    if (inFence) { return; }
    let n: number = 0;
    for (const m of line.matchAll(/\]\(#([^)]*)\)/g)) { map.set(`${i}:${n++}`, m[1] || ""); }
  });
  return map;
};
...
const walk: (dir: string) => Array<string> = (dir: string): Array<string> => {
  ...
  for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (e.isDirectory()) { out.push(...walk(p)); } else if (p.endsWith(".md")) { out.push(p); }
  }
  return out;
};

let already: number = 0; let repointed: number = 0;
const unresolved: Array<string> = []; const touched: Set<string> = new Set();

for (const loc of fs.readdirSync(CONTENT_DIR).filter((d: string) => { return fs.statSync(path.join(CONTENT_DIR, d)).isDirectory(); })) {
  for (const file of walk(path.join(CONTENT_DIR, loc))) {
    ...
    const mine: Array<string> = mineH.map((h: string) => { return Markdown.slugify(h); });
    ...
    const enNew: Array<string> = enH.map((h: string) => { return Markdown.slugify(h); });
    ...
    const enPos: Map<string, string> = lineParity ? anchorsByPosition(enFile) : new Map();
    ...
    const out: string = src.split("\n").map((line: string, lineNo: number): string => {
      ...
      let nth: number = -1;
      return line.replace(/\]\(#([^)]*)\)/g, (whole: string, a: string): string => {
        nth++;
        if (mine.includes(a)) { already++; return whole; }
         ...
         */
        if (idx === -1 && lineParity) {
          const enAnchor: string | undefined = enPos.get(`${lineNo}:${nth}`);
          if (enAnchor) {
            idx = enNew.indexOf(enAnchor);
            if (idx === -1) { idx = enOld.indexOf(enAnchor); }
          }
        }

        if (idx === -1 || !mine[idx]) { unresolved.push(`${loc}/${rel}:${lineNo + 1} -> #${a}`); return whole; }
        repointed++; touched.add(file);
        return `](#${mine[idx]})`;
      });
    }).join("\n");

    if (out !== src && APPLY) { fs.writeFileSync(file, out); }
  }
}

console.log(APPLY ? "APPLIED" : "DRY RUN");
console.log(`  already correct : ${already}`);
console.log(`  repointed       : ${repointed} across ${touched.size} files`);
