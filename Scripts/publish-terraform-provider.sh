...
generate_provider() {
    ...
    local go_files=$(find "$PROVIDER_FRAMEWORK_DIR" -name "*.go" | wc -l)
    print_status "Generated $go_files Go files"

    # Ensure we're in the provider framework directory for Go operations
    cd "$PROVIDER_FRAMEWORK_DIR"

    # Verify essential files were generated - go.mod is required
    if [[ ! -f "go.mod" ]]; then
        print_error "go.mod file not found in $PROVIDER_FRAMEWORK_DIR"
        print_error "The 'npm run generate-terraform-provider' command should create a proper Go module"
        print_error "Please check the generator script to ensure it initializes the Go module properly"
        exit 1
    fi

    print_success "Found go.mod file generated by provider generation"

    # Verify main.go exists (check various possible locations)
    if [[ ! -f "cmd/main.go" && ! -f "main.go" && ! -f "cmd/provider/main.go" ]]; then
        print_error "No main.go file found (checked cmd/main.go, main.go, and cmd/provider/main.go)"
        print_error "The provider generation should create the main entry point"
        exit 1
    fi

    # Ensure Go module dependencies are properly resolved
    print_status "Resolving Go module dependencies..."
    if ! go mod tidy; then
        print_error "Failed to resolve Go module dependencies"
        print_error "There may be import issues in the generated code"
        exit 1
    fi

    # Verify the module can be built
    print_status "Verifying provider can be built..."
    if go build -v ./...; then
        print_success "Provider builds successfully"
    else
        print_error "Provider build failed"
        print_error "There are compilation issues in the generated code"
        exit 1
    fi
