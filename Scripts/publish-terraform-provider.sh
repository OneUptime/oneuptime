...
generate_provider() {
    ...
    # Ensure we're in the terraform directory for Go operations
    cd "$TERRAFORM_DIR"

    # Verify essential files were generated - go.mod is required
    if [[ ! -f "go.mod" ]]; then
        print_error "go.mod file not generated by provider generation"
    ...
    if [[ ! -f "cmd/main.go" && ! -f "main.go" ]]; then
        print_error "No main.go file found (checked cmd/main.go and main.go)"
