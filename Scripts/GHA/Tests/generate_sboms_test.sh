...
if [[ -n "${FAKE_SYFT_FAIL_REF}" && "$ref" == *"${FAKE_SYFT_FAIL_REF}"* ]] \
...
# --- A registry that never recovers still fails the job. ---
reset_fake_syft_state
...
status=0
output="$(run_generate "$OUT_DIR" "linux/amd64,linux/arm64")" || status=$?
assert_eq 1 "$status" "fails when a scan never recovers"
...
assert_eq 4 "$(cat "${FAKE_SYFT_STATE_DIR}"/*home*amd64* 2>/dev/null)" "made delays+1 attempts before giving up"
...
assert_eq 1 "$(cat "${FAKE_SYFT_STATE_DIR}"/*home*amd64* 2>/dev/null)" "does not retry a missing tag"
