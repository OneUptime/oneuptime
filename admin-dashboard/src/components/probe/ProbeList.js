...
export class ProbeList extends Component {
    ...
    render() {
        const { probes,deleteRequesting,probeRequesting,addRequesting } = this.props;
        ...
        let canNext = (probes && probes.count) && (probes.count > (probes.skip + probes.limit)) ? true : false;
        let canPrev = (probes && probes.skip <= 0) ? false : true;

        if ((probes && !probes.data) || probeRequesting || addRequesting || deleteRequesting) {
            canNext = false;
            canPrev = false;
        }
        return (
            <div>
                <table className="Table">
                    <thead className="Table-body">
                        <tr className="Table-row db-ListViewItem db-ListViewItem-header">
                            ...
                            <td className="Table-cell Table-cell--align--left Table-cell--verticalAlign--top Table-cell--width--minimized Table-cell--wrap--noWrap db-ListViewItem-cell" style={{ height: '1px' }}>
                                <div className="db-ListViewItem-cellContent Box-root Padding-all--8"><span className="db-ListViewItem-text Text-color--dark Text-display--inline Text-fontSize--13 Text-fontWeight--medium Text-lineHeight--20 Text-typeface--upper Text-wrap--wrap"><span>Action</span></span></div>
                            </td>
                    ...
                    <tbody className="Table-body">
                        {probeRequesting ?
                            ...
                            :
                            probes && probes.data && probes.data.length > 0 ? (
                                probes.data.map((probe) => {
                                    return <tr key={probe._id} className="Table-row db-ListViewItem bs-ActionsParent db-ListViewItem--hasLink">
                                        <td className="Table-cell Table-cell--align--left Table-cell--verticalAlign--top Table-cell--width--minimized Table-cell--wrap--wrap db-ListViewItem-cell db-ListViewItem-cell--breakWord" style={{ height: '1px', minWidth: '270px' }}>
                                            <div className="db-ListViewItem-cellContent Box-root Padding-all--8">
                                                <span className="db-ListViewItem-text Text-color--cyan Text-display--inline Text-fontSize--14 Text-fontWeight--medium Text-lineHeight--20 Text-typeface--base Text-wrap--wrap">
                                                    <div className="Box-root Margin-right--16"><span>{probe.probeName}</span></div>
                                                </span>
                                            </div>
                                        </td>
                                        <td className="Table-cell Table-cell--align--left Table-cell--verticalAlign--top Table-cell--width--minimized Table-cell--wrap--noWrap db-ListViewItem-cell" style={{ height: '1px' }}>
                                            <div className="db-ListViewItem-link" >
                                                <div className="db-ListViewItem-cellContent Box-root Padding-all--8">
                                                    <span className="db-ListViewItem-text Text-color--inherit Text-display--inline Text-fontSize--14 Text-fontWeight--regular Text-lineHeight--20 Text-typeface--base Text-wrap--wrap">
                                                        <div className="Box-root"><span>{probe.lastAlive ? moment(probe.lastAlive).format('dddd, MMMM Do YYYY, h:mm a') : ''}</span></div>
                                                    </span>
                                                </div>
                                            </div>
                                        </td>
                                                        ...
                                                        <div className="Box-root Flex-flex">
                                                            <div className="Box-root Flex-flex">
                                                                <div className="db-RadarRulesListUserName Box-root Flex-flex Flex-alignItems--center Flex-direction--row Flex-justifyContent--flexStart">
                                                                    <ProbeStatus lastAlive={probe && probe.lastAlive} />
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </span>
                                        ...
                                        <td className="Table-cell Table-cell--align--left Table-cell--verticalAlign--top Table-cell--width--minimized Table-cell--wrap--noWrap db-ListViewItem-cell" style={{ height: '1px' }}>
                                            <div className="db-ListViewItem-link" >
                                                <div className="db-ListViewItem-cellContent Box-root Padding-all--8">
                                                    <button id="delete_probe" className="bs-Button bs-DeprecatedButton Margin-left--8" disabled={probeRequesting || addRequesting || deleteRequesting} onClick={() => this.handleClick(probe._id)}>
                                                        <ShouldRender if={!deleteRequesting}>
                                                            <span>Remove</span>
                                                        </ShouldRender>
                                                        <ShouldRender if={deleteRequesting}>
                                                            <FormLoader />
                ...
                <div style={{ textAlign: 'center', marginTop: '10px' }}>
                    {probes && (!probes.data || !probes.data.length) && !probes.requesting && !probes.error ? 'We don\'t have any probes yet' : null}
                ...
                <div className="Box-root Flex-flex Flex-alignItems--center Flex-justifyContent--spaceBetween">
                    <div className="Box-root Flex-flex Flex-alignItems--center Padding-all--20">
                        <span className="Text-color--inherit Text-display--inline Text-fontSize--14 Text-fontWeight--regular Text-lineHeight--20 Text-typeface--base Text-wrap--wrap">
                            <span>
                                <span className="Text-color--inherit Text-display--inline Text-fontSize--14 Text-fontWeight--medium Text-lineHeight--20 Text-typeface--base Text-wrap--wrap">{probes && probes.count ? probes.count + (probes && probes.count > 1 ? ' probes' : ' Probe') : null}</span>
                            </span>
                        </span>
                    </div>
                    <div className="Box-root Padding-horizontal--20 Padding-vertical--16">
                        <div className="Box-root Flex-flex Flex-alignItems--stretch Flex-direction--row Flex-justifyContent--flexStart">
                            <div className="Box-root Margin-right--8">
                                <button id="btnPrev" onClick={() => { this.props.prevClicked(probes.skip, probes.limit) }} className={'Button bs-ButtonLegacy' + (canPrev ? '' : 'Is--disabled')} disabled={!canPrev} data-db-analytics-name="list_view.pagination.previous" type="button">
                            ...
                            <div className="Box-root">
                                <button id="btnNext" onClick={() => { this.props.nextClicked(probes.skip, probes.limit) }} className={'Button bs-ButtonLegacy' + (canNext ? '' : 'Is--disabled')} disabled={!canNext} data-db-analytics-name="list_view.pagination.next" type="button">
...
function mapStateToProps(state) {
    return {
        ...
        deleteRequesting : state.probe.deleteProbe && state.probe.deleteProbe.requesting,
...
ProbeList.propTypes = {
  ...
  probes: PropTypes.oneOfType([PropTypes.object, PropTypes.oneOf([null, undefined])]),
