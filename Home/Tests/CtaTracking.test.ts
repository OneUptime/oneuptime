...
describe("the compliance CTA reaches GA4", () => {
  ...
  test("the documented regex and the container config agree", () => {
    const container: string = fs.readFileSync(
      path.join(REPOSITORY_ROOT, "Docs", "analytics", "gtm-key-events.json"),
      "utf-8",
    );
    const funnelDoc: string = fs.readFileSync(
      path.join(REPOSITORY_ROOT, "Docs", "analytics", "revenue-funnel.md"),
      "utf-8",
    );

    const pattern: RegExpMatchArray | null = container.match(
      /\^\((signup_started[^)]*)\)\$/,
    );

    expect(pattern).not.toBeNull();
    expect(funnelDoc).toContain(pattern![1]!);
  });
});
