...
describe('Dmca page test', () => {
    ...
    test(
        'Title of the page',
        async () => {
            await page.goto(`${HOME_URL.toString()}/legal/soc-2`, {
    ...
    test(
        ...
        async () => {
            ...
            const report: VALUE_TYPE = await PuppeteerHelper.getTextContent(
                page,
                '#report'
            );

            expect(pageTittle).toBe('SOC 2');
            expect(report).toBe('Request Report');
        },
        OPERATION_TIMEOUT
    );
});
