...
describe('Subprocessors page test', () => {
    beforeAll(async () => {
    ...
    test(
        'Title of the page',
        async () => {
            await page.goto(`${HOME_URL.toString()}/legal/subprocessors`, {
    ...
    test(
        ...
        async () => {
            ...
            const updates: VALUE_TYPE = await PuppeteerHelper.getTextContent(
                page,
                '#updates'
            );

            expect(pageTittle).toBe('Subprocessors');
            expect(updates).toBe('Updates');
        },
        OPERATION_TIMEOUT
    );
});
