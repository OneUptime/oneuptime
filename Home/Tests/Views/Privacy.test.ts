...
describe('Privacy policy page test', () => {
    ...
    test(
        'Title of the page',
        async () => {
            await page.goto(`${HOME_URL.toString()}/legal/privacy`, {
    ...
    test(
        ...
        async () => {
            ...
            const dataUse: VALUE_TYPE = await PuppeteerHelper.getTextContent(
                page,
                '#data-use'
            );
            const security: VALUE_TYPE = await PuppeteerHelper.getTextContent(
                page,
                '#security'
            );

            expect(pageTittle).toBe('Privacy Policy');
            expect(dataUse).toBe('Data Use');
