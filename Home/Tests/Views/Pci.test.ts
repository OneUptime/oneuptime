...
describe('Dmca page test', () => {
    ...
    test(
        'Title of the page',
        async () => {
            await page.goto(`${HOME_URL.toString()}/legal/pci`, {
    ...
    test(
        ...
        async () => {
            ...
            const audit = await PuppeteerHelper.getTextContent(page, '#audit');

            expect(pageTittle).toBe('PCI DSS');
            expect(audit).toBe('Audit Report');
        },
        OPERATION_TIMEOUT
    );
});
