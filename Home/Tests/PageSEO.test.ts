...
describe("PageSEOConfig", () => {
  ...
  test.each(entries)("%s has a non-empty title and description", (
    _key: string,
    data: PageSEOData,
  ) => {
  ...
  test.each(entries)("%s has a rooted canonical path", (
    key: string,
    data: PageSEOData,
  ) => {
  ...
  test.each(entries)("%s has a valid page type", (
    _key: string,
    data: PageSEOData,
  ) => {
  ...
  test.each(entries)("%s has a well-formed breadcrumb trail", (
    _key: string,
    data: PageSEOData,
  ) => {
    ...
    // canonical URL agree.
    const last: BreadcrumbItem = data.breadcrumbs[data.breadcrumbs.length - 1]!;
    expect(last.url).toBe(data.canonicalPath);
  });

  test.each(entries)("%s optional OG fields are non-empty when present", (
    _key: string,
    data: PageSEOData,
  ) => {
