 ...
 *
 * EMPTY at merge. Until the key ceremony ships a public key here, every
 * license is "unverified" (accepted only while legacy acceptance is on), so the
 * signed license format protects nothing yet. The ceremony order matters:
 * release a build that trusts the new key, deploy it to the license server,
 * and only then give the license server the private key.
 *
