...
- **OneUptime テレメトリ取り込みトークン** — _プロジェクト設定 → テレメトリと APM → 取り込みキー_ から作成し、`x-oneuptime-token` の値をコピーします
...
4. フリートの下の **メトリクス** を開いて、上記の任意の `iot_*` 系列をチャート化します。
...
取り込みトークンが無効、失効、または欠落しています。_プロジェクト設定 → テレメトリと APM → 取り込みキー_ から新しいトークンを生成し、`x-oneuptime-token` ヘッダーを更新してください。
