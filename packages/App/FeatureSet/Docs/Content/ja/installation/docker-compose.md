...
- **キャッシュとキュー**: `valkey` サービスでは [Valkey](https://valkey.io)（Redis 7.2 の BSD ライセンスのフォーク）が動作し、`config.env` の `VALKEY_*` 設定で構成します。Redis プロトコルを扱うサーバーであれば何でも動作します — 希望する場合は `VALKEY_HOST` をマネージドの Redis に向けてください。これらの設定は 13.0.0 までは `REDIS_*` という名前でした。古い名前も引き続き読み取られ、コンテナはホスト名 `redis` にも応答し、`npm run update` がそれらを書き換えることもないため、古い `config.env` でも編集は不要です。
