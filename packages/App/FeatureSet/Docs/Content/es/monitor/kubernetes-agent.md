...
OBI también puede propagar el contexto de traza a través de los límites de servicio, de modo que el span en el lado del pod B se enlace con la misma traza que la petición saliente del pod A — sin cambios de SDK en ninguna aplicación. **Esto está desactivado por defecto; actívalo con `--set ebpf.contextPropagation=true`.**

| Opción                     | Por defecto | Descripción                                                                                                                                                                                        |
| -------------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ebpf.contextPropagation`  | off         | Inyecta un W3C `traceparent` en el tráfico saliente para que los spans se enlacen entre servicios. Desactivado por defecto — lee antes la advertencia de abajo. Sin requisito de kernel adicional; ninguna versión de kernel la deja inerte. |
| `ebpf.contextPropagationMode` | headers  | Cómo se inyecta el `traceparent`: `headers` (cabeceras de petición HTTP/1.1, el más acotado), `tcp` (opción TCP vía Linux Traffic Control) o `all` (ambos). Sólo se lee cuando `contextPropagation` es true. |
...
> **⚠️ Por qué está desactivado por defecto.** Todos los modos de propagación modifican tráfico que ya está en vuelo: las peticiones HTTP en claro se ensanchan in situ en el kernel, mientras que TLS y TCP en bruto reciben una opción TCP añadida desde un hook de Traffic Control. Si la contabilidad de bytes de la conexión no se corrige con exactitud, el flujo se desincroniza — el síntoma reportado es que las transferencias a través de un proxy L7 como nginx se quedan colgadas a mitad del cuerpo en cuanto la respuesta supera los ~64KB, mientras que las peticiones pequeñas siguen funcionando, por lo que parece un fallo de la aplicación y no del agente. Las trazas, las métricas RED y el mapa de servicios no dependen de ello; para enlazar entre servicios, un SDK de OpenTelemetry que propaga la cabecera en espacio de usuario sin reescritura en el kernel es la opción más segura.
...
| `profiling.tracers`           | `""` _(todos los runtimes)_ | Lista separada por comas de tracers de lenguaje a cargar.                                                                                                 |
