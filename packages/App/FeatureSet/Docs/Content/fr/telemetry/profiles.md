...
exporters:
  otlphttp:
    ...
    # Sans cette ligne, l'exportateur envoie vers /otlp/v1development/profiles, que OneUptime ne sert pas
...
service:
  pipelines:
    # Nécessite de démarrer le collecteur avec --feature-gates=service.profilesSupport
    profiles:
      receivers: [otlp]
      exporters: [otlphttp]
```
