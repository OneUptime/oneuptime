...
Les règles de runbook attachent automatiquement des runbooks lorsqu'un **incident**, une **alerte** ou un **événement de maintenance planifiée** est créé. Elles se gèrent depuis le menu Règles de chaque entité :
...
| **Activé**                | Bouton pour suspendre une règle sans la supprimer.                                                      |
| **Modèle de titre**       | Regex insensible à la casse comparée au titre de l'entité. Vide = tout titre correspond.                |
| **Modèle de description** | Regex insensible à la casse comparée à la description de l'entité. Vide = toute description correspond. |
| **Runbooks à démarrer**   | Un ou plusieurs runbooks à démarrer lorsque la règle se déclenche.                                      |
