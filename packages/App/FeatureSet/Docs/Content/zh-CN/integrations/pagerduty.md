...
1. 打开 **工作流 → 创建工作流**，命名为 `Incidents → PagerDuty`，并打开 **生成器**。
...
1. 在**同一个**工作流中再添加一个 **事件** 触发器？不——一个工作流只能有一个触发器。请创建一个**第二个**工作流，命名为 `Resolve PagerDuty`，使用 **事件 → On Update** 触发器。
2. 添加 **条件** 模块，检查事件是否已解决（分支判断事件状态 `{{Incident.currentIncidentState.name}}` 是否等于你的已解决状态名称）。
3. 从 **是** 出发，添加 **API** 模块向 PagerDuty 发送请求，使用**相同的 `dedup_key`**，并将 `event_action` 设置为 `resolve`：
...
PagerDuty 的 `severity` 接受 `critical`、`error`、`warning` 或 `info`。要从 OneUptime 严重程度映射，在 API 模块之前对 `{{Incident.incidentSeverity.name}}` 添加 **条件** 分支，并从每个分支发送不同的正文。
...
- **日志中什么都没有**——确认工作流处于 **已启用** 状态且触发器为 **On Create**。
