...
> **版本：** SSO 属于 OneUptime Enterprise Edition（企业版）功能。在 OneUptime Cloud 上，**Scale** 及以上套餐可用。自托管部署需要使用企业版镜像并拥有许可证。请参阅 [Enterprise Edition](/docs/self-hosted/enterprise)。没有有效的许可证时（14 天试用期结束后，或许可证过期 30 天后），在激活许可证之前，SSO 登录将停止，且不会强制执行“Require SSO”。
