...
> **版本：** SCIM 属于 OneUptime Enterprise Edition（企业版）功能。在 OneUptime Cloud 上，**Scale** 及以上套餐可用。自托管部署需要使用企业版镜像并拥有许可证。请参阅 [Enterprise Edition](/docs/self-hosted/enterprise)。没有有效的许可证时（14 天试用期结束后，或许可证过期 30 天后），在激活许可证之前，SCIM 请求将被拒绝。
...
2. **配置 SCIM 设置**
   ...
   - 启用 **自动预配用户** 以在 IdP 中分配用户时自动添加用户
   - 启用 **自动取消预配用户** 以在 IdP 中取消分配用户时自动移除用户
   - 选择新用户应加入的 **默认团队**
...
3. **团队成员资格**：用户被自动添加到配置的默认团队。在 OneUptime Cloud 上，已有 OneUptime 账号的人会改为收到邀请，并在接受邀请后加入（参见下方的常见问题）
...
2. **配置 SCIM 设置**
   ...
   - 启用 **自动预配用户** 以在 IdP 中分配用户时自动添加订阅者
   - 启用 **自动取消预配用户** 以在 IdP 中取消分配用户时自动移除订阅者
   - 复制 **SCIM 基础 URL** 和 **Bearer Token** 用于 IdP 配置
...
当 SCIM 尝试创建已存在（通过电子邮件匹配）的用户时，OneUptime 不会创建重复用户。接下来会发生什么，取决于 OneUptime 的运行位置：
...
由 SCIM 自己创建的用户、已加入您项目的用户，以及已确认您项目 SSO 的用户，在两种环境下都会被立即添加。

### SCIM 能否更改用户的电子邮件地址或姓名？

OneUptime 账号的电子邮件地址是该用户登录其所属每个项目时使用的地址，也是其密码重置链接的接收地址。因此：
...
姓名在两种环境下都遵循相同的规则：只有当用户已加入此项目、不属于任何其他项目且不是 OneUptime 管理员时，SCIM 才会更新其姓名。对于其他用户，姓名保持不变，请求的其余部分仍会成功。
