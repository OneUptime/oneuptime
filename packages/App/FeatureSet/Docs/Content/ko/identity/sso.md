...
> **에디션:** SSO는 OneUptime Enterprise Edition의 기능입니다. OneUptime Cloud에서는 **Scale** 플랜 이상에서 사용할 수 있습니다. 셀프 호스팅 설치에는 Enterprise Edition 이미지와 라이선스가 필요합니다. [Enterprise Edition](/docs/self-hosted/enterprise)을 참고하세요. 유효한 라이선스가 없으면(14일 평가판 종료 후 또는 라이선스 만료 30일 후) 라이선스가 활성화될 때까지 SSO 로그인이 중지되고 "Require SSO"가 적용되지 않습니다.
