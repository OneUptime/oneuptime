...
> **에디션:** SCIM은 OneUptime Enterprise Edition의 기능입니다. OneUptime Cloud에서는 **Scale** 플랜 이상에서 사용할 수 있습니다. 셀프 호스팅 설치에는 Enterprise Edition 이미지와 라이선스가 필요합니다. [Enterprise Edition](/docs/self-hosted/enterprise)을 참고하세요. 유효한 라이선스가 없으면(14일 평가판 종료 후 또는 라이선스 만료 30일 후) 라이선스가 활성화될 때까지 SCIM 요청이 거부됩니다.
...
2. **SCIM 설정 구성**
   ...
   - IdP에서 사용자가 할당될 때 자동으로 사용자를 추가하려면 **사용자 자동 프로비저닝**을 활성화합니다
   - IdP에서 사용자가 할당 해제될 때 자동으로 사용자를 제거하려면 **사용자 자동 프로비저닝 해제**를 활성화합니다
...
3. **팀 구성원 자격**: 사용자가 구성된 기본 팀에 자동으로 추가됨. OneUptime Cloud에서는 이미 OneUptime 계정이 있던 사람은 대신 초대되며, 초대를 수락하면 참여함(아래 자주 묻는 질문 참조)
...
1. **상태 페이지 설정으로 이동**
   ...
   - **상태 페이지 설정** > **보안** > **SCIM**으로 이동합니다

2. **SCIM 설정 구성**

   - IdP에서 사용자가 할당될 때 자동으로 구독자를 추가하려면 **사용자 자동 프로비저닝**을 활성화합니다
   - IdP에서 사용자가 할당 해제될 때 자동으로 구독자를 제거하려면 **사용자 자동 프로비저닝 해제**를 활성화합니다
   - IdP 구성을 위한 **SCIM 기본 URL** 및 **Bearer 토큰**을 복사합니다
...
5. 다음 옵션을 구성합니다:
   - **사용자 자동 프로비저닝**: 사용자를 자동으로 생성하려면 활성화
   - **사용자 자동 프로비저닝 해제**: 사용자를 자동으로 제거하려면 활성화
   - **기본 팀**: 새 사용자가 추가될 팀 선택
   - **푸시 그룹 활성화**: Entra ID 그룹을 통해 팀 구성원 자격을 관리하려면 활성화
6. 구성을 저장합니다
...
5. 다음 옵션을 구성합니다:
   ...
   - **사용자 자동 프로비저닝 해제**: 사용자를 자동으로 제거하려면 활성화
   - **기본 팀**: 새 사용자가 추가될 팀 선택
   - **푸시 그룹 활성화**: Okta 그룹을 통해 팀 구성원 자격을 관리하려면 활성화
6. 구성을 저장합니다
...
SCIM이 이미 존재하는 사용자(이메일로 일치)를 생성하려고 할 때 OneUptime은 중복 사용자를 생성하지 않습니다. 그다음 동작은 OneUptime이 실행되는 환경에 따라 다릅니다:

- **셀프 호스팅**: 기존 사용자가 구성된 기본 팀(푸시 그룹을 사용하는 경우 그룹의 팀)에 즉시 추가됩니다.
...
SCIM이 직접 생성한 사용자, 이미 프로젝트에 참여한 사용자, 프로젝트의 SSO를 확인한 사용자는 어느 환경에서든 즉시 추가됩니다.

### SCIM이 사용자의 이메일 주소나 이름을 변경할 수 있나요?

OneUptime 계정의 이메일 주소는 그 사람이 속한 모든 프로젝트에 로그인할 때 쓰는 주소이며, 비밀번호 재설정 링크가 전송되는 주소이기도 합니다. 따라서:
...
- **셀프 호스팅**: SCIM은 이 프로젝트에 참여했고, 다른 프로젝트에 속하지 않으며, OneUptime 관리자가 아닌 사용자의 이메일 주소만 변경합니다. 그 밖의 변경은 같은 방식으로 거부됩니다.

이름에도 어느 환경에서든 같은 규칙이 적용됩니다. SCIM은 이 프로젝트에 참여했고, 다른 프로젝트에 속하지 않으며, OneUptime 관리자가 아닌 사용자의 이름만 업데이트합니다. 그 외의 사용자는 이름이 그대로 유지되며 요청의 나머지 부분은 정상적으로 성공합니다.

### 기본 팀과 그룹 푸시의 차이점은 무엇인가요?

- **기본 팀**: SCIM을 통해 프로비저닝된 모든 사용자가 동일한 사전 정의된 팀에 추가됩니다
- **푸시 그룹**: 팀 구성원 자격은 ID 공급자에 의해 관리되어 IdP 그룹 구성원 자격을 기반으로 다른 사용자가 다른 팀에 있을 수 있습니다
