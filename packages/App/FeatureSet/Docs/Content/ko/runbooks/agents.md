...
**런북 → 설정 → 에이전트**로 가서 새 에이전트를 만듭니다. 입력 항목:
...
에이전트를 만든 뒤 그 행에서 **설정 지침 표시**를 클릭합니다. 이 에이전트의 ID와 키가 들어간 `docker run` 명령이 보입니다. **지금 키를 저장하세요** — 나중에 재설정할 수는 있지만, 모달을 닫은 뒤에는 같은 키 값을 다시 볼 수 없습니다.
...
**런북 → 설정 → 에이전트**로 돌아갑니다. 약 60초 안에 에이전트의 행이 `Connected`로 바뀌고 **마지막 확인** 시각이 갱신되어야 합니다. 계속 `Disconnected`라면:
...
둘 다 단계별로 설정할 수 있습니다. **런북 &rsaquo; 여러분의 Runbook &rsaquo; 단계**를 열고, Bash 또는 JavaScript 단계를 펼쳐, 스크립트 아래에서 **실행 타임아웃**과 **클레임 타임아웃**을 초 단위로 설정하세요. 필드를 비워 두면 기본값이 사용됩니다. 각각 1초에서 1시간까지 지정할 수 있고, 그 범위를 벗어난 값은 단계가 실행될 때 범위 안으로 제한됩니다.
