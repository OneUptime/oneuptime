...
- **백업**: 데이터베이스(Clickhouse, Postgres)를 정기적으로 백업합니다. 캐시는 무상태이므로 무시해도 됩니다.
- **캐시 및 큐**: `valkey` 서비스는 Redis 7.2의 BSD 라이선스 포크인 [Valkey](https://valkey.io)를 실행하며, `config.env`의 `VALKEY_*` 설정으로 구성합니다. Redis 프로토콜을 사용하는 서버라면 무엇이든 동작하므로, 원한다면 `VALKEY_HOST`가 관리형 Redis를 가리키도록 하십시오. 이 설정들은 13.0.0까지 `REDIS_*`라는 이름이었습니다. 기존 이름도 여전히 읽히고, 컨테이너는 여전히 `redis` 호스트 이름에 응답하며, `npm run update`가 이를 다시 쓰지 않으므로 이전 `config.env`는 수정할 필요가 없습니다.
