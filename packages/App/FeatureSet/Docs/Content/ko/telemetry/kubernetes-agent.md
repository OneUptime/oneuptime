...
- 설치된 `helm` v3
- **OneUptime API 키** — *프로젝트 설정 → API 키*에서 생성하세요
...
`namespaceFilters.rules` 네임스페이스 패턴을 네 가지 범위에 독립적으로 적용합니다:

- `podLogs`: hostPath filelog 수신기 또는 API 로그 테일러에서 Pod stdout/stderr를 필터링합니다. Kubernetes 이벤트와 감사 로그에는 영향을 주지 않습니다.
- `ebpfDiscovery`: OBI 프로세스 검색을 필터링하므로 eBPF 트레이스와 eBPF 메트릭을 모두 제어합니다.
- `metrics`: 메타데이터 보강 후 네임스페이스가 있는 메트릭 시리즈를 필터링합니다. 네임스페이스가 없는 노드 및 클러스터 시리즈는 유지됩니다.
- `traces`: 메타데이터 보강 후 eBPF 스팬과 애플리케이션이 보낸 OTLP 스팬을 모두 필터링합니다.

네임스페이스 패턴은 전체 이름과 일치하며 team-*처럼 *를 와일드카드로 지원합니다. 어떤 범위에 include 규칙이 하나라도 있으면 해당 범위에서는 일치하는 네임스페이스만 유지됩니다. exclude 규칙이 항상 우선합니다. 기본 규칙은 kube-system을 podLogs와 ebpfDiscovery에서만 제외합니다.

Pod 로그와 eBPF 검색을 특정 네임스페이스로 제한하려면:
...
노이즈가 많은 네임스페이스의 로그만 중지하고 eBPF 트레이스, 서비스 맵, 메트릭은 유지하려면 제외 범위를 podLogs로 한정합니다:
...
podLogs 및 ebpfDiscovery 규칙은 소스에서 필터링합니다. 제외된 로그 파일은 열리지 않고 제외된 워크로드는 계측되지 않습니다. metrics 및 traces 규칙은 네임스페이스 메타데이터가 추가된 뒤 collector에서 실행됩니다.

#### 네임스페이스별 메트릭 및 트레이스 필터링

네임스페이스가 있는 메트릭이나 스팬도 필터링하려면 해당 범위를 규칙에 직접 추가합니다:

```bash
  --set-json 'namespaceFilters.rules=[{"action":"exclude","namespaces":["kube-system","noisy-*"],"scopes":["podLogs","ebpfDiscovery","metrics","traces"]}]'
```

> **노드 및 클러스터 수준 메트릭은 항상 유지됩니다. 네임스페이스는 노드가 아니라 Pod의 속성이므로 네임스페이스가 없는 시리즈는 규칙과 일치하지 않고 삭제되지 않습니다.**

Kubernetes 이벤트는 에이전트에서 네임스페이스별로 필터링할 수 없습니다. k8sobjects 수신기에서 k8s.namespace.name 속성 없이 도착하며 네임스페이스는 이벤트 본문에 있습니다. 대신 서버 측에서 필터링하세요.
...
> **Kubernetes 이벤트와 리소스 스펙은 이것으로 절대 필터링되지 않습니다.** 이들은 자체 심각도 없이 Kubernetes API에서 도착하므로, 임계값을 적용하면 피드를 다듬는 것이 아니라 통째로 삭제하게 됩니다 — 가장 필요로 하는 `FailedScheduling`, `BackOff`, `OOMKilling` 경고까지 포함해서 말입니다. 이들은 볼륨이 적고 가치가 높으므로 에이전트는 항상 이들을 전송합니다. 이들을 줄이려면 대신 대시보드의 서버 측 **로그 → 설정 → 드롭 필터**를 사용하세요.
...
> **이 값을 설정할 때는 스팬 기반 모니터를 다시 튜닝하세요.** 샘플링은 OneUptime에 도달하는 스팬을 줄이므로, 그것을 세는 것은 무엇이든 더 적게 세게 됩니다: `Span Count`를 사용하는 **트레이스** 모니터와 `Exception Count`를 사용하는 **예외** 모니터는 어제 볼륨의 대략 `percentage`%만 보게 됩니다. 샘플링하지 않은 트래픽에 맞춰 튜닝한 임계값은 조용히 더 이상 넘지 않게 됩니다 — 모니터가 오류를 내는 것이 아니라 그저 침묵합니다. 비율을 설정할 때 해당 임계값을 같은 배수로 나누세요. 이 비율은 클러스터 전체에 적용되므로 개별 서비스만 예외로 두는 방법은 없습니다. 오류 **그룹화**는 선형보다 더 나쁘게 저하됩니다: 흔한 예외는 여전히 드러나지만, 드물게 한 번 발생하는 예외는 10분의 1의 빈도로 나타나기보다 아예 사라져 버릴 가능성이 더 큽니다.
...
| **메트릭 카디널리티**            | 고유한 시리즈 수 (컨테이너별, PVC별, …)              | `filters.metrics.exclude`, `namespaceFilters.rules` (`metrics`), `cadvisor.metricsAllowlist`, `kubeletstats.volumeMetrics` |
...
세 가지 모두 **되돌릴 수 없습니다**: 여기에서 삭제한 것은 절대 OneUptime에 도달하지 않으며, 세 가지 모두 모니터를 조용하게 만들 수 있습니다. 앞의 두 가지는 모니터가 감시하는 시그널 자체를 제거해 모니터를 침묵시킵니다. 샘플링은 그보다 범위가 좁습니다: eBPF RED 메트릭은 샘플러가 실행되기 전에 계산되므로 메트릭 기반 모니터는 정확한 상태로 유지됩니다 — 하지만 *스팬*을 세는 모니터(`Span Count`를 사용하는 **트레이스** 모니터, `Exception Count`를 사용하는 **예외** 모니터)는 그에 비례해 더 적게 세므로 임계값을 같은 배수로 다시 튜닝해야 합니다. 나중에 결정하고 싶다면 OneUptime이 서버 측에서 데이터를 삭제할 수도 있습니다(**로그 → 설정 → 드롭 필터**, **메트릭 → 설정 → 파이프라인 규칙**) — 이는 여전히 이그레스 비용이 들지만, 재배포 없이 변경할 수 있는 설정입니다.

### 레버 1 — 파드 로그는 보통 가장 큰 단일 소스입니다

컨테이너 로그는 클러스터의 모든 컨테이너에서 로그 라인당 하나의 레코드이기 때문에 거의 항상 수집에서 가장 큰 부분을 차지합니다.

- **특정 네임스페이스의 로그만 필요합니까? podLogs 범위의 include 규칙을 사용하세요. 로그 소스에서 일치 여부를 판단하므로 필터링된 네임스페이스는 읽히지 않고 eBPF 텔레메트리는 독립적으로 유지됩니다.**
  ...
  노이즈가 많은 네임스페이스 그룹만 제외하려면 namespaces: [noisy-*] 및 scopes: [podLogs]를 지정한 exclude 규칙을 사용하세요.
...
- **전체 네임스페이스의 메트릭을 삭제하시겠습니까? metrics 범위의 exclude 규칙을 추가하세요. Pod 및 컨테이너별 시리즈는 필터링되고 네임스페이스가 없는 노드와 클러스터 시리즈는 유지됩니다.**
...
예외가 하나 있으며, 롤아웃하기 전에 확인해야 할 것은 바로 이것입니다: 메트릭이 아니라 **스팬을 세는** 모니터 — `Span Count`를 사용하는 **트레이스** 모니터, `Exception Count`를 사용하는 **예외** 모니터 — 는 그에 비례해 더 적게 세므로, 임계값을 같은 배수로 다시 튜닝해야 합니다. [트레이스 샘플링](#트레이스-샘플링)을 참조하세요.
...
필요하면 더 줄일 수 있습니다. minSeverity를 ERROR로 올리고, 네임스페이스 규칙의 scopes에 metrics를 추가하거나, OTel SDK에서 이미 트레이스를 보내는 경우 ebpf.enabled=false로 설정하세요.

> **무엇을 잘라내는지 주의하세요.** 일부 모니터는 특정 시그널에 의존합니다: `cadvisor`를 비활성화하면 OOM-kill 및 CPU 스로틀링 모니터가 제거되고, `kubeletstats.volumeMetrics`를 비활성화하면 PVC 저디스크 모니터가 제거되며, 로그를 비활성화하면 로그 기반 알림이 제거됩니다. 그리고 `sampling.traces.percentage`는 모니터를 제거하지는 않지만 스팬 기반 모니터(`Span Count`를 사용하는 **트레이스** 모니터, `Exception Count`를 사용하는 **예외** 모니터)를 함께 축소하므로, 그에 맞춰 임계값을 다시 튜닝하세요. 모니터가 감시하는 시그널이 아니라 대응하지 않는 시그널을 다듬으세요.

### 효과 측정

텔레메트리 사용량은 하루 단위로 집계되므로, 감소를 확인하려면 **프로젝트 설정 → 사용 기록**에서 하루 이틀에 걸친 추세를 확인하세요 — 변경 사항을 적용하는 즉시 움직이지는 않습니다. 모든 것을 한 번에 낮춰서 실제로 의존하던 모니터를 잃는 대신, 한 번에 하나의 레버씩 변경하여 차이를 귀속시킬 수 있도록 하세요 — 로그 끄기, 그다음 간격 늘리기, 그다음 eBPF 다듬기.
...
3. **수집 키를 검증하세요.** OneUptime에 토큰이 수락되는지 직접 문의하세요(`200` = 유효, `401` = 알 수 없음/취소됨):
   ...
   `401`을 반환하면 릴리스의 키가 잘못되었거나 취소된 것입니다. *프로젝트 설정 → 텔레메트리 및 APM → 수집 키*에서 유효한 키를 복사하여 다시 배포하세요:
...
4. 매우 큰 클러스터에서는 단일 복제본이 병목이 될 수 있습니다. namespaceFilters.rules의 podLogs 범위 include 규칙을 사용해 별도 릴리스를 샤딩하세요.
