...
| `same-origin-propagation`   | Logged once when the recorder starts: whether it adds trace context (`traceparent`, and a `tracestate` carrying the session id) to requests to the page's own origin. `enabled: true` with `reason: "on"` is the default. `reason: "policy-off"` means **Same-origin trace propagation** is off on the application's _Replay Policy_ page; `reason: "opaque-origin"` means the page has no origin of its own (a sandboxed iframe, `about:blank`, `srcdoc` or `file:`), so nothing counts as same-origin. Even when enabled, the headers go only on requests made while the session uploads. See [Correlating with your other telemetry](/docs/telemetry/session-replay#correlating-with-your-other-telemetry). |
| `same-origin-propagation-tripped` | A request the recorder had added trace context to failed at the network level, usually a same-origin request redirected to another origin that does not allow `traceparent` and `tracestate`. The recorder stopped adding them for the rest of this page load. `retried: true` means it was a `GET` or `HEAD` `fetch` without a body and was sent once more exactly as your page made it; `false` means it could not be retried (an XHR, or a request with a body) and failed. Your page's own aborts and timeouts do not trip it, and neither does a failure while the browser reports itself offline; the next page load starts fresh. See [Downloads or redirects fail after enabling Session Replay](#downloads-or-redirects-fail-after-enabling-session-replay). |
...
2. **Is your API on another origin?** Its requests get nothing unless the origin is in **Trace propagation origins**, and then only a `traceparent`, matched by trace id. If the request's row on the Network tab has no trace link either, the origin is not listed and your page does not set `traceparent` itself.
...
4. **Did the request already carry a `tracestate`?** Datadog RUM, Elastic RUM and some OpenTelemetry setups send their own. The recorder does not write into another vendor's header, so those spans are not stamped and link only by trace id.
5. **Traces there, Logs empty?** Logs join by trace id, so your backend has to write them inside the request's span, with OpenTelemetry logging that records the active trace id.
6. **Sampled away?** A trace your backend drops never reaches OneUptime; see [Sampling of browser-started traces](/docs/telemetry/session-replay#sampling-of-browser-started-traces).
...
The recorder trips a breaker on the first such failure. It logs `same-origin-propagation-tripped`, stops adding headers for the rest of the page load, and retries a `GET` or `HEAD` `fetch` without a body once, exactly as your page made it (`retried: true` in the detail), so your page gets the retry's outcome. An `XMLHttpRequest`, or a `fetch` with a body, cannot be retried, so that one request still fails, and the next page load adds the headers again. To fix it for good, either:

- allow `traceparent` and `tracestate` in the other origin's CORS configuration (on S3, the bucket's CORS `AllowedHeaders`), or
- turn **Same-origin trace propagation** off on the application's _Replay Policy_ page. Backend telemetry then links only for origins in **Trace propagation origins** and for requests your page traces itself.
