...
| Guest Down                    | Warning  | `pve_up` and `pve_onboot_status` filtered to `pve.scope=guest`, Min per `id`                  | A guest configured to start on boot (`pve_onboot_status` = 1) reports `pve_up` below 1 for the whole window. Guests you stopped on purpose are excluded automatically, so they never fire |
...
| High Guest CPU Usage          | Warning  | `pve_cpu_usage_ratio` filtered to `pve.scope=guest`, Avg per `id`, **past 15 minutes**        | > 0.95 (95% of allocated vCPUs) every minute of the 15-minute window — a guest is meant to use its allocation, so only a guest that never comes down pages                                |
...
> Notes on the choices baked in: down/offline templates use **Min** so a single down scrape trips the threshold instead of being masked by scrapes where the resource was still up — and **Guest Down** additionally requires `pve_onboot_status` on the same `id`, so a guest you stopped deliberately is never paged for; CPU templates use **Avg** because `pve_cpu_usage_ratio` is already a 0–1 ratio, so the per-minute average is the sustained utilization; state-style templates (HA, backup, replication) use **Max** so one bad scrape fires. Ratio formulas aggregate both sides with **Sum** — numerator and denominator ride the same exporter scrape, so the scrape multiple cancels and the result is a true percentage. Templates are pre-scoped with `pve.scope` / `pve.type` filters and grouped by `id`, so one incident fires per affected resource.
