...
The probe looks names up through its own DNS servers — the ones in `/etc/resolv.conf` where it runs: the host's for the bundled Docker Compose probes, the cluster's DNS for a probe in Kubernetes. They may not be the servers your workstation asks, so when a host you know has a PTR record shows "no PTR record", run `nslookup <address>` from inside the probe's container or pod to compare. A lookup that times out or fails is retried once, asking each configured nameserver in turn (up to three); when lookups still fail, the scan's status message says for how many hosts, and a rescan tries again. Names in the probe's `/etc/hosts` are used before DNS is asked.
