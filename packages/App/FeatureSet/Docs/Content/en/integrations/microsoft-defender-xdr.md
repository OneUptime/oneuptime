...
- **A poll reads at most 24 hours past the cursor.** A connection whose cursor fell behind (workers were down, the tenant was throttled) catches up in consecutive windows rather than skipping to the present.
- **Bounded reads.** One run reads at most 20 pages of 100 alerts (2,000 alerts). See [When a window holds more than one poll can read](#when-a-window-holds-more-than-one-poll-can-read) for what happens when a window holds more.
- **A failed run changes nothing.** A transport failure or a `4xx`/`5xx` from Graph leaves the cursor and the window length as they were; the error is stored on the connection's **Last Error** with credentials redacted, and the next poll retries the same window. The one exception is a request that **timed out**: the cursor is kept but the next window is halved, since a window too heavy to answer in time would otherwise time out on every retry.

### When a window holds more than one poll can read

When the 20-page limit stops a run before the window is read, the run is reported as **Partial** with this warning:
...
The [List alerts_v2](https://learn.microsoft.com/en-us/graph/api/security-list-alerts_v2) method documents `$filter`, `$top`, `$skip` and `$count` but not `$orderby`, and returns the most recent alerts first. The 2,000 alerts a stopped run read are therefore the newest in its window, not the oldest, and moving the cursor past them would skip the older alerts it never reached. OneUptime shortens the window instead:

- The next poll reads a window **half as long from the same starting point**, and keeps halving down to one minute until a window reads completely. The run's warning says so: `This window holds more records than one poll can read; the next poll reads a {n} minute window from the same starting point.` Alerts already imported from the longer window are counted as duplicates, not stored again.
- After a window reads completely, the cursor moves to its end and the next window is **twice as long again**, back up to 24 hours.
- If even a one minute window holds more than 2,000 alerts, polling **moves past that minute** so newer alerts keep arriving. The run stays **Partial** and **Last Error** reads `More records were created in the one minute from {cursor} to {end} than one poll can read. Polling moved past this minute so newer records keep arriving; use Import this time range in Diagnostics on this minute to recover what one run can read.` Run **Import history** on that minute to recover the alerts it skipped.
- A run that is **Partial** for another reason, such as an alert that could not be normalized, is retried the same way: the window shortens from the same starting point.
...
- **Polls succeed but a known alert is missing** — check the alert's `createdDateTime` in the Defender portal against the run's window in **Diagnostics**; the window is on creation time, not activity time. An alert created before the connection existed is older than the initial 24 hour lookback: use **Import history** for it. A run reported as **Partial** was stopped by the 2,000-alert limit; the next poll reads a shorter window from the same start, so the alert is imported once a window containing it reads completely. If **Last Error** names a skipped minute, run **Import history** on that minute.
