...
1. In Google Cloud, create a **service account** on the project your SecOps instance is bound to, and grant it the **Chronicle API Viewer** role (`roles/chronicle.viewer`). That role includes the `legacySearchDetections`, `legacySearchCuratedDetections`, `countAllCuratedRuleSetDetections` and `legacyFetchAlertsView` permissions the connector reads with.
...
2. **Curated rule detections by created time** — `legacy:legacySearchCuratedDetections`, the same way, once per curated (Google-authored) rule. That endpoint takes one curated rule id (`ur_...`) and has no "every rule" wildcard, so the pass first calls `:countAllCuratedRuleSetDetections` for the week before the window through its end, and searches each curated rule that produced detections. A curated rule that has not fired in that week has nothing to import. A tenant without curated-rule access answers this pass with `400`, `403` or `404`; that is recorded as a warning on the run, not as a failure.
3. **The alerts view by detection time** — `legacy:legacyFetchAlertsView`, which covers telemetry alerts, SOAR alerts and machine-intelligence alerts as well as rule alerts. The endpoint has no pagination, so a response Google limits is split into smaller windows and read again.

The alerts view can only filter by detection time, and Google often makes an alert readable long after its detection time (ingestion delay, scheduled rules, re-runs over late data). A scheduled poll therefore also **sweeps the alerts view over the 24 hours before its window**, alerts only, recorded as the **Read late alerts by detection time** check. Records already imported are skipped, so the sweep's repeats show up under **Already imported**. The sweep is best effort: when Google limits it, its budget runs out or it fails, the run gets a warning, but the poll is not Partial or Failed and the cursor still moves, because the day it reaches back over was already read by earlier polls.
...
The connector gives each poll fixed budgets: the rule detections pass has **20** search pages, the curated pass has up to **200** requests (the count plus at least one page per curated rule that fired in the past week, busiest rules first), the alerts view and its late-alert sweep share **16** requests, and the whole read stops after **4 minutes**. The curated pass starts no further curated rule once 60% of that time is spent, so the alerts view still runs. When a poll cannot search every active curated rule, it records a warning naming how many were not searched rather than holding the cursor, because a shorter window would still have every one of those rules to search. A pass that a budget stopped part way is recorded as a warning check that names the budget (for example `Read rule detections by created time was stopped by the request budget after 20 requests.`), and a pass the budget never let start is a skipped check with the same reason in the run's warnings — never a success. A window with more detections than that cannot be read in one poll, and polling never re-reads the same window forever:
...
3. **Read rule detections**, **Read curated rule detections** and **Read the alerts view** — a one-record probe of each pass over the last 24 hours. The curated check reads the curated rule counts for the last 7 days and probes the curated rule with the most detections; with no curated rule fired in that week it passes with nothing to probe. A curated read answered with `400`, `403` or `404` is a warning, because the tenant may simply have no curated-rule access.
...
- **Last Error is populated** — a poll was attempted and something in it failed, a poll could not read its whole window, or the scheduler could not queue a poll. For a failed poll the field stores the complete error message with credentials redacted. When a connection has an error, select **View Error** in its **Actions** column to open the full message, then **Copy Error** in the dialog to copy it for support. The field clears on the next complete poll, which also removes the **View Error** action; run history keeps it. Read the prefix first. A message beginning `More records were created in the one minute from` is that skipped minute rather than a failure; see **Health is Partial import** above. Only four prefixes carry an HTTP status, so a message without one is not evidence of a fault on OneUptime's side:
  ...
  - `Google SecOps curated rule detection counts failed (HTTP ...)` — Chronicle rejected `countAllCuratedRuleSetDetections`, the call that names the curated rules the curated pass searches. A `400`, `403` or `404` is downgraded to a warning for the same reason as the curated search; anything else fails the poll.
  - `Google SecOps curated rule detection counts returned ...` — Chronicle answered `200` to the curated rule counts, but the body was empty, not JSON, or a shape the parser does not recognize. Reported rather than read as "no curated rule fired", so curated detections are not silently skipped.
