...
From 1280px wide the player is sized to the window: the stage takes every pixel the header, the timeline and the transport do not, so the recording is drawn as large as the screen allows rather than inside a fixed box. Below that width the page falls back to ordinary flow — the stage keeps the recording's aspect ratio, capped at 70% of the window height, and the events rail stacks underneath it.
...
The controls under the stage sit on one row: play/pause, the current time and duration, −10s / +10s, a speed menu (0.25× to 8×), **Skip idle**, previous / next error, next frustration, a **?** button that lists every keyboard shortcut, and a menu for the mouse trail, rail following and the timeline's signal lanes.
...
When the tab you are watching ends while another has later footage, a **Continue in Tab 2** chip appears. A page the user came back to through the browser's back/forward cache appears as a tab of its own.
