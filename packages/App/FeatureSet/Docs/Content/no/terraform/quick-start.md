...
2. Go to **Prosjektinnstillinger** > **API-nøkler**.
3. Click **Create API Key**.
4. Give it a name (for example `terraform`) and an expiry.
5. Grant permissions. Terraform needs **Opprett**, **Read**, **Update (Edit)**, and **Slett** on every resource type you plan to manage — for this guide: Label, Monitor, and Status Page.
...
- **Overvåkere** — the `Homepage` monitor is listed with the `critical` label.
- **Statussider** — `Internal Status` appears.
- **Prosjektinnstillinger > Etiketter** — the `critical` label exists with the color you set.
