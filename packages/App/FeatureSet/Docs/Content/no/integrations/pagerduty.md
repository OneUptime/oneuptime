...
Denne integrasjonen er **utgående**: OneUptime kaller PagerDutys [Events API v2](https://developer.pagerduty.com/docs/events-api-v2/overview/). Den bruker en OneUptime **[Arbeidsflyt](/docs/workflows/index)** med en **Hendelse → On Create**-trigger og en **API-komponent**.
...
1. Åpne **Arbeidsflyter → Opprett arbeidsflyt**, gi den navnet `Incidents → PagerDuty`, og åpne **Bygger**.
...
1. I den **samme** arbeidsflyten, legge til en ny **Hendelse**-trigger? Nei — en arbeidsflyt har én trigger. Opprett i stedet en **andre** arbeidsflyt kalt `Resolve PagerDuty` med en **Hendelse → On Update**-trigger.
2. Legg til en **Betingelser**-blokk for å sjekke at hendelsen nå er løst (forgren på hendelsens tilstand/`{{Incident.currentIncidentState.name}}` lik ditt løste tilstandsnavn).
3. Fra **Ja**, legg til en **API**-blokk til PagerDuty med den **samme `dedup_key`** og `event_action` satt til `resolve`:
...
PagerDutys `severity` godtar `critical`, `error`, `warning` eller `info`. For å mappe fra OneUptime-alvorlighetsgrader, legg til **Betingelser**-grener på `{{Incident.incidentSeverity.name}}` før API-blokken og send en annen body fra hver.
...
- **Ingenting i loggene** — bekreft at arbeidsflyten er **Aktivert** og at triggeren er **On Create**.
