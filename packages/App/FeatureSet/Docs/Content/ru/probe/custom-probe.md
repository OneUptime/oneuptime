...
- `PROBE_MONITOR_RETRY_LIMIT` — количество повторных попыток после первой при неудачной проверке монитора, используется, когда шаг не задаёт своё значение (по умолчанию: 3, то есть до 4 попыток)
...
- `PROBE_MONITOR_CHECK_TIMEOUT_IN_MS` — предельное время проверки одного монитора; по его истечении проверка прерывается и повторяется в следующем цикле (по умолчанию: 900000)
