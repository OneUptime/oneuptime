...
1. Откройте **Рабочие процессы → Создать рабочий процесс**, назовите его `Datadog → Incidents` и откройте **Конструктор**.
...
4. Из выхода **Да** добавьте блок **Создать инцидент**:
   - **Заголовок**: `{{Datadog.Request Body.title}}`
   - **Описание**: `{{Datadog.Request Body.body}}\nHost: {{Datadog.Request Body.host}}\n{{Datadog.Request Body.link}}`
   - **Серьёзность**: выберите один.
...
`$ALERT_TRANSITION` принимает значение `Recovered` при сбросе монитора. Добавьте вторую ветвь **Условия** (`transition == Recovered`), найдите соответствующий инцидент (совпадение по отправленному `id`) и переведите его в состояние разрешено с помощью **Update Incident**.

## Устранение неполадок

- **Запуск не появляется** — убедитесь, что сообщение монитора содержит `@webhook-oneuptime` и рабочий процесс **Включено**.
- **Поля пустые** — Datadog подставляет только те переменные шаблона, которые применимы к событию. Проверьте вывод триггера на вкладке **Журналы** и скорректируйте нагрузку webhook.
