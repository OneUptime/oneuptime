...
- **Токен приёма телеметрии OneUptime** — создайте его в разделе _Настройки проекта → Телеметрия и APM → Ключи приема_ и скопируйте значение `x-oneuptime-token`.
...
| `faas.instance`                                        | нет          | Отслеживается по экземплярам на вкладке **Экземпляры**      |
...
> Функция, которая также задаёт `service.name`, по-прежнему появляется и в разделе **Сервисы**. Представление **Serverless-функции** — это ракурс, ориентированный на FaaS и ограниченный по `faas.name`.
...
Как только функция отправит спан, лог или метрику, она появляется в разделе **Serverless-функции**. В обзоре отображаются:

- **Вызовы**, **error rate** и **длительность p95** — выводятся из ваших трассировок за выбираемый интервал времени, с графиками трендов.
- **Экземпляры** — текущее количество увиденных значений `faas.instance`.
- Полноценные вкладки **Журналы**, **Трассировки** и **Метрики**, ограниченные этой функцией.

Вы также можете автоматически применять метки и владельцев через _Serverless → Настройки → Правила меток / Правила владельца_.
