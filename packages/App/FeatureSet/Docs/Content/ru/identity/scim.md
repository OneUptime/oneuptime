...
> **Редакция:** SCIM входит в OneUptime Enterprise Edition. В OneUptime Cloud он доступен начиная с тарифа **Scale**. Для самостоятельно размещённых установок нужны образ Enterprise Edition и лицензия. См. [Enterprise Edition](/docs/self-hosted/enterprise). Без действующей лицензии (после 14-дневного пробного периода или через 30 дней после истечения лицензии) запросы SCIM отклоняются, пока не будет активирована лицензия.
...
3. **Членство в команде**: Пользователь автоматически добавляется в настроенные команды по умолчанию. В OneUptime Cloud человек, у которого уже был аккаунт OneUptime, вместо этого получает приглашение и присоединяется, когда его примет (см. «Часто задаваемые вопросы» ниже)
...
Когда SCIM пытается создать пользователя, который уже существует (совпадение по email), OneUptime не создаёт дублирующего пользователя. Что происходит дальше, зависит от того, где работает OneUptime:

- **Самостоятельное размещение**: Существующий пользователь сразу добавляется в настроенные команды по умолчанию (или, при использовании Push Groups, в команду группы).
...
Пользователи, которых создаёт сам SCIM, пользователи, уже присоединившиеся к вашему проекту, и пользователи, подтвердившие SSO вашего проекта, в обоих случаях добавляются сразу.

### Может ли SCIM изменить адрес электронной почты или имя пользователя?

Адрес электронной почты аккаунта OneUptime — это то, с чем человек входит во все проекты, в которых состоит, и куда приходят ссылки для сброса его пароля. Поэтому:
...
- **Самостоятельное размещение**: SCIM изменяет адрес электронной почты только у пользователя, который присоединился к этому проекту, не состоит ни в одном другом проекте и не является администратором OneUptime. Любое другое изменение отклоняется так же.

Для имён в обоих случаях действует то же правило: SCIM обновляет имя только у пользователя, который присоединился к этому проекту, не состоит ни в одном другом проекте и не является администратором OneUptime. У всех остальных имя остаётся как есть, а остальная часть запроса всё равно выполняется успешно.
