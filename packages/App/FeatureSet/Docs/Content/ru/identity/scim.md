...
> **Редакция:** SCIM входит в OneUptime Enterprise Edition. В OneUptime Cloud он доступен начиная с тарифа **Scale**. Для самостоятельно размещённых установок нужны образ Enterprise Edition и лицензия. См. [Enterprise Edition](/docs/self-hosted/enterprise).
...
2. **Настройте параметры SCIM**
   ...
   - Включите **Автоматическое предоставление пользователей**, чтобы автоматически добавлять пользователей при их назначении в вашем IdP
   - Включите **Автоматический отзыв доступа пользователей**, чтобы автоматически удалять пользователей при снятии назначения в вашем IdP
   - Выберите **Команды по умолчанию**, в которые должны добавляться новые пользователи
...
2. **Настройте параметры SCIM**
   ...
   - Включите **Автоматическое предоставление пользователей**, чтобы автоматически добавлять подписчиков при их назначении в вашем IdP
   - Включите **Автоматический отзыв доступа пользователей**, чтобы автоматически удалять подписчиков при снятии назначения в вашем IdP
   - Скопируйте **SCIM Base URL** и **Bearer Token** для настройки вашего IdP
...
5. Настройте следующие параметры:
   - **Автоматическое предоставление пользователей**: Включите для автоматического создания пользователей
   - **Автоматический отзыв доступа пользователей**: Включите для автоматического удаления пользователей
   - **Команды по умолчанию**: Выберите команды, в которые должны добавляться новые пользователи
   - **Включить push-группы**: Включите, если хотите управлять членством в команде через группы Entra ID
...
5. Настройте следующие параметры:
   ...
   - **Автоматический отзыв доступа пользователей**: Включите для автоматического удаления пользователей
   - **Команды по умолчанию**: Выберите команды, в которые должны добавляться новые пользователи
   - **Включить push-группы**: Включите, если хотите управлять членством в команде через группы Okta
6. Сохраните конфигурацию
...
- **Команды по умолчанию**: Все пользователи, созданные через SCIM, добавляются в одни и те же предопределённые команды
