...
- Un **Token di Ingestione Telemetria OneUptime** — creane uno da _Impostazioni del progetto → Telemetria e APM → Chiavi di acquisizione_.
...
- **CPU** e **Memoria** per task/istanza in esecuzione (da `container.cpu.utilization` / `container.memory.usage`), oltre a un elenco **Istanze principali per CPU**.
- **Istanze** — un conteggio in tempo reale dei task.
- **Richieste** e grafici di tendenza derivati dalle tue tracce.
- Schede complete **Registri**, **Tracce**, **Metriche** e **Istanze**.
