...
2. Go to **Configurações do projeto** > **Chaves de API**.
...
- **Monitores** — the `Homepage` monitor is listed with the `critical` label.
- **Páginas de status** — `Internal Status` appears.
- **Configurações do projeto > Rótulos** — the `critical` label exists with the color you set.
