...
- 每步驟執行逾時預設值：JavaScript、Bash 與 HTTP 均為 **30 秒**。請在該 runbook 的 **步驟** 頁面上依步驟設定——將欄位留空即維持預設值。
- Bash 與 JavaScript 步驟的每步驟**認領逾時**：**2 分鐘**——即 Worker 在判定失敗之前，等待所選 agent 認領該工作的時間。同樣可依步驟設定。
- 兩種逾時都接受 **1 秒到 1 小時**。超出該範圍的值會在步驟執行時被限制在範圍內，因此輸入錯誤的設定既無法停用逾時，也無法無限期佔用一個 Worker 並行名額。
