...
2. Go to **Projektindstillinger** > **API-nøgler**.
...
- **Overvågninger** — the `Homepage` monitor is listed with the `critical` label.
- **Statussider** — `Internal Status` appears.
- **Projektindstillinger > Etiketter** — the `critical` label exists with the color you set.
