...
> Eine Funktion, die zusätzlich `service.name` setzt, erscheint weiterhin auch unter **Dienste**. Die Ansicht **Serverlose Funktionen** ist die FaaS-fokussierte Perspektive, eingegrenzt durch `faas.name`.
...
Sobald die Funktion einen Span, ein Log oder eine Metrik ausgibt, erscheint sie unter **Serverlose Funktionen**. Der Überblick zeigt:
...
- Vollständige Tabs **Protokolle**, **Traces** und **Metriken**, eingegrenzt auf diese Funktion.

Sie können außerdem Labels und Verantwortliche automatisch zuweisen über _Serverlos → Einstellungen → Beschriftungsregeln / Eigentümerregeln_.
