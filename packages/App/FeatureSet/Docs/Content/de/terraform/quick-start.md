...
2. Go to **Projekteinstellungen** > **API-Schlüssel**.
...
- **Monitore** — the `Homepage` monitor is listed with the `critical` label.
- **Statusseiten** — `Internal Status` appears.
- **Projekteinstellungen > Beschriftungen** — the `critical` label exists with the color you set.
