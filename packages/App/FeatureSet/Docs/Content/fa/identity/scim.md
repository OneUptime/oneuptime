...
> **نسخه:** SCIM بخشی از نسخه سازمانی OneUptime (Enterprise Edition) است. در OneUptime Cloud از طرح **Scale** به بالا در دسترس است. نصب‌های خودمیزبان به ایمیج نسخه سازمانی و یک مجوز نیاز دارند. [Enterprise Edition](/docs/self-hosted/enterprise) را ببینید. بدون مجوز معتبر (پس از دوره آزمایشی ۱۴ روزه، یا ۳۰ روز پس از انقضای مجوز)، درخواست‌های SCIM تا زمان فعال شدن یک مجوز رد می‌شوند.
...
3. **عضویت تیمی**: کاربر خودکار به تیم‌های پیش‌فرض پیکربندی‌شده افزوده می‌شود. در OneUptime Cloud، کسی که از پیش حساب OneUptime داشته، به‌جای آن دعوت می‌شود و پس از پذیرفتن دعوت عضو می‌شود (پرسش‌های پرتکرار پایین را ببینید)
...
وقتی SCIM می‌کوشد کاربری را بسازد که از پیش وجود دارد (تطبیق بر پایه ایمیل)، OneUptime کاربر تکراری نمی‌سازد. آنچه پس از آن رخ می‌دهد به این بستگی دارد که OneUptime کجا اجرا می‌شود:

- **خودمیزبان**: کاربر موجود بی‌درنگ به تیم‌های پیش‌فرض پیکربندی‌شده (یا با Push Groups، به تیم آن گروه) افزوده می‌شود.
...
کاربرانی که خود SCIM می‌سازد، کاربرانی که پیش‌تر به پروژه شما پیوسته‌اند و کاربرانی که SSO پروژه شما را تأیید کرده‌اند، در هر دو حالت بی‌درنگ افزوده می‌شوند.

### آیا SCIM می‌تواند نشانی ایمیل یا نام کاربر را تغییر دهد؟

نشانی ایمیل حساب OneUptime همان چیزی است که شخص با آن به همه پروژه‌هایی که عضوشان است وارد می‌شود، و پیوندهای بازنشانی گذرواژه‌اش به همان نشانی فرستاده می‌شود. بنابراین:
...
- **خودمیزبان**: SCIM نشانی ایمیل را تنها برای کاربری تغییر می‌دهد که به این پروژه پیوسته، عضو هیچ پروژه دیگری نیست و مدیر OneUptime نیست. هر تغییر دیگری به همان شکل رد می‌شود.

نام‌ها در هر دو حالت از همین قاعده پیروی می‌کنند: SCIM نام را تنها برای کاربری به‌روز می‌کند که به این پروژه پیوسته، عضو هیچ پروژه دیگری نیست و مدیر OneUptime نیست. برای دیگران نام همان‌گونه که هست می‌ماند و باقی درخواست همچنان موفق می‌شود.
