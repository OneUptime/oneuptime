...
‏Azure SQL Database و Azure SQL Managed Instance به‌عنوان **Microsoft SQL Server** وصل می‌شوند. دسترسی‌های متفاوتی لازم دارند — [ساخت کاربر مانیتورینگ](#ساخت-کاربر-مانیتورینگ) را ببینید.
...
اگر از هر پایگاه داده دیگری اجرا شود، `GRANT VIEW SERVER STATE` با Msg 4621 شکست می‌خورد، «Permissions at the server scope can only be granted when the current database is master».

> **دسترسی خواندن به جدول‌هایتان کافی نیست.** ورودی‌ای که فقط می‌تواند داده بخواند — `db_datareader`، یا هر نقش «دسترسی خواندن» دیگری — می‌تواند وصل شود و اندازه پایگاه داده را می‌گیرد، و نه چیز دیگری. SQL Server نماهایی را که مانیتور می‌خواند با `The user does not have permission to perform this action.` رد می‌کند (Msg 297). پیام پیش از آن، چیزی را که رد شده نام می‌برد: Msg 300 `VIEW SERVER STATE` (روی ۲۰۲۲ ‏`VIEW SERVER PERFORMANCE STATE`) برای نماهای کارساز، از جمله فضای گزارش تراکنش و فضای آزاد tempdb، یا Msg 262 `VIEW DATABASE STATE` (روی ۲۰۲۲ ‏`VIEW DATABASE PERFORMANCE STATE`) برای نمای همانندسازی. مانیتور آنلاین می‌ماند، گروه‌های Connections، Activity، Throughput، Locks، Storage و Replication را به‌عنوان فاقد دسترسی گزارش می‌دهد، و `GRANT` بالا را کنارشان نشان می‌دهد. `VIEW SERVER STATE` همه آن‌ها را پوشش می‌دهد.
>
> دو نما رد نمی‌کنند: بدون این دسترسی، `sys.dm_exec_sessions` و `sys.dm_exec_requests` بی‌صدا فقط نشست خودِ مانیتور را نشان می‌دهند. مانیتور هرگز آن‌ها را به‌تنهایی نمی‌خواند — همیشه در کنار نمایی که رد می‌کند — پس دسترسی گمشده هرگز نمی‌تواند به‌صورت «۱ اتصال» ثبت شود.

### Azure SQL Database

‏Azure SQL Database هیچ دسترسی سطح کارسازی ندارد — ‏`GRANT VIEW SERVER STATE` آنجا شکست می‌خورد — پس همان نماها به‌جایش با دسترسی‌ای در سطح پایگاه داده باز می‌شوند. ورودی‌ای در `master` بسازید، در پایگاه داده‌ای که مانیتور می‌کنید کاربری به آن بدهید، و دسترسی را همان‌جا اعطا کنید، نه در `master`:
...
این روی پایگاه‌های داده vCore و روی پایگاه‌های داده DTU در S2 و بالاتر کافی است. روی **Basic، S0 و S1**، و برای هر پایگاه داده‌ای در یک **استخر کشسان** (elastic pool)، ‏Azure فقط به مدیر کارساز، مدیر Microsoft Entra، یا اعضای نقش کارساز `##MS_ServerStateReader##` اجازه خواندن این نماها را می‌دهد، صرف‌نظر از اینکه دسترسی‌های پایگاه داده چه می‌گویند. آنجا، مدیر کارساز ورودی را به آن نقش هم می‌افزاید:
...
‏`##MS_ServerStateReader##` روی هر رده‌ای کار می‌کند، پس اگر `VIEW DATABASE STATE` کافی از آب درنیاید راه جایگزین هم همین است. عضویت تازه در نقش ممکن است چند دقیقه طول بکشد تا اعمال شود، و فقط به اتصال‌های تازه می‌رسد؛ پروب در هر بررسی اتصال تازه‌ای باز می‌کند.

کاربر پایگاه داده محصور (`CREATE USER oneuptime_health WITH PASSWORD = '...'` در پایگاه داده مانیتورشده، بدون ورودی) روی S2 و بالاتر با `VIEW DATABASE STATE` کار می‌کند، اما نمی‌تواند به `##MS_ServerStateReader##` بپیوندد: نقش‌های کارساز فقط ورودی می‌پذیرند. برای بردن کاربر محصور به آن نقش، حذفش کنید (`DROP USER oneuptime_health;`) و دستورهای بالا را دنبال کنید.

پروب Azure SQL Database را از روی `SERVERPROPERTY('EngineEdition')` بازمی‌شناسد نه از روی نسخه‌اش — ‏Azure SQL Database هرچه واقعاً اجرا کند `12.0.2000.8` گزارش می‌دهد، که مانند SQL Server ‏۲۰۱۴ خوانده می‌شود. پس **Engine** مانیتور `Azure SQL Database 12.0.2000.8` را نشان می‌دهد، و دسترسی گمشده به‌صورت دستور Azure بالا نشان داده می‌شود، هرگز به‌صورت `VIEW SERVER STATE` نه.

- **همانندسازی روی Azure SQL Database جمع‌آوری نمی‌شود.** ‏Azure SQL Database نمای `sys.dm_hadr_database_replica_states` را ندارد، پس گروه Replication آنجا رد می‌شود، نه اینکه در هر بررسی شکست‌خورده گزارش شود. نماهای نسخه‌های خودِ Azure (`sys.dm_database_replica_states`، `sys.dm_geo_replication_link_status`) هنوز خوانده نمی‌شوند.
- **اتصال‌ها به ازای هر پایگاه داده‌اند.** با `VIEW DATABASE STATE`، ‏Azure SQL Database فقط نشست‌های پایگاه داده مانیتورشده را نشان می‌دهد، پس گروه Connections همان پایگاه داده را می‌شمارد نه کارساز منطقی را. هر پایگاه داده‌ای را که برایتان مهم است مانیتور کنید.
- **TempDB Free Space در استخر کشسان مال خودِ استخر است.** پایگاه‌های داده یک استخر یک tempdb مشترک دارند.
...
- **Database Name** — پایگاه داده‌ای که به آن وصل می‌شوید. سنجه‌های محدود به پایگاه داده (اندازه، نرخ برخورد حافظه نهان، ریزش موقت) برای همین پایگاه داده گزارش می‌شوند؛ سنجه‌های محدود به کارساز (اتصال‌ها، آپ‌تایم، همانندسازی) برای کل کارساز — جز روی Azure SQL Database، که در آن اتصال‌ها فقط برای پایگاه داده مانیتورشده شمرده می‌شوند.
...
گروه واحدی است که روشن یا خاموشش می‌کنید، و واحدی که دسترسی گمشده برای آن گزارش می‌شود. گروه‌ها هستند تا یک دسترسی گمشده یک گروه برایتان هزینه داشته باشد نه کل مانیتور. دستورهای یک گروه یکی‌یکی اجرا می‌شوند، پس گروهی ممکن است تا حدی جمع‌آوری شود: آنگاه هم در `collectedGroups` و هم در `unavailableGroups` پدیدار می‌شود. گروه Storage در SQL Server برای ورودی‌ای بدون `VIEW SERVER STATE` حالت رایج است — اندازه پایگاه داده جمع‌آوری می‌شود، فضای گزارش و فضای آزاد tempdb نه.
...
| Storage | اندازه پایگاه داده، ریزش موقت، فضای گزارش، فضای آزاد tempdb | PostgreSQL: چیزی فراتر از `CONNECT` نه. MySQL: `SELECT` روی پایگاه داده. SQL Server: برای اندازه پایگاه داده چیزی لازم نیست؛ `VIEW SERVER STATE` برای فضای گزارش و فضای آزاد tempdb |
| Replication | نسخه‌های متصل، تأخیر همانندسازی بر حسب ثانیه و بایت، جایگاه‌های غیرفعال، وضعیت بازیابی | PostgreSQL: `pg_monitor`. MySQL: `REPLICATION CLIENT`. SQL Server: `VIEW SERVER STATE`؛ روی Azure SQL Database جمع‌آوری نمی‌شود |
...
روی Azure SQL Database، هرجا این جدول می‌گوید `VIEW SERVER STATE`، ‏`VIEW DATABASE STATE` بخوانید — یا روی Basic، S0، S1 و استخرهای کشسان، ‏`##MS_ServerStateReader##`. [Azure SQL Database](#azure-sql-database) را ببینید.
...
- **گروهی نمی‌تواند اجرا شود** — دسترسی گمشده، `performance_schema` غیرفعال، اتمام مهلت دستور. مانیتور **آنلاین می‌ماند**. سنجه‌هایی که آن گروه نتوانست بخواند **غایب**اند، نه صفر. هیچ خط نموداری کشیده نمی‌شود، هیچ آستانه‌ای روی آن سری‌ها نمی‌تواند مطابقت کند، و هیچ حادثه‌ای نمی‌تواند از آن‌ها برانگیخته شود. بررسی یک مسئله جمع‌آوری ثبت می‌کند که گروه، دلیل، و — هرجا هست — دستور دقیق `GRANT` را نام می‌برد، که روی خلاصه مانیتور نشان داده می‌شود و در **Metric Groups Failed** شمرده می‌شود.
...
این پیام SQL Server (Msg 297) برای ورودی‌ای است که می‌تواند وصل شود اما نمی‌تواند نماهای وضعیت کارساز را بخواند. همیشه یعنی دسترسی گمشده، هرگز خرابی‌ای در پایگاه داده نه. SQL Server آن را به‌عنوان پیام دوم می‌فرستد، پس از پیامی که دسترسیِ ردشده را نام می‌برد، و مانیتور هر دو را نشان می‌دهد: برای نمونه `VIEW SERVER STATE permission was denied on object 'server', database 'master'. The user does not have permission to perform this action.` کنارش دستوری است که آن را برای سکویی که پروب به آن وصل شده رفع می‌کند:

- **SQL Server یا Azure SQL Managed Instance** — ‏`GRANT VIEW SERVER STATE TO oneuptime_health;` را در `master` اجرا کنید (مانیتور آن را به‌صورت `GRANT VIEW SERVER STATE TO [<monitoring_login>]; -- run in master` نشان می‌دهد).
- **Azure SQL Database** — ‏`GRANT VIEW DATABASE STATE TO oneuptime_health;` را در پایگاه داده مانیتورشده اجرا کنید؛ روی Basic، S0، S1 و استخرهای کشسان، به‌جایش عضویت در `##MS_ServerStateReader##`. [Azure SQL Database](#azure-sql-database) را ببینید.

در این میان اندازه پایگاه داده همچنان جمع‌آوری می‌شود، چون تنها سنجه Storage است که هر ورودی می‌تواند بخواند.
...
| `engineVersion` | رشته | رشته نسخه‌ای که کارساز گزارش کرد (روی SQL Server، ‏`ProductVersion` خام؛ خلاصه مانیتور سکو را کنارش نام می‌برد) |
...
| `unavailableGroups` | آرایه | گروه‌هایی با دستوری که نتوانست جمع‌آوری شود، هرکدام با دلیلی و اصلاحی. گروهی که تا حدی جمع‌آوری شده در هر دو فهرست هست |
...
- **یک مانیتور به ازای هر نمونه، نه به ازای هر پایگاه داده**، مگر مشخصاً اندازه و سنجه‌های حافظه نهان به ازای هر پایگاه داده را بخواهید — وگرنه پرس‌وجوهای محدود به کارساز را بدون اطلاعات تازه‌ای ضرب می‌کنید. Azure SQL Database استثناست: اتصال‌ها را به ازای هر پایگاه داده گزارش می‌دهد، پس آنجا هر پایگاه داده را مانیتور کنید.
