...
2. **آیا API شما روی مبدأی دیگر است؟** درخواست‌هایش چیزی نمی‌گیرند مگر اینکه مبدأ در **Trace propagation origins** باشد، و آنگاه فقط یک `traceparent` که بر پایه شناسه ردیابی جفت می‌شود. اگر سطر آن درخواست در زبانه Network هم پیوند ردیابی ندارد، مبدأ فهرست نشده و صفحه شما خودش `traceparent` نمی‌گذارد.
3. **آیا بک‌اند شما زمینه ردیابی W3C را ادامه می‌دهد؟** انتشاردهنده‌های OpenTelemetry آن باید `tracecontext` را در بر داشته باشند (SDK‏ Go تا `otel.SetTextMapPropagator(propagation.TraceContext{})` هیچ‌چیز منتشر نمی‌کند)، و هیچ پراکسی یا CDNای در جلویش نباید `traceparent` و `tracestate` را بزداید. ردیابی بک‌اندی که شناسه‌اش با شناسه ردیابی سطر Network یکی است اما اسپن‌هایش در زبانه نیستند، به همین‌جا اشاره دارد.
4. **آیا درخواست از پیش `tracestate` داشت؟** ‏Datadog RUM، ‏Elastic RUM و برخی پیکربندی‌های OpenTelemetry مال خودشان را می‌فرستند. ضبط‌کننده در هدر فروشنده‌ای دیگر نمی‌نویسد، پس آن اسپن‌ها مهر نمی‌خورند و فقط بر پایه شناسه ردیابی پیوند می‌خورند.
5. **‏Traces پر است اما Logs خالی؟** گزارش‌ها بر پایه شناسه ردیابی پیوند می‌خورند، پس بک‌اند شما باید آن‌ها را درون اسپن درخواست بنویسد، با گزارش‌گیری OpenTelemetry که شناسه ردیابی فعال را ثبت کند.
6. **نمونه‌برداری کنارش گذاشته؟** ردیابی‌ای که بک‌اند شما بیندازد هرگز به OneUptime نمی‌رسد؛ [نمونه‌برداری از ردیابی‌هایی که از مرورگر آغاز می‌شوند](/docs/telemetry/session-replay#sampling-of-browser-started-traces) را ببینید.
