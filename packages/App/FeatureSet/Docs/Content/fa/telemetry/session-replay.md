...
- **کدام درخواست‌ها.** فراخوانی‌های `fetch` و `XMLHttpRequest` به **مبدأ خود** صفحه، که وقتی نشست **در حال فرستادن** است زده می‌شوند، به‌جز صادرات OpenTelemetry خود صفحه (درخواست‌های `POST` به مسیرهایی که به `/v1/traces`، `/v1/logs` یا `/v1/metrics` ختم می‌شوند). پیش از رضایت زیر _Require explicit_، پس از `revokeConsent()`، زیر _On error or frustration_ تا وقتی تریگری شلیک نکرده، در نشستی که نمونه‌برداری نشده، یا پس از `stop()` چیزی افزوده نمی‌شود. `traceparent` یا `tracestate`ای که درخواست از پیش حمل می‌کند هرگز جایگزین نمی‌شود: درخواست مال خودش را نگه می‌دارد و ضبط‌کننده فقط آنچه را کم است می‌افزاید. `;p:` فقط وقتی می‌آید که خود ضبط‌کننده `traceparent` را ساخته باشد. درخواست‌ها به مبدأهای دیگر چیزی نمی‌گیرند مگر اینکه مبدأ را در **Trace propagation origins** فهرست کنید، و آنگاه فقط یک `traceparent` که هیچ شناسه نشستی حمل نمی‌کند.
...
ضبط‌کننده `traceparent` خود را با پرچم نمونه‌برداری‌شده (`-01`) می‌سازد، چون بک‌اندی که از تصمیم والدش پیروی می‌کند وگرنه همه اسپن‌های درخواست را می‌اندازد. روی دیگرش این است که نمونه‌بردارهای پیش‌فرض `ParentBased` **همه** ردیابی‌هایی را که ضبط‌کننده آغاز می‌کند نگه می‌دارند، حتی اگر بک‌اند شما ردیابی‌های ریشه خودش را مثلاً با نرخ ۱۰٪ نمونه بگیرد: درخواست‌های هم‌مبدأ هر نشستی که فرستاده می‌شود، و درخواست‌ها به مبدأهای فهرست‌شده در **Trace propagation origins** از هر نشستی که ضبط‌کننده در آن اجرا می‌شود، ضبط‌شده یا نه. با اجرای ضبط‌کننده روی صفحه‌هایتان حجم ردیابی و هزینه دریافت می‌تواند بالا برود. `OTEL_TRACES_SAMPLER=parentbased_traceidratio` هم همین‌گونه رفتار می‌کند: نرخش فقط بر اسپن‌های ریشه اعمال می‌شود.
...
- **صادرات OpenTelemetry خود صفحه‌تان.** یک `POST` هم‌مبدأ که مسیرش به `/v1/traces`، `/v1/logs` یا `/v1/metrics` ختم شود (صادرکننده مرورگر که از راه پراکسی‌ای روی مبدأ خودتان می‌فرستد) از مسیر هم‌مبدأ ضبط‌کننده چیزی نمی‌گیرد، پس صادر کردن تله‌متری مرورگری‌تان به ازای هر دسته یک ردیابی بک‌اند به ضبط نمی‌افزاید. درخواست‌های دیگر به چنین مسیرهایی، مثلاً `GET /api/v1/logs` روی API خودتان، طبق معمول پیوند می‌خورند. اگر مبدأ خودتان در **Trace propagation origins** هم فهرست شده باشد (نسخه‌های پیشین این صفحه می‌گفتند فهرستش کنید)، آن صادرات همچنان `traceparent` آن فهرست را می‌گیرند، پس مبدأ خودتان را از آن فهرست بردارید.
