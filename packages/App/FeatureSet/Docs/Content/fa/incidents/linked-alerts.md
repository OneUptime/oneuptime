...
**هشداری که از پیش حادثه‌ای دارد مشخص می‌شود.** با بودن **Declare Incident** روی صفحه هر هشدار، ممکن است دو پاسخ‌دهنده که برای یک قطعی فراخوانده شده‌اند هر کدام جداگانه آن را اعلام کنند. پس کادری که هشدارها را فهرست می‌کند کنار هر هشداری که از پیش به حادثه‌ای پیوند شده می‌نویسد «(already linked to Incident INC-42)»، با پیوندی به آن حادثه، و پیام «This alert is already linked to an incident.» (یا «Some of these alerts are already linked to an incident.») را می‌افزاید تا یادتان بیندازد بررسی کنید که با همان مشکل سروکار ندارید. این فقط یک یادآوری است، نه مانع: اگر همان مشکل است، به‌جای اعلام حادثه‌ای دیگر، هشدار را به حادثه موجود پیوند دهید. فقط حادثه‌هایی نام برده می‌شوند که اجازه دیدنشان را دارید.
...
هشدارها در پس‌زمینه، درست پس از پیوند شدن، تصدیق می‌شوند، پس صفحه حادثه ممکن است لحظه‌ای پیش از آن باز شود. هشداری که نتوان تصدیقش کرد — مثلاً چون در این فاصله حذف شده — ثبت می‌شود و هرگز جلوی بقیه یا حادثه را نمی‌گیرد، و هشداری که کس دیگری در این فاصله تصدیق یا برطرفش کند همان‌طور که او گذاشته می‌ماند. اگر حادثه با روشن بودن کلیدهای هشدارهای پیوندشده پروژه مستقیم در وضعیتی تصدیق‌شده یا برطرف‌شده اعلام شود، کلیدها هشدارها را هنگام پیوند جابه‌جا می‌کنند و جعبه انتخاب آن هشدارها را به آن‌ها می‌سپارد.
