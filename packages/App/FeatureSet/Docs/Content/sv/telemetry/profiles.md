...
exporters:
  otlphttp:
    ...
    # Utan den här raden skickar exportören till /otlp/v1development/profiles, som OneUptime inte tar emot
...
service:
  pipelines:
    # Kräver att collectorn startas med --feature-gates=service.profilesSupport
    profiles:
      receivers: [otlp]
      exporters: [otlphttp]
```
