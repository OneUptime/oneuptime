...
`namespaceFilters.rules` tillämpar namespace-mönster oberoende på fyra scopes:

- `podLogs`: filtrerar poddarnas stdout/stderr vid hostPath-filelog-mottagaren eller API-loggtailern; Kubernetes-händelser och granskningsloggar påverkas inte.
- `ebpfDiscovery`: filtrerar OBI-processupptäckt och därmed både eBPF-spår och eBPF-mätvärden.
- `metrics`: filtrerar namespace-specifika mätserier efter metadataberikning; nod- och klusterserier utan namespace behålls.
- `traces`: filtrerar både eBPF-spans och OTLP-spans som skickas av program efter metadataberikning.

Namespace-mönster matchar hela namnet och stöder * som jokertecken, till exempel team-*. Om ett scope har en include-regel behålls endast matchande namespaces i det scopet. exclude-regler vinner alltid. Standardregeln utesluter kube-system endast från podLogs och ebpfDiscovery.

Så här begränsar du poddloggar och eBPF-upptäckt till vissa namespaces:
...
Om du vill stoppa loggar från ett stökigt namespace men behålla dess eBPF-spår, tjänstekarta och mätvärden begränsar du uteslutningen till podLogs:
...
Regler för podLogs och ebpfDiscovery filtrerar vid källan: uteslutna loggfiler öppnas aldrig och uteslutna workloads instrumenteras aldrig. Regler för metrics och traces körs senare i collectorn, efter att namespace-metadata har lagts till.

#### Filtrera mätvärden och spår efter namespace

Lägg till dessa scopes direkt i regeln när du även vill filtrera namespace-specifika mätvärden eller spans:

```bash
  --set-json 'namespaceFilters.rules=[{"action":"exclude","namespaces":["kube-system","noisy-*"],"scopes":["podLogs","ebpfDiscovery","metrics","traces"]}]'
```

> **Mätvärden på nod- och klusternivå behålls alltid. Ett namespace tillhör en podd, inte en nod, så serier utan namespace matchar inte regeln och tas inte bort.**

Kubernetes-händelser kan inte filtreras efter namespace i agenten. De kommer från k8sobjects-mottagaren utan attributet k8s.namespace.name; namespace finns i händelsens body. Filtrera dem i stället på serversidan.
...
### Sampling av spårningar

Filtren ovan tar bort en **kategori** av telemetri — en namnrymd, en allvarlighetsgrad, ett måttnamn. Sampling är annorlunda: den behåller varje kategori och tunnar ut populationen i stället. Sätt `sampling.traces.percentage` till den andel av spårningarna du vill behålla:
...
Det behåller en spårning av tio och släpper de andra nio i agenten, innan de lämnar ditt kluster.

**Du får hela spårningar, inte fragment.** Beslutet är en hash av spårnings-ID:t snarare än en slantsingling per span, så varje span i en spårning behålls eller släpps tillsammans — de spårningar som överlever är fullständiga och läsbara från början till slut. Det är den egenskapen som gör sampling säker att slå på.

**Dina måttbaserade monitorer rör sig inte.** eBPF:s RED-mått — begärandefrekvens, felfrekvens, varaktighet — är en *mått*familj. OBI beräknar dem från varje begäran och de färdas genom måttpipelinen, som samplaren inte sitter i. Vid `percentage: 10` får du en tiondel av spårningarna och 100 % korrekt frekvens/fel/latens. Instrumentpaneler och monitorer som byggts på de måtten påverkas inte.

**Dina span-baserade monitorer gör det.** Allt som OneUptime härleder från själva spans skalas ner med frekvensen — se varningen nedan innan du slår på detta.

| Nyckel                        | Betydelse                                                                     |
| ----------------------------- | ----------------------------------------------------------------------------- |
| `sampling.traces.percentage`  | Procentandel spårningar att **behålla**, 0-100. Standard `100` (behåll allt). |
| `sampling.traces.hashSeed`    | Frö för spårnings-ID-hashen. Standard `22`.                                   |

Noteringar som kommer att bespara dig en incident:

- **`0` behåller inga spårningar alls.** Det är en frekvens, inte en av-knapp — det raderar varje spårning medan eBPF-DaemonSet:en fortsätter köra och kosta dig pengar. Om du inte vill ha några spårningar, använd `ebpf.enabled=false`. Om du inte vill ha spårningar men *vill* ha RED-mått och tjänstekartan, låt eBPF vara på och sätt detta till `0` med avsikt.
- **Gäller endast när `ebpf.enabled`.** Spårningspipelinen existerar inte annars, så vid `ebpf.enabled=false` gör det här värdet ingenting.
- **Endast spårningar.** Det finns inget `sampling.logs` eller `sampling.metrics`, och det är avsiktligt — se noteringen nedan.
- **Bråkdelar kräver `--set-json`, och de har en nedre gräns.** `--set sampling.traces.percentage=0.5` misslyckas, eftersom Helm läser `0.5` som en sträng. Använd `--set-json 'sampling.traces.percentage=0.5'` eller en values-fil. Heltal fungerar utmärkt med `--set`. Under ungefär `0.0061` kvantiseras frekvensen till noll och beter sig exakt som `0` — varje spårning släpps, utan något fel. `0.01` (en av tiotusen) är det minsta värdet som gör det som det säger.
...
| **eBPF-spårningar och span-mått** | En spårning per begäran från varje instrumenterad process  | `sampling.traces.percentage`, `ebpf.enabled`, `ebpf.features.*`, `ebpf.autoTargetExe`, `ebpf.excludeExePaths` |
...
| **Måttkardinalitet**              | Antal distinkta serier (per container, per PVC, …)         | `filters.metrics.exclude`, `namespaceFilters.rules` (`metrics`), `cadvisor.metricsAllowlist`, `kubeletstats.volumeMetrics` |
...
Det finns tre sätt att skära ner volymen, och det är värt att veta vilket du använder:
...
- **Vill du bara ha loggar från vissa namespaces? Använd en include-regel med scopet podLogs. Matchningen sker vid loggkällan, så filtrerade namespaces läses aldrig och eBPF-telemetrin förblir oberoende.**
  ...
  Om du vill behålla alla namespaces utom en stökig grupp använder du en exclude-regel med namespaces: [noisy-*] och scopes: [podLogs].
...
- **Vill du ta bort mätvärden för ett helt namespace? Lägg till en exclude-regel med scopet metrics. Serier per podd och container filtreras, medan nod- och klusterserier utan namespace behålls.**
...
Varje spak ovan köper volym genom att ge upp något: en namnrymd du slutar bevaka, en allvarlighetsgrad du slutar behålla, en måttfamilj du slutar samla in. Sampling är undantaget, och på ett upptaget kluster är det ofta den största minskningen som finns att få för den minsta förlusten:
...
Det är en minskning av spårningsvolymen med 90 % för en smalare förlust än någon annan spak här:

- De spårningar du behåller är **hela** — beslutet hashar spårnings-ID:t, så alla spans i en spårning delar det. Du får färre spårningar, inte trasiga.
- Dina **RED-mått förblir exakta**. Begärandefrekvens, felfrekvens och varaktighet beräknas av OBI från varje begäran och färdas genom måttpipelinen, som samplaren inte sitter i. Varje instrumentpanel och monitor som byggts på dem visar detsamma som tidigare.

Det du ger upp är mest exempelspårningar: när en monitor larmar har du en tiondel så många spårningar att öppna. På ett kluster som gör tusentals identiska begäranden i sekunden är det oftast ett bra byte — den hundrade identiska `/healthz`-spanen lär dig ingenting som den första inte gjorde. På ett tyst kluster är det ett dåligt byte, eftersom du kanske inte har något exempel på den sällsynta begäran som gick sönder.

Undantaget, och det enda du bör kontrollera innan du rullar ut detta: monitorer som **räknar spans** i stället för mått — Traces på `Span Count`, Exceptions på `Exception Count` — ser proportionellt färre, så deras trösklar behöver omjusteras med samma faktor. Se [Sampling av spårningar](#sampling-av-spårningar).

Ta till detta när eBPF-spårningar utgör en stor del av din ingest men du fortfarande vill ha tjänstekartan och RED-måtten intakta. Föredra Spak 2 när du vill sluta instrumentera något helt.

Se [Sampling av spårningar](#sampling-av-spårningar) för det fullständiga beteendet, inklusive varför `0` är en frekvens snarare än en av-knapp och varför det inte finns någon logg- eller måttmotsvarighet.
...
Begränsa mer vid behov: höj minSeverity till ERROR, lägg till metrics i en namespace-regels scopes eller ange ebpf.enabled=false om du redan skickar spår från OTel-SDK:er.

> **Var uppmärksam på vad du skär bort.** Vissa monitorer är beroende av specifika signaler: att inaktivera `cadvisor` tar bort OOM-kill- och CPU-throttling-monitorerna; att inaktivera `kubeletstats.volumeMetrics` tar bort monitorn för lågt PVC-diskutrymme; att inaktivera loggar tar bort loggbaserade varningar; och `sampling.traces.percentage` tar inte bort någon monitor men skalar ner de span-baserade (Traces på `Span Count`, Exceptions på `Exception Count`), så justera om deras trösklar så att de matchar. Trimma de signaler du inte agerar på, inte de som en monitor bevakar.
...
Den vanligaste anledningen — särskilt efter en ominstallation — är en **felaktig eller återkallad ingestnyckel**. OneUptime avvisar en sådan nyckel: OTLP-ingestslutpunkterna svarar HTTP `401` för en saknad, okänd eller utgången nyckel och `422` för en inaktiverad nyckel eller en webbläsarnyckel. Ingen av statuskoderna görs om, så insamlaren kastar varje batch och loggar ett `Exporting failed. Dropping data.`-fel per batch. Poddarna förblir Running och Ready, så den raden är lätt att missa.

1. Kontrollera att agentens poddar körs: `kubectl get pods -n oneuptime-agent`
2. Kontrollera metrics-collector-loggarna: `kubectl logs -n oneuptime-agent -l component=metrics-collector -c otel-collector` (en `Exporting failed`-rad med `HTTP Status Code 401` eller `422` betyder att nyckeln avvisades — se ovan)
...
4. I mycket stora kluster kan en enda replika bli en flaskhals — dela upp separata releaser med podLogs-scopade include-regler i namespaceFilters.rules.
