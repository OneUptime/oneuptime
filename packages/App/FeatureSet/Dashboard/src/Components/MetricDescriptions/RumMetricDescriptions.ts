...
export const RUM_METRIC_DESCRIPTIONS: Record<RumMetric, string> = {
  ...
  pageLoadTime:
    "How long full page loads took, from the start of navigation to the browser's load event. p95: 95% of page loads in the range were faster than this and the slowest 5% took longer. The median below is the typical load - half were faster.",
  events:
  ...
  errorRate:
    "The share of events in the selected range that were marked as errors, such as a failed page load or network request. The bar turns amber at 1% and red at 5%.",
  eventDuration:
    "p95 means the 95th percentile: 95% of events (page loads, requests and interactions) finished faster than this, and the slowest 5% took longer. Worked out for each interval on the chart, then averaged over the selected range.",
  exceptions:
