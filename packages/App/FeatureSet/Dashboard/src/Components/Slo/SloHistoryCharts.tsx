...
import { SloWidgetMetric } from "Common/Types/Dashboard/DashboardComponents/DashboardSloComponent";
...
/*
 * The SloHistory series names, from the one mapping the dashboard SLO widget
 * also reads. They are persisted strings the worker writes, so they are looked
 * up here, never retyped.
 */
const SLI_METRIC_NAME: string = getSloHistoryMetricName(SloWidgetMetric.Sli);
const BUDGET_METRIC_NAME: string = getSloHistoryMetricName(
  SloWidgetMetric.ErrorBudgetRemaining,
);
const BURN_RATE_METRIC_NAME: string = getSloHistoryMetricName(
...
/*
 * The SLO's long-range history: SLI, remaining error budget and burn rate from
 * SloHistory (400 days), with the target, the at-risk / exhausted boundaries
 * and every enabled burn-rate rule's threshold drawn as reference lines. It is
 * the Error Budget History tab of the Metrics page and the whole of the legacy
 * Charts route. The oneuptime.slo.* metrics on the other tab keep only the
 * monitor-metric retention and cannot draw value reference lines, which is why
 * this view still reads SloHistory directly.
 *
 ...
 * view can no longer pin a bucket size per hardcoded range. The
 ...
 */
const SloHistoryCharts: FunctionComponent<ComponentProps> = (
  props: ComponentProps,
): ReactElement => {
  /*
   * Effects key on the id STRING: a parent that builds a new ObjectID on every
   * render must not restart the fetch each time.
   */
      ...
      try {
        ...
        const [slo, rules, sli, budget, burnRate]: [
          ...
          ModelAPI.getList<ServiceLevelObjectiveBurnRateRule>({
            ...
            query: {
              serviceLevelObjectiveId: new ObjectID(sloIdString),
              projectId: ProjectUtil.getCurrentProjectId()!,
              isEnabled: true,
            },
            select: {
  ...
  const syncId: string = `slo-charts-${sloIdString}`;
...
export default SloHistoryCharts;
