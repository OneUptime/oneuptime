...
export const getRumDocMarkdown: (vars: DocVars) => string = (
  ...
  return [
    ...
    "web, or `device.id` / `device.model.identifier` for mobile. The application",
    "is identified by `service.name`, and its telemetry is owned by this RUM",
    "application (it is never duplicated as a backend Service).",
    "",
    "`device.manufacturer` also marks a batch as mobile, but only on a resource",
    "that carries no `host.name` or `host.id` — it doubles as the make of a",
    "physical machine on a host's inventory item. Set `device.id` or",
    "`device.model.identifier` as well and the distinction never comes up.",
    "",
