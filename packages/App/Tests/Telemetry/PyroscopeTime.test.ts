...
describe("parsePyroscopeTimeParam reads the unit off the digit count", () => {
  test.each([
    ...
    expect(parsePyroscopeTimeParam(raw, NOW_NANO)).toBe(expected);
  });

  test("keeps sub-second precision exactly for millisecond values", () => {
    expect(parsePyroscopeTimeParam("1790380815123", NOW_NANO)).toBe(
      WINDOW_START_NANO + BigInt(15_123_000_000),
    );
  });
  ...
  test("keeps nanosecond values exact, beyond Number precision", () => {
    ...
    expect(parsePyroscopeTimeParam(raw, NOW_NANO)!.toString()).toBe(raw);
  });

  test("strips the separators attime.Parse strips", () => {
    expect(parsePyroscopeTimeParam("1_790_380_800", NOW_NANO)).toBe(
      WINDOW_START_NANO,
    );
    expect(parsePyroscopeTimeParam("1,790,380,800", NOW_NANO)).toBe(
      WINDOW_START_NANO,
    );
    expect(parsePyroscopeTimeParam(" 1790380800000 ", NOW_NANO)).toBe(
      WINDOW_START_NANO,
    );
  });

  test("accepts a number as well as a string", () => {
    expect(parsePyroscopeTimeParam(1790380800, NOW_NANO)).toBe(
      WINDOW_START_NANO,
    );
  });
  ...
  test.each([
    ...
    expect(parsePyroscopeTimeParam(raw, NOW_NANO)).toBeNull();
  });

  test("treats a value before 2000 as not provided", () => {
    // 8-digit seconds: 1973. A unit mix-up, not a real capture time.
    expect(parsePyroscopeTimeParam("99999999", NOW_NANO)).toBeNull();
  });
  ...
  test("treats a value more than a day in the future as not provided", () => {
    ...
    expect(
      parsePyroscopeTimeParam(twoDaysAheadSeconds.toString(), NOW_NANO),
    ).toBeNull();
  });
  ...
  test("tolerates clock skew of under a day", () => {
    ...
    expect(
      parsePyroscopeTimeParam(anHourAheadSeconds.toString(), NOW_NANO),
    ).toBe(anHourAheadSeconds * NANOS_PER_SECOND);
  });
...
describe("toNonNegativeUnixNano reads decoded pprof int64 fields", () => {
  test.each([
    ...
    expect(toNonNegativeUnixNano(raw)).toBe(expected);
  });
  ...
  test.each([
    ...
    expect(toNonNegativeUnixNano(raw)).toBeNull();
  });
});
...
describe("resolveProfileWindow", () => {
  ...
  test(".NET <= 0.13: no pprof time, window from from/until", () => {
    ...
    expect(window).toEqual({
      startUnixNano: WINDOW_START_NANO,
      endUnixNano: WINDOW_START_NANO + FIFTEEN_SECONDS,
    });
  });
  ...
  test("a pprof with time and duration wins over the query params", () => {
    ...
    expect(window.startUnixNano).toBe(pprofStart);
    expect(window.endUnixNano).toBe(pprofStart + BigInt(10_000_000_000));
  });
  ...
  test("Go non-CPU profiles: pprof time but no duration end at `until`", () => {
    ...
    expect(window.endUnixNano).toBe(WINDOW_START_NANO + FIFTEEN_SECONDS);
  });
  ...
  test("a 1970 pprof time (microseconds in time_nanos) falls back to `from`", () => {
    ...
    expect(window.startUnixNano).toBe(WINDOW_START_NANO);
    expect(window.endUnixNano).toBe(WINDOW_START_NANO + FIFTEEN_SECONDS);
  });
  ...
  test("with no usable time anywhere, the profile is stamped with ingestion time", () => {
    ...
    expect(window).toEqual({
      startUnixNano: NOW_NANO,
      endUnixNano: NOW_NANO,
    });
  });
  ...
  test("an `until` before the start is ignored rather than producing a negative window", () => {
    ...
    expect(window.endUnixNano).toBe(WINDOW_START_NANO);
  });
