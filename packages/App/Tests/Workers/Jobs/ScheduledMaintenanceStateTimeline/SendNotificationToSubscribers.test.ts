...
describe("ScheduledMaintenanceStateTimeline email subjects are sent as written", () => {
  test.each(TEMPLATE_SYNTAX_DESCRIPTIONS)(
    ...
    async ({ markdown, text }: TemplateSyntaxDescription) => {
      ...
      useCustomTemplates({
        body: "{{scheduledMaintenanceDescription}}",
        subject:
          "{{scheduledMaintenanceTitle}} is {{scheduledMaintenanceState}}: {{scheduledMaintenanceDescription}}",
      });
      ...
      expect(sentMail()).toHaveLength(1);
      expect(sentMail()[0]!["subject"]).toBe(
        `Subject|${EVENT_TITLE} is ${STATE_NAME}: ${text}`,
      );
