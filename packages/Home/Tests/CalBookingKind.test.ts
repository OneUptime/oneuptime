...
/*
 * Which conversation a booked meeting is, on the record the CRM receives.
 *
 ...
 * but the Cal BOOKING_CREATED webhook — which is delivered to the CRM, not to
 * OneUptime, and is the record that survives ad blockers — could not tell a
 * free user's support call from a net-new enterprise demo.
 *
 * These assert the writing half: each embed hands Cal its own kind, in the
 * bracketed shape Cal actually accepts. The reading half lives in the CRM and
 * is not testable from here, which is exactly why the writing half is pinned
 * this tightly.
 */
...
for (const page of BOOKING_PAGES) {
  describe(page.template, () => {
    ...
    test("the browser event and the embed agree on the kind", () => {
       ...
       * drift, the browser funnel and the CRM disagree about what the same
...
describe("the shared Cal metadata helper", () => {
  ...
  test("sends the kind under the key the receiver reads", () => {
