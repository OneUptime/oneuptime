...
import {
  useAlertEpisodes,
  useUnresolvedAlertEpisodeCount,
} from "./useAlertEpisodes";
...
import type { AlertEpisodeItem, ListResponse } from "../api/types";
 ...
 * served page one out of the cache - an infinite list that scrolls forever
 * showing the same twenty episodes, with no error and nothing in the logs. The
 * tests prove it the only way that regression can be caught: render the hook
 * twice against ONE QueryClient with different windows, and watch the api be
 * asked twice.
 *
...
function fetchMock(): jest.MockedFunction<typeof fetchAlertEpisodes> {
  return fetchAlertEpisodes as jest.MockedFunction<typeof fetchAlertEpisodes>;
}
...
interface EpisodesRender {
  result: {
    current: UseQueryResult<ListResponse<AlertEpisodeItem>, Error>;
  };
...
async function renderEpisodes(
  ...
  return renderHook(
    (current: EpisodesArgs) => {
      return useAlertEpisodes(current.projectId, current.skip, current.limit);
    },
    {
      initialProps: args,
...
async function renderUnresolvedCount(
  ...
  return renderHook(
    () => {
      return useUnresolvedAlertEpisodeCount(projectId);
    },
    {
      wrapper: createQueryWrapper(client),
    },
...
describe("useAlertEpisodes", () => {
  ...
  beforeEach(() => {
    ...
    fetchMock().mockReset();
    fetchMock().mockResolvedValue(makeListResponse<AlertEpisodeItem>([]));
  });
  ...
  test("hands the whole page envelope back to the caller, count and all", async () => {
     ...
     */
    const page: ListResponse<AlertEpisodeItem> = makeListResponse(
      [
        makeAlertEpisode({ _id: "episode-1", episodeNumber: 3 }),
        makeAlertEpisode({ _id: "episode-2", episodeNumber: 2 }),
      ],
      { count: 57 },
    );
  ...
  test("stores the page under a key naming the hook, the project and the window", async () => {
     ...
     * unresolved count below hangs off the same "alert-episodes" prefix and
    ...
    expect(
      client.getQueryData(["alert-episodes", "project-1", 20, 20]),
    ).toEqual(page);
  ...
  test("fetches the second page rather than serving the first one again", async () => {
     ...
     */
    const firstPage: ListResponse<AlertEpisodeItem> = makeListResponse(
      [makeAlertEpisode({ _id: "episode-1" })],
      { count: 40 },
    );
    const secondPage: ListResponse<AlertEpisodeItem> = makeListResponse(
      [makeAlertEpisode({ _id: "episode-21" })],
      { count: 40, skip: 20 },
    );
    fetchMock()
  ...
  test("asking for a bigger page fetches it instead of reusing the smaller one", async () => {
     ...
     * the data.
     */
    const smallPage: ListResponse<AlertEpisodeItem> = makeListResponse(
      [makeAlertEpisode({ _id: "episode-1" })],
      { count: 40, limit: 20 },
    );
    const largePage: ListResponse<AlertEpisodeItem> = makeListResponse(
      [
        makeAlertEpisode({ _id: "episode-1" }),
        makeAlertEpisode({ _id: "episode-2" }),
      ],
      { count: 40, limit: 50 },
    );
    fetchMock()
  ...
  test("never serves one project's episodes for another project", async () => {
     ...
     */
    const firstProjectPage: ListResponse<AlertEpisodeItem> = makeListResponse([
      makeAlertEpisode({ _id: "episode-in-project-1" }),
    ]);
    const secondProjectPage: ListResponse<AlertEpisodeItem> = makeListResponse([
      makeAlertEpisode({ _id: "episode-in-project-2" }),
    ]);
    fetchMock()
...
describe("useUnresolvedAlertEpisodeCount", () => {
  let client: QueryClient;
  ...
  test("reports the server's total, not the number of rows it was sent", async () => {
    ...
    fetchMock().mockResolvedValue(
      makeListResponse([makeAlertEpisode()], { count: 42, limit: 1 }),
    );
  ...
  test("a project with nothing unresolved reports zero", async () => {
     ...
     */
    fetchMock().mockResolvedValue(
      makeListResponse<AlertEpisodeItem>([], { count: 0, limit: 1 }),
    );
  ...
  test("counts each project separately", async () => {
    fetchMock()
      ...
      .mockResolvedValueOnce(
        makeListResponse([makeAlertEpisode()], { count: 7, limit: 1 }),
      );
  ...
  test("does not share a cache entry with the first page of the list", async () => {
    /*
     * Both keys begin with "alert-episodes" - deliberately, so that
     ...
     */
    fetchMock().mockResolvedValue(
      makeListResponse([makeAlertEpisode()], { count: 42 }),
    );
