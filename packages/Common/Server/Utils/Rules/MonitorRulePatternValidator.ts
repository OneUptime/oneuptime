...
/*
 * Write-time validation for the monitor name / description patterns on the
 * rules that attach monitors to something - status page monitor rules and SLO
 * monitor rules.
 *
 ...
 *
 * The messages name the field ("Monitor Name Pattern"), never the product the
 * rule belongs to, which is what lets every monitor rule share this one
 * validator instead of drifting apart copy by copy.
 */
export default class MonitorRulePatternValidator {
  public static validate(data: {
    namePattern?: string | null | undefined;
    descriptionPattern?: string | null | undefined;
  }): void {
    MonitorRulePatternValidator.validateOne(
