...
export class Service extends DatabaseService<DatabaseServerFeed> {
  ...
  @CaptureSpan()
  public async createDatabaseServerFeedItem(data: {
    databaseServerId: ObjectID;
    feedInfoInMarkdown: string;
    databaseServerFeedEventType: DatabaseServerFeedEventType;
    projectId: ObjectID;
     ...
     */
    try {
      const feed: DatabaseServerFeed = new DatabaseServerFeed();

      if (!data.databaseServerId) {
        throw new BadDataException("Database ID is required");
      }
      ...
      if (!data.databaseServerFeedEventType) {
        throw new BadDataException("Database feed event is required");
      }
      ...
      feed.databaseServerId = data.databaseServerId;
      feed.feedInfoInMarkdown = data.feedInfoInMarkdown;
      feed.databaseServerFeedEventType = data.databaseServerFeedEventType;
      ...
      logger.error("Error in creating database feed", {
        projectId: data.projectId?.toString(),
        databaseServerId: data.databaseServerId?.toString(),
      } as LogAttributes);
      logger.error(e, {
        projectId: data.projectId?.toString(),
        databaseServerId: data.databaseServerId?.toString(),
