...
export class Service extends DatabaseService<Model> {
   ...
   * log records batched into one UPDATE), never once per record, and never
   * on the request path. Runs no hooks and does not bump `version`, so it
   * cannot conflict with a human editing the same filter in the dashboard.
   *
