...
export class Service extends DatabaseService<ProxmoxClusterFeed> {
  public constructor() {
    super(ProxmoxClusterFeed);
  ...
  @CaptureSpan()
  public async createProxmoxClusterFeedItem(data: {
    proxmoxClusterId: ObjectID;
    feedInfoInMarkdown: string;
    proxmoxClusterFeedEventType: ProxmoxClusterFeedEventType;
    projectId: ObjectID;
     ...
     */
    try {
      const feed: ProxmoxClusterFeed = new ProxmoxClusterFeed();

      if (!data.proxmoxClusterId) {
        throw new BadDataException("Proxmox Cluster ID is required");
      }
      ...
      if (!data.proxmoxClusterFeedEventType) {
        throw new BadDataException("Proxmox Cluster feed event is required");
      }
      ...
      feed.proxmoxClusterId = data.proxmoxClusterId;
      feed.feedInfoInMarkdown = data.feedInfoInMarkdown;
      feed.proxmoxClusterFeedEventType = data.proxmoxClusterFeedEventType;
      ...
      logger.error("Error in creating Proxmox cluster feed", {
        projectId: data.projectId?.toString(),
        proxmoxClusterId: data.proxmoxClusterId?.toString(),
      } as LogAttributes);
      logger.error(e, {
        projectId: data.projectId?.toString(),
        proxmoxClusterId: data.proxmoxClusterId?.toString(),
