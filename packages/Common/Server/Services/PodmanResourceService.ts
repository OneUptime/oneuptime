...
export interface ParsedPodmanContainer {
   ...
   * Testcontainers keys, see CONTAINER_CLASSIFIER_LABEL_KEYS): MERGED into
   * the row's labels, never replacing what the inventory snapshot wrote.
   * Null / omitted leaves the labels alone.
   */
    ...
    for (let i: number = 0; i < containers.length; i += UPSERT_BATCH_SIZE) {
      ...
      const sql: string = `
        ...
        DO UPDATE SET
          ...
          "labels" = CASE
            WHEN EXCLUDED."labels" IS NULL THEN "PodmanResource"."labels"
            WHEN lower(left(EXCLUDED."containerId", 12)) <> lower(left("PodmanResource"."containerId", 12))
              THEN EXCLUDED."labels"
            WHEN jsonb_typeof("PodmanResource"."labels") = 'object'
              THEN "PodmanResource"."labels" || EXCLUDED."labels"
            ELSE EXCLUDED."labels"
          END,
          "resourceCreationTimestamp" = CASE
            WHEN EXCLUDED."resourceCreationTimestamp" IS NULL THEN "PodmanResource"."resourceCreationTimestamp"
            WHEN "PodmanResource"."resourceCreationTimestamp" IS NULL
              OR lower(left(EXCLUDED."containerId", 12)) <> lower(left("PodmanResource"."containerId", 12))
              OR EXCLUDED."resourceCreationTimestamp" > "PodmanResource"."resourceCreationTimestamp" + interval '1 minute'
              THEN EXCLUDED."resourceCreationTimestamp"
            ELSE "PodmanResource"."resourceCreationTimestamp"
          END,
          "updatedAt" = now()
