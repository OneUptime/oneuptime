...
/**
 * The edition a frontend build bundles: "community" or "enterprise".
 *
 * The same decision esbuild makes for the frontend's Enterprise plugin (see
 * Common/UI/esbuild-enterprise.js): ONEUPTIME_EDITION=community or
 * enterprise decides outright, and "auto" (the default) means enterprise
 * exactly when this frontend's ee plugin, ee/<frontend directory name>/
 * Index.tsx, is on disk. A frontend with no ee plugin (Accounts, Status Page)
 * is therefore always "community". An unknown ONEUPTIME_EDITION throws, like
 * the build.
 *
 * The edition goes into the service worker's cache version: the Community and
 * Enterprise images of one release share APP_VERSION and GIT_SHA, and without
 * it a browser that cached one edition's assets would keep them after the
 * deployment switched to the other.
 * @param {string} frontendDir - The frontend's directory (where sw.js.template lives)
 * @param {Object} [env] - Environment to read (defaults to process.env)
 * @returns {string}
 */
function getEdition(frontendDir, env) {
  const source = env || process.env;
  const requested = readRequestedEdition(source);

  if (requested === EDITION_COMMUNITY || requested === EDITION_ENTERPRISE) {
    return requested;
  }

  const candidates = getEnterpriseCandidates(
    frontendDir,
    path.basename(frontendDir),
    source,
  );

  const hasPlugin = candidates.some((candidate) => {
    try {
  ...
  return hasPlugin ? EDITION_ENTERPRISE : EDITION_COMMUNITY;
}
...
function generateServiceWorker(
  ...
  const edition = getEdition(path.dirname(templatePath));
...
`;
  ...
  console.log(
    `   Cache version: oneuptime-v${appVersion}-${edition}-${gitSha}`,
  );
}
