...
const prepareIssue: RunStepFunction = step(
  "jira-create-issue-for-incident",
  "prepare-issue-1",
);
const planTransition: RunStepFunction = step(
  "jira-transition-issue-on-incident-state",
  "plan-transition-1",
);
const privateNoteComment: RunStepFunction = step(
  "jira-comment-from-private-note",
  "build-comment-1",
);
const publicNoteComment: RunStepFunction = step(
  "jira-comment-from-public-note",
  "build-comment-1",
);
const updateComment: RunStepFunction = step(
  "jira-comment-on-incident-update",
  "build-comment-1",
);
const prepareIncident: RunStepFunction = step(
  "jira-declare-incident-from-issue",
  "prepare-incident-1",
);
const confirmUnlinked: RunStepFunction = step(
  "jira-declare-incident-from-issue",
  "confirm-unlinked-1",
);
const readEvent: RunStepFunction = step(
  "jira-status-to-incident-state",
  "read-event-1",
);
const decideState: RunStepFunction = step(
  "jira-status-to-incident-state",
  "decide-state-1",
);
const readComment: RunStepFunction = step(
...
const readChanges: RunStepFunction = step(
...
/** A jira:issue_updated event on the issue OneUptime linked to INCIDENT_ID. */
...
const INCIDENT_LABEL: string = incidentLabel();
const LINK_LABELS: Array<string> = [JIRA_LINK_LABEL, INCIDENT_LABEL];
const ISSUE_LINK: string = `[${JIRA_ISSUE_KEY}](${JIRA_SITE}/browse/${JIRA_ISSUE_KEY})`;
const ONEUPTIME_URL: string = "https://oneuptime.com";

const NO_ISSUE_REASON: string = `No Jira issue is labelled ${INCIDENT_LABEL}, or the Jira credentials cannot see it.`;
const ALREADY_LINKED_REASON: string =
  "Jira issue OPS-17 is already linked to OneUptime, so no incident was declared.";
const NOT_RETURNED_REASON: string =
  "Jira did not return the issue, so no incident was declared.";

/** A second issue carrying the incident's label, as a clone of OPS-17 would. */
const CLONE_ISSUE_KEY: string = "OPS-30";

type IssueKeysReasonFunction = (keys: Array<string>) => string;

/** Why the outbound scripts refuse to pick one of several labelled issues. */
const moreThanOneReason: IssueKeysReasonFunction = (
  keys: Array<string>,
): string => {
  return `More than one Jira issue is labelled ${INCIDENT_LABEL} (${keys.join(", ")}). A cloned issue copies the label: remove it from every issue except the one filed for the incident.`;
};
...
/*
 * Every outbound script documents this line as the switch that lets private
 * incidents through; replacing it also pins that it is still there.
 */
const SYNC_PRIVATE_INCIDENTS: EditCodeFunction = replaceInCode(
...
type NoteOnIncidentFunction = (
  isPrivate: boolean | undefined,
  note?: string | undefined,
) => JSONObject;

/** A note whose hydrated incident is, or is not, private. */
const noteOnIncident: NoteOnIncidentFunction = (
  isPrivate: boolean | undefined,
  note?: string | undefined,
): JSONObject => {
  const model: JSONObject = noteModel({ note: note });

  return {
    ...model,
    incident: withPrivacy(model["incident"] as JSONObject, isPrivate),
  };
};
const NOT_LINKED_REASON: string = `Jira issue ${JIRA_ISSUE_KEY} is not linked to an incident: it has no ${JIRA_INCIDENT_LABEL_PREFIX}<id> label.`;
...
const UNRESOLVED_INCIDENT: string =
...
const STATES_WITH_MONITORING: Array<JSONObject> = [
  { ...RESOLVED_STATE, order: 4 },
  MONITORING_STATE,
  CREATED_STATE,
  ACKNOWLEDGED_STATE,
];
...
const issueArgs: IssueArgsFunction = (
  ...
  return {
    ...
    incident: asArgument(incident),
  };
};

describe("prepare-issue-1 shapes a new incident into a Jira issue", () => {
  test("writes the summary, description, link label and link back to the incident", async () => {
    const result: JSONObject = await prepareIssue(issueArgs(incidentModel()));

    expect(result).toEqual({
      proceed: true,
      summary: "[OneUptime] INC-42: Checkout latency high",
      description:
        'INC-42 was declared in OneUptime.\nSeverity: Critical Incident\nState: Identified\n\np99 latency on /checkout is above 2s.\nStarted after the "v4.2" deploy.',
      incidentLabel: INCIDENT_LABEL,
      incidentUrl: `https://oneuptime.com/dashboard/${PROJECT_ID}/incidents/${INCIDENT_ID}`,
    });
  });
  ...
  test("the label it writes is the one every other template searches for", async () => {
    ...
    expect(textOf(result, "incidentLabel")).toBe(
  ...
  test("tidies the OneUptime URL however it was typed", async () => {
    const cases: Array<{ typed: string; base: string }> = [
      { typed: "https://status.acme.com///", base: "https://status.acme.com" },
    ...
    for (const oneuptimeUrl of cases) {
      const result: JSONObject = await prepareIssue(
        issueArgs(incidentModel(), oneuptimeUrl.typed),
      );

      expect(textOf(result, "incidentUrl")).toBe(
        `${oneuptimeUrl.base}/dashboard/${PROJECT_ID}/incidents/${INCIDENT_ID}`,
      );
    }
  });
  ...
  test("folds a title with line breaks, tabs and control characters onto one line", async () => {
    const result: JSONObject = await prepareIssue(
      issueArgs(
        incidentModel({ title: "Checkout\n\tlatency\u0000 high\r\n\u0007" }),
      ),
    );

    // Jira rejects a summary with a line break in it.
    expect(textOf(result, "summary")).toBe(
      "[OneUptime] INC-42: Checkout latency high",
    );
  });

  test("caps the summary at Jira's 255 characters", async () => {
    const result: JSONObject = await prepareIssue(
      issueArgs(incidentModel({ title: "x".repeat(400) })),
    );
    const summary: string = textOf(result, "summary");

    expect(summary).toHaveLength(255);
    expect(summary.startsWith("[OneUptime] INC-42: xxx")).toBe(true);
    expect(summary.endsWith("…")).toBe(true);
  });

  test("names an untitled incident rather than sending an empty summary", async () => {
    const result: JSONObject = await prepareIssue(
      issueArgs(incidentModel({ title: "" })),
    );

    expect(textOf(result, "summary")).toBe(
      "[OneUptime] INC-42: Untitled incident",
    );
  });

  test("caps the description at 30000 characters", async () => {
    const result: JSONObject = await prepareIssue(
      issueArgs(incidentModel({ description: "y".repeat(40000) })),
    );
  ...
  test("leaves the description block out when the incident has none", async () => {
    const result: JSONObject = await prepareIssue(
      issueArgs(incidentModel({ description: null })),
    );
  ...
  test("leaves severity and state out when the trigger did not hydrate them", async () => {
    const result: JSONObject = await prepareIssue(
      issueArgs(
        omit(incidentModel(), ["incidentSeverity", "currentIncidentState"]),
      ),
    );
  ...
  test("defuses double braces in the title and description", async () => {
    ...
    expect(textOf(result, "summary")).toBe(
      `[OneUptime] INC-42: Leak ${DEFUSED_SECRET_REFERENCE}`,
    );
  ...
  test("files no issue for an incident declared from Jira, naming its issue", async () => {
    const result: JSONObject = await prepareIssue(
      issueArgs(incidentModel({ customFields: { jiraIssueKey: "OPS-17" } })),
    );

    expectSkipped(
      result,
      "INC-42 was declared from Jira issue OPS-17, so no new issue was created.",
    );
  });
  ...
  test("files an issue when custom fields are missing or hold other keys", async () => {
    for (const customFields of [null, { team: "payments" }]) {
      const result: JSONObject = await prepareIssue(
        issueArgs(incidentModel({ customFields: customFields })),
      );
  ...
  test("files no issue when the incident has no usable id", async () => {
    // The label is the only link, so an issue without an id could never be found again.
    const incidents: Array<JSONObject> = [
      omit(incidentModel(), ["_id"]),
      incidentModel({ _id: "INC-42" }),
      { ...incidentModel(), _id: { _type: "ObjectID", value: INCIDENT_ID } },
    ];

    for (const incident of incidents) {
      expectSkipped(
        await prepareIssue(issueArgs(incident)),
        "The trigger did not hand over an incident id.",
      );
    }
  });
...
const transitionArgs: TransitionArgsFunction = (
  ...
  return {
    incident: quoted(incidentModel({ state: state })),
...
describe("plan-transition-1 picks the Jira transition for the incident's new state", () => {
  test("moves the issue of an acknowledged incident to In Progress", async () => {
    const result: JSONObject = await planTransition(
      transitionArgs(ACKNOWLEDGED_STATE, jiraSearchResponse()),
    );
    ...
    expect(result).toEqual({
      ...
      reason: "INC-42 is Acknowledged in OneUptime.",
    });
  });

  test("moves the issue of a resolved incident to Done", async () => {
    const result: JSONObject = await planTransition(
      transitionArgs(RESOLVED_STATE, jiraSearchResponse()),
    );
    ...
    expect(result).toEqual({
      ...
      transitionId: "31",
      jiraStatus: "Done",
      reason: "INC-42 is Resolved in OneUptime.",
    });
  });
  ...
  test("goes by status category, so any Done status counts as done", async () => {
    ...
    expectSkipped(
      await planTransition(
        transitionArgs(RESOLVED_STATE, jiraSearchResponse({ status: closed })),
      ),
  ...
  test("does nothing for the created state", async () => {
    expectSkipped(
      await planTransition(transitionArgs(CREATED_STATE, jiraSearchResponse())),
      "INC-42 moved to Identified, which has no Jira status mapped to it.",
    );
  });
  ...
  test("does nothing for a custom state nobody mapped", async () => {
    expectSkipped(
      await planTransition(
        transitionArgs(MONITORING_STATE, jiraSearchResponse()),
      ),
      "INC-42 moved to Monitoring, which has no Jira status mapped to it.",
    );
  });
  ...
  test("skips, naming the label, when no issue carries it", async () => {
    for (const search of [EMPTY_JIRA_SEARCH, UNRESOLVED_SEARCH, "null"]) {
      expectSkipped(
        await planTransition(transitionArgs(ACKNOWLEDGED_STATE, search)),
        NO_ISSUE_REASON,
      );
    }
  });
  ...
  test("skips an issue whose key fails validation", async () => {
    ...
    for (const key of ["../../myself", "OPS 17"]) {
      expectSkipped(
        await planTransition(
          transitionArgs(ACKNOWLEDGED_STATE, jiraSearchResponse({ key: key })),
        ),
        NO_ISSUE_REASON,
      );
    }
  ...
  test("skips when the search did not expand the issue's transitions", async () => {
    expectSkipped(
      await planTransition(
        transitionArgs(ACKNOWLEDGED_STATE, {
  ...
  test("STATE_TO_JIRA_STATUS maps a custom state to a Jira status by name, in any case", async () => {
    ...
    expect(result).toEqual({
      ...
      reason: "INC-42 is Monitoring in OneUptime.",
    });
  });
...
describe("plan-transition-1 only moves the Jira issue forward", () => {
  test("leaves a Done issue alone when the incident is acknowledged", async () => {
    // Someone closed the issue first; acknowledging the incident must not reopen it.
    expectSkipped(
      await planFrom({
  ...
  test("a named mapping that would move the issue backwards is not chosen", async () => {
    const backwards: Array<{
      ...
      {
        // A To Do-category name for an acknowledged incident whose issue is In Progress.
...
describe("plan-transition-1 chooses among several statuses by meaning", () => {
...
const NOTE_TEMPLATES: Array<NoteTemplateCase> = [
  {
    ...
    run: privateNoteComment,
  },
  {
    templateId: "jira-comment-from-public-note",
    noteKind: "public note",
    run: publicNoteComment,
  },
];

// The two note templates ship separate copies of the script, so both are run.
for (const noteTemplate of NOTE_TEMPLATES) {
  describe(`build-comment-1 in ${noteTemplate.templateId} turns a note into a Jira comment`, () => {
    ...
    test("posts the note under a heading naming the note, the incident and the author", async () => {
      const result: JSONObject = await run(noteArgs(noteModel()));

      expect(result).toEqual({
        proceed: true,
        issueKey: JIRA_ISSUE_KEY,
        comment: `Synced from OneUptime: ${noteTemplate.noteKind} on INC-42 by Jane Doe.\n\nFailed over to the **secondary** database. Error rate is dropping.`,
      });
    ...
    test("leaves the author out when the note has none", async () => {
      const result: JSONObject = await run(
        noteArgs(noteModel({ authorName: null })),
      );

      expect(
        textOf(result, "comment").startsWith(
          `Synced from OneUptime: ${noteTemplate.noteKind} on INC-42.\n\n`,
        ),
      ).toBe(true);
    });

    test("says 'the incident' when the note's incident was not hydrated", async () => {
      const result: JSONObject = await run(
        noteArgs(omit(noteModel(), ["incident"])),
      );

      expect(
        textOf(result, "comment").startsWith(
          `Synced from OneUptime: ${noteTemplate.noteKind} on the incident by Jane Doe.`,
        ),
      ).toBe(true);
    });
    ...
    test("keeps the note's line breaks and drops its control characters", async () => {
      const result: JSONObject = await run(
        noteArgs(noteModel({ note: "Line\u0000 one\r\nLine two" })),
      );

      expect(textOf(result, "comment").endsWith("\n\nLine one\nLine two")).toBe(
        true,
      );
    });
    ...
    test("does not post a note that came from Jira back to it", async () => {
      ...
      for (const note of notes) {
        expectSkipped(
          await run(noteArgs(noteModel({ note: note }))),
    ...
    test("skips, naming the label, when no issue is linked", async () => {
      for (const search of [EMPTY_JIRA_SEARCH, UNRESOLVED_SEARCH, "null"]) {
        expectSkipped(
          await run(noteArgs(noteModel(), search)),
          NO_ISSUE_REASON,
        );
      }
    });
    ...
    test("caps the comment at 30000 characters", async () => {
      const result: JSONObject = await run(
        noteArgs(noteModel({ note: "z".repeat(40000) })),
      );
...
describe("build-comment-1 in jira-comment-on-incident-update describes the incident as it stands", () => {
  test("lists every field the trigger listens on, in order", async () => {
  ...
  test("is just the heading when nothing is filled in", async () => {
    const result: JSONObject = await updateComment(
      updateArgs(
        omit(incidentModel({ title: "", description: null }), [
          "incidentSeverity",
  ...
  test("caps each field on its own line", async () => {
    const result: JSONObject = await updateComment(
      updateArgs(
        incidentModel({ title: "t".repeat(800), rootCause: "r".repeat(8000) }),
      ),
    );
  ...
  test("skips, naming the label, when no issue is linked", async () => {
    for (const search of [EMPTY_JIRA_SEARCH, UNRESOLVED_SEARCH, "null"]) {
      expectSkipped(
        await updateComment(updateArgs(incidentModel(), search)),
        NO_ISSUE_REASON,
      );
    }
  });

  test("defuses double braces in the incident's text", async () => {
    const result: JSONObject = await updateComment(
      updateArgs(
        incidentModel({
          title: `Leak ${SECRET_REFERENCE}`,
...
interface LinkedIssueCase {
  templateId: string;
  componentId: string;
  /** Runs the step for a public incident that would otherwise proceed. */
  withSearch: WithSearchFunction;
}

/* Every script that acts on the issue a label search found. */
const LINKED_ISSUE_CASES: Array<LinkedIssueCase> = [
  {
    ...
    withSearch: async (search: JSONValue): Promise<JSONObject> => {
      return await planTransition(transitionArgs(ACKNOWLEDGED_STATE, search));
    },
  },
  ...NOTE_TEMPLATES.map((noteTemplate: NoteTemplateCase): LinkedIssueCase => {
    return {
      templateId: noteTemplate.templateId,
      componentId: "build-comment-1",
      withSearch: async (search: JSONValue): Promise<JSONObject> => {
        return await noteTemplate.run(noteArgs(noteModel(), search));
      },
    };
  }),
  {
    ...
    componentId: "build-comment-1",
    withSearch: async (search: JSONValue): Promise<JSONObject> => {
      return await updateComment(updateArgs(incidentModel(), search));
    },
  },
];
 ...
 */
describe("the outbound scripts act only on the one issue carrying the incident's label", () => {
  for (const linked of LINKED_ISSUE_CASES) {
    ...
    test(`${name} skips, naming the label, when the search found none`, async () => {
      ...
      for (const search of searches) {
        expectSkipped(await linked.withSearch(search), NO_ISSUE_REASON);
      }
    });
    ...
    test(`${name} refuses to choose between two labelled issues, naming both`, async () => {
      expectSkipped(
        ...
        moreThanOneReason([JIRA_ISSUE_KEY, CLONE_ISSUE_KEY]),
      );
    });
...
interface PrivateIncidentCase {
  templateId: string;
  componentId: string;
  /** Runs the step on an incident that is otherwise ready to sync. */
  run: PrivacyRunFunction;
  reason: string;
}

const PRIVATE_INCIDENT_CASES: Array<PrivateIncidentCase> = [
  {
    ...
    run: async (
      ...
      return await prepareIssue(
        issueArgs(withPrivacy(incidentModel(), isPrivate)),
        editCode,
      );
    },
    reason: "INC-42 is a private incident, so no Jira issue was created.",
  },
  {
    templateId: "jira-transition-issue-on-incident-state",
    componentId: "plan-transition-1",
      ...
      return await planTransition(
        {
          incident: quoted(
            withPrivacy(
              incidentModel({ state: ACKNOWLEDGED_STATE }),
              isPrivate,
            ),
          ),
    ...
    reason: "INC-42 is a private incident, so its Jira issue was not moved.",
  },
  ...NOTE_TEMPLATES.map(
    (noteTemplate: NoteTemplateCase): PrivateIncidentCase => {
      return {
        templateId: noteTemplate.templateId,
        componentId: "build-comment-1",
        run: async (
          ...
          return await noteTemplate.run(
            noteArgs(noteOnIncident(isPrivate)),
            editCode,
          );
        },
        reason:
          "The note is on INC-42, a private incident, so it was not posted to Jira.",
      };
    },
  ),
  {
    ...
    run: async (
      ...
      return await updateComment(
        updateArgs(withPrivacy(incidentModel(), isPrivate)),
        editCode,
      );
    },
    reason:
      "INC-42 is a private incident, so its changes were not posted to Jira.",
  },
];

describe("the outbound scripts keep private incidents in OneUptime", () => {
  for (const privacy of PRIVATE_INCIDENT_CASES) {
    const name: string = `${privacy.componentId} in ${privacy.templateId}`;

    test(`${name} skips a private incident`, async () => {
      expectSkipped(await privacy.run(true), privacy.reason);
    });

    test(`${name} syncs a private incident once SYNC_PRIVATE_INCIDENTS is true`, async () => {
      const result: JSONObject = await privacy.run(
        true,
        SYNC_PRIVATE_INCIDENTS,
      );
    ...
    test(`${name} syncs an incident that is not private, or does not say`, async () => {
  ...
  test("the private check comes before the linked-issue lookup", async () => {
    // Nothing about the Jira side is worth reporting for an incident that stays in OneUptime.
    const privateIncident: JSONObject = withPrivacy(incidentModel(), true);

    expectSkipped(
      await updateComment(updateArgs(privateIncident, EMPTY_JIRA_SEARCH)),
      "INC-42 is a private incident, so its changes were not posted to Jira.",
    );
    expectSkipped(
      await planTransition({
        incident: quoted(
          withPrivacy(incidentModel({ state: CREATED_STATE }), true),
        ),
        search: quoted(searchWithKeys([JIRA_ISSUE_KEY, CLONE_ISSUE_KEY])),
      }),
      "INC-42 is a private incident, so its Jira issue was not moved.",
    );
  });

  test("prepare-issue-1 checks privacy before the Jira issue key an incident was declared with", async () => {
    expectSkipped(
      await prepareIssue(
        issueArgs(
          withPrivacy(
            incidentModel({ customFields: { jiraIssueKey: JIRA_ISSUE_KEY } }),
            true,
          ),
        ),
      ),
      "INC-42 is a private incident, so no Jira issue was created.",
    );
  });
  ...
  for (const noteTemplate of NOTE_TEMPLATES) {
      ...
      for (const note of notes) {
        expectSkipped(
          await noteTemplate.run(
            noteArgs(noteOnIncident(true, note), EMPTY_JIRA_SEARCH),
          ),
          "The note is on INC-42, a private incident, so it was not posted to Jira.",
        );
      }
    ...
    test(`build-comment-1 in ${noteTemplate.templateId} still skips an empty note once private incidents sync`, async () => {
      expectSkipped(
        await noteTemplate.run(
          noteArgs(noteOnIncident(true, "  ")),
          SYNC_PRIVATE_INCIDENTS,
        ),
        "The note is empty, so there is nothing to post.",
      );
...
type IncidentArgsFunction = (
  payload: JSONValue,
  severities?: JSONValue,
) => JSONObject;

/* The Jira payload goes last, as it does in the template. */
const incidentArgs: IncidentArgsFunction = (
  payload: JSONValue,
  severities?: JSONValue,
): JSONObject => {
  return {
    severities: asArgument(
      severities === undefined ? INCIDENT_SEVERITIES : severities,
    ),
...
type DeclaredSeverityFunction = (
  priority: string | null,
  severities?: JSONValue,
  editCode?: EditCodeFunction | undefined,
) => Promise<string>;

/** The severity name an issue with this priority is declared with. */
const declaredSeverity: DeclaredSeverityFunction = async (
  priority: string | null,
  severities?: JSONValue,
  editCode?: EditCodeFunction | undefined,
): Promise<string> => {
  const result: JSONObject = await prepareIncident(
    incidentArgs(jiraIssueCreatedEvent({ priority: priority }), severities),
    editCode,
  );
...
describe("prepare-incident-1 turns a new Jira issue into an incident", () => {
  test("declares the issue with a severity from its priority and a link back", async () => {
    const result: JSONObject = await prepareIncident(
      incidentArgs(jiraIssueCreatedEvent()),
    );
    ...
    expect(result).toEqual({
      ...
      description: `Declared from Jira issue ${ISSUE_LINK}.\nPriority: High\nReported by: Priya Patel\n\nCustomers in *eu-west-1* see 502s at checkout.\nStarted around 09:40 UTC.`,
    });
  });

  test("ignores every event but jira:issue_created", async () => {
    expectSkipped(
      await prepareIncident(
        incidentArgs(jiraIssueUpdatedEvent({ items: STARTED_CHANGELOG })),
      ),
      "Ignored a jira:issue_updated event: this workflow only handles jira:issue_created.",
    );
    expectSkipped(
      await prepareIncident(incidentArgs(jiraCommentEvent())),
  ...
  test("ignores a request that is not a Jira event at all", async () => {
    ...
    for (const payload of [{}, "ping", "null"]) {
      expectSkipped(
        await prepareIncident(incidentArgs(payload)),
        NOT_A_JIRA_EVENT_REASON,
      );
    }
  });

  test("declares nothing for an issue OneUptime filed or already linked", async () => {
    const labelSets: Array<Array<string>> = [
      LINK_LABELS,
      [JIRA_LINK_LABEL],
      ["ONEUPTIME"],
      [`OneUptime-Incident-${OTHER_INCIDENT_ID.toUpperCase()}`],
      ["backend", `${JIRA_INCIDENT_LABEL_PREFIX}not-even-an-id`],
    ];

    for (const labels of labelSets) {
      expectSkipped(
        await prepareIncident(
          incidentArgs(jiraIssueCreatedEvent({ labels: labels })),
        ),
        "Jira issue OPS-17 is already linked to OneUptime, so no incident was declared.",
      );
    }
  ...
  test("skips an event whose issue key is not a Jira key", async () => {
    ...
    for (const key of ["../../myself", "OPS 1", "", "17", "OPS-17/comment"]) {
      expectSkipped(
        await prepareIncident(
          incidentArgs(withIssue(jiraIssueCreatedEvent(), { key: key })),
        ),
        NO_ISSUE_KEY_REASON,
      );
    }
  ...
  test("maps Jira priorities onto the default three severities", async () => {
    const expected: Array<{ priority: string | null; severity: string }> = [
    ...
    for (const mapping of expected) {
      expect({
        ...
        severity: await declaredSeverity(mapping.priority),
      }).toEqual(mapping);
    }
  });
  ...
  test("hands on the chosen severity's id", async () => {
    ...
    for (const priority of ["High", "Medium", "Low"]) {
      const result: JSONObject = await prepareIncident(
        incidentArgs(jiraIssueCreatedEvent({ priority: priority })),
      );
      ids.push(textOf(result, "incidentSeverityId"));
    }
  ...
  test("spreads the priorities across five severities", async () => {
    expect(await declaredSeverity("High", severityList(5))).toBe("SEV1");
    expect(await declaredSeverity("Medium", severityList(5))).toBe("SEV3");
    expect(await declaredSeverity("Low", severityList(5))).toBe("SEV5");
  });

  test("spreads the priorities across two severities", async () => {
    expect(await declaredSeverity("High", severityList(2))).toBe("SEV1");
    expect(await declaredSeverity("Medium", severityList(2))).toBe("SEV2");
    expect(await declaredSeverity("Low", severityList(2))).toBe("SEV2");
  });

  test("maps every priority to the only severity there is", async () => {
    for (const priority of ["Highest", "Medium", "Lowest", null]) {
      expect(await declaredSeverity(priority, severityList(1))).toBe("SEV1");
    }
  });
  ...
  test("orders severities by their order field, not by how find-many returns them", async () => {
    ...
    expect(await declaredSeverity("Highest", reversed)).toBe("SEV1");
    expect(await declaredSeverity("Lowest", reversed)).toBe("SEV5");
  });

  test("ignores severity rows without an id", async () => {
    expect(
      await declaredSeverity("Highest", [
        { name: "Ghost", order: 0 },
        ...INCIDENT_SEVERITIES,
      ]),
    ).toBe("Critical Incident");
  });

  test("declares nothing when the project has no severities to choose from", async () => {
    for (const severities of [
      ...
      expectSkipped(
        await prepareIncident(
          incidentArgs(jiraIssueCreatedEvent(), severities as JSONValue),
        ),
        "This project has no incident severities to choose from.",
      );
    }
  });
  ...
  test("caps the title at 500 characters", async () => {
    const result: JSONObject = await prepareIncident(
      incidentArgs(jiraIssueCreatedEvent({ summary: "s".repeat(700) })),
    );
  ...
  test("names the issue when it has no summary", async () => {
    const result: JSONObject = await prepareIncident(
      incidentArgs(jiraIssueCreatedEvent({ summary: "" })),
    );
  ...
  test("writes the key without a link when the issue has no REST URL to read the site from", async () => {
    const result: JSONObject = await prepareIncident(
      incidentArgs(jiraIssueCreatedEvent({ self: "" })),
    );

    expect(
      textOf(result, "description").startsWith(
        "Declared from Jira issue OPS-17.\n",
      ),
    ).toBe(true);
  });
  ...
  test("leaves the priority line out when the issue has no priority", async () => {
    const result: JSONObject = await prepareIncident(
      incidentArgs(jiraIssueCreatedEvent({ priority: null })),
    );
  ...
  test("flattens a description in Atlassian Document Format to text", async () => {
    const result: JSONObject = await prepareIncident(
      incidentArgs(jiraIssueCreatedEvent({ description: ADF_TEXT })),
    );
  ...
  test("caps the description at 20000 characters", async () => {
    const result: JSONObject = await prepareIncident(
      incidentArgs(jiraIssueCreatedEvent({ description: "d".repeat(25000) })),
    );
  ...
  test("reads a request body that arrived as JSON text", async () => {
    ...
    const fromText: JSONObject = await prepareIncident(
      incidentArgs(JSON.stringify(jiraIssueCreatedEvent())),
    );
    const fromObject: JSONObject = await prepareIncident(
      incidentArgs(jiraIssueCreatedEvent()),
    );
  ...
  test("PRIORITY_RANK can be extended, and ranks outside 0 to 2 are clamped", async () => {
    ...
    expect(
      await declaredSeverity("P1", INCIDENT_SEVERITIES, addPriorities),
    ).toBe("Critical Incident");
    expect(
      await declaredSeverity("SEV0", INCIDENT_SEVERITIES, addPriorities),
    ).toBe("Critical Incident");
    expect(
      await declaredSeverity("SEV9", INCIDENT_SEVERITIES, addPriorities),
    ).toBe("Minor Incident");
  });
});
...
/*
 * The declare template's second look, at the issue as Jira has it. The
 * webhook's own labels were already checked by prepare-incident-1, but anyone
 * with the workflow's URL can write a webhook, and a delivery Jira retries
 * arrives after the first one labelled the issue.
 */
describe("confirm-unlinked-1 declares nothing for an issue Jira says is linked", () => {
  ...
  test("lets an issue through whose labels only look like the link labels", async () => {
    const labelSets: Array<Array<string>> = [
      ["backend", "sev1"],
      ["oneuptime-team", "incident-review"],
      [`x-${INCIDENT_LABEL}`, `${JIRA_LINK_LABEL}s`],
    ];
  ...
  test("declares nothing for an issue carrying the link labels", async () => {
    expectSkipped(
      await confirmUnlinked(confirmArgs(jiraIssueLabelsResponse(LINK_LABELS))),
      ALREADY_LINKED_REASON,
    );
  });

  test("declares nothing for an issue carrying just the bare oneuptime label, in any case", async () => {
  ...
  test("declares nothing for an issue with any oneuptime-incident- label, whatever the id", async () => {
    // Linked to another incident, or labelled by hand: either way it is not free to declare.
    const labels: Array<string> = [
      incidentLabel(OTHER_INCIDENT_ID),
      `OneUptime-Incident-${OTHER_INCIDENT_ID.toUpperCase()}`,
      `${JIRA_INCIDENT_LABEL_PREFIX}42`,
      `${JIRA_INCIDENT_LABEL_PREFIX}not-even-an-id`,
      JIRA_INCIDENT_LABEL_PREFIX,
    ];
  ...
  test("goes by Jira's labels, not the webhook's", async () => {
    ...
    const prepared: JSONObject = await prepareIncident(incidentArgs(event));
  ...
  test("declares nothing when Jira did not return an issue", async () => {
    const responses: Array<JSONValue> = [
  ...
  test("reads the response whether it arrives as JSON text or as an object", async () => {
    const response: JSONObject = jiraIssueLabelsResponse(LINK_LABELS);
...
describe("read-event-1 keeps only status changes on linked issues", () => {
  ...
  test("reads work starting on an issue", async () => {
    const result: JSONObject = await readEvent(
      linkedUpdate(STARTED_CHANGELOG, {
        status: IN_PROGRESS,
  ...
  test("ignores every event but jira:issue_updated", async () => {
    ...
    expectSkipped(
      await readEvent(jiraIssueCreatedEvent({ labels: LINK_LABELS })),
  ...
  test("skips an issue that is not linked to an incident", async () => {
  ...
  test("skips a link label whose id is not a UUID", async () => {
    const labelSets: Array<Array<string>> = [
      [JIRA_LINK_LABEL, `${JIRA_INCIDENT_LABEL_PREFIX}42`],
      [JIRA_LINK_LABEL, `${INCIDENT_LABEL}-old`],
      [JIRA_LINK_LABEL, `x-${INCIDENT_LABEL}`],
    ];
  ...
  test("reads a link label in any case and hands the id on lowercased", async () => {
    ...
    const result: JSONObject = await readEvent(
      jiraIssueUpdatedEvent({
        ...
        labels: [`OneUptime-Incident-${INCIDENT_ID.toUpperCase()}`],
      }),
    );
  ...
  test("takes the first valid link label", async () => {
    const result: JSONObject = await readEvent(
      jiraIssueUpdatedEvent({
        items: STARTED_CHANGELOG,
        labels: [
          `${JIRA_INCIDENT_LABEL_PREFIX}garbage`,
          incidentLabel(OTHER_INCIDENT_ID),
          INCIDENT_LABEL,
        ],
      }),
    ...
    expect(result["incidentId"]).toBe(OTHER_INCIDENT_ID);
  });

  test("skips an event whose issue key is not a Jira key", async () => {
    expectSkipped(
      await readEvent(
        withIssue(linkedUpdate(STARTED_CHANGELOG), { key: "../../myself" }),
      ),
      NO_ISSUE_KEY_REASON,
    );
  });
  ...
  test("leaves changedBy empty when the event names no user", async () => {
    const result: JSONObject = await readEvent(
      omit(linkedUpdate(STARTED_CHANGELOG, { status: IN_PROGRESS }), ["user"]),
    );
  ...
  test("reads a body that arrived as JSON text", async () => {
    ...
    const event: JSONObject = linkedUpdate(RESOLVED_CHANGELOG, {
      status: DONE,
    });
...
type StatusEventFunction = (props?: StatusEventProps | undefined) => JSONObject;
...
const IN_PROGRESS_EVENT: JSONObject = statusEvent({
  jiraStatus: "In Progress",
  jiraStatusCategory: "indeterminate",
});
...
const decideArgs: DecideArgsFunction = (
  ...
  return {
    incident: asArgument(incident === undefined ? incidentModel() : incident),
    states: asArgument(states === undefined ? INCIDENT_STATES : states),
    event: asArgument(event),
  };
};

describe("decide-state-1 maps the Jira status onto an incident state", () => {
  test("resolves the incident when the issue reaches a Done status", async () => {
    const result: JSONObject = await decideState(decideArgs(statusEvent()));
  ...
  test("acknowledges the incident when work starts on the issue", async () => {
    const result: JSONObject = await decideState(decideArgs(IN_PROGRESS_EVENT));

    expect(result["stateId"]).toBe(ACKNOWLEDGED_STATE_ID);
  ...
  test("does nothing when the issue moves back to To Do", async () => {
    expectSkipped(
      await decideState(
        decideArgs(
          statusEvent({ jiraStatus: "To Do", jiraStatusCategory: "new" }),
        ),
      ),
  ...
  test("does not move the incident to the state it is already in", async () => {
    expectSkipped(
      await decideState(
        decideArgs(
          IN_PROGRESS_EVENT,
      ...
      "INC-42 is already Acknowledged, so Jira issue OPS-17 moving to In Progress changes nothing.",
    );
  });

  test("never moves the incident backwards", async () => {
    expectSkipped(
      await decideState(
        decideArgs(IN_PROGRESS_EVENT, incidentModel({ state: RESOLVED_STATE })),
      ),
      "INC-42 is already Resolved, so Jira issue OPS-17 moving to In Progress changes nothing.",
    );
  });

  test("settles the echo of a transition OneUptime made itself", async () => {
    // The OneUptime-to-Jira template moved the issue to Done because the incident resolved.
    expectSkipped(
      await decideState(
        decideArgs(statusEvent(), incidentModel({ state: RESOLVED_STATE })),
      ),
      "INC-42 is already Resolved, so Jira issue OPS-17 moving to Done changes nothing.",
    );
  });

  test("skips, naming the id, when no incident was found", async () => {
    for (const incident of ["null", UNRESOLVED_INCIDENT]) {
      expectSkipped(
        await decideState(decideArgs(statusEvent(), incident)),
        `No incident with id ${INCIDENT_ID} exists in this project.`,
      );
    }
  });

  test("skips when the incident's states could not be read", async () => {
    for (const states of ["null", UNRESOLVED_STATES, []]) {
      expectSkipped(
        await decideState(
          decideArgs(statusEvent(), incidentModel(), states as JSONValue),
        ),
  ...
  test("finds the state by its flag whatever order the states arrive in", async () => {
    for (const states of [
      INCIDENT_STATES,
      [...INCIDENT_STATES].reverse(),
      STATES_WITH_MONITORING,
    ]) {
      const result: JSONObject = await decideState(
        decideArgs(IN_PROGRESS_EVENT, incidentModel(), states),
      );
  ...
  test("a status named in JIRA_STATUS_TO_STATE wins over its category", async () => {
    const result: JSONObject = await decideState(
      decideArgs(statusEvent(), incidentModel(), STATES_WITH_MONITORING),
      replaceInCode(
    ...
    // Done would otherwise resolve the incident; the name asks for Monitoring instead.
  ...
  test("leaves the actor out of the root cause when Jira named none", async () => {
    const result: JSONObject = await decideState(
      decideArgs(statusEvent({ changedBy: "" })),
    );
  ...
  test("caps the root cause at 1000 characters and defuses double braces in it", async () => {
    const longName: JSONObject = await decideState(
      decideArgs(statusEvent({ changedBy: "c".repeat(2000) })),
    );
  ...
  test("accepts read-event-1's real output", async () => {
    const event: JSONObject = await readEvent(
      linkedUpdate(RESOLVED_CHANGELOG, { status: DONE }),
    );
    const result: JSONObject = await decideState(decideArgs(event));

    expect(result["stateId"]).toBe(RESOLVED_STATE_ID);
  });
});

describe("read-comment-1 turns a Jira comment into the text of a note", () => {
  ...
  test("ignores an edited comment by default", async () => {
    expectSkipped(
      await readComment(jiraCommentEvent({ webhookEvent: "comment_updated" })),
  ...
  test("does not copy back a comment OneUptime posted", async () => {
    ...
    const bodies: Array<string> = [
      `${JIRA_SYNCED_FROM_ONEUPTIME_MARKER}: private note on INC-42 by Jane Doe.\n\nFailed over.`,
      `Agreed with this:\n\n> ${JIRA_SYNCED_FROM_ONEUPTIME_MARKER}: public note on INC-42.`,
    ];
...
describe("find-link-1 reads the incident id off the issue's labels", () => {
  test("finds the incident from the issue's link label", async () => {
    const result: JSONObject = await findLink(
      findLinkArgs(jiraIssueLabelsResponse(LINK_LABELS)),
    );

    expect(result).toEqual({ proceed: true, incidentId: INCIDENT_ID });
  });
  ...
  test("hands the id on lowercased", async () => {
    const result: JSONObject = await findLink(
      findLinkArgs(
        jiraIssueLabelsResponse([
          `ONEUPTIME-INCIDENT-${INCIDENT_ID.toUpperCase()}`,
        ]),
      ),
    );
  ...
  test("skips when the issue response has no fields or never resolved", async () => {
    ...
    for (const issue of responses) {
      expectSkipped(await findLink(findLinkArgs(issue)), NOT_LINKED_REASON);
    }
  });

  test("accepts read-comment-1's real output", async () => {
    const comment: JSONObject = await readComment(jiraCommentEvent());
    const result: JSONObject = await findLink(
      findLinkArgs(jiraIssueLabelsResponse(LINK_LABELS), comment),
    );
...
describe("read-changes-1 lists what changed on a linked issue", () => {
  test("notes a priority change", async () => {
    const result: JSONObject = await readChanges(
      linkedUpdate([PRIORITY_RAISED]),
    );
  ...
  test("notes assignee, summary and description changes, one line each", async () => {
    const result: JSONObject = await readChanges(
      linkedUpdate([ASSIGNED, SUMMARY_CHANGED, DESCRIPTION_CHANGED]),
    );
  ...
  test("leaves status, resolution and time tracking to the status template", async () => {
    expectSkipped(
      await readChanges(
        linkedUpdate([...RESOLVED_CHANGELOG, ...TIME_LOGGED], { status: DONE }),
      ),
  ...
  test("ignores the Rank change from dragging the issue on a board", async () => {
    expectSkipped(
      await readChanges(linkedUpdate([RANKED])),
      "Nothing that changed on Jira issue OPS-17 is worth a note.",
    );
  });
  ...
  test("ignores a field by its display name, whatever custom field id the site gave it", async () => {
    ...
    for (const item of items) {
      expectSkipped(
        await readChanges(linkedUpdate([item])),
        "Nothing that changed on Jira issue OPS-17 is worth a note.",
      );
    }
  });
  ...
  test("ignores the link labels the declare template adds", async () => {
    // Its label PUT arrives back here as a labels change on a freshly linked issue.
    for (const item of [
      labelsChanged("", `${JIRA_LINK_LABEL} ${INCIDENT_LABEL}`),
      labelsChanged("backend", `backend ${JIRA_LINK_LABEL} ${INCIDENT_LABEL}`),
    ]) {
      expectSkipped(
        await readChanges(linkedUpdate([item])),
        "Nothing that changed on Jira issue OPS-17 is worth a note.",
  ...
  test("reports a labels change that adds another label next to the link", async () => {
    const result: JSONObject = await readChanges(
      linkedUpdate([
        labelsChanged("", `${JIRA_LINK_LABEL} ${INCIDENT_LABEL} sev1`),
      ]),
    );

    expect(textOf(result, "note")).toContain(
      `\n- labels: (empty) → ${JIRA_LINK_LABEL} ${INCIDENT_LABEL} sev1`,
    );
  });

  test("lists the noteworthy changes and drops the rest", async () => {
    const result: JSONObject = await readChanges(
      linkedUpdate([...RESOLVED_CHANGELOG, PRIORITY_RAISED], { status: DONE }),
    );
  ...
  test("skips when there is no changelog", async () => {
    expectSkipped(
      await readChanges(omit(linkedUpdate([PRIORITY_RAISED]), ["changelog"])),
      "Nothing that changed on Jira issue OPS-17 is worth a note.",
    );
  });
  ...
  test("ignores every event but jira:issue_updated", async () => {
    expectSkipped(
      await readChanges(jiraIssueCreatedEvent({ labels: LINK_LABELS })),
      "Ignored a jira:issue_created event: this workflow only handles jira:issue_updated.",
    );
    expectSkipped(
  ...
  test("skips an event whose issue key is not a Jira key", async () => {
    expectSkipped(
      await readChanges(
        withIssue(linkedUpdate([PRIORITY_RAISED]), { key: "OPS 17" }),
      ),
      NO_ISSUE_KEY_REASON,
    );
  });
  ...
  test("says Someone updated the issue when the event names no user", async () => {
    const result: JSONObject = await readChanges(
      omit(linkedUpdate([PRIORITY_RAISED]), ["user"]),
    );
...
const JIRA_SCRIPT_STEPS: Array<ScriptStep> = [
  {
    templateId: "jira-create-issue-for-incident",
    componentId: "prepare-issue-1",
    argumentNames: ["oneuptimeUrl", "incident"],
  },
  {
    templateId: "jira-transition-issue-on-incident-state",
    componentId: "plan-transition-1",
    argumentNames: ["incident", "search"],
  },
  {
    templateId: "jira-comment-from-private-note",
    componentId: "build-comment-1",
  ...
  },
  {
    ...
    argumentNames: ["incident", "states", "event"],
  },
  {
...
describe("every Jira script", () => {
  test("is covered here, with the arguments its template really passes", () => {
    ...
    expect(jiraTemplates).toHaveLength(9);
...
const ADVERSARIAL_PROCEEDING: Array<AdversarialCase> = [
  {
    templateId: "jira-create-issue-for-incident",
    componentId: "prepare-issue-1",
    args: issueArgs(
      incidentModel({
        ...
        state: { ...CREATED_STATE, name: ADVERSARIAL_TEXT },
      }),
    ),
  },
  {
  ...
  ...NOTE_TEMPLATES.map((noteTemplate: NoteTemplateCase): AdversarialCase => {
    return {
      templateId: noteTemplate.templateId,
      componentId: "build-comment-1",
      args: noteArgs(
        noteModel({ note: ADVERSARIAL_TEXT, authorName: ADVERSARIAL_TEXT }),
      ),
    };
  }),
  {
  ...
  },
  {
    templateId: "jira-status-to-incident-state",
    componentId: "read-event-1",
    args: linkedUpdate(STARTED_CHANGELOG, {
      status: jiraStatus({
        name: ADVERSARIAL_TEXT,
        categoryKey: "indeterminate",
      }),
  ...
  },
  {
    ...
    componentId: "find-link-1",
    args: findLinkArgs(
      jiraIssueLabelsResponse([ADVERSARIAL_TEXT, ...LINK_LABELS]),
 ...
 */
describe("no Jira script hands on double braces", () => {
  for (const adversarial of ADVERSARIAL_PROCEEDING) {
  ...
  test("in the reasons the webhook scripts give for skipping", async () => {
    ...
    const cases: Array<AdversarialCase> = [
      ...
      },
      {
        ...
        componentId: "confirm-unlinked-1",
        args: confirmArgs({
          ...jiraIssueLabelsResponse(LINK_LABELS),
  ...
  test("in the reasons plan-transition-1 gives for skipping", async () => {
    ...
    for (const search of [
      ...
      const result: JSONObject = await planTransition(
        transitionArgs(ACKNOWLEDGED_STATE, search),
      );
    ...
    expect(reasons[3]).toContain("Name the one to use in STATE_TO_JIRA_STATUS");
  ...
  test("in the reasons the outbound scripts give for skipping", async () => {
    /*
     * The incident number's prefix is a project setting, and a private
     * incident's reason quotes it. skip() defuses every reason regardless.
     */
    const hostileIncident: JSONObject = {
      ...incidentModel({ state: ACKNOWLEDGED_STATE }),
      incidentNumberWithPrefix: ADVERSARIAL_TEXT,
      isPrivate: true,
    };
    const hostileNote: JSONObject = {
      ...noteModel(),
      incident: { incidentNumberWithPrefix: ADVERSARIAL_TEXT, isPrivate: true },
    };
    const results: Array<JSONObject> = [
      await prepareIssue(issueArgs(hostileIncident)),
      await planTransition({
        incident: quoted(hostileIncident),
        search: quoted(jiraSearchResponse()),
      }),
      await privateNoteComment(noteArgs(hostileNote)),
      await publicNoteComment(noteArgs(hostileNote)),
      await updateComment(updateArgs(hostileIncident)),
  ...
  test("from a hostile status change carried all the way through the status template", async () => {
    ...
    const decision: JSONObject = await decideState(decideArgs(event));
