...
/*
 * The HTTP face of Phase 2.
 *
 * The team compliance route (/team/compliance-status/:teamId) and the
 * TeamComplianceService rebuild behind it are Enterprise Edition code since the
 * Community / Enterprise split; their HTTP tests moved with them to
 * ee/Tests/Server/TeamCompliance/TeamComplianceAPI.test.ts.
 *
 ...
 *   across tenants.
 *
 *   MASKING is tested end to end, with the REAL OnCallReadinessService and only
 *   the database-facing services stubbed. A masking test that stubs the service
 *   it is testing proves nothing: the whole claim is that a raw phone number put
 *   into the database cannot come out of the HTTP response, and the only way to
 *   test that claim is to put a raw phone number in one end and read the JSON
 *   out of the other.
 *
...
// Every database-facing read the readiness path can make.
...
/*
 * --------------------------------------------------------------------------- *
 * A refused caller costs nothing. Every database-facing read the readiness
 * routes can reach is stubbed above; an anonymous caller must be turned away
 * before ANY of them runs, on every read route - the refusal is the only gate,
 * so it has to come first.
 * ---------------------------------------------------------------------------
 */

describe("the readiness read routes - a refused caller reads nothing", () => {
  test.each<[string]>([[POLICY_ROUTE], [PROJECT_ROUTE], [USER_ROUTE]])(
    "%s",
    async (uri: string) => {
      propsSpy.mockResolvedValue({} as never);
      ...
      for (const spy of [...everyFindBySpy(), projectFindOneById]) {
