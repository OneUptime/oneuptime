...
import MicrosoftTeamsIncidentMessages from "../../../../../../Server/Utils/Workspace/MicrosoftTeams/Messages/Incident";
...
describe("MicrosoftTeamsIncidentMessages.getIncidentCreateMessageBlocks", () => {
  ...
  test("returns a divider followed by a single buttons block", async () => {
    const blocks: Array<WorkspaceMessageBlock> =
      await MicrosoftTeamsIncidentMessages.getIncidentCreateMessageBlocks({
        incidentId,
        projectId,
      });
  ...
  test("emits the 6 buttons in order with the expected action ids and titles", async () => {
    ...
    expect(
      ...
      MicrosoftTeamsIncidentActionType.ViewIncident,
  ...
  test("throws BadDataException when the incident id is missing", async () => {
    ...
    await expect(
      MicrosoftTeamsIncidentMessages.getIncidentCreateMessageBlocks({
