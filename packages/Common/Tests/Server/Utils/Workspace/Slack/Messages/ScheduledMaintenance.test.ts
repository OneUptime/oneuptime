...
import SlackScheduledMaintenanceMessages from "../../../../../../Server/Utils/Workspace/Slack/Messages/ScheduledMaintenance";
...
describe("SlackScheduledMaintenanceMessages.getScheduledMaintenanceCreateMessageBlocks", () => {
  ...
  test("emits the 5 buttons in order with the expected action ids and titles", async () => {
    ...
    expect(
      ...
      SlackActionType.ViewChangeScheduledMaintenanceState,
      SlackActionType.ViewAddScheduledMaintenanceNote,
    ]);
  ...
  test("throws BadDataException when the scheduled maintenance id is missing", async () => {
    ...
    await expect(
      SlackScheduledMaintenanceMessages.getScheduledMaintenanceCreateMessageBlocks(
        {
  ...
  test("does not offer on-call execution or acknowledge buttons", async () => {
    const buttons: Array<WorkspaceMessagePayloadButton> = getButtons(
      await SlackScheduledMaintenanceMessages.getScheduledMaintenanceCreateMessageBlocks(
        {
          scheduledMaintenanceId,
    ...
    const titles: Array<string> = buttons.map(
      (button: WorkspaceMessagePayloadButton) => {
        return button.title;
      },
    );

    expect(titles).not.toContain("📞 Execute On Call");
    expect(
      titles.some((title: string) => {
        return title.includes("Acknowledge");
      }),
    ).toBe(false);
  });
});
