 ...
 * which detached every member alert.
 *
...
describe("Alert Episode overview: details column", () => {
  ...
  test("no longer repeats the title the header already shows", async () => {
    ...
    expect(screen.queryByText("Last Alert Added At")).toBeNull();
   ...
   * "Edit Alert Episode" beside the title and squeezed the title and description
  ...
  test("stacks the details card header and keeps a short, gated Edit button", async () => {
    ...
    expect(screen.queryByText("Edit Alert Episode")).toBeNull();
    expect(
...
describe("Alert Episode overview: loading and layout", () => {
  ...
  test("renders the header, a four-cell stat bar and the member list", async () => {
    ...
    expect(within(statBar).getByText("Alerts")).toBeInTheDocument();
    expect(statValue("Alerts")).toBe("0");
  ...
  test("measures the stat bar from createdAt with the project's state names", async () => {
    const startedAt: Date = new Date("2026-09-14T18:00:00.000Z");

    const ackState: AlertState = new AlertState();
    ackState.id = new ObjectID(ACK_STATE_ID);
    ackState.name = "Triaged";
    ackState.isAcknowledgedState = true;

    const resolvedState: AlertState = new AlertState();
    resolvedState.id = new ObjectID(RESOLVED_STATE_ID);
    resolvedState.name = "Fixed";
    resolvedState.isResolvedState = true;

    const ackTimeline: AlertEpisodeStateTimeline =
      new AlertEpisodeStateTimeline();
    ackTimeline.alertStateId = new ObjectID(ACK_STATE_ID);
    // Written long after creation, so a first-timeline start would be wrong.
    ackTimeline.startsAt = new Date(startedAt.getTime() + 42 * 60 * 1000);

    const resolvedTimeline: AlertEpisodeStateTimeline =
      new AlertEpisodeStateTimeline();
    resolvedTimeline.alertStateId = new ObjectID(RESOLVED_STATE_ID);
    resolvedTimeline.startsAt = new Date(startedAt.getTime() + 125 * 60 * 1000);
    ...
    getListMock.mockImplementation((...args: Array<any>) => {
      ...
      if (modelType === AlertEpisodeStateTimeline) {
        return Promise.resolve({
    ...
    // Resolved now, so the duration is closed: created -> resolved.
    expect(statValue("Duration")).toBe("2 hours, 5 minutes");
    expect(statValue("Alerts")).toBe("3");
  });
  ...
  test("a state change refreshes the page in place instead of unmounting it", async () => {
    ...
    const memberCallsBefore: number = listCallsFor(Alert).filter(
      (call: Array<any>): boolean => {
        return call[0].limit === 8;
      },
    ).length;
    ...
    await waitFor((): void => {
      expect(listCallsFor(AlertEpisodeStateTimeline).length).toBe(
        timelineCallsBefore + 1,
      );
      // The member list reloads too: member states moved with the episode.
      expect(
        listCallsFor(Alert).filter((call: Array<any>): boolean => {
          return call[0].limit === 8;
        }).length,
...
describe("Alert Episode overview: moving to another episode on the same route", () => {
  /*
  ...
  const navigateTo: NavigateToFunction = (episodeId: string): void => {
    const path: string = `/dashboard/${PROJECT_ID}/alerts/episodes/${episodeId}`;
  ...
  type ServeEpisodesFunction = (otherPageRead: Promise<AlertEpisode>) => void;

  const serveEpisodes: ServeEpisodesFunction = (
    otherPageRead: Promise<AlertEpisode>,
  ): void => {
  ...
  test("shows the skeleton first and renders nothing for the new id before its numbers land", async () => {
    let resolveOther: (episode: AlertEpisode) => void = (): void => {};
    const otherPageRead: Promise<AlertEpisode> = new Promise<AlertEpisode>(
      (resolve: (episode: AlertEpisode) => void) => {
    ...
    view.rerender(<AlertEpisodeView {...pageProps} />);
    ...
    const otherEpisode: AlertEpisode = buildEpisode({});
    otherEpisode.id = new ObjectID(OTHER_EPISODE_ID);
    otherEpisode.alertCount = 5;
    ...
    await waitFor((): void => {
      expect(statValue("Alerts")).toBe("5");
    });
