...
export default class LogDropFilter extends BaseModel {
  ...
  /*
   * Ingest-owned counters. A drop filter used to discard logs with no
  ...
  @TableColumn({
    ...
    canReadOnRelationQuery: true,
    description:
      "Total number of logs this filter has discarded since it was created.",
    defaultValue: 0,
  })
  @Column({
  ...
  @TableColumn({
    ...
    canReadOnRelationQuery: true,
    description:
      "When this filter most recently discarded a log. Null means it has never matched anything.",
  })
  @Column({
