...
export default class MonitorOverviewCriteriaUtil {
   ...
   * because that is the one that fires first. Criteria that are switched
   * off are skipped: the server does not evaluate them, so quoting their
   * window would promise an alert that never comes.
   */
  public static getMissingSignalMinutes(data: {
    ...
    for (const step of getSteps(data.monitorSteps)) {
      ...
      for (const rawInstance of instances) {
        ...
        for (const filter of filters as Array<CriteriaFilter>) {
          if (!filter || filter.checkOn !== data.checkOn) {
            continue;
          }
