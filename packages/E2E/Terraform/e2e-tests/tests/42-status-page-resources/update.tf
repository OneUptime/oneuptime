...
# apply + drift gate. Changed vs main.tf: group name, resource display_name
...
resource "oneuptime_status_page_group" "group" {
  ...
  name           = "terraform-e2e-sp-group-updated"
...
resource "oneuptime_status_page_resource" "resource" {
  ...
  display_name         = "terraform-e2e-sp-resource-updated"
  display_description  = "Resource updated by Terraform E2E tests"
}
