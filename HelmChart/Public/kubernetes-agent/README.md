...
In every mode, bypass approval included, a write in **kube-system**, **kube-public** or **kube-node-lease** always needs a human; so do a `drain` and a `taint`, because draining a node evicts pods in every namespace — those three and the agent's own included — and a `NoExecute` taint evicts them too; and the Runner never changes anything in its own namespace. No allowlist entry makes any of these run on its own. An allowlist entry is matched word by word: `*` stands for exactly one word (an image, a name), never for extra objects, flags or a second `-n`, and every flag the command uses must be written out in the entry. Only a Project Owner, a Project Admin or someone with **Edit Auto Remediation Rule** may switch a cluster to automatic or bypass approval, write its allowlist, or bind a Runner or credential to it (binding a credential also needs **Read Runbook Credential**); anyone who can edit the cluster may turn AI access down.
...
| `remediation.namespaces` | `[]` | Bind the write role only in these namespaces (one RoleBinding each). Each must already exist, or the install or upgrade fails. Empty binds it cluster-wide; `=null` resets a stored list. |
| `remediation.nodeOperations` | `true` | With `remediation.enabled`, also grant cordon/uncordon/taint/drain, cluster-wide. `false` grants no node RBAC and tells the Runner to refuse node operations. |
...
> **An upgrade fails with `namespaces "<name>" not found`?** `aiAccess.remediation.namespaces` lists a namespace that does not exist (or no longer does): the chart puts a RoleBinding in each listed namespace and never creates one. Create the namespace, or upgrade with the list minus that namespace — `--set "aiAccess.remediation.namespaces={web}"`, or `--set aiAccess.remediation.namespaces=null` to go back to the cluster-wide binding. Take a namespace off the list before you delete it.
