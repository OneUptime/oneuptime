...
| `podDisruptionBudget.enabled`                     | Create a PodDisruptionBudget for each stateless deployment (app, worker, nginx, home, ai-agent, probes, pgbouncer) to cap voluntary disruptions during node drains / cluster upgrades  | `false`         |                 |
