...
export const loginSuccess = (user) => {
    ...
    if (user.redirect && ((_a = user === null || user === void 0 ? void 0 : user.tokens) === null || _a === void 0 ? void 0 : _a.jwtAccessToken)) {
...
export const verifyTokenRequest = (promise) => {
    return {
...
export const loginUserSso = (values) => {
    return async (dispatch) => {
        try {
            const response = await BackendAPI.get(`user/sso/login?email=${values.email}`);
...
export const verifyAuthToken = (values) => {
    const initialUrl = User.initialUrl();
    const redirect = getQueryVar('redirectTo', initialUrl);
    if (redirect) {
        values.redirect = redirect;
    ...
    return function (dispatch) {
        const promise = BackendAPI.post(new Route('user/totp/verifyToken'), values);
...
export const useBackupCodeRequest = (promise) => {
    return {
        type: types.BACKUP_CODE_VERIFICATION_REQUEST,
        payload: promise,
    };
};
export const useBackupCodeError = (error) => {
    return {
        type: types.BACKUP_CODE_VERIFICATION_FAILED,
        payload: error,
    };
};
export const verifyBackupCode = (values) => {
    const initialUrl = User.initialUrl();
    const redirect = getQueryVar('redirectTo', initialUrl);
    if (redirect) {
        values.redirect = redirect;
    ...
    values.email = values.email || email;
    return function (dispatch) {
        const promise = BackendAPI.post(new Route('user/verify/backupCode'), values);
...
export const saveStatusPage = (data) => {
    return {
        type: types.SAVE_STATUS_PAGE,
        payload: data,
    };
};
export const masterAdminExistsRequest = (promise) => {
    return {
        type: types.MASTER_ADMIN_EXISTS_REQUEST,
        payload: promise,
    };
};
export const masterAdminExistsError = (error) => {
    return {
        type: types.MASTER_ADMIN_EXISTS_FAILED,
        payload: error,
    };
};
export const masterAdminExistsSuccess = (data) => {
    return {
...
// Calls the API to register a user.
export const checkIfMasterAdminExists = (values) => {
    return function (dispatch) {
