...
const Layers: FunctionComponent<ComponentProps> = (
    ...
    const [layers, setLayers] = React.useState<Array<OnCallDutyPolicyScheduleLayer>>([]);

    const [isAddbuttonLoading, setIsAddButtonLoading] = React.useState<boolean>(false);

    const [error, setError] = React.useState<string>('');

    const [isDeletetingLayerId, setIsDeletingLayerId] = React.useState<Array<ObjectID>>([]);


    const [showCannotDeleteOnlyLayerError, setShowCannotDeleteOnlyLayerError] = React.useState<boolean>(false);
    ...
    const addLayer: Function = async () => {
        ...
        try {
            ...
            const onCallPolicyScheduleLayer = new OnCallDutyPolicyScheduleLayer();
            onCallPolicyScheduleLayer.onCallDutyPolicyScheduleId = props.onCallDutyPolicyScheduleId;
    ...
    const deleteLayer: Function = async (item: OnCallDutyPolicyScheduleLayer) => {
        if (!item.id) {
        ...
        try {
            ...
            const newLayers = layers.filter((layer: OnCallDutyPolicyScheduleLayer) => {
        ...
        setIsDeletingLayerId(isDeletetingLayerId.filter((id: ObjectID) => {
    ...
    const fetchLayers: Function = async () => {
        ...
        try {
            const layers = await ModelAPI.getList<OnCallDutyPolicyScheduleLayer>(
                OnCallDutyPolicyScheduleLayer,
                {
        ...
        <div>
            <div>
                {layers.map((layer: OnCallDutyPolicyScheduleLayer) => {
                    return (
                        <Layer
                            isDeleteButtonLoading={!!isDeletetingLayerId.find((id: ObjectID) => {
                            ...
                            onLayerChange={(layer: OnCallDutyPolicyScheduleLayer) => {
                                ...
                                const newLayers = layers.map((item: OnCallDutyPolicyScheduleLayer) => {
                                    if (item.id === layer.id) {
                                        return layer;
                                    }
                                    return item;
            ...
            <div>
                <Button title='Add New Layer' isLoading={isAddbuttonLoading} onClick={()=>{
                    addLayer();
                }} icon={IconProp.Add} />
            </div>
            ...
            {showCannotDeleteOnlyLayerError ? (
                <ConfirmModal
                    ...
                    description={
                        <div>
                            Schedule must have at least one layer.
                        </div>
                    }
                    isLoading={false}
