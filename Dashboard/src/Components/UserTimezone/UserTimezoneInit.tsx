...
const UseTimezoneInitElement: FunctionComponent = (
    ...
    const [timezoneToSave, setTimezoneToSave] = React.useState<Timezone | null>(null);


    const updateUserTimezone = async (timezone: Timezone): Promise<void> => {
        try {
            ...
            ShowToastNotification({
                ...
                createdAt: OneUptimeDate.getCurrentDate()
            })
        }
    ...
    useAsyncEffect(async () => {
        if (User.isLoggedIn()) {
            ...
            if (User.getSavedUserTimezone() === null || User.getSavedUserTimezone() !== guessTimezone) {
    ...
    if (showConfirmModal) {
        return <ConfirmModal
            ...
            description={
                <div>
                    <p>
                        We have detected that your timezone is different from the
                        timezone you have saved in your profile. Would you like to update
                        your timezone to{" "}
                        <strong>{timezoneToSave?.toString()}</strong>?
                    </p>
                </div>
            }
            onClose={() => {
