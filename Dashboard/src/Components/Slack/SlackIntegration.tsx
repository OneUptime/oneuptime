...
import ProjectAuthToken, { ProjectAuthtokenServiceProviderType } from "Common/Models/DatabaseModels/ProjectAuthToken";
import ModelAPI from "Common/UI/Utils/ModelAPI/ModelAPI";
import SortOrder from "Common/Types/BaseDatabase/SortOrder";
import ListResult from "Common/UI/Utils/BaseDatabase/ListResult";
import UserAuthToken, { UserAuthTokenServiceProviderType } from "Common/Models/DatabaseModels/UserAuthToken";
...
const SlackIntegration: FunctionComponent = (): ReactElement => {
    ...
    const [isUserAccountAlreadyConnected, setIsUserAccountAlreadyConnected] = React.useState<boolean>(false);
    const [userAuthTokenId, setUserAuthTokenId] = React.useState<ObjectID | null>(null);
    const [projectAuthTokenId, setProjectAuthTokenId] = React.useState<ObjectID | null>(null);
    const [isProjectAccountAlreadyConnected, setIsProjectAccountAlreadyConnected] = React.useState<boolean>(false);
    ...
    const loadItems = async () => {
        try {
            ...
            const projectAuth: ListResult<ProjectAuthToken> = await ModelAPI.getList<ProjectAuthToken>({
            ...
            const userAuth: ListResult<UserAuthToken> = await ModelAPI.getList<UserAuthToken>({
    ...
    return (
        <Fragment>
            <div>
                ...
                {isUserAccountAlreadyConnected && isProjectAccountAlreadyConnected && <Card
                    ...
                    buttons={[
                        {
                            ...
                            onClick: () => {
                                if (SlackAppClientId) {
                                    ...
                                    Navigation.navigate(URL.fromString(`https://slack.com/oauth/v2/authorize?scope=${botScopes.join(",")
