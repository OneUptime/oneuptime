      ...
      runFunction: async () => {
        ...
        for (const cert of certs) {
          try {
            ...
            logger.error("Create directory err", { service: "ingress", job: "AcmeWriteCertificates" });
            logger.error(err, { service: "ingress", job: "AcmeWriteCertificates" });
          }
          ...
          logger.debug(
            ...
            { service: "ingress", job: "AcmeWriteCertificates", domain: cert.domain?.toString().trim().toLocaleLowerCase() || "" },
          );
        }
      },
    });
