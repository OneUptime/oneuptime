...
- `ai-report-summary.png`: the AI Investigation card. The Summary is its own section (TL;DR
  plus the report's summary), then the report section by section: the amber "Most likely
  root cause" callout, Evidence and Suggested next steps, with citation chips and incident
  links inline and no brand heading.
...
- `ai-report-evidence.png`: "Evidence checked" with C1 expanded: what the query asked
  (search, time window, limit, when it ran, how long it took, the tool), an Open in
  Incidents link, the notice that the rows are current data under the viewer's permissions,
  and the three incidents it returned. The other nine queries show their plain-language
  description, local run time and row count; C8 returned no rows.
- `incident-overview.png`: the incident page at 1440px. Hero with number, title, state,
  ...
  report leading the left column above the feed; Incident Details, Incident Roles and
...
`output/playwright/event-overview-ui/` under their `-synthetic` names (`ai-report-summary`,
`ai-report-references` and `ai-report-evidence` at device scale factor 2). The "before"
