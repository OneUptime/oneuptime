...
- Runs the engine's receiver against `DATABASE_ENDPOINT` and enables the useful metrics that are off upstream (for example `postgresql.blks_hit` / `postgresql.blks_read` for the cache hit ratio, `mysql.query.count`, `sqlserver.deadlock.rate`, `oracledb.tablespace.utilization`, `redis.maxmemory`, `mongodb.health`, `jvm.memory.heap.utilization`). Each config lists further optional metrics in a comment.
